...
export function activate(context: vscode.ExtensionContext) {
  ...
  const port = vscode.workspace.getConfiguration('csQuill').get<number>('daemonPort') ?? 8443;
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeTextDocument((event) => {
      ...
      debounceTimer = setTimeout(() => {
        ...
        const lang = doc.languageId === 'typescript' || doc.languageId === 'typescriptreact'
  ...
  context.subscriptions.push(
    vscode.languages.registerCodeActionsProvider(
      ...
      { providedCodeActionKinds: QuillCodeActionProvider.providedCodeActionKinds },
    ),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cs-quill.analyzeFile', async () => {
      ...
      if (!editor) { vscode.window.showWarningMessage('파일을 열어주세요'); return; }

      if (!client.isConnected()) {
        vscode.window.showErrorMessage('CS Quill 데몬 미연결. cs daemon --port 8443 실행 필요');
        return;
      }

      const doc = editor.document;
      vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: '🦔 CS Quill 분석 중...' }, async () => {
        const result = await client.analyzeFile(doc.uri.fsPath, doc.getText());
        if (result?.findings) {
          diagnosticProvider.updateDiagnostics(doc.uri, result.findings);
          vscode.window.showInformationMessage(`🦔 분석 완료: ${result.findings.length}건 (${result.score}/100, ${result.duration}ms)`);
        } else {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cs-quill.fixAll', async () => {
      ...
      const quillDiags = diagnostics.filter(d => d.source?.startsWith('CS Quill'));
      ...
      if (fixCount > 0) {
        ...
        vscode.window.showInformationMessage('자동 수정 가능한 항목이 없습니다');
      }
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cs-quill.showStatus', () => {
      if (client.isConnected()) {
        vscode.window.showInformationMessage(`🦔 CS Quill 연결됨 (Session: ${client.getSessionId()})`);
      } else {
        vscode.window.showWarningMessage('🦔 CS Quill 미연결. cs daemon 실행 필요');
      }
    }),
  );
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('cs-quill.reconnect', async () => {
      ...
      vscode.window.showInformationMessage(ok ? '🦔 재연결 성공' : '🦔 재연결 실패');
    }),
  );
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider('eh-universe-sidebar', sidebarProvider),
  );
