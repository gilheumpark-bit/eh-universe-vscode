...
interface AgentDiffPreviewProps {
  ...
  onReject: () => void;
  onPartialAccept?: (
    accepted: FileChangePreview[],
    rejected: FileChangePreview[],
  ) => void;
}
...
export default function AgentDiffPreview({
  changes,
  onAccept,
  onReject,
  onPartialAccept,
}: AgentDiffPreviewProps) {
  const { lang } = useLang();
  const [selectedIndex, setSelectedIndex] = useState(0);
  const [decisions, setDecisions] = useState<
    Map<string, "accept" | "reject" | "pending">
  ...
  const stats = useMemo(() => {
    ...
    const totalLinesAdded = changes.reduce(
      (sum, c) =>
        c.status === "deleted" ? sum : sum + c.modified.split("\n").length,
      0,
    );
    const totalLinesRemoved = changes.reduce(
      (sum, c) =>
        c.status === "added" ? sum : sum + c.original.split("\n").length,
      0,
    );
  ...
  const rejectedCount = useMemo(
    () => Array.from(decisions.values()).filter((d) => d === "reject").length,
    [decisions],
  );

  const handleDecision = useCallback(
    (filePath: string, decision: "accept" | "reject") => {
      setDecisions((prev) => {
        const next = new Map(prev);
        next.set(filePath, decision);
        return next;
      });
    },
    [],
  ...
  const handleApplyDecisions = useCallback(() => {
    const accepted = changes.filter(
      (c) => decisions.get(c.filePath) !== "reject",
    );
    const rejected = changes.filter(
      (c) => decisions.get(c.filePath) === "reject",
    );
    if (onPartialAccept && rejected.length > 0) {
      onPartialAccept(accepted, rejected);
    } else {
      onAccept(accepted);
    }
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent) => {
      if (e.key === "Tab" && !e.shiftKey) {
        ...
        handleDecision(selected.filePath, "accept");
        handleNext();
  ...
  return (
    ...
    >
      <div
        className="flex flex-col w-full max-w-[1200px] mx-auto my-4 bg-bg-primary rounded-xl shadow-2xl overflow-hidden border border-border"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-bg-secondary">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold">
              {L4(lang, {
                ko: "에이전트 변경 확인",
                en: "Agent Changes Review",
              })}
            </span>
            <span className="text-xs text-text-secondary">
              {L4(lang, {
                ko: `${changes.length}개 파일 변경됨`,
                en: `${changes.length} files changed`,
              })}
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            >
              <CheckCircle size={12} />{" "}
              {L4(lang, { ko: "모두 수락", en: "Accept All" })}
            </button>
            <button
            ...
            >
              <XCircle size={12} />{" "}
              {L4(lang, { ko: "모두 거절", en: "Reject All" })}
            </button>
            <button
              onClick={onReject}
              aria-label="닫기"
              className="p-1 hover:bg-bg-secondary/50 rounded transition-colors"
            >
              <X size={14} />
            </button>
          </div>
        ...
        <div className="flex items-center gap-4 px-4 py-2 text-[10px] border-b border-border bg-bg-secondary/50">
          <span className="text-green-400">
            {L4(lang, {
              ko: `+${stats.totalLinesAdded}줄`,
              en: `+${stats.totalLinesAdded} lines`,
            })}
          </span>
          <span className="text-red-400">
            {L4(lang, {
              ko: `-${stats.totalLinesRemoved}줄`,
              en: `-${stats.totalLinesRemoved} lines`,
            })}
          </span>
          <span className="text-text-secondary">
            ...
            {stats.modified > 0 && (
              <span className="ml-2">
                {L4(lang, {
                  ko: `${stats.modified} 수정됨`,
                  en: `${stats.modified} modified`,
                })}
              </span>
            )}
            {stats.removed > 0 && (
              <span className="ml-2 text-red-400">
                {L4(lang, {
                  ko: `${stats.removed} 삭제됨`,
                  en: `${stats.removed} deleted`,
                })}
              </span>
            )}
          </span>
          <span className="ml-auto text-text-secondary">
            {L4(lang, {
              ko: `수락: ${acceptedCount} | 거절: ${rejectedCount} | 대기: ${changes.length - acceptedCount - rejectedCount}`,
              en: `Accept: ${acceptedCount} | Reject: ${rejectedCount} | Pending: ${changes.length - acceptedCount - rejectedCount}`,
            })}
          </span>
        </div>
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 text-xs text-left hover:bg-bg-tertiary/50 transition-colors ${
                    isSelected
                      ? "bg-bg-tertiary/50 border-l-2 border-accent-amber"
                      : ""
                  }`}
                >
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            {selected ? (
              <>
                <div className="flex items-center justify-between px-4 py-2 border-b border-border">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {showDiff
                        ? L4(lang, { ko: "코드 보기", en: "Code View" })
                        : L4(lang, { ko: "변경 비교", en: "Diff View" })}
                    </button>
                    <button
                      onClick={() =>
                        handleDecision(selected.filePath, "accept")
                    ...
                    >
                      <Check size={10} />{" "}
                      {L4(lang, { ko: "수락", en: "Accept" })}
                    </button>
                    <button
                      onClick={() =>
                        handleDecision(selected.filePath, "reject")
                      }
                ...
                <div className="flex items-center justify-between px-4 py-2 border-t border-border">
                  ...
                  >
                    <ChevronLeft size={12} />{" "}
                    {L4(lang, { ko: "이전", en: "Previous" })}
                  </button>
                  <span className="text-[10px] text-text-tertiary">
                    {L4(lang, {
                      ko: `${selectedIndex + 1} / ${changes.length} (Tab: 다음, Enter: 수락, Esc: 닫기)`,
                      en: `${selectedIndex + 1} / ${changes.length} (Tab: next, Enter: accept, Esc: close)`,
                    })}
                  </span>
                  <button
                    onClick={handleNext}
              ...
              <div className="flex-1 flex items-center justify-center text-text-secondary">
                {L4(lang, { ko: "파일을 선택하세요", en: "Select a file" })}
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-3 px-4 py-3 border-t border-border bg-bg-secondary">
          ...
          >
            {L4(lang, {
              ko: `선택 적용 (${acceptedCount} 수락됨)`,
              en: `Apply Selection (${acceptedCount} accepted)`,
            })}
          </button>
        </div>
      </div>
...
function DecisionBadge({
  ...
  return (
    <span className="w-2.5 h-2.5 rounded-full border border-text-secondary" />
  );
}
...
function InlineDiffView({
  ...
  while (oi < origLines.length || mi < modLines.length) {
    if (
      ...
      oi < origLines.length &&
      (mi >= modLines.length || origLines[oi] !== modLines[mi])
    ) {
    ...
    <div>
      {rows.map((row, i) => (
        ...
        >
          ...
          <span className="w-10 text-right pr-2 shrink-0 text-text-tertiary select-none opacity-50">
            {row.num}
          </span>
          <span
            className={
