...
import {
  ...
  connectServer,
  callTool,
} from "@/lib/code-studio/features/mcp-client";
...
import {
  detectPreset,
  buildPresetPrompt,
} from "@/lib/code-studio/core/design-presets";
import {
  runDesignLint,
  formatDesignLintReport,
} from "@/lib/code-studio/pipeline/design-lint";
...
export function ChatPanel({
    ...
    ? (() => {
        ...
        const doc = servers
          .flatMap((s) =>
            s.tools.map(
              (t) =>
                `- /mcp call ${s.name} ${t.name} (args: ${JSON.stringify(t.inputSchema)})`,
            ),
          )
          .join("\n");
  ...
  const chat = useCodeStudioChat({
    systemInstruction:
      chatMode === "nod"
        ? (ko ? NOD_SYSTEM_PROMPT : NOD_SYSTEM_PROMPT_EN) +
...
${mcpToolsDoc}`,
  ...
  useEffect(() => {
    ...
    try {
      ...
      logger.warn(
        "code-studio.chat.seed",
        "Failed to load project-spec chat seed",
        err,
      );
    }
  }, [isMounted]);
  ...
  const handleSend = useCallback(async () => {
    ...
    if (text.startsWith("/mcp")) {
      ...
      if (cmd === "connect") {
        ...
        await chat.sendMessage(
          `User executed: /mcp connect ${name}\n\nSystem Response:\n${msg}`,
        );
        return;
      }
      ...
      if (cmd === "call") {
        ...
        try {
          ...
          await chat.sendMessage(
            `[MCP Tool Result: ${serverName}.${toolName}]\n${result.content}\n\nPlease analyze this result.`,
          );
        } catch (e) {
          await chat.sendMessage(
            `System Error: Tool call failed. Invalid JSON arguments or execution error. ${e}`,
          );
        }
        return;
      }
    ...
    if (
      text.startsWith(">") ||
      (text.startsWith("/") && !text.startsWith("/mcp"))
    ) {
      const rawCmd = text.startsWith(">")
        ? text.slice(1).trim()
        : text.slice(1).trim();
      ...
      if (nlResult.type === "shell") {
        ...
        await chat.sendMessage(
          `[Terminal] ${nlResult.command}${nlResult.terminalId != null ? ` (terminal ${nlResult.terminalId + 1})` : ""}`,
        );
        return;
      }
      if (nlResult.type === "action") {
        ...
        await chat.sendMessage(
          `[Action] ${nlResult.action}: ${JSON.stringify(nlResult.params)}`,
        );
        return;
      }
    ...
    const presetHint =
      presetId !== null ||
      /컴포넌트|component|UI|버튼|button|모달|modal|폼|form|카드|card|페이지|page|랜딩|landing|대시보드|dashboard/i.test(
        text,
      )
  ...
  const handleMentionSelect = useCallback(
    (mention: string) => {
      ...
      if (atIdx >= 0) {
        setInput(
          input.slice(0, atIdx) + mention + " " + input.slice(cursorPos),
        );
      }
      setShowMentions(false);
    ...
    <div className="flex flex-col h-full min-h-0 bg-bg-secondary">
        ...
        <div className="border-b border-border bg-bg-primary max-h-[200px] overflow-y-auto">
          ...
          {chatSessions.length === 0 ? (
            ...
            chatSessions.map((session) => (
              <div
                key={session.id}
                className="flex items-center gap-2 px-3 py-1.5 hover:bg-bg-tertiary group"
              >
                {renamingId === session.id ? (
                  ...
                  <button
                    ...
                    className="flex-1 text-left text-xs truncate text-text-primary"
        ...
        {chat.messages.length === 0 && !chat.isStreaming && (
          <div className="flex flex-col items-center justify-center gap-3 py-12 px-4 text-center">
            ...
            <p className="text-xs text-text-tertiary leading-relaxed max-w-[240px]">
              Ask about your code, request reviews, or generate implementations.
            </p>
            <div className="flex flex-wrap justify-center gap-2 mt-2 max-w-[280px]">
              {["Review this file", "Find bugs", "Refactor", "Add tests"].map(
                (s, i) => (
                  <button
                    key={i}
                    onClick={() => {
                  ...
                  >
                    <Zap size={9} className="inline opacity-50 mr-1" />
                    {s}
                  </button>
                ),
              )}
          ...
          return (
            <div key={msg.id} className="text-xs leading-relaxed">
              ...
              {codeBlocks.map((block, idx) => {
                const isUI =
                  /tsx?/.test(block.language ?? "") &&
                  (/</.test(block.code) || /className/.test(block.code));
                ...
                return (
                  <div key={idx} className="mt-1 space-y-1">
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() =>
                          onApplyCode?.(block.code, block.fileName)
                        }
                      ...
                      {lint && (
                        <span
                          className={`text-[9px] px-1.5 py-0.5 rounded ${lint.passed ? "bg-accent-green/15 text-accent-green" : "bg-accent-red/15 text-accent-red"}`}
                        >
                    ...
                    {lint && lint.issues.length > 0 && (
                      <details className="text-[9px] text-text-tertiary">
                        <summary className="cursor-pointer hover:text-text-secondary">
                          {lint.issues.length} design issue
                          {lint.issues.length !== 1 ? "s" : ""} found
                        </summary>
      ...
      <div className="border-t border-border p-2 relative">
        ...
        <div className="flex items-center gap-1 px-2 mb-1">
          <button
            onClick={() => setChatMode("nod")}
            className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold transition-colors ${chatMode === "nod" ? "bg-accent-green/15 text-accent-green" : "text-text-tertiary hover:text-text-secondary"}`}
          >
            <Stethoscope size={11} /> NOD
          </button>
          <button
            onClick={() => setChatMode("code")}
            className={`flex items-center gap-1 px-2 py-1 rounded text-[10px] font-bold transition-colors ${chatMode === "code" ? "bg-accent-blue/15 text-accent-blue" : "text-text-tertiary hover:text-text-secondary"}`}
          >
            <Code2 size={11} /> Code
          </button>
        </div>
        <div className="flex items-center gap-2 bg-bg-tertiary rounded-lg px-3 py-2">
          ...
          <input
            ...
            placeholder={
              chatMode === "nod"
                ? ko
                  ? "NOD에게 물어보세요... 뭐든 쉽게 설명해드려요"
                  : "Ask NOD anything... I'll explain it simply"
                : "Ask about your code..."
            }
          ...
          {chat.isStreaming ? (
            ...
            <button
              ...
              disabled={!input.trim()}
              aria-label="전송"
              className="text-blue-400 hover:text-white disabled:opacity-30 transition-colors"
            >
              <Send size={14} />
            </button>
          )}
