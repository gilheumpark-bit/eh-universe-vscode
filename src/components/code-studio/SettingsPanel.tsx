...
export function SettingsPanel({
  ...
  const [internalSettings, setInternalSettings] = useState<IDESettings>(
    () => settingsProp ?? loadIDESettings(),
  );
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b border-white/8 px-4 py-3">
        <h2 className="font-mono text-[10px] uppercase tracking-[0.2em] text-accent-purple">
          Settings
        </h2>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-1">
        {tab === "editor" && (
          <>
            ...
            <Row label="Bracket Guides">
              <Toggle
                checked={settings.bracketGuides}
                onChange={(v) => update("bracketGuides", v)}
              />
            </Row>
        ...
        {tab === "ai" && (
          <>
                ...
                {[
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (
                        mode === "pro" &&
                        settings.actionApprovalMode !== "pro"
                      ) {
                ...
                <div className="mt-2 rounded-lg border border-accent-red/40 bg-accent-red/5 p-3 animate-in fade-in slide-in-from-bottom-2 duration-200">
                  <div className="flex items-start gap-2">
                    ...
                    <div className="flex-1 space-y-2">
                      <p className="text-[10px] font-bold text-accent-red">
                        Pro 모드 경고
                      </p>
                      <p className="text-[9px] text-text-secondary leading-relaxed">
                        ...
                        터미널 명령어, 파일 덮어쓰기, 패키지 설치 등을 즉시
                        실행합니다. 예기치 않은 시스템 변경이 발생할 수
                        있습니다.
                      </p>
              ...
              <div className="flex gap-1.5 mt-2">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      setCodingModeState(mode);
                      setCodingMode(mode);
                      localStorage.setItem("eh_coding_mode", mode);
                    }}
                    className={`flex-1 flex flex-col items-center gap-1 py-2.5 px-2 rounded-lg border text-[10px] transition-all duration-200 hover:scale-[1.02] active:scale-95 ${
                      codingMode === mode
                        ? `border-${color}/40 bg-${color}/10 text-${color}`
            ...
            <Row label="Ghost Text">
              <Toggle
                checked={settings.aiGhostText}
                onChange={(v) => update("aiGhostText", v)}
              />
            </Row>
            ...
            <Row label="Auto Suggest Delay (ms)">
              <NumberInput
                value={settings.aiAutoSuggestDelay}
                onChange={(v) => update("aiAutoSuggestDelay", v)}
                min={100}
                max={3000}
                step={100}
              />
        ...
        {tab === "pipeline" && (
          <>
            <Row label="Pass Threshold">
              <NumberInput
                value={settings.pipelinePassThreshold}
                onChange={(v) => update("pipelinePassThreshold", v)}
                min={0}
                max={100}
              />
            </Row>
            <p className="pt-2 text-[10px] text-text-tertiary">
              Score below this threshold will be marked as &quot;fail&quot;.
              Default: 77.
            </p>
          </>
        )}
