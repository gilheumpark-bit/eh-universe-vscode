...
const TEMPLATES: AppTemplate[] = [
  {
    ...
    name: "React 기본",
    framework: "React",
    description: "React + TypeScript 기본 프로젝트",
    files: [
      {
        name: "src/App.tsx",
  ...
  },
  {
    ...
    name: "Next.js 기본",
    framework: "Next.js",
    description: "Next.js 14 App Router 프로젝트",
    files: [
      {
        name: "app/page.tsx",
        content:
  ...
  },
  {
    ...
    name: "React Todo",
    framework: "React",
    description: "할일 관리 앱 (React + Tailwind)",
    files: [
      {
        name: "src/App.tsx",
        content:
  ...
  },
  {
    ...
    name: "Next.js 블로그",
    framework: "Next.js",
    description: "마크다운 기반 블로그 (MDX)",
    files: [
      {
        name: "app/page.tsx",
        content: "export default function Home() { return <main>Blog</main>; }",
      },
    ],
  },
  {
    ...
    name: "Node CLI",
    framework: "Node.js",
    description: "Node.js CLI 도구 템플릿",
    files: [
      {
        name: "src/cli.ts",
...
export function TemplateGallery({ onSelectTemplate, onClose }: Props) {
  ...
  useEffect(() => {
    if (showAiPrompt) aiInputRef.current?.focus();
  ...
  const filtered = useMemo(
    () =>
      TEMPLATES.filter((t) => {
        const matchFw =
          activeFramework === "전체" || t.framework === activeFramework;
        const matchSearch =
          !searchQuery.trim() ||
  ...
  return (
    <div
      ...
      onKeyDown={(e) => {
        ...
        if (e.key === "Enter" && (e.metaKey || e.ctrlKey) && showAiPrompt)
          handleAiGenerate();
      }}
    >
      <div
        className="bg-[#0f1419] border border-white/10 rounded-xl shadow-2xl w-[680px] max-h-[80vh] overflow-hidden flex flex-col"
        ...
        {showAiPrompt && (
          <div className="px-4 py-3 border-b border-white/8 bg-amber-800/5">
            ...
            <textarea
              ...
              className="w-full h-20 p-2 rounded-lg bg-[#0a0e17] border border-white/10 text-sm text-white placeholder:text-white/50 outline-none resize-none"
              disabled={isGenerating}
            />
            <div className="flex items-center justify-between mt-2">
              <span className="text-[10px] text-white/50">
                {isGenerating ? "생성 중..." : "Ctrl+Enter로 생성"}
              </span>
              <button
        ...
        <div className="flex items-center gap-1 px-4 py-2 border-b border-white/8 overflow-x-auto">
          {FRAMEWORK_FILTERS.map((fw) => (
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs whitespace-nowrap transition-colors ${activeFramework === fw ? "bg-amber-900/22 text-amber-400 font-medium" : "text-white/60 hover:bg-white/5"}`}
            >
              {fw}
            </button>
          ))}
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-white/50">
              ...
              <p className="text-xs">일치하는 템플릿이 없습니다.</p>
            </div>
          ) : (
