...
import {
  getReviewChecklist,
  type ChecklistItem,
} from "@/lib/code-studio/pipeline/pipeline-utils";
  ...
  return (
    <div className="p-2 space-y-1 overflow-y-auto" style={{ maxHeight: 200 }}>
      {filtered.map((f, i) => (
        <div
          key={i}
          className="flex items-start gap-2 text-[10px] px-2 py-1 rounded hover:bg-bg-tertiary"
        >
          ...
          {f.line != null && (
            <span className="text-text-tertiary flex-shrink-0">L{f.line}</span>
          )}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="p-2 space-y-2">
      <div className="flex items-center justify-between text-[10px] text-text-tertiary px-2">
        <span>
          Review Checklist ({passCount}/{total})
        </span>
        <span
          className={
            score >= 77
  ...
  return (
    <div className="space-y-1 p-2 max-h-[180px] overflow-y-auto">
      {files.map((file) => (
        <div
          key={file.name}
          className="bg-bg-primary rounded border border-border"
        >
          <button
            onClick={() =>
          ...
          {expanded === file.name && (
            <div className="px-2 pb-2 space-y-1">
              {file.findings.map((f, i) => (
                <div key={i} className="text-[9px] text-text-tertiary pl-4">
                  {f.message}
                </div>
              ))}
              <div className="flex items-center gap-1 pl-4 pt-1">
                <button
                  onClick={() => onApprove?.(file.name)}
                  className="text-[9px] px-2 py-0.5 rounded bg-green-500/15 text-green-400 hover:bg-green-500/25"
                >
                  <ThumbsUp size={8} className="inline mr-1" />
                  Approve
                </button>
                <button
                  onClick={() => onReject?.(file.name)}
                  className="text-[9px] px-2 py-0.5 rounded bg-red-500/15 text-red-400 hover:bg-red-500/25"
                >
                  <ThumbsDown size={8} className="inline mr-1" />
                  Reject
                </button>
              </div>
            </div>
...
export function ReviewCenter({
  ...
  return (
    <div
      className="flex flex-col border-t border-border bg-bg-secondary overflow-hidden"
      style={{ minHeight: 160 }}
    >
      {/* Tab Bar */}
      <div
        className="flex items-center h-8 border-b border-border bg-bg-secondary px-1 gap-0.5 flex-shrink-0"
        role="tablist"
      >
        {TABS.map((tab) => (
          <button
          ...
          >
            ...
            {tab === "problems" && problemCount > 0 && (
              <span className="ml-0.5 px-1.5 py-0 text-[9px] rounded-full bg-red-500/20 text-red-400 font-semibold">
                {problemCount}
              </span>
            )}
          </button>
        ...
        {activeTab === "pipeline" && pipelineResult && (
          ...
          >
            <div className="flex items-center gap-2 text-xs text-text-primary px-2 py-1">
              <span>
                Overall: <strong>{pipelineResult.overallScore}</strong>/100
              </span>
              <span
                className={
            ...
            {pipelineResult.stages.map((s) => (
              ...
              >
                <span className="w-20 text-text-tertiary truncate">
                  {s.stage}
                </span>
                <div className="flex-1 h-1.5 bg-bg-tertiary rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full ${s.score >= 80 ? "bg-accent-green" : s.score >= 60 ? "bg-accent-amber" : "bg-accent-red"}`}
                    style={{ width: `${s.score}%` }}
                  />
                </div>
                <span className="text-text-primary font-mono w-6 text-right">
                  {s.score}
                </span>
              </div>
            ))}
            ...
            {bugScanning ? (
              ...
              <>
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs rounded bg-bg-tertiary hover:bg-border text-text-primary"
                >
                  Scan Again
                </button>
              </>
              ...
              <>
                <Bug size={24} className="text-text-tertiary opacity-40" />
                <p className="text-xs text-text-tertiary">
                  AI-powered bug detection across your codebase
                </p>
                <button
                  onClick={handleBugScan}
                  disabled={!onBugScan}
