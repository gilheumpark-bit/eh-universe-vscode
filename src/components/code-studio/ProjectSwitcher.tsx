        ...
        <div className="absolute top-full left-0 mt-1 w-80 rounded-lg shadow-xl z-50 bg-[#0a0e17] border border-white/10 overflow-hidden">
          ...
          <div className="flex items-center gap-1 p-2 border-b border-white/8">
            <button
              ...
              className="flex items-center gap-1 px-2 py-1 text-xs rounded hover:bg-white/5 text-white/50"
            >
            ...
            <div className="p-3 border-b border-white/8 bg-white/3">
              ...
              <div className="flex gap-2 justify-end">
                <button
                  ...
                  className="px-2 py-1 text-xs rounded hover:bg-white/5 text-white/60"
                >
                  취소
                </button>
                <button
                  ...
                  className="px-3 py-1 text-xs rounded bg-amber-800 text-stone-100 hover:bg-amber-700 disabled:opacity-50"
                >
                  만들기
                </button>
              </div>
          ...
          <div className="max-h-64 overflow-y-auto">
            {!searchQuery && filtered.length > 0 && (
              <div className="px-2 pt-2 pb-1">
                <span className="text-[10px] uppercase tracking-wider text-white/50 font-medium flex items-center gap-1">
                  <Clock className="w-2.5 h-2.5" /> 최근
                </span>
              </div>
            )}
                ...
                {confirmDeleteId === project.id ? (
                  <div className="flex items-center gap-2 px-3 py-2 bg-red-500/10 border-b border-white/8">
                    ...
                    <button
                      ...
                      className="px-2 py-0.5 text-xs rounded hover:bg-white/5 text-white/60"
                    >
                      아니오
                    </button>
                    <button
                  ...
                  >
                    <FolderOpen
                      className={`w-4 h-4 mt-0.5 shrink-0 ${project.id === currentProjectId ? "text-amber-400" : "text-white/50"}`}
                    />
            ...
            {filtered.length === 0 && searchQuery && (
              <div className="px-3 py-4 text-xs text-center text-white/50">
                일치하는 프로젝트 없음
              </div>
            )}
          </div>
