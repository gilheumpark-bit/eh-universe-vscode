...
const StepRow = memo(function StepRow({
  ...
  return (
    <div className="border-l-2 border-[#30363d] pl-3 ml-1">
      <button
        onClick={onToggle}
        className="flex items-center gap-2 w-full text-left py-1 hover:bg-[#21262d]/50 rounded px-1 -ml-1"
      >
        {statusIcon}
        <span className="text-[#8b949e]">{icon}</span>
        <span className="text-xs flex-1 truncate text-[#e6edf3]">
          {step.label}
        </span>
        {step.durationMs != null && (
          <span className="text-[9px] text-[#8b949e]">{step.durationMs}ms</span>
        )}
        {step.output &&
          (expanded ? (
...
const AgentBadge = memo(function AgentBadge({ mode }: { mode: AgentMode }) {
  ...
  return (
    <span
      className={`text-[10px] px-1.5 py-0.5 rounded bg-current/10 ${c.color}`}
    >
      {mode === "executing" && (
...
function ConfidenceBar({ value }: { value: number }) {
  ...
  const color =
    pct >= 80 ? "bg-green-400" : pct >= 50 ? "bg-accent-amber" : "bg-red-400";
  return (
...
export function pickAgentApplyCandidate(
  session: AgentSession | null,
): AgentApplyCandidate | null {
  if (!session) {
    return null;
  }
  ...
  const candidates = session.messages.flatMap((message, messageIndex) =>
    extractCodeBlocks(message.content).map((block, blockIndex) => ({
      ...
      score:
        (CATEGORY_PRIORITY[AGENT_REGISTRY[message.role]?.category ?? ""] || 0) *
          10_000 +
        messageIndex * 100 +
        blockIndex,
    })),
...
export function AgentPanel({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const agentMode =
        localStorage.getItem("eh-cs-agent-mode") || "generate-verify";
      localStorage.removeItem("eh-cs-agent-mode");
      setInput(seeded);
      // 모드별 에이전트 프리셋 적용
      setAgentPreset(
        agentMode === "verify" ? VERIFY_ONLY_ROLES : GENERATE_AND_VERIFY_ROLES,
      );
  ...
  const activeAgentIdx = useMemo(() => {
    ...
    const idx = AGENT_ROLES.findIndex(
      (a) => a.role === agent.progress.currentRole,
    );
    return idx >= 0 ? idx : 0;
  ...
  const handleRun = useCallback(async () => {
    ...
    try {
      ...
      const confidence = Math.round(
        (result.summary?.finalConfidence ?? agent.averageConfidence) * 100,
      );
    ...
    if (applyCandidate) {
      ...
      saga.addStep({
        ...
        compensate: async () => {
          ...
          setSummary(
            L4(lang, {
              ko: "Saga 롤백: 이전 코드로 복원됨",
              en: "Saga rollback: reverted to previous code",
            }),
          );
        },
      });
      ...
      try {
        const { runHarnessLoop, errorsToPrompt } =
          await import("@/lib/code-studio/harness");
        const { createWebContainer } =
          await import("@/lib/code-studio/features/webcontainer");
          ...
          onFixRequest: async (errors, currentCode) => {
            ...
            await agent
              ...
              .then((session) => {
                ...
                if (lastMsg) {
                  const codeMatch = lastMsg.content.match(
                    /```[\s\S]*?\n([\s\S]*?)```/,
                  );
        ...
        if (result.success) {
          ...
          setSummary(
            L4(lang, {
              ko: `정적 검증 통과. 동적 테스트 실행 중...`,
              en: `Static checks passed. Running dynamic tests...`,
            }),
          );
          try {
            const { runDynamicSuite, runFrontendGate1, runFrontendGate2 } =
              await import("@/lib/code-studio/harness");
            ...
            const dynamic = await runDynamicSuite(wc, applyCandidate.code, {
              ...
              onProgress: (gate, status) => {
                setSummary(
                  L4(lang, {
                    ko: `동적 테스트: ${gate} ${status}`,
                    en: `Dynamic: ${gate} ${status}`,
                  }),
                );
              },
            });
            ...
            const totalScore = Math.round(
              (fg1.score + fg2.score + dynamic.totalScore) / 3,
            );
            ...
            setSummary(
              L4(lang, {
                ko: `하네스 통과 (${result.iterations}회)`,
                en: `Harness passed (${result.iterations} iteration(s))`,
              }),
            );
          }
        } else {
        ...
        /* harness is best-effort, don't block */
      }
    }
  ...
  const hasVerification = steps.some(
    (s) =>
      s.label.includes("guard") ||
      s.label.includes("optimizer") ||
      s.label.includes("scanner"),
  );
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-[#30363d]">
        ...
        <div className="flex items-center gap-1">
          {mode === "executing" && (
            ...
            >
              <Pause size={12} className="text-accent-amber" />
            </button>
          )}
          {(mode === "complete" ||
            mode === "error" ||
            ...
            <button
              ...
              className="p-1 hover:bg-[#21262d] rounded"
            >
              <Square size={12} className="text-[#8b949e]" />
            </button>
          )}
        </div>
      </div>
        ...
        {AGENT_ROLES.map((a, i) => (
          <div
            ...
            className={`flex flex-col items-center gap-0.5 px-2 py-1 rounded text-[9px] min-w-[60px] transition-all ${
              i === activeAgentIdx && mode === "executing"
                ? "bg-[#21262d] ring-1 ring-amber-700/35"
                : "bg-[#010409]"
            }`}
          >
        ...
        {mode === "idle" && !session ? (
          <div className="text-center text-[#8b949e] py-8">
            ...
            <p className="text-xs mb-2">
              {L4(lang, {
                ko: "Action Dock (에이전트 조율)",
                en: "Action Dock (Agent Orchestration)",
              })}
            </p>
            <p className="text-[10px] opacity-60">
              {L4(lang, {
                ko: "지능형 팀이 실행할 작업을 설명하세요.",
                en: "Describe a task for the 5-agent team to execute.",
              })}
            </p>
            <div className="mt-4 space-y-1 text-[10px] text-left max-w-[220px] mx-auto">
              <p className="text-green-400">
                {L4(lang, { ko: "예시:", en: "Examples:" })}
              </p>
              <p>
                {L4(lang, {
                  ko: "이 파일을 여러 모듈로 리팩터링하기",
                  en: "Refactor this file into modules",
                })}
              </p>
              <p>
                {L4(lang, {
                  ko: "모든 비동기 호출에 에러 핸들링 추가",
                  en: "Add error handling to all async calls",
                })}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            {steps.length > 10 && !expandedSteps.has("__show_all__") && (
              ...
              >
                {L4(lang, {
                  ko: `+${steps.length - 10}개 이전 스텝 보기`,
                  en: `Show ${steps.length - 10} earlier steps`,
                })}
              </button>
            )}
            {(steps.length <= 10 || expandedSteps.has("__show_all__")
      ...
      <div className="border-t border-[#30363d] p-2">
        <div className="flex items-center gap-2 bg-[#21262d] rounded-lg px-3 py-2">
          ...
          <input
            ...
            placeholder={L4(lang, {
              ko: "에이전트가 수행할 작업을 설명하세요...",
              en: "Describe a task for the agents...",
            })}
          ...
          <button
            onClick={handleRun}
            disabled={!input.trim() || agent.running || mode === "staged"}
