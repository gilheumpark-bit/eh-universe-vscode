...
function TeamStatusIcon({ status }: { status: string }) {
  ...
  return (
    <div
      className="w-3 h-3 rounded-full bg-border animate-pulse"
      aria-label="Pending"
    />
  );
}
...
function StatusBadge({ status, lang }: { status: string; lang: string }) {
  ...
  return (
    <span
      className={`text-[10px] px-1.5 py-0.5 rounded inline-flex items-center gap-1 ${colors}`}
    >
      {icon}
      {text}
    </span>
    ...
    return (
      <div className="h-64 border-t border-border bg-bg-secondary flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-xs text-text-tertiary">
          {L4(lang, {
            ko: "파이프라인 결과 없음",
            en: "No pipeline results yet",
          })}
        </p>
        {onRun && (
          <button
            onClick={onRun}
            className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium rounded-lg bg-amber-800 text-stone-100 hover:opacity-90 transition-opacity"
          >
            <Play size={12} />{" "}
    ...
    return (
      <div className="h-64 border-t border-border bg-bg-secondary flex flex-col items-center justify-center gap-3">
        ...
        {onAbort && (
          <button
            onClick={onAbort}
            className="flex items-center gap-1 px-3 py-1.5 text-xs rounded border border-border text-accent-amber hover:bg-bg-tertiary"
          >
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-border">
        ...
        <span className="flex items-center gap-1">
          {onRun && (
            <button
              onClick={onRun}
              className="p-1 rounded hover:bg-bg-tertiary text-blue-400"
              title={L4(lang, { ko: "다시 실행", en: "Re-run" })}
              aria-label={L4(lang, { ko: "다시 실행", en: "Re-run" })}
            >
              <Play size={12} />
            </button>
          )}
          ...
          <button
            ...
            className="p-1 rounded hover:bg-bg-tertiary text-text-tertiary"
            title={L4(lang, { ko: "보고서 다운로드", en: "Download report" })}
            aria-label={L4(lang, {
              ko: "보고서 다운로드",
              en: "Download report",
            })}
      ...
      <div className="flex-1 overflow-y-auto p-2">
        <div className="grid grid-cols-4 gap-2">
          {result.stages.map((stage) => {
            ...
            const config = TEAM_CONFIG[stageKey] ?? {
              label: stageKey,
              icon: <Shield size={14} />,
              ...
              <div key={stageKey} className="flex flex-col">
                ...
                >
                  <div className="flex items-center gap-1.5 mb-1.5">
                    ...
                    <span className="text-[10px] font-semibold flex-1 text-left text-text-primary">
                      {getTeamLabel(stageKey)}
                    </span>
                    <TeamStatusIcon status={stage.status} />
                  </div>
                  <div className="w-full h-1.5 bg-bg-secondary rounded-full overflow-hidden mb-1">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${stage.score >= 80 ? "bg-accent-green" : stage.score >= 60 ? "bg-accent-amber" : "bg-accent-red"}`}
                  ...
                  <div className="mt-1 p-2 bg-bg-primary border border-border rounded text-[9px] space-y-1 max-h-32 overflow-y-auto">
                    {(stage.findings.length > 10
                      ...
                      <div key={i} className="flex items-start gap-1">
                        ...
                        {f.line != null && (
                          <span className="text-text-tertiary">L{f.line}</span>
                        )}
                      </div>
                    ))}
                    {stage.findings.length > 10 && (
