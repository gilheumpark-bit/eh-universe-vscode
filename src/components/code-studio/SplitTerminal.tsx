...
export function SplitTerminal() {
  ...
  const closePane = useCallback(
    (id: string) => {
      setPanes((p) => {
        ...
        if (next.length === 0)
          return [{ id: "t1", name: `${t.termShellLabel} 1` }];
  ...
  const handleResize = (e: React.MouseEvent) => {
    ...
    const onMove = (me: MouseEvent) => {
      ...
      const delta =
        splitDir === "horizontal" ? me.clientX - startX : me.clientY - startY;
  ...
  return (
    <div className="flex flex-col h-48 border-t border-white/8">
      <div className="flex items-center bg-[#0f1419] border-b border-white/8 px-1 shrink-0">
        ...
        <button
          onClick={toggleSplit}
          className="p-1 text-white/60 hover:text-white hover:bg-white/5 rounded ml-auto"
          title="분할"
          aria-label="분할 방향 전환"
        >
          {splitDir === "horizontal" ? (
      ...
      <div
        className={`flex-1 overflow-hidden flex ${splitDir === "vertical" ? "flex-col" : "flex-row"}`}
      >
        {splitDir === "none" ? (
          panes.map((p) => (
            <div
              ...
              className={
                p.id === activeId
                  ? "flex-1 bg-[#0a0e17] p-2 font-mono text-xs text-green-400"
                  : "hidden"
              }
            >
          ...
          <>
            <div
              ...
              className="overflow-hidden bg-[#0a0e17] p-2 font-mono text-xs text-green-400"
            >
            ...
            <div
              className={`${splitDir === "horizontal" ? "w-1 cursor-col-resize" : "h-1 cursor-row-resize"} bg-white/10 hover:bg-amber-800/50 transition-colors`}
