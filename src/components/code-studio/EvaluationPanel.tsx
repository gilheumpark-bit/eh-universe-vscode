...
function evaluateFiles(files: FileNode[]): EvalScore {
  ...
  return {
    overall: base,
    grade: base >= 90 ? "S" : base >= 80 ? "A" : base >= 70 ? "B" : "C",
    summary: `${totalLines} lines analyzed`,
    categories: {
      codeQuality: base + 2,
      trendAlignment: base - 5,
      marketReadiness: base - 3,
      maintainability: base + 1,
      scalability: base - 2,
    },
    recommendations: [
      "타입 안전성 강화",
...
export function EvaluationPanel({ files, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#0a0e17] border border-white/10 rounded-xl shadow-2xl flex flex-col"
        style={{ width: 520, maxHeight: "80vh" }}
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-white/8">
          <div className="flex items-center gap-2 text-sm font-semibold text-white">
            <BarChart3 size={16} /> 프로젝트 평가
          </div>
          <button
            onClick={onClose}
            aria-label="닫기"
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {!result && !loading && (
            <div className="text-center py-8">
              <Trophy
                size={48}
                className="mx-auto mb-4 opacity-30 text-white/50"
              />
              <p className="text-sm text-white/50 mb-4">
                프로젝트 코드 품질, 시장 적합도, 유지보수성을 평가합니다
              </p>
              <button
                onClick={handleRun}
                className="px-4 py-2 rounded-lg text-sm font-medium bg-amber-800 text-stone-100 hover:bg-amber-700"
              >
                평가 시작
              </button>
            </div>
          ...
          {error && (
            <div className="text-sm text-red-400 p-3 rounded bg-white/5">
              오류: {error}
            </div>
          )}
          {result && (
            <>
              ...
              <ScoreBar
                label="코드 품질"
                value={result.categories.codeQuality}
                icon={<Shield size={12} />}
              />
              <ScoreBar
                label="트렌드 적합도"
                value={result.categories.trendAlignment}
                icon={<TrendingUp size={12} />}
              />
              <ScoreBar
                label="시장 준비도"
                value={result.categories.marketReadiness}
                icon={<BarChart3 size={12} />}
              />
              <ScoreBar
                label="유지보수성"
                value={result.categories.maintainability}
                icon={<Wrench size={12} />}
              />
              <ScoreBar
                label="확장성"
                value={result.categories.scalability}
              ...
              <div className="mt-4 text-center">
                <button
                  onClick={handleRun}
                  disabled={loading}
                  className="px-3 py-1.5 text-xs rounded bg-white/5 hover:bg-white/10 text-white/50 transition-colors"
                >
                  재평가
                </button>
              </div>
