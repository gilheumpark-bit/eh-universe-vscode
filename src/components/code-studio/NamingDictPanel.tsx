      ...
      for (const id of identifiers) {
        if (
          ...
          violations.push({
            line: i + 1,
            text: id,
            rule: `${rule.pattern}: ${rule.convention}`,
          });
          break;
        }
        if (
          convLower.includes("start with use") &&
...
export function NamingDictPanel({ activeFileContent = "" }: Props) {
  ...
  const addPattern = useCallback(() => {
    ...
    persist({
      ...dict,
      patterns: [...dict.patterns, { pattern: p, convention: c }],
    });
    setNewPattern("");
    setNewPatternConv("");
  ...
  const filteredTerms = dict.terms.filter(
    (r) =>
      ...
      r.convention.toLowerCase().includes(searchQuery.toLowerCase()),
  );
          ...
          <>
            ...
            {filteredTerms.map((rule, i) => {
              ...
              return editIdx === realIdx ? (
                ...
                <div
                  key={i}
                  className="group relative flex flex-col gap-1 p-3 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 hover:border-white/10 transition-all"
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button
                        onClick={() => startEditTerm(realIdx)}
                        className="p-1 rounded hover:bg-white/10 text-text-tertiary hover:text-text-secondary"
                      >
                  ...
                  <span className="text-[13px] text-accent-cyan/90">
                    {rule.convention}
                  </span>
                </div>
              );
          ...
          <>
            ...
            {violations.length === 0 && (
              <div className="px-4 py-6 text-[12px] text-text-tertiary italic text-center bg-white/2 rounded-lg border border-white/3">
                {activeFileContent
                  ? "No violations found."
                  : "Open a file to scan for violations."}
              </div>
            )}
            {violations.map((v, i) => (
              <div
                key={i}
                className="flex flex-col gap-1 p-3 rounded-lg bg-amber-500/5 border border-amber-500/20"
              >
        ...
        <div className="p-3 shrink-0 border-t border-white/5 pointer-events-auto space-y-2">
          <div className="grid grid-cols-1 gap-2">
            ...
            <input
              ...
              placeholder='Convention (e.g. "userId")'
              className="w-full bg-black/40 border border-white/10 rounded-md py-1.5 px-3 text-[12px] text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent-cyan/40"
            />
          </div>
          <button
        ...
        <div className="p-3 shrink-0 border-t border-white/5 pointer-events-auto space-y-2">
          <div className="grid grid-cols-1 gap-2">
            <input
              ...
              placeholder='Scope (e.g. "components")'
              className="w-full bg-black/40 border border-white/10 rounded-md py-1.5 px-3 text-[12px] text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent-cyan/40"
            />
            <input
              value={newPatternConv}
              onChange={(e) => setNewPatternConv(e.target.value)}
              placeholder='Convention (e.g. "PascalCase")'
              className="w-full bg-black/40 border border-white/10 rounded-md py-1.5 px-3 text-[12px] text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent-cyan/40"
            />
          </div>
          <button
