...
describe("Rule 1: CR_NO_SEMANTIC_TOKENS", () => {
  it("passes when semantic tokens are used", () => {
    const code =
      '<div className="bg-bg-primary text-text-primary p-4">Hello</div>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "CR_NO_SEMANTIC_TOKENS"),
    ).toBeUndefined();
  });

  it("warns when no semantic tokens in long code", () => {
    const code =
      '<div className="flex items-center justify-center p-8 m-4 rounded-lg border shadow-md hover:opacity-80 transition-colors duration-200">Long code without semantic tokens at all here</div>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "CR_NO_SEMANTIC_TOKENS"),
    ).toBeDefined();
...
describe("Rule 3: YELLOW_TEXT_ON_YELLOW", () => {
  it("catches yellow text on yellow bg", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "YELLOW_TEXT_ON_YELLOW"),
    ).toBeDefined();
  });

  it("passes with dark text on yellow bg", () => {
    const code =
      '<div className="bg-amber-300 text-text-primary">Warning</div>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "YELLOW_TEXT_ON_YELLOW"),
    ).toBeUndefined();
...
describe("Rule 4: OUTLINE_NONE", () => {
  it("catches outline:none", () => {
    const code =
      ".input { outline: none; border: 1px solid var(--color-border); }";
    const result = runDesignLint(code);
  ...
  it("passes without outline override", () => {
    const code =
      '<button className="bg-bg-primary text-text-primary">OK</button>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "OUTLINE_NONE"),
    ).toBeUndefined();
  });
});
...
describe("Rule 5: COLOR_ONLY_STATUS", () => {
  it("warns when color-only status (no icon, no role)", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "COLOR_ONLY_STATUS"),
    ).toBeDefined();
  });

  it("passes with icon companion", () => {
    const code =
      '<span className="text-accent-red"><AlertTriangle aria-hidden="true" /> Error</span>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "COLOR_ONLY_STATUS"),
    ).toBeUndefined();
  ...
  it('passes with role="alert"', () => {
    const code =
      '<span className="text-accent-green" role="status">Success</span>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "COLOR_ONLY_STATUS"),
    ).toBeUndefined();
...
describe("Rule 6: NON_4_MULTIPLE_SPACING", () => {
  it("catches 15px", () => {
    ...
    const issue = result.issues.find(
      (i) => i.rule === "NON_4_MULTIPLE_SPACING",
    );
  ...
  it("catches 13px", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "NON_4_MULTIPLE_SPACING"),
    ).toBeDefined();
  });
...
describe("Rule 7: HEX_HARDCODED", () => {
  it("catches hex in inline style", () => {
    const code =
      '<div style={{ color: "#FF0000", background: "#1E1E1E" }}>Text</div>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "HEX_HARDCODED_STYLE"),
    ).toBeDefined();
  });
  ...
  it("catches hex in Tailwind arbitrary class", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "HEX_HARDCODED_CLASS"),
    ).toBeDefined();
  });
...
describe("Rule 8: TOUCH_TARGET_SMALL", () => {
  it("warns when button has no min-height", () => {
    const code =
      '<button className="px-2 py-1 text-xs bg-bg-primary">Small</button>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "TOUCH_TARGET_SMALL"),
    ).toBeDefined();
  });

  it("passes with min-h-[44px]", () => {
    const code =
      '<button className="min-h-[44px] px-4 py-2 bg-bg-primary">Big</button>';
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "TOUCH_TARGET_SMALL"),
    ).toBeUndefined();
...
describe("Rule 9: TRANSITION_ALL", () => {
  it("catches transition: all 0.3s", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "TRANSITION_ALL"),
    ).toBeDefined();
  });

  it("passes with specific transition", () => {
    const code =
      ".btn { transition: background-color var(--transition-normal); }";
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "TRANSITION_ALL"),
    ).toBeUndefined();
...
describe("Rule 10: ARBITRARY_ZINDEX", () => {
  it("catches z-index: 9999", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "ARBITRARY_ZINDEX"),
    ).toBeDefined();
  });
...
describe("Rule 11: COLOR_BLIND_PAIR", () => {
  it("warns red+green without icon differentiation", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "COLOR_BLIND_PAIR"),
    ).toBeDefined();
  });
...
describe("Rule 12: TOUCH_TARGET_PRECISE", () => {
  it("warns on button with p-1 and no min-h", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "TOUCH_TARGET_PRECISE"),
    ).toBeDefined();
  });
...
describe("Rule 13: DUAL_THEME_CR", () => {
  it("passes text-text-primary on bg-bg-primary (high CR both themes)", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "DUAL_THEME_CR_FAIL"),
    ).toBeUndefined();
  });

  it("warns text-text-tertiary on bg-bg-tertiary (low CR)", () => {
    const code =
      '<div className="bg-bg-tertiary text-text-tertiary">Faded</div>';
    const result = runDesignLint(code);
...
describe("Rule 14: Responsive overflow", () => {
  ...
  it("info when no responsive classes in long code", () => {
    const lines = Array.from(
      { length: 20 },
      (_, i) => `<div className="bg-bg-primary p-4">Line ${i}</div>`,
    );
    const code = lines.join("\n");
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "NO_RESPONSIVE_CLASSES"),
    ).toBeDefined();
  });
});
...
describe("Bonus: Component reuse", () => {
  it("suggests premium-button for complex custom button", () => {
    ...
    const result = runDesignLint(code);
    expect(
      result.issues.find((i) => i.rule === "COMPONENT_REBUILD"),
    ).toBeDefined();
  });
  ...
  it("suggests ds-input for styled input", () => {
    const code =
      '<input className="w-full px-3 py-2 border rounded-lg focus:border-amber-500 bg-bg-secondary" />';
    const result = runDesignLint(code);
...
describe("Scoring", () => {
  it("perfect score for clean code", () => {
    const code =
      '<div className="bg-bg-primary text-text-primary p-4 min-h-[44px]">Clean</div>';
    const result = runDesignLint(code);
  ...
  it("deducts 15 per error", () => {
    const code =
      '<div style={{ color: "#FF0000" }} className="bg-[#000000]">Bad</div>';
    const result = runDesignLint(code);
...
describe("formatDesignLintReport", () => {
  it("returns summary for clean code", () => {
    const result = runDesignLint(
      '<div className="bg-bg-primary text-text-primary">OK</div>',
    );
