...
const INTENT_PATTERNS: { type: IntentType; patterns: RegExp[] }[] = [
  ...
  {
    type: "repair",
    patterns: [/(?:수정|고치|fix|repair|bug|버그|에러|error|오류|patch)\b/i],
  },
  {
    type: "review",
    patterns: [/(?:리뷰|검토|review|analyze|분석|check|확인|audit)\b/i],
  },
  {
    type: "refactor",
    patterns: [/(?:리팩토링|refactor|개선|optimize|최적화|정리|cleanup)\b/i],
  },
  {
    type: "explain",
    patterns: [/(?:설명|explain|이해|understand|뭐|what|어떻게|how|왜|why)\b/i],
  },
];
...
export function runTeam1Simulation(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
  ...
  if (cyclomatic > 20) {
    findings.push({
      severity: "major",
      message: `순환 복잡도 ${cyclomatic} (매우 높음)`,
      rule: "HIGH_CYCLOMATIC",
    });
  } else if (cyclomatic > 10) {
    findings.push({
      severity: "minor",
      message: `순환 복잡도 ${cyclomatic} (높음)`,
      rule: "MOD_CYCLOMATIC",
    });
  }
  ...
  if (cognitive > 30) {
    findings.push({
      severity: "major",
      message: `인지 복잡도 ${cognitive} (매우 높음)`,
      rule: "HIGH_COGNITIVE",
    });
  } else if (cognitive > 15) {
    findings.push({
      severity: "minor",
      message: `인지 복잡도 ${cognitive} (높음)`,
      rule: "MOD_COGNITIVE",
    });
  }
  ...
  if (importCount > 10) {
    findings.push({
      severity: "minor",
      message: `다수 의존성 (${importCount}개 import)`,
      rule: "MANY_IMPORTS",
    });
  }
...
export function runTeam2Generation(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/:\s*any\b/.test(stripped) || /<any>/.test(stripped)) {
      findings.push({
        severity: "major",
        message: `'any' 타입 사용 감지`,
        line: i + 1,
        rule: "TYPE_SAFETY_ANY",
      });
    }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (classMatch && /^[a-z]/.test(classMatch[1])) {
      findings.push({
        severity: "minor",
        message: `클래스명 소문자 시작: ${classMatch[1]}`,
        line: i + 1,
        rule: "NAMING_CLASS",
      });
    }
  ...
  for (let i = 0; i < lines.length - 2; i++) {
    ...
    if (prev !== undefined && i - prev > 3) {
      findings.push({
        severity: "minor",
        message: `중복 코드 블록 (L${prev + 1}과 유사)`,
        line: i + 1,
  ...
  const avgLen =
    totalFunctions > 0
      ? Math.round(fns.reduce((s, f) => s + f.bodyLines, 0) / totalFunctions)
      : 0;
...
const VALIDATION_RULES: {
  ...
  },
  {
    ...
    severity: "critical",
    message: "하드코딩된 API 키/토큰",
    rule: "NO_HARDCODED_KEY",
  },
  {
    pattern: /\bTODO\b|\bFIXME\b|\bHACK\b|\bXXX\b/,
    severity: "major",
    message: "TODO/FIXME/HACK 잔존",
    rule: "NO_TODO",
  },
  {
    pattern: /debugger\s*;?/,
    severity: "major",
    message: "debugger 문 잔존",
    rule: "NO_DEBUGGER",
  },
];
...
function checkNullSafety(code: string): Finding[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    const stripped = lines[i].replace(/"[^"]*"|'[^']*'|`[^`]*`/g, '""');
    if (
      /\.(find|querySelector|getElementById)\s*\([^)]*\)\s*\./.test(stripped)
    ) {
      if (
        !/\.(find|querySelector|getElementById)\s*\([^)]*\)\s*\?\./.test(
          stripped,
        )
      ) {
        findings.push({
          severity: "major",
          message: "null 가능 반환값에 직접 속성 접근",
          line: i + 1,
...
const NODE_ONLY_APIS = [
  ...
  {
    pattern: /\bimport\b.*\bfrom\s+['"]fs['"]/,
    api: "fs",
    rule: "NODE_ONLY_FS",
  },
  {
    pattern: /\bimport\b.*\bfrom\s+['"]child_process['"]/,
    api: "child_process",
    rule: "NODE_ONLY_CP",
  },
...
const BROWSER_ONLY_APIS = [
  {
    pattern: /\bdocument\.(getElementById|querySelector|createElement)\b/,
    api: "document DOM",
    rule: "BROWSER_ONLY_DOM",
  },
  {
    pattern: /\bwindow\.(location|history|navigator|innerWidth|innerHeight)\b/,
    api: "window.*",
    rule: "BROWSER_ONLY_WINDOW",
  },
  {
    pattern: /\blocalStorage\b|\bsessionStorage\b/,
    api: "localStorage/sessionStorage",
    rule: "BROWSER_ONLY_STORAGE",
  },
  {
    pattern: /\balert\s*\(|\bconfirm\s*\(|\bprompt\s*\(/,
    api: "alert/confirm/prompt",
    rule: "BROWSER_ONLY_DIALOG",
  },
];
...
function checkRuntimeCompat(code: string, lines: string[]): Finding[] {
  ...
  const isComponent =
    /\bexport\s+(?:default\s+)?function\s+[A-Z]/.test(code) ||
    /\bexport\s+default\s+[A-Z]/.test(code);
    ...
    for (let i = 0; i < lines.length; i++) {
      for (const { pattern, api, rule } of NODE_ONLY_APIS) {
        if (pattern.test(lines[i]) && rule !== "NODE_PROCESS_ENV") {
          findings.push({
            severity: "major",
            message: `클라이언트 코드에서 Node.js API '${api}' 사용`,
            line: i + 1,
            rule,
          });
        }
    ...
    for (let i = 0; i < lines.length; i++) {
      for (const { pattern, api, rule } of BROWSER_ONLY_APIS) {
        if (pattern.test(lines[i])) {
          findings.push({
            severity: "major",
            message: `서버 코드에서 브라우저 API '${api}' 사용`,
            line: i + 1,
            rule,
          });
        }
    ...
    for (let i = 0; i < lines.length; i++) {
      if (/\bwindow\b/.test(lines[i]) && !/typeof\s+window/.test(lines[i])) {
        ...
        if (!/typeof\s+window/.test(prevLines)) {
          findings.push({
            severity: "minor",
            message: "SSR 환경에서 typeof 가드 없이 window 접근",
            line: i + 1,
            rule: "SSR_WINDOW_GUARD",
          });
          break; // 한 번만 보고
        }
      }
...
export function runTeam3Validation(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
...
export function runTeam4SizeDensity(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
  ...
  if (blankRatio > 0.4) {
    findings.push({
      severity: "minor",
      message: `빈 줄 비율 ${(blankRatio * 100).toFixed(0)}% — 코드 밀도 낮음`,
      rule: "SPARSE_CODE",
    });
  } else if (blankRatio < 0.05 && totalLines > 30) {
    findings.push({
      severity: "minor",
      message: `빈 줄 비율 ${(blankRatio * 100).toFixed(0)}% — 코드 너무 밀집`,
      rule: "DENSE_CODE",
    });
  }

  const avgLineLen =
    nonBlank.reduce((s, l) => s + l.length, 0) / Math.max(1, nonBlank.length);
  if (avgLineLen > 100) {
    findings.push({
      severity: "minor",
      message: `평균 줄 길이 ${avgLineLen.toFixed(0)}자 — 줄 바꿈 권장`,
      rule: "LONG_LINES",
    });
  }
...
export function runTeam5AssetTrace(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
  ...
  if (importLines.length > 15) {
    findings.push({
      severity: "minor",
      message: `import ${importLines.length}개 — 모듈 분리 권장`,
      rule: "TOO_MANY_IMPORTS",
    });
  }
  if (externalImports.length > 10) {
    findings.push({
      severity: "minor",
      message: `외부 의존성 ${externalImports.length}개 — 번들 크기 주의`,
      rule: "HEAVY_EXTERNAL_DEPS",
    });
  }
  ...
  if (exportCount > 15) {
    findings.push({
      severity: "minor",
      message: `export ${exportCount}개 — 모듈 분리 권장`,
  ...
  if (
    nonEmpty.length > 0 &&
    exportOnlyLines.length === nonEmpty.length &&
    exportOnlyLines.length > 5
  ) {
    findings.push({
      severity: "minor",
      message: `배럴 파일 감지 (${exportOnlyLines.length}개 re-export)`,
      rule: "BARREL_FILE",
    });
  }
  ...
  for (const [base, paths] of importPaths) {
    ...
    if (
      ...
      uniquePaths.some((p) => p.startsWith(".")) &&
      uniquePaths.some((p) => p.startsWith("@"))
    ) {
      findings.push({
        severity: "minor",
        message: `모듈 '${base}' 다중 경로 import (상대+절대 혼용)`,
        rule: "MIXED_IMPORT_PATHS",
      });
    }
  }
...
export function runTeam6Stability(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
  ...
  for (let i = 0; i < lines.length; i++) {
    const chainMatch = lines[i].match(
      /\.(sort|filter|map|reduce|flatMap|find|every|some)\s*\(/g,
    );
    if (chainMatch && chainMatch.length >= 3) {
      findings.push({
        severity: "minor",
        message: `배열 메서드 체인 ${chainMatch.length}단계`,
        line: i + 1,
        rule: "ARRAY_CHAIN",
      });
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/setInterval\s*\(/.test(lines[i]) && !/clearInterval/.test(code)) {
      findings.push({
        severity: "major",
        message: "setInterval without clearInterval — 메모리 누수 가능",
        line: i + 1,
    ...
    if (
      /addEventListener\s*\(/.test(lines[i]) &&
      !/removeEventListener/.test(code)
    ) {
      findings.push({
        severity: "minor",
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      /\w+=\{\s*\{/.test(trimmed) &&
      /<\w/.test(lines[Math.max(0, i - 3)]?.trim() ?? "")
    ) {
      findings.push({
        severity: "minor",
        message: "JSX prop에 인라인 객체 — 불필요 리렌더 유발 가능",
        line: i + 1,
        rule: "INLINE_OBJECT_PROP",
      });
    }
    if (
      /\w+=\{\s*\(\)\s*=>/.test(trimmed) ||
      /\w+=\{\s*function\s*\(/.test(trimmed)
    ) {
      findings.push({
        severity: "minor",
        message: "JSX prop에 인라인 함수 — useCallback 권장",
        line: i + 1,
        rule: "INLINE_FUNCTION_PROP",
      });
    }
  }
    ...
    if (effectMatch) {
      ...
      if (depsMatch) {
        ...
        if (deps >= 6) {
          findings.push({
            severity: "minor",
            message: `useEffect 의존성 ${deps}개 — 분리 또는 커스텀 훅 권장`,
            line: i + 1,
            rule: "EFFECT_DEPS_OVERLOAD",
          });
        }
      }
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    const trimmed = lines[i].trim();
    if (
      /^(?:const|let)\s+\w+\s*=\s*\w+\.(?:filter|map|sort|reduce)\s*\(/.test(
        trimmed,
      )
    ) {
      ...
      if (!/useMemo/.test(prevLines) && !/useCallback/.test(prevLines)) {
        ...
        if (/return\s*\(?\s*</.test(belowBlock)) {
          findings.push({
            severity: "minor",
            message:
              "컴포넌트 내 배열 변환 — useMemo 미적용 시 렌더마다 재계산",
            line: i + 1,
            rule: "RENDER_BLOCKING_COMPUTE",
          });
        }
      }
    }
    ...
    if (/\bfor\s*\(|for\s+.*\bof\b/.test(trimmed)) {
      ...
      if (awaitCount >= 2) {
        findings.push({
          severity: "major",
          message:
            "루프 내 직렬 await — Promise.all 또는 Promise.allSettled 권장",
          line: i + 1,
...
const SECRET_PATTERNS: { pattern: RegExp; message: string }[] = [
  ...
  {
    pattern: /(?:aws_access_key_id|aws_secret)\s*[:=]/i,
    message: "AWS 자격증명 감지",
  },
];
...
const DB_CONN_PATTERNS: { pattern: RegExp; message: string }[] = [
  {
    ...
    message: "MongoDB 연결 문자열 감지",
  },
  {
    pattern: /postgres(?:ql)?:\/\/[^\s"']+/i,
    message: "PostgreSQL 연결 문자열 감지",
  },
  { pattern: /mysql:\/\/[^\s"']+/i, message: "MySQL 연결 문자열 감지" },
];

export function runTeam7ReleaseIP(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
  ...
  const hasLicense = /licen[sc]e|copyright|©|\(c\)/i.test(
    lines.slice(0, 10).join("\n"),
  );
  if (!hasLicense && lines.length > 20) {
    findings.push({
      severity: "minor",
      message: "[라이선스] 라이선스 선언 누락",
  ...
  const score = Math.max(
    0,
    100 - criticals * 30 - (findings.length - criticals) * 5,
  );
...
export function runTeam8Governance(
  code: string,
  _language: string,
  _fileName: string,
): TeamResult {
  ...
  if (maxNesting > GOVERNANCE_BUDGET.maxNesting) {
    findings.push({
      severity: "major",
      message: `네스팅 깊이 ${maxNesting} > 최대 ${GOVERNANCE_BUDGET.maxNesting}`,
      rule: "NESTING_DEPTH",
    });
  }
  ...
  for (const fn of fns) {
    ...
    if (fn.bodyLines > GOVERNANCE_BUDGET.maxLinesPerFunction) {
      findings.push({
        severity: "major",
        message: `함수 ${fn.name}: ${fn.bodyLines}줄 > 제한 ${GOVERNANCE_BUDGET.maxLinesPerFunction}줄`,
        line: fn.line,
    ...
    if (fn.params > GOVERNANCE_BUDGET.maxParams) {
      findings.push({
        severity: "minor",
        message: `함수 ${fn.name}: 파라미터 ${fn.params}개 > 제한 ${GOVERNANCE_BUDGET.maxParams}개`,
        line: fn.line,
        rule: "PARAM_COUNT",
      });
    }
  ...
  if (cc > GOVERNANCE_BUDGET.maxCyclomatic) {
    findings.push({
      severity: "major",
      message: `순환 복잡도 ${cc} > 제한 ${GOVERNANCE_BUDGET.maxCyclomatic}`,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (classMatch && !/^[A-Z][a-zA-Z0-9]*$/.test(classMatch[1])) {
      findings.push({
        severity: "minor",
        message: `클래스명 '${classMatch[1]}' PascalCase 위반`,
        line: i + 1,
    ...
    if (typeMatch && !/^[A-Z][a-zA-Z0-9]*$/.test(typeMatch[1])) {
      findings.push({
        severity: "minor",
        message: `타입명 '${typeMatch[1]}' PascalCase 위반`,
        line: i + 1,
  ...
  if (hasLegacy) {
    findings.push({
      severity: "minor",
      message: "레거시 패턴 감지 (var / module.exports)",
      rule: "LEGACY_PATTERN",
    });
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/style\s*=\s*\{/.test(trimmed) && /#[0-9a-fA-F]{3,8}\b/.test(trimmed)) {
      ...
      if (hardcodedColorCount <= 2) {
        findings.push({
          severity: "minor",
          message: "인라인 스타일에 하드코딩 색상 — CSS 변수 권장",
          line: i + 1,
          rule: "HARDCODED_COLOR_INLINE",
        });
      }
    }
    // className에 색상 하드코딩 (tailwind zinc-500 등은 허용, #hex는 비허용)
    if (
      /(?:color|background|border)\s*:\s*['"]?#[0-9a-fA-F]{3,8}/.test(trimmed)
    ) {
      hardcodedColorCount++;
      if (hardcodedColorCount <= 2) {
        findings.push({
          severity: "minor",
          message: "CSS 속성에 하드코딩 hex 색상",
          line: i + 1,
  ...
  if (hardcodedColorCount > 2) {
    findings.push({
      severity: "major",
      message: `하드코딩 색상 ${hardcodedColorCount}건 — 테마 시스템 미적용 우려`,
      rule: "HARDCODED_COLOR_BULK",
    });
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/console\.\w+\s*\(/.test(trimmed)) {
      if (
        /(?:password|token|secret|apiKey|api_key|bearer|credential)/i.test(
          trimmed,
        )
      ) {
        findings.push({
          severity: "major",
          message: "민감정보 포함 가능한 console 출력",
          line: i + 1,
  ...
  const errorLogCount = (code.match(/console\.(?:error|warn)\s*\(/g) ?? [])
    .length;
  if (logCount > 5 && errorLogCount === 0) {
    findings.push({
      severity: "minor",
      message: `console.log ${logCount}회, error/warn 0회 — 로그 레벨 미분류`,
      rule: "LOG_LEVEL_MISSING",
    });
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (
      /\.stack\b/.test(lines[i]) &&
      /(?:res|response|json|send|render)/.test(lines[i])
    ) {
      findings.push({
        severity: "major",
        message: "에러 스택 트레이스가 응답에 노출될 수 있음",
        line: i + 1,
  ...
  const baseTrust =
    Math.max(0, 100 - findings.length * 15 - todoCount * 5) / 100;
  const trustState =
    baseTrust < 0.3 ? "untrusted" : baseTrust < 0.6 ? "degraded" : "trusted";

  if (trustState === "untrusted") {
    findings.push({
      severity: "critical",
      message: `신뢰도 저하: ${trustState} (${(baseTrust * 100).toFixed(0)}%)`,
      rule: "TRUST_DEGRADATION",
    });
  } else if (trustState === "degraded") {
    findings.push({
      severity: "major",
      message: `신뢰도 주의: ${trustState} (${(baseTrust * 100).toFixed(0)}%)`,
      rule: "TRUST_DEGRADATION",
    });
  }
  ...
  return {
    ...
    metrics: {
      ...
      trustScore: Math.round(baseTrust * 100),
    },
  };
}
