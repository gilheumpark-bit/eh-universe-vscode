...
const SCENARIOS: FailureSimulation[] = [
  {
    ...
    injectionPoint: "db/api calls",
  },
  {
    scenario: "api-timeout",
    label: "API Timeout",
    description: "External API takes 30+ seconds",
    injectionPoint: "fetch/axios calls",
  },
  {
    scenario: "memory-leak",
    label: "Memory Leak",
    description: "Unbounded data accumulation",
    injectionPoint: "event listeners, caches",
  },
  {
    scenario: "null-data",
    label: "Null Data",
    description: "API returns null/undefined",
    injectionPoint: "response handlers",
  },
  {
    scenario: "auth-expired",
    label: "Auth Expired",
    description: "Token/session expired mid-use",
    injectionPoint: "auth middleware",
  },
  {
    scenario: "network-partition",
    label: "Network Split",
    description: "Intermittent connectivity",
    injectionPoint: "all network calls",
  },
  {
    scenario: "high-load",
    label: "High Load",
    description: "100x normal traffic",
    injectionPoint: "request handlers",
  },
  {
    scenario: "race-condition",
    label: "Race Condition",
    description: "Concurrent state mutations",
    injectionPoint: "shared state",
  },
  {
    scenario: "cascade-failure",
    label: "Cascade Failure",
    description: "One service failure triggers others",
    injectionPoint: "service dependencies",
  },
  {
    scenario: "config-corruption",
    label: "Config Corrupt",
    description: "Env vars missing or invalid",
    injectionPoint: "config loading",
  },
  {
    scenario: "data-corruption",
    label: "Data Corrupt",
    description: "Malformed data in storage",
    injectionPoint: "data parsing",
  },
  {
    scenario: "disk-full",
    label: "Disk Full",
    description: "Storage write failures",
    injectionPoint: "file/log writes",
  },
];
...
function computeResilienceMetrics(code: string): ResilienceMetrics {
  ...
  const catchWithoutHandler = (
    code.match(/catch\s*(?:\([^)]*\))?\s*\{\s*\}/g) ?? []
  ).length;
  ...
  const errorBoundaryCount = (
    code.match(/ErrorBoundary|componentDidCatch|onError/g) ?? []
  ).length;
  const retryPatterns = (
    code.match(/retry|backoff|maxRetries|maxAttempts/gi) ?? []
  ).length;
  const fallbackPatterns = (
    code.match(/fallback|default[A-Z]|placeholder|skeleton/gi) ?? []
  ).length;
  const resourceCleanup = (
    code.match(
      /\bfinally\b|cleanup|dispose|removeEventListener|clearTimeout|clearInterval/g,
    ) ?? []
  ).length;
...
function buildResilienceBlock(m: ResilienceMetrics): string {
  ...
  if (m.fetchWithoutTimeout > 0)
    warnings.push(
      `⚠ timeout 없는 fetch ${m.fetchWithoutTimeout}건 — 무한 대기 위험`,
    );
