...
/** 코드에서 빈깡통 패턴 감지 */
export function scanForHollowCode(
  code: string,
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      /(?:function\s+\w+|(?:const|let|var)\s+\w+\s*=\s*(?:async\s+)?(?:\([^)]*\)|[a-zA-Z_$]\w*)\s*=>)\s*\{\s*\}/.test(
        trimmed,
      )
    ) {
      const nameMatch = trimmed.match(
        /(?:function\s+(\w+)|(?:const|let|var)\s+(\w+))/,
      );
      findings.push({
        type: "empty-function",
        file: fileName,
    ...
    if (
      /^\s*return\s+(null|undefined|\{\s*\}|\[\s*\])\s*;?\s*$/.test(trimmed)
    ) {
    ...
    if (/\/\/\s*TODO|\/\/\s*FIXME|\/\/\s*HACK|\/\/\s*XXX/.test(trimmed)) {
      ...
      if (
        /^return\s+(null|undefined|\{\}|\[\])/.test(nextLine) ||
        nextLine === "}" ||
        nextLine === ""
      ) {
        findings.push({
    ...
    if (fnMatch) {
      ...
      const bodyLines = lines
        .slice(i + 1, Math.min(i + 30, lines.length))
        .join(" ");
...
/** 여러 파일을 스캔하고 결과 합산 */
export function scanProjectForHollowCode(
