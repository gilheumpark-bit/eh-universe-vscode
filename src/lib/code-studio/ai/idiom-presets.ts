...
const DETECTION_SIGNALS: DetectionSignal[] = [
  // Next.js signals (check before React — Next.js IS React)
  {
    ...
    contentPattern: /"next"\s*:/,
    framework: "nextjs",
    weight: 10,
  },
  {
    filePattern: (n) => n.endsWith(".tsx") || n.endsWith(".ts"),
    contentPattern: /['"]use client['"]|['"]use server['"]/,
    framework: "nextjs",
    weight: 8,
  },
  {
    filePattern: (n) =>
      /^(page|layout|loading|error|not-found)\.(tsx|ts|jsx|js)$/.test(n),
    contentPattern: /./,
    framework: "nextjs",
    weight: 6,
  },
  ...
  {
    filePattern: "package.json",
    contentPattern: /"vue"\s*:/,
    framework: "vue",
    weight: 10,
  },
  {
    filePattern: (n) => n.endsWith(".vue"),
    contentPattern: /<script\s+setup/,
    framework: "vue",
    weight: 8,
  },
  {
  ...
  {
    filePattern: "package.json",
    contentPattern: /"svelte"\s*:/,
    framework: "svelte",
    weight: 10,
  },
  {
    filePattern: (n) => n.endsWith(".svelte"),
    contentPattern: /./,
    framework: "svelte",
    weight: 8,
  },
  ...
  {
    filePattern: "package.json",
    contentPattern: /"@angular\/core"\s*:/,
    framework: "angular",
    weight: 10,
  },
  {
    filePattern: (n) => n.endsWith(".component.ts"),
    contentPattern: /@Component/,
    framework: "angular",
    weight: 8,
  },
  ...
  {
    filePattern: "package.json",
    contentPattern: /"react"\s*:/,
    framework: "react",
    weight: 5,
  },
  {
    filePattern: (n) => n.endsWith(".tsx") || n.endsWith(".jsx"),
    contentPattern: /import\s+.*React|from\s+['"]react['"]|useState|useEffect/,
    framework: "react",
    weight: 3,
  },
];
...
export function detectFramework(
  ...
  for (const file of files) {
    for (const signal of DETECTION_SIGNALS) {
      const nameMatch =
        typeof signal.filePattern === "string"
          ? file.name === signal.filePattern ||
            file.name.endsWith(signal.filePattern)
