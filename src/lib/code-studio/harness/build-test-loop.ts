...
} from "@/lib/code-studio/pipeline/ast-hollow-scanner";
import {
  buildHarnessFeedback,
  type HarnessFeedback,
} from "@/lib/code-studio/harness/adversarial-core";
...
function parseSingleError(
  ...
  if (jestMatch) {
    return {
      ...
      line: 0,
      column: 0,
      message: `Test suite failed: ${jestMatch[1]}`,
      severity: "error",
      source: "test",
    };
  }
...
export async function runHarnessLoop(
  ...
  while (iteration < cfg.maxIterations) {
    ...
    if (hollowErrors.length > 0) {
      ...
      if (iteration < cfg.maxIterations) {
        const fixedCode = await cfg.onFixRequest(
          hollowErrors.map((f) => ({
            file: f.file,
            line: f.line,
            column: 0,
            message: `[AST] ${f.type}: ${f.message}`,
            severity: "error" as const,
            source: "build" as const,
          })),
          currentCode,
    ...
    for (const step of cfg.steps) {
      ...
      if (result.exitCode !== 0 && result.errors.length > 0) {
        ...
        for (const err of result.errors) {
          switch (err.source) {
            case "build":
              allBuildErrors.push(err);
              break;
            case "test":
              allTestErrors.push(err);
              break;
            case "lint":
              allLintErrors.push(err);
              break;
            case "typecheck":
              allTypeErrors.push(err);
              break;
          }
        }
  ...
  const buildMsgs = allErrors.map(
    (e) => `[${e.source}] ${e.file}:${e.line} ${e.message}`,
  );
...
/** 에러를 에이전트 프롬프트로 변환 */
export function errorsToPrompt(
  errors: ParsedError[],
  feedback?: HarnessFeedback,
): string {
  ...
  if (errors.length > 0) {
    ...
    for (const [file, errs] of grouped) {
      ...
      for (const err of errs) {
        parts.push(
          `- Line ${err.line}: [${err.source}] ${err.message}${err.code ? ` (${err.code})` : ""}`,
        );
      }
    }
  }
