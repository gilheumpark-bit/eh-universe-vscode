  ...
  private save(): void {
    ...
    try {
      localStorage.setItem(
        HISTORY_KEY,
        JSON.stringify(this.entries.slice(-MAX_HISTORY)),
      );
    } catch {
      /* ignore */
    }
...
export class EnvironmentManager {
  ...
  private loadPersisted(): void {
    ...
    try {
      ...
      if (envRaw)
        for (const [k, v] of Object.entries(
          JSON.parse(envRaw) as Record<string, string>,
        ))
    ...
    try {
      ...
      if (aliasRaw)
        for (const [k, v] of Object.entries(
          JSON.parse(aliasRaw) as Record<string, string>,
        ))
  ...
  unsetVar(name: string): boolean {
    const d = this.variables.delete(name);
    if (d) this.saveVars();
    return d;
  }
  ...
  removeAlias(name: string): boolean {
    const d = this.aliases.delete(name);
    if (d) this.saveAliases();
    return d;
  }
  ...
  setCwd(path: string): void {
    if (path === "~" || path === "$HOME") {
      ...
      const p = this.cwd.split("/");
      if (p.length > 1) p.pop();
      this.cwd = p.join("/") || "~";
    } else if (path === "-") {
      const old = this.variables.get("OLDPWD") ?? "~";
      ...
      this.cwd = `${this.cwd}/${path}`.replace(/\/+/g, "/");
    }
    this.variables.set("OLDPWD", this.cwd);
  }
...
export class JobManager {
  ...
  resume(id: number): void {
    const j = this.jobs.get(id);
    if (j?.status === "stopped") j.status = "running";
  }
  ...
  foreground(): TerminalJob | null {
    const stopped = Array.from(this.jobs.values()).filter(
      (j) => j.status === "stopped",
    );
    if (stopped.length === 0) return null;
    const job = stopped[stopped.length - 1];
    job.status = "running";
    return job;
  }
...
export function highlightInput(
  ...
  for (const part of parts) {
    if (/^\s+$/.test(part)) {
      spans.push({ text: part, color: "inherit" });
      continue;
    }
    if (isFirstWord) {
      isFirstWord = false;
      if (BUILTIN_SET.has(part))
        spans.push({ text: part, color: "var(--accent-yellow)", bold: true });
      else if (env.getAlias(part) !== undefined)
        spans.push({ text: part, color: "var(--accent-purple)" });
      else if (COMMON_COMMANDS.includes(part))
        spans.push({ text: part, color: "var(--accent-green)", bold: true });
      else spans.push({ text: part, color: "var(--accent-green)" });
    } else if (part.startsWith("-"))
      spans.push({ text: part, color: "var(--accent-blue)" });
    else if (part.startsWith("$"))
      spans.push({ text: part, color: "var(--accent-purple)" });
    else if (part === "|" || part === "&&" || part === "||" || part === ";")
      spans.push({ text: part, color: "var(--accent-red)", bold: true });
    else if (part === ">" || part === ">>" || part === "2>&1")
      spans.push({ text: part, color: "var(--accent-red)" });
...
export function executeBuiltin(
  ...
  switch (command) {
    case "cd": {
      env.setCwd(env.expandVariables(args[0] || "~"));
      return { output: [], exitCode: 0 };
    }
    case "pwd":
      return { output: [ln(env.getCwd())], exitCode: 0 };
    case "echo":
      return { output: [ln(env.expandVariables(args.join(" ")))], exitCode: 0 };
    case "export": {
      if (args.length === 0) {
        return {
          output: Array.from(env.getAllVars()).map(([k, v]) =>
            ln(`export ${k}="${v}"`, "green"),
          ),
          exitCode: 0,
        };
      ...
      for (const a of args) {
        const eq = a.indexOf("=");
        if (eq > 0)
          env.setVar(a.slice(0, eq), env.expandVariables(a.slice(eq + 1)));
      }
      return { output: [], exitCode: 0 };
    }
    case "unset": {
      for (const n of args) env.unsetVar(n);
      return { output: [], exitCode: 0 };
    }
    case "alias": {
      if (args.length === 0) {
        return {
          output: Array.from(env.getAllAliases()).map(([k, v]) =>
            ln(`alias ${k}='${v}'`, "yellow"),
          ),
          exitCode: 0,
        };
      ...
      for (const a of args) {
        ...
        if (eq > 0)
          env.setAlias(
            a.slice(0, eq),
            a.slice(eq + 1).replace(/^['"]|['"]$/g, ""),
          );
      }
      return { output: [], exitCode: 0 };
    }
    case "unalias": {
      for (const n of args) env.removeAlias(n);
      return { output: [], exitCode: 0 };
    }
    case "history": {
      if (args[0] === "-c") {
        history.clear();
        return { output: [ln("History cleared.", "yellow")], exitCode: 0 };
      }
      const limit = args[0] ? parseInt(args[0], 10) : 50;
      return {
        output: history
          .getFormatted(isNaN(limit) ? 50 : limit)
          .map((l) => ln(l)),
        exitCode: 0,
      };
    }
    case "jobs": {
      ...
      return list.length === 0
        ? { output: [ln("No jobs.", "yellow")], exitCode: 0 }
        : { output: list.map((l) => ln(l)), exitCode: 0 };
    }
    case "fg": {
      const j = jobs.foreground();
      return j
        ? {
            output: [ln(`[${j.id}] resumed: ${j.command}`, "green")],
            exitCode: 0,
          }
        : { output: [ln("No stopped jobs.", "red")], exitCode: 1 };
    }
    case "bg": {
      const j = jobs.background();
      return j
        ? {
            output: [
              ln(`[${j.id}] running in background: ${j.command}`, "green"),
            ],
            exitCode: 0,
          }
        : { output: [ln("No stopped jobs.", "red")], exitCode: 1 };
    ...
    case "env":
      return {
        output: Array.from(env.getAllVars()).map(([k, v]) => ln(`${k}=${v}`)),
        exitCode: 0,
      };
    case "set": {
      if (args.length === 0)
    ...
    case "type": {
      const t = args[0];
      if (!t)
        return { output: [ln("usage: which <command>", "red")], exitCode: 1 };
      if (BUILTIN_SET.has(t))
        return { output: [ln(`${t}: shell builtin`, "blue")], exitCode: 0 };
      const al = env.getAlias(t);
      if (al)
        return {
          output: [ln(`${t}: aliased to '${al}'`, "yellow")],
          exitCode: 0,
        };
      if (COMMON_COMMANDS.includes(t))
        return { output: [ln(`/usr/bin/${t}`)], exitCode: 0 };
      return { output: [ln(`${t}: not found`, "red")], exitCode: 1 };
    }
    case "clear":
      return { output: [], exitCode: 0 };
    case "help":
      return { output: buildHelp(), exitCode: 0 };
    case "cat":
      return catBuiltin(args, files, env);
    case "ls":
      return lsBuiltin(args, files, env);
    case "true":
      return { output: [], exitCode: 0 };
    case "false":
      return { output: [], exitCode: 1 };
    case "test":
      return { output: [], exitCode: args.length > 0 ? 0 : 1 };
    case "time":
      return null;
    default:
      return null;
...
function catBuiltin(
  ...
  if (args.length === 0)
    return { output: [ln("usage: cat <file>", "red")], exitCode: 1 };
  ...
  if (!node)
    return {
      output: [ln(`cat: ${fname}: No such file or directory`, "red")],
      exitCode: 1,
    };
  if (node.type === "folder")
    return {
      output: [ln(`cat: ${fname}: Is a directory`, "red")],
      exitCode: 1,
    };
  const lines = (node.content ?? "").split("\n");
  return {
    output: lines.map((l, i) =>
      ln(showN ? `${String(i + 1).padStart(4)} ${l}` : l),
    ),
    exitCode: 0,
  };
}
...
function lsBuiltin(
  ...
  const entries =
    targetDir === "." || targetDir === "./"
      ? files
      : (resolveDir(files, targetDir, env.getCwd()) ?? files);
  ...
  for (const node of entries) {
    ...
    if (showLong) {
      ...
      const size = isDir
        ? "-"
        : String(node.content?.length ?? 0).padStart(6, " ");
      out.push(
        ln(
          `${perms}  ${size}  ${node.name}${isDir ? "/" : ""}`,
          isDir ? "blue" : undefined,
        ),
      );
    } else {
      out.push(
        ln(`${node.name}${isDir ? "/" : ""}`, isDir ? "blue" : undefined),
      );
    }
  }
...
function buildHelp(): TerminalLine[] {
  return [
    ...
    ln(
      "  Navigation: cd, pwd, echo, export, unset, alias, history, env, which, clear",
      "yellow",
    ),
    ln(
      "  File Ops: cat, ls, head, tail, wc, touch, mkdir, rm, cp, mv, grep, find, sort",
      "yellow",
    ),
    ln(
      "  Shortcuts: Ctrl+L: Clear | Tab: Autocomplete | Up/Down: History",
      "blue",
    ),
  ];
}
...
export function preprocessCommand(
  ...
  const expanded = alias
    ? alias + (firstSpace > 0 ? trimmed.slice(firstSpace) : "")
    : trimmed;
  const withVars = env.expandVariables(expanded);

  let tokens: Token[];
  try {
    tokens = tokenize(withVars);
  } catch {
    return {
      segments: [
        {
          command: firstWord,
          args: withVars.split(/\s+/).slice(1),
          redirects: [],
          background: false,
        },
      ],
  ...
  for (let i = 0; i < tokens.length; i++) {
    const t = tokens[i];
    if (t.type === "word") {
      if (!cur.command) cur.command = t.value;
      else cur.args!.push(t.value);
    } else if (
      t.type === "pipe" ||
      t.type === "and" ||
      t.type === "or" ||
      ...
      t.type === "redirect_out" ||
      t.type === "redirect_append" ||
      t.type === "redirect_err"
    ) {
      const next = tokens[i + 1];
      if (next?.type === "word") {
        const m = {
          redirect_out: "out" as const,
          redirect_append: "append" as const,
          redirect_err: "err" as const,
        };
        cur.redirects!.push({ type: m[t.type], target: next.value });
        i++;
      }
    } else if (t.type === "redirect_err_out") {
      cur.redirects!.push({ type: "err_out", target: "" });
    }
  }
  if (cur.command)
    segments.push({
      ...
      args: cur.args || [],
      redirects: cur.redirects || [],
      background: cur.background || false,
    });
  return { segments };
}
...
function findFileNode(files: FileNode[], name: string): FileNode | null {
  for (const f of files) {
    ...
    if (f.children) {
      const found = findFileNode(f.children, name);
