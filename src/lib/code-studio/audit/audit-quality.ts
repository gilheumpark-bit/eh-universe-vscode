...
export function auditTesting(ctx: AuditContext): AuditAreaResult {
  ...
  const testFiles = ctx.files.filter((f) =>
    /\.(test|spec)\.(ts|tsx|js)$/.test(f.path),
  );
  ...
  const ratioThreshold =
    srcFiles.length > 200 ? 0.1 : srcFiles.length > 100 ? 0.15 : 0.3;
  if (ratio >= ratioThreshold) {
    ...
    findings.push({
      ...
      message: `테스트/소스 비율 ${(ratio * 100).toFixed(0)}% — ${(ratioThreshold * 100).toFixed(0)}% 이상 권장`,
      rule: "LOW_TEST_RATIO",
    });
  }
  ...
  const componentTests = testFiles.filter((f) =>
    f.path.includes("/components/"),
  );
  if (componentTests.length > 0 || componentFiles.length === 0) {
    ...
    findings.push({
      ...
      area: "testing",
      severity: "high",
      message: `UI 컴포넌트 ${componentFiles.length}개, 테스트 ${componentTests.length}건`,
      rule: "NO_COMPONENT_TESTS",
    });
  }
  ...
  if (hookTests.length >= hookFiles.length * 0.3 || hookFiles.length === 0) {
    ...
    findings.push({
      ...
      area: "testing",
      severity: "medium",
      message: `훅 ${hookFiles.length}개 중 ${hookTests.length}개만 테스트`,
      rule: "LOW_HOOK_TESTS",
    });
  }
  ...
  // Check 5: E2E tests
  checks++;
  const e2eFiles = ctx.files.filter(
    (f) => f.path.includes("/e2e/") || f.path.includes("playwright"),
  );
  if (e2eFiles.length >= 3) {
    ...
    findings.push({
      ...
      area: "testing",
      severity: "medium",
      message: `E2E 테스트 ${e2eFiles.length}건 — 최소 3건 권장`,
      rule: "LOW_E2E_TESTS",
    });
  }
...
export function auditErrorHandling(ctx: AuditContext): AuditAreaResult {
  ...
  for (const f of ctx.files) {
    ...
    if (!hasAsync || !hasAwait || hasTryCatch) {
      ...
      findings.push({
        ...
        area: "error-handling",
        severity: "medium",
        message: "async/await 사용 중 try-catch 없음",
        file: f.path,
        rule: "ASYNC_NO_TRYCATCH",
      });
    }
    ...
    const unhandledPromise =
      /\.then\s*\([^)]*\)\s*(?!\.catch)/.test(f.content) &&
      !/\.catch\s*\(/.test(f.content);
    if (!unhandledPromise) {
      passed++;
    } else {
      findings.push({
        ...
        area: "error-handling",
        severity: "medium",
        message: "Promise .then() 에 .catch() 없음",
        file: f.path,
  ...
  checks++;
  const hasErrorBoundary = ctx.files.some((f) =>
    /ErrorBoundary|componentDidCatch/.test(f.content),
  );
  if (hasErrorBoundary) {
    passed++;
  } else {
...
export function auditFeatureCompleteness(ctx: AuditContext): AuditAreaResult {
  ...
  if (stubCount <= stubThreshold) {
    ...
    findings.push({
      ...
      severity: stubCount > 10 ? "high" : "medium",
      message: `stub/placeholder 패턴 ${stubCount}개 파일 감지`,
      rule: "STUB_PATTERN",
    });
  }
  ...
  for (const f of ctx.files) {
    ...
    for (const line of lines) {
      if (
        /=>\s*\{\s*\}/.test(line) &&
        !/\.catch|createContext|default|noop|eslint|cleanup|abort|dispose|unmount|unsubscribe|Ref\.current|removeEventListener/i.test(
          line,
        )
      ) {
        noopCount++;
  ...
  if (noopCount <= noopThreshold) {
    ...
    findings.push({
      ...
      area: "feature-completeness",
      severity: "medium",
      message: `빈 함수 본문 ${noopCount}건 — 미구현 의심`,
      rule: "NOOP_FUNCTIONS",
    });
  }
  ...
  for (const f of ctx.files) {
    const exportMatches = f.content.matchAll(
  ...
  for (const [name, srcPath] of exportedNames) {
    const importedAnywhere = ctx.files.some(
      (f) => f.path !== srcPath && new RegExp(`\\b${name}\\b`).test(f.content),
    );
  ...
  if (unusedExports <= unusedThreshold) {
    ...
    findings.push({
      ...
      area: "feature-completeness",
      severity: "medium",
      message: `미사용 export ${unusedExports}건 — 데드코드 또는 미연결`,
      rule: "UNUSED_EXPORTS",
    });
  }
  ...
  for (const f of ctx.files) {
    ...
    if (
      ...
      f.path.includes("translations") ||
      f.path.includes("articles")
    )
      continue;
    if (
      /setTimeout\s*\(\s*\(\)\s*=>\s*\{[^}]*resolve/i.test(f.content) ||
      /(?:^|\s)(?:const|let|var|function)\s+\w*(?:simulate|mock|fake|dummy)\w*/im.test(
        f.content,
      )
    ) {
      mockInProd++;
  ...
  if (mockInProd <= mockThreshold) {
    ...
    findings.push({
      ...
      area: "feature-completeness",
      severity: "medium",
      message: `프로덕션 코드에 simulate/mock 패턴 ${mockInProd}건`,
      rule: "MOCK_IN_PROD",
    });
  }
...
export function auditDocumentation(ctx: AuditContext): AuditAreaResult {
  ...
  const docRatio =
    exportedFunctions > 0 ? documentedFunctions / exportedFunctions : 1;
  if (docRatio >= 0.3) {
    passed++;
  } else {
    findings.push({
      ...
      area: "documentation",
      severity: "medium",
      message: `export 함수 JSDoc 비율 ${(docRatio * 100).toFixed(0)}% — 30% 이상 권장`,
      rule: "LOW_JSDOC",
    });
  }
  ...
  checks++;
  if (
    ctx.files.some((f) => /changelog|releases|CHANGELOG/i.test(f.path)) ||
    ctx.projectName
  ) {
    passed++;
  } else {
  ...
  checks++;
  const sealedFiles = ctx.files.filter((f) =>
    /IDENTITY_SEAL|PART\s+\d+/.test(f.content),
  );
  if (sealedFiles.length >= 5) {
    ...
    findings.push({
      ...
      area: "documentation",
      severity: "low",
      message: `구조 마커(PART/SEAL) ${sealedFiles.length}건 — 대형 파일에 권장`,
      rule: "LOW_STRUCTURE_MARKERS",
    });
  }
    ...
    if (mentionedPanels) {
      ...
      if (claimed > 50) {
        findings.push({
          ...
          area: "documentation",
          severity: "low",
          message: `README에 패널 ${claimed}개 표기 — 실제 수와 대조 필요`,
          rule: "STALE_README",
        });
      } else {
        passed++;
  ...
  return {
    ...
    metrics: {
      exportedFunctions,
      documentedFunctions,
      docRatio: Math.round(docRatio * 100),
      sealedFiles: sealedFiles.length,
    },
  };
}
