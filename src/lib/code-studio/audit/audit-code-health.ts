...
export function auditOperations(ctx: AuditContext): AuditAreaResult {
  ...
  const totalLines = ctx.files.reduce(
  ...
  if (largeFiles.length <= largeFileThreshold) {
    ...
    for (const f of largeFiles.slice(0, 5)) {
      ...
      findings.push({
        id: fid("ops"),
        area: "operations",
        severity: lines > 2000 ? "high" : "low",
        message: `거대 파일: ${lines}줄 (허용 ${largeFileThreshold}건)`,
        file: f.path,
        rule: "LARGE_FILE",
        suggestion: "모듈 분리 권장",
      });
  ...
  if (anyCount <= anyThreshold) {
    ...
    findings.push({
      id: fid("ops"),
      area: "operations",
      severity: anyCount > tsFiles * 0.1 ? "high" : "medium",
      message: `any/@ts-ignore ${anyCount}건 감지 (허용 ${anyThreshold}건)`,
      rule: "TYPE_SAFETY_BYPASS",
  ...
  if (consoleCount <= 10) {
    ...
    findings.push({
      id: fid("ops"),
      area: "operations",
      severity: consoleCount > 50 ? "high" : "medium",
      message: `console.log/debug ${consoleCount}건 — 구조화 로깅 전환 권장`,
      rule: "CONSOLE_LOG_PROD",
    });
  }
  ...
  if (todoCount <= 3) {
    ...
    findings.push({
      ...
      area: "operations",
      severity: "low",
      message: `TODO/FIXME ${todoCount}건 잔존`,
      rule: "TODO_REMAINING",
    });
  }
      ...
      if (existing && existing !== f.path) {
        ...
        if (dupeCount <= 3) {
          findings.push({
            ...
            area: "operations",
            severity: "low",
            message: `중복 코드 블록 (${existing}과 유사)`,
            file: f.path,
            line: i + 1,
  ...
  checks++;
  const fileNodes = ctx.files.map((f) => ({
    name: f.path,
    type: "file" as const,
    id: f.path,
    content: f.content,
  ...
  if (deadCodeFindings.length <= 10) {
    ...
    findings.push({
      id: fid("ops"),
      area: "operations",
      severity: deadCodeFindings.length > 50 ? "high" : "medium",
      message: `미사용 코드/데드코드 ${deadCodeFindings.length}건 감지`,
      rule: "DEAD_CODE_DETECTED",
  ...
  return {
    ...
    metrics: {
      ...
      consoleCount,
      todoCount,
      deadCodeCount: deadCodeFindings.length,
    },
  };
}
...
export function auditComplexity(ctx: AuditContext): AuditAreaResult {
  ...
  for (const f of ctx.files) {
    if (
      !f.language.includes("typescript") &&
      !f.language.includes("tsx") &&
    ...
    if (maxNest <= 5) {
      ...
      findings.push({
        id: fid("cplx"),
        area: "complexity",
        severity: maxNest > 8 ? "high" : "medium",
        message: `네스팅 깊이 ${maxNest}`,
        file: f.path,
        rule: "DEEP_NESTING",
      ...
      if (funcLen > 50) {
        ...
        if (longFunctions <= 5) {
          findings.push({
            ...
            message: `함수 ${funcLen}줄 (L${startLine})`,
            file: f.path,
            line: startLine,
            rule: "LONG_FUNCTION",
          });
  ...
  return {
    ...
    metrics: {
      avgCyclomatic: Math.round(
        totalCyclomatic / Math.max(ctx.files.length, 1),
      ),
      highComplexityFunctions,
      deepNestingFiles,
      longFunctions,
...
export function auditArchitecture(ctx: AuditContext): AuditAreaResult {
  ...
  if (barrelFiles.length <= 5) {
    ...
    findings.push({
      ...
      area: "architecture",
      severity: "medium",
      message: `배럴 파일 ${barrelFiles.length}개 — 순환 의존 위험`,
      rule: "BARREL_FILES",
    });
  }
  ...
  let circularRisk = 0;
  for (const f of ctx.files) {
    const deepRelatives = (f.content.match(/from\s+['"]\.\.\/..\//g) ?? [])
      .length;
    if (deepRelatives > 3) {
      circularRisk++;
      if (circularRisk <= 3) {
        findings.push({
          ...
          area: "architecture",
          severity: "low",
          message: `깊은 상대 경로 import ${deepRelatives}건`,
          file: f.path,
  ...
  if (starExports <= starThreshold) {
    ...
    findings.push({
      ...
      area: "architecture",
      severity: "medium",
      message: `export * from ${starExports}건 — 트리셰이킹 방해 가능`,
      rule: "STAR_EXPORTS",
    });
  }
  ...
  const hasContext = ctx.files.filter((f) =>
    /createContext|useContext/.test(f.content),
  ).length;
  const hasLocalStorage = ctx.files.filter((f) =>
    /localStorage\.\w+/.test(f.content),
  ).length;
  if (
    (hasZustand ? 1 : 0) + (hasRedux ? 1 : 0) + (hasContext > 0 ? 1 : 0) <=
    2
  ) {
    passed++;
  } else {
    findings.push({
      ...
      area: "architecture",
      severity: "medium",
      message: "상태 관리 3개 이상 혼합 (Zustand/Redux/Context)",
      rule: "MIXED_STATE",
    });
  }
  ...
  const oversizedDirs = [...dirCounts.entries()].filter(
    ([, c]) => c > dirSizeLimit,
  );
  if (oversizedDirs.length === 0) {
    ...
    for (const [dir, count] of oversizedDirs) {
      findings.push({
        ...
        area: "architecture",
        severity: "medium",
        message: `디렉토리 ${dir} — ${count}개 파일 (분리 권장)`,
        rule: "OVERSIZED_DIR",
      });
    }
  }
  ...
  return {
    ...
    metrics: {
      ...
      starExports,
      contextCount: hasContext,
      localStorageUsage: hasLocalStorage,
    },
  };
}
...
export function auditDependencies(ctx: AuditContext): AuditAreaResult {
      ...
      findings: [
        {
          ...
          area: "dependencies",
          severity: "high",
          message: "package.json 미발견",
          rule: "NO_PACKAGE_JSON",
        },
      ],
      checks: 1,
      passed: 0,
  ...
  const wildcards = Object.entries(allDeps).filter(
    ([, v]) => v === "*" || v === "latest",
  );
  ...
  if (deps.length <= 20) {
    ...
    findings.push({
      ...
      area: "dependencies",
      severity: "low",
      message: `프로덕션 의존성 ${deps.length}개 — 번들 크기 주의`,
      rule: "MANY_PROD_DEPS",
    });
  }
  ...
  if (heavyFound.length === 0) {
    ...
    for (const d of heavyFound) {
      findings.push({
        ...
        area: "dependencies",
        severity: "medium",
        message: `대형 패키지 '${d}' — 트리셰이킹 또는 대안 검토`,
        rule: "HEAVY_PACKAGE",
        suggestion:
          d === "moment"
            ? "dayjs 또는 date-fns 권장"
            : d === "lodash"
              ? "lodash-es 또는 개별 import 권장"
              : undefined,
      });
    }
  }
