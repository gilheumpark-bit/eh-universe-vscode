...
export function getActFromEpisode(
  episode: number,
  totalEpisodes: number,
): ActInfo {
  const x = episode / totalEpisodes;
  if (x <= 0.2)
    return { act: 1, name: "도입/설정", nameEN: "Setup", progress: x / 0.2 };
  if (x <= 0.4)
    return {
      act: 2,
      name: "상승/갈등",
...
export const GENRE_TENSION_PARAMS: Record<string, GenreParams> = {
  SF: { base: 0.4, amp: 0.15, accel: 0.3 },
  FANTASY: { base: 0.35, amp: 0.18, accel: 0.28 },
  ROMANCE: { base: 0.3, amp: 0.12, accel: 0.2 },
  THRILLER: { base: 0.5, amp: 0.2, accel: 0.4 },
