...
function checkBlur(lines: string[]): {
  findings: DirectorFinding[];
  count: number;
} {
...
function checkEscape(text: string): {
  findings: DirectorFinding[];
  count: number;
} {
  const findings: DirectorFinding[] = [];
...
function checkGainVsCost(lines: string[]): {
  findings: DirectorFinding[];
  gainCount: number;
  noCostCount: number;
} {
...
function checkSimilarContext(text: string): {
  findings: DirectorFinding[];
  count: number;
} {
  const findings: DirectorFinding[] = [];
...
function checkAITone(text: string): {
  findings: DirectorFinding[];
  count: number;
} {
  const findings: DirectorFinding[] = [];
...
function checkTypo(lines: string[]): {
  findings: DirectorFinding[];
  count: number;
} {
  const findings: DirectorFinding[] = [];
...
function checkEndingMono(text: string): {
  findings: DirectorFinding[];
  ratio: number;
} {
  const findings: DirectorFinding[] = [];
...
function checkHallucinationRisk(text: string): {
  findings: DirectorFinding[];
...
function checkPlatformRules(
  ...
  if (
    preset.nodChecks.includes("length_min_warning") &&
    charCount < preset.episodeLength.min
  ) {
    findings.push({
  ...
  if (preset.nodChecks.includes("density_check")) {
    ...
    const longNarrationBlocks = paras.filter(
      (p) =>
        p.length > 500 &&
        !p.includes('"') &&
        !p.includes("\u201C") &&
...
export function calculateQualityTag(
  ...
  if (score < 40) {
    return {
      tag: "🔴",
      label: "ALERT",
      visibleFindings: findings.filter((f) => f.severity >= 3),
    };
  }
  if (score < 70) {
    return {
