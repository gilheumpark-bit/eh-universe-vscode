...
export function tensionCurve(
  ...
  const params =
    GENRE_TENSION_PARAMS[genre.toUpperCase()] ?? GENRE_TENSION_PARAMS.SF;
  ...
  if (x > 0.8) {
    tension += 0.3 * ((x - 0.8) / 0.2) ** 2;
  }

  // Midpoint boost at 40-60%
  if (x > 0.4 && x < 0.6) {
    tension += 0.1 * Math.sin(((x - 0.4) * Math.PI) / 0.2);
  }
 ...
 */
export function getActPosition(
  episode: number,
