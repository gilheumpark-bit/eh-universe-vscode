...
export function getDefaultSuggestionConfig(
  level: SkillLevel,
): SuggestionConfig {
...
export function generateSuggestions(
  ...
  previousSuggestions.forEach((s) => {
    if (s.dismissed)
      dismissCounts[s.category] = (dismissCounts[s.category] || 0) + 1;
  });
  ...
  const canSuggest = (cat: SuggestionCategory): boolean => {
    ...
    if ((dismissCounts[cat] || 0) >= sgConfig.suppressAfterDismiss)
      return false;
    return true;
  };
    ...
    for (const name of ctx.characterNames) {
      ...
      if (ep - lastEp >= 3) {
        add(
          ...
          "warning",
          msg(
            `캐릭터 '${name}'이(가) ${ep - lastEp}화째 등장하지 않았습니다.`,
            `Character '${name}' hasn't appeared for ${ep - lastEp} episodes.`,
            lang,
          ),
          msg(
            `다음 장면에 등장시키거나, 다른 캐릭터의 대화에서 언급하세요.`,
            `Include them in the next scene or mention them in dialogue.`,
            lang,
          ),
        );
        break; // 한 명만
  ...
  if (ctx.recentMetrics.length > 0) {
    ...
    if (delta > 30) {
      add(
        ...
        "warning",
        msg(
          `긴장도가 목표 대비 ${Math.round(delta)}% 벗어났습니다. (현재: ${last.tension}, 목표: ${Math.round(expectedTension)})`,
          `Tension is ${Math.round(delta)}% off target. (Current: ${last.tension}, Target: ${Math.round(expectedTension)})`,
          lang,
        ),
        msg(
          `갈등 요소를 추가하거나 위기감을 조절하세요.`,
          `Add conflict elements or adjust urgency.`,
          lang,
        ),
      );
    }
  ...
  if (ctx.recentMetrics.length >= 3) {
    ...
    if (lastThree.every((m) => m.eos < 30)) {
      add(
        ...
        "critical",
        msg(
          `감정 밀도가 3화 연속 낮습니다. (EOS: ${lastThree.map((m) => m.eos).join(", ")})`,
          `Emotion density has been low for 3 consecutive chapters. (EOS: ${lastThree.map((m) => m.eos).join(", ")})`,
          lang,
        ),
        msg(
          `내면 독백, 감각 묘사, 감정 대화를 늘려보세요.`,
          `Add inner monologue, sensory details, and emotional dialogue.`,
          lang,
        ),
      );
    }
  ...
  if (ctx.recentMetrics.length > 0) {
    ...
    if (last.pacing < 30) {
      add(
        ...
        "warning",
        msg(
          `페이싱이 매우 느립니다. (${last.pacing}/100)`,
          `Pacing is very slow. (${last.pacing}/100)`,
          lang,
        ),
        msg(
          `문장 길이에 변화를 주고, 짧은 대화를 섞어보세요.`,
          `Vary sentence length and mix in short dialogue.`,
          lang,
        ),
      );
      ...
      add(
        ...
        "info",
        msg(
          `페이싱이 매우 빠릅니다. (${last.pacing}/100) 독자가 숨 돌릴 여유가 필요할 수 있습니다.`,
          `Pacing is very fast. (${last.pacing}/100) Readers may need breathing room.`,
          lang,
        ),
        msg(
          `묘사 장면이나 내면 독백으로 호흡을 조절하세요.`,
          `Use description or inner monologue to control pacing.`,
          lang,
        ),
      );
    }
  ...
  if (ctx.recentMetrics.length >= 3) {
    ...
    if (avgGradeNum < 65) {
      add(
        ...
        "warning",
        msg(
          `최근 3화 평균 등급이 C+입니다. AI 문투가 누적되었을 수 있습니다.`,
          `Average grade for last 3 chapters is C+. AI tone may be accumulating.`,
          lang,
        ),
        msg(
          `"요약하자면", "결론적으로" 같은 패턴을 확인하세요.`,
          `Check for patterns like "in summary", "in conclusion".`,
          lang,
        ),
      );
    }
...
export function dismissSuggestion(
  ...
  return suggestions.map((s) =>
    s.id === id
      ? { ...s, dismissed: true, dismissCount: s.dismissCount + 1 }
      : s,
  );
}
