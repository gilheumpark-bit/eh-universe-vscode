...
function evaluateWorldCheck(ctx: PipelineContext): PipelineStageResult {
  ...
  // 세계관 필수 필드 존재 여부
  const c = ctx.config;
  if (!c.synopsis || c.synopsis.length < 20) {
    score -= 30;
    warnings.push("synopsis_too_short");
  }
  if (!c.genre) {
    score -= 20;
    warnings.push("genre_not_set");
  }
  if (!c.characters || c.characters.length === 0) {
    score -= 20;
    warnings.push("no_characters");
  }
  if (!c.setting || c.setting.length < 10) {
    score -= 10;
    warnings.push("setting_vague");
  }
...
function evaluateCharacterSync(ctx: PipelineContext): PipelineStageResult {
  ...
  for (const ch of chars) {
    if (!ch.traits || ch.traits.length < 10) {
      score -= 10;
      warnings.push(`${ch.name}: traits_incomplete`);
    }
    if (!ch.role) {
      score -= 5;
      warnings.push(`${ch.name}: role_missing`);
    }
  }

  // POV 캐릭터 존재 여부
  if (
    ctx.config.povCharacter &&
    !chars.some((c) => c.name === ctx.config.povCharacter)
  ) {
    score -= 15;
...
function evaluateDirectionSetup(ctx: PipelineContext): PipelineStageResult {
  ...
  if (!c.sceneDirection) {
    ...
    if (!sd.hooks || sd.hooks.length === 0) {
      score -= 20;
      warnings.push("no_hooks");
    }
    if (!sd.goguma || sd.goguma.length === 0) {
      score -= 15;
      warnings.push("no_tension_devices");
    }
  }

  // 긴장도 가드레일 설정 여부
  if (!c.guardrails || c.guardrails.min === 0) {
    score -= 10;
    warnings.push("guardrails_not_set");
  }
...
export function executePipeline(
  ...
  for (const stageName of stageOrder) {
    ...
    switch (stageName) {
      case "world_check":
        result = evaluateWorldCheck(ctx);
        break;
      case "character_sync":
        result = evaluateCharacterSync(ctx);
        break;
      case "direction_setup":
        result = evaluateDirectionSetup(ctx);
        break;
      default:
        result = {
          stage: stageName,
  ...
  const allPassed = stages.every(
    (s) =>
      s.status === "passed" || s.status === "skipped" || s.status === "pending",
  );
...
export function buildPipelineSummary(
  ...
  const stageLabels: Record<PipelineStage, string> = isKO
    ? {
        world_check: "세계관 검증",
        character_sync: "캐릭터 동기화",
        direction_setup: "연출 설정",
        generation: "집필 생성",
      }
    : {
        world_check: "World Check",
        character_sync: "Character Sync",
        direction_setup: "Direction Setup",
  ...
  const details = execution.stages.map(
    (s) =>
