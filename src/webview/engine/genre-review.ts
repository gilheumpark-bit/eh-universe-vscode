...
export interface GenreBenchmark {
  ...
  benchmarks: Partial<
    Record<ReviewAspectKey, { min: number; max: number; unit: string }>
  >;
}
...
export const READER_LEVELS: ReaderLevelMeta[] = [
  {
    ...
    desc: {
      ko: "재미 판단 — 몰입 유지, 이탈 구간 탐지",
      en: "Fun factor — immersion, drop-off detection",
    },
  ...
  },
  {
    ...
    desc: {
      ko: "장르 충족도 — 클리셰, 기대전개, 떡밥 회수",
      en: "Genre satisfaction — tropes, expectations, payoff",
    },
    focusKeys: [
      "clicheUsage",
      "foreshadowing",
  ...
  },
  {
    ...
    desc: {
      ko: "구조 검수 — 시점, 페이싱, 상업성",
      en: "Structure review — POV, pacing, commercial viability",
    },
    focusKeys: [
      "structureIntegrity",
      "commercialViability",
  ...
  },
  {
    ...
    desc: {
      ko: "서사 완결성 — 주제의식, 문학 장치, 서사 구조",
      en: "Narrative integrity — theme, literary devices, structure",
    },
    focusKeys: [
      "thematicDepth",
      "literaryDevice",
...
const ASPECT_LABELS: Record<ReviewAspectKey, { ko: string; en: string }> = {
  ...
  characterEntry: {
    ko: "신규 캐릭터 등장 빈도",
    en: "Character Introduction Rate",
  },
...
export function extractTextMetrics(text: string): TextMetrics {
  ...
  const avgSentenceLen =
    sentences.length > 0
      ? sentences.reduce((acc, s) => acc + s.trim().length, 0) /
        sentences.length
      : 0;
  ...
  const namePatterns =
    text.match(
      /(?:^|["\s])([가-힣]{2,4})(?:이|가|은|는|을|를|의|에게|한테|라고|라며)/gm,
    ) ?? [];
  const nameSet = new Set<string>();
  for (const match of namePatterns) {
    const clean = match
      .replace(/^[""\s]+/, "")
      .replace(/(?:이|가|은|는|을|를|의|에게|한테|라고|라며)$/, "");
  ...
  return {
    ...
    exclamationDensity:
      Math.round((exclamationCount / totalSentences) * 100) / 100,
    ...
    lineBreakDensity:
      totalLines > 0
        ? Math.round(
            ((text.split("\n").length - totalLines) / totalLines) * 100,
          ) / 100
        : 0,
  };
}
 ...
 */
function metricsToAspectScores(
  m: TextMetrics,
): Partial<Record<ReviewAspectKey, number>> {
  ...
  const hookBase = Math.min(
    100,
    40 +
      m.exclamationDensity * 20 +
      m.questionDensity * 15 +
      (hasFirstLineDialogue ? 15 : 0),
  );
  scores.hook = Math.round(hookBase);

  // pacing: 평균 문장 길이 역비례 + 줄바꿈 밀도
  const pacingFromSentence =
    m.avgSentenceLen > 0 ? Math.max(0, 100 - (m.avgSentenceLen - 15) * 2) : 50;
  scores.pacing = Math.round(
    Math.min(100, Math.max(0, pacingFromSentence + m.lineBreakDensity * 10)),
  );
  ...
  scores.emotionDensity = Math.round(
    Math.min(
      100,
      (m.exclamationDensity + m.ellipsisDensity + m.questionDensity) * 25,
    ),
  );
  ...
  const clicheBase = Math.max(
    0,
    80 - m.uniqueNames.length * 10 - m.dialogueRatio * 0.3,
  );
  scores.clicheUsage = Math.round(
    Math.min(100, Math.max(0, clicheBase + m.ellipsisDensity * 5)),
  );
  ...
  const avgParagraphLen =
    m.paragraphCount > 0 ? m.totalChars / m.paragraphCount : m.totalChars;
  scores.structureIntegrity = Math.round(
    Math.min(100, Math.max(0, 80 - Math.abs(avgParagraphLen - 200) * 0.2)),
  );

  // commercialViability: hook + pacing 복합
  scores.commercialViability = Math.round(
    ((scores.hook ?? 50) + (scores.pacing ?? 50)) / 2,
  );

  // thematicDepth: 문장 길이가 긴 편이면 높음 (사색적 서술)
  scores.thematicDepth = Math.round(
    Math.min(100, Math.max(0, 20 + m.avgSentenceLen * 1.5)),
  );

  // literaryDevice: 줄임표 + 은유적 표현 밀도 프록시
  scores.literaryDevice = Math.round(
    Math.min(100, m.ellipsisDensity * 30 + 20),
  );
...
function generateComment(
  ...
  const delta =
    pos === "below" ? min - value : pos === "above" ? value - max : 0;

  // Lv.1 라이트 독자 시점 코멘트
  if (level === 1) {
    if (key === "hook" && pos === "below")
      return {
        ko: `도입부가 약해서 첫 3줄에서 이탈할 수 있음`,
        en: `Weak opening — readers may drop within 3 lines`,
      };
    if (key === "pacing" && pos === "below")
      return {
        ko: `전개가 느림 — 지루함 느낄 구간`,
        en: `Slow pacing — boredom risk zone`,
      };
    if (key === "pacing" && pos === "above")
      return {
        ko: `전개가 너무 빠름 — 상황 이해 전에 넘어감`,
        en: `Too fast — events pass before comprehension`,
      };
    if (key === "dialogueRatio" && pos === "below")
      return {
        ko: `대화가 적어 답답할 수 있음`,
        en: `Low dialogue — may feel heavy`,
      };
    if (key === "characterEntry" && pos === "above")
      return {
        ko: `캐릭터가 한꺼번에 너무 많이 등장함`,
        en: `Too many characters introduced at once`,
      };
    if (pos === "within")
      return {
        ko: `이 장르 독자 기대치 충족`,
        en: `Meets reader expectations`,
      };
    return {
      ko: `기준 범위에서 ${Math.round(delta)}p 벗어남`,
      en: `${Math.round(delta)}p outside benchmark`,
    };
  }

  // Lv.2 장르 매니아 시점
  if (level === 2) {
    if (key === "clicheUsage" && pos === "below")
      return {
        ko: `장르 관습 부족 — 기대 전개가 빠져 있을 수 있음`,
        en: `Low trope usage — genre expectations unmet`,
      };
    if (key === "clicheUsage" && pos === "above")
      return {
        ko: `클리셰 과다 — 뻔한 느낌`,
        en: `Trope overload — feels predictable`,
      };
    if (key === "foreshadowing" && pos === "below")
      return {
        ko: `복선/떡밥 부족 — 다음 화 기대감 약함`,
        en: `Low foreshadowing — weak next-episode anticipation`,
      };
    if (key === "emotionDensity" && pos === "below")
      return {
        ko: `감정선 약함 — 이 장르 팬이 원하는 몰입감 부족`,
        en: `Weak emotional arc for genre fans`,
      };
    if (key === "worldExposition" && pos === "above")
      return {
        ko: `설정 설명이 과다 — 이미 아는 독자에겐 지루함`,
        en: `Over-exposition — veterans find it boring`,
      };
    if (pos === "within")
      return {
        ko: `장르 매니아 기대 충족`,
        en: `Meets enthusiast expectations`,
      };
    return {
      ko: `장르 평균 대비 ${pos === "below" ? "부족" : "과다"}`,
      en: `${pos === "below" ? "Below" : "Above"} genre average`,
    };
  }

  // Lv.3 편집자 시점
  if (level === 3) {
    if (key === "structureIntegrity" && pos === "below")
      return {
        ko: `구조적 문제 — 시점 혼란 또는 장면 전환 불안정`,
        en: `Structural issue — POV confusion or unstable transitions`,
      };
    if (key === "commercialViability" && pos === "below")
      return {
        ko: `상업성 약함 — 이 장르 시장에서 경쟁력 부족`,
        en: `Low commercial viability for this genre market`,
      };
    if (key === "pacing" && pos !== "within")
      return {
        ko: `페이싱 조정 필요 — 이 장르 상위 작품 대비 ${pos === "below" ? "하위" : "상위"} ${Math.round(delta)}p`,
        en: `Pacing adjustment needed — ${Math.round(delta)}p ${pos === "below" ? "below" : "above"} top works`,
      };
    if (key === "narrativeVoice" && pos === "below")
      return {
        ko: `화자 일관성 부족 — 시점 혼재 가능성`,
        en: `Voice inconsistency — possible POV mixing`,
      };
    if (pos === "within")
      return { ko: `편집 기준 통과`, en: `Passes editorial standards` };
    return {
      ko: `편집자 관점: 조정 권장`,
      en: `Editor view: adjustment recommended`,
    };
  }

  // Lv.4 비평가 시점
  if (key === "thematicDepth" && pos === "below")
    return {
      ko: `주제의식 약함 — 서사가 표면에 머묾`,
      en: `Shallow theme — narrative stays surface-level`,
    };
  if (key === "literaryDevice" && pos === "below")
    return {
      ko: `문학 장치 부족 — 은유·대비·상징 활용 여지 있음`,
      en: `Low literary device usage — room for metaphor, contrast, symbolism`,
    };
  if (key === "narrativeVoice" && pos === "below")
    return {
      ko: `서사 목소리가 불분명 — 작가의 고유 색이 약함`,
      en: `Unclear narrative voice — author's unique color is weak`,
    };
  if (key === "structureIntegrity" && pos === "above")
    return {
      ko: `지나치게 정형적 — 파격적 구조 시도 여지 있음`,
      en: `Too formulaic — room for structural experimentation`,
    };
  if (pos === "within")
    return { ko: `비평적 기준 충족`, en: `Meets critical standards` };
  return {
    ko: `비평 관점: ${pos === "below" ? "보강" : "절제"} 필요`,
    en: `Critic: needs ${pos === "below" ? "more depth" : "restraint"}`,
  };
}
...
export function runGenreLevelReview(
  ...
  const levelMeta =
    READER_LEVELS.find((l) => l.level === level) ?? READER_LEVELS[0];
