...
export function formatSocialProfile(
  ...
  const relation =
    RELATION_LABELS[language]?.[profile.relationDistance] ??
    profile.relationDistance;
  const age =
    AGE_LABELS[language]?.[profile.ageRegister] ?? profile.ageRegister;
  const explicit =
    EXPLICIT_LABELS[language]?.[profile.explicitness] ?? profile.explicitness;
  const profanity =
    PROFANITY_LABELS[language]?.[profile.profanityLevel] ??
    profile.profanityLevel;
  ...
  if (isKO) {
    ...
    parts.push(`나이대: ${age}`);
    if (profile.professionRegister)
      parts.push(`직업: ${profile.professionRegister}`);
    ...
    parts.push(`Age: ${age}`);
    if (profile.professionRegister)
      parts.push(`Profession: ${profile.professionRegister}`);
