...
export function buildPrismBlock(config: StoryConfig, isKO: boolean): string {
  ...
  if (isKO) {
    parts.push(
      `- 원문 보존 우선: 원문의 단어/문장/단락을 허가 없이 삭제·재배치 금지`,
    );
    parts.push(`- 시점 잠금 (POV Lock): 설정된 시점 캐릭터 외 내면 서술 금지`);
    parts.push(
      `- 캐릭터 말맛 보존: 등록된 말투/대사 스타일을 절대 평준화하지 마라`,
    );
    parts.push(
      `- 설정 환각 금지: 등록되지 않은 지명/인명/설정을 창작하지 마라`,
    );
    parts.push(
      `- AI 냄새 차단: "그러나", "한편", "결론적으로" 등 요약형 연결사 사용 금지`,
    );
    parts.push(
      `- 감정은 직접 명명 대신 장면으로: "슬펐다" 대신 행동/감각/환경으로 전달`,
    );
  } else {
    parts.push(
      `- Preserve original: Never delete/reorder words/sentences/paragraphs without permission`,
    );
    parts.push(`- POV Lock: No inner narration outside the set POV character`);
    parts.push(
      `- Character voice preservation: Never flatten registered speech styles`,
    );
    parts.push(
      `- No setting hallucination: Do not invent unregistered names/places/lore`,
    );
    parts.push(
      `- AI tone suppression: Avoid summary-style connectors ("however", "in conclusion")`,
    );
    parts.push(
      `- Show emotion through scenes: Use action/sensation/environment instead of naming emotions`,
    );
  }
  ...
  if (genrePreset) {
    parts.push(
      `- ${isKO ? "장르 리듬" : "Genre rhythm"}: ${genrePreset.pacing}`,
    );
  }
  ...
  if (isKO) {
    ...
    parts.push(
      `- When expanding, increase density of existing scenes rather than adding new events`,
    );
    parts.push(
      `- Prioritize: character action, sensory detail, environmental response`,
    );
...
export function buildPrismModeBlock(
  config: StoryConfig,
  isKO: boolean,
): string {
  ...
  if (mode === "ALL") {
    ...
    const custom = config.prismCustom ?? {
      sexual: 0,
      violence: 0,
      profanity: 0,
    };
    ...
    const sexLabels = isKO
      ...
      : [
          "No sexual content",
          "Light implication only",
          "Up to kissing/tension",
          "Heavy romance",
          "Explicit allowed",
          "No limits",
        ];
    const violLabels = isKO
      ...
      : [
          "No violence",
          "Impacts only, no blood",
          "Wounds/blood allowed",
          "Graphic violence",
          "Extreme violence",
          "No limits",
        ];
    ...
    parts.push(
      `- ${isKO ? "성적 수위" : "Sexual"} [${custom.sexual}/5]: ${sexLabels[custom.sexual]}`,
    );
    parts.push(
      `- ${isKO ? "폭력 수위" : "Violence"} [${custom.violence}/5]: ${violLabels[custom.violence]}`,
    );
    parts.push(
      `- ${isKO ? "비속어 수위" : "Profanity"} [${custom.profanity}/5]: ${profLabels[custom.profanity]}`,
    );
  }
