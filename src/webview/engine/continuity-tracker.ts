...
const KOREAN_NAME_RE =
  /(?:^|[\s"'\u201C\u2018])([가-힣]{2,4})(?:이|가|은|는|을|를|의|에게|한테|라고|라며|도|만|까지)/gm;
...
function countDialogue(text: string, name: string): number {
  ...
  const patterns = [
    new RegExp(
      `${name}[이가은는]?\\s*(?:말했|대답했|외쳤|중얼|속삭|물었|소리쳤)`,
      "g",
    ),
    new RegExp(`"[^"]*"[^\\n]*${name}`, "g"),
  ];
...
function extractThreads(text: string): {
  planted: string[];
  resolved: string[];
} {
...
function analyzeEpisodePair(
  ...
  for (const cc of curr.characters) {
    ...
    const prevInjured = pc.stateFlags.some(
      (f) => f.includes("부상") || f.includes("상처"),
    );
    const currInjured = cc.stateFlags.some(
      (f) => f.includes("부상") || f.includes("상처"),
    );
  ...
  if (prev.location && curr.location && prev.location !== curr.location) {
    const hasTransition =
      curr.eventSummary.includes("이동") || curr.eventSummary.includes("도착");
    if (!hasTransition) {
      warnings.push({
        type: "location_jump",
        message: {
...
export function buildContinuityReport(
  ...
  const overallScore =
    episodes.length > 0
      ? Math.round(
          episodes.reduce((acc, e) => acc + e.continuityScore, 0) /
            episodes.length,
        )
      : 100;
  ...
  return {
    ...
    totalWarnings,
    threadStatus: {
      open: allOpenThreads.length,
      resolved: allResolvedThreads.length,
    },
  };
}
