...
export function calculateArcPhase(
  episode: number,
  totalEpisodes: number,
): ArcShadow {
  const x = episode / totalEpisodes;
  if (x <= 0.2) return { phase: "INTRO", progress: x / 0.2 };
  if (x <= 0.6) return { phase: "PROGRESS", progress: (x - 0.2) / 0.4 };
  if (x <= 0.85) return { phase: "CLIMAX", progress: (x - 0.6) / 0.25 };
...
export function detectDrift(shadow: CharacterShadow): DriftLevel {
  ...
  if (shadow.baselineDrift < 0.3) return "MINOR_DRIFT";
  if (shadow.baselineDrift < 0.5) return "DRIFT";
  return "CRITICAL";
}
...
export function getOverdueThreads(
  ...
  return threads.filter(
    (t) => !t.resolved && currentEpisode - t.introducedEpisode >= 7,
  );
}

export function getHighPriorityUnresolved(
  threads: NarrativeThread[],
): NarrativeThread[] {
...
export function buildShadowPrompt(
  ...
  parts.push(
    `[${t("shadow.arcState")}] ${arc.phase} (${t("shadow.progress")}: ${Math.round(arc.progress * 100)}%)`,
  );
  ...
  if (shadow.characters.length > 0) {
    ...
    shadow.characters.forEach((c) => {
      ...
      parts.push(
        `  ${c.name}: ${t("shadow.emotion")}=${c.emotion}, ${t("shadow.goal")}=${c.goal}, drift=${drift}`,
      );
      if (c.unresolvedObservations.length > 0) {
        parts.push(
          `    ${t("shadow.unresolved")}: ${c.unresolvedObservations.join(", ")}`,
        );
      }
    });
  }
  ...
  if (shadow.world.location) {
    parts.push(
      `[${t("shadow.worldState")}] ${shadow.world.location} | ${shadow.world.timeMarker} | ${shadow.world.environmentalMood}`,
    );
    if (shadow.world.activeThreats.length > 0) {
      parts.push(
        `  ${t("shadow.activeThreats")}: ${shadow.world.activeThreats.join(", ")}`,
      );
    }
  }
  ...
  if (overdue.length > 0) {
    parts.push(
      `[${t("shadow.urgentThreads")}] ${t("shadow.urgentThreadsDesc")}: ${overdue.map((th) => th.description).join(", ")}`,
    );
  }
  if (highPri.length > 0 && arc.phase === "CLIMAX") {
    parts.push(
      `[${t("shadow.mustResolve")}] ${t("shadow.mustResolveDesc")}: ${highPri.map((th) => th.description).join(", ")}`,
    );
  }
