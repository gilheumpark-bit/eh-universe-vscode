...
const SLIDER_LABELS: Record<
  string,
  { name: string; nameEN: string; levels: string[]; levelsEN: string[] }
> = {
  s1: {
    name: "문장 길이",
    nameEN: "Sentence Length",
    levels: [
      "짧고 단단하게",
      "짧은 호흡",
      "균형",
  ...
  s2: {
    name: "감정 밀도",
    nameEN: "Emotional Density",
    levels: ["감정 절제", "건조한 편", "균형", "정서 강조", "감정 밀도 높음"],
    levelsEN: [
      "Restrained",
      "Dry-leaning",
      "Balanced",
  ...
  s3: {
    name: "묘사 방식",
    nameEN: "Description",
    levels: ["사실 위주", "직설 묘사", "균형", "이미지 강조", "감각 몰입"],
    levelsEN: [
      "Factual",
      "Direct",
      "Balanced",
  ...
  s4: {
    name: "서술 시점",
    nameEN: "Narrative Distance",
    levels: ["멀리 조망", "관찰자 시점", "균형", "인물 밀착", "내면 침투"],
    levelsEN: [
      "Panoramic",
      "Observer",
      "Balanced",
  ...
  s5: {
    name: "어휘 수준",
    nameEN: "Vocabulary",
    levels: ["편한 말맛", "담백한 어휘", "균형", "정교한 어휘", "전문적 질감"],
    levelsEN: ["Plainspoken", "Clean", "Balanced", "Refined", "Specialized"],
  },
};
...
export function buildStyleDNA(
  ...
  // DNA identity
  const dnaNames = profile.selectedDNA
    .map((i) => (isKO ? DNA_NAMES[i] : DNA_NAMES_EN[i]))
    .join(" + ");
  ...
  if (profile.sliders) {
    ...
    for (const [key, rawVal] of Object.entries(profile.sliders)) {
      ...
      sliderParts.push(
        `${isKO ? meta.name : meta.nameEN}: ${label} (${val}/5)`,
      );
    }
...
export function buildLanguagePackBlock(
  ...
  }
  {
    ...
    parts.push(
      `- ${label}: ${pack.dialogueMarkers.open}...${pack.dialogueMarkers.close}`,
    );
  }
  {
    const label = isKO ? "문장 리듬" : "Sentence rhythm";
    parts.push(
      `- ${label}: ${pack.sentenceRhythm.minWords}~${pack.sentenceRhythm.maxWords} ${isKO ? "단어" : "words"}`,
    );
  }
...
export function buildEHRules(ruleLevel: number, isKO: boolean): string {
  ...
  const PCT_MAP: Record<number, number> = {
    1: 0,
    2: 15,
    3: 25,
    4: 35,
  ...
  if (ruleLevel >= 2) {
    sections.push(
      `[${t("pipeline.enforcerHeader")} Lv${ruleLevel}]\n${t("pipeline.enforcerBody")}`,
    );
  }
  ...
  if (ruleLevel >= 3) {
    ...
    sections.push(
      `[${t("pipeline.costInflictionHeader")}]\n${t("pipeline.costInflictionBody")}\n${costNote}`,
    );
  }
  ...
  if (ruleLevel >= 5) {
    sections.push(
      `[${t("pipeline.narrativeLockHeader")}]\n${t("pipeline.narrativeLockBody")}`,
    );
  }
...
export function buildSystemInstruction(
  ...
  const targetTension = Math.round(
    tensionCurve(config.episode, totalEpisodes, config.genre) * 100,
  );
  const platformTarget = getTargetCharRange(platform);
  // 가드레일이 설정되어 있으면 사용자 수치 우선, 아니면 플랫폼 기본값
  const charTarget = {
    min:
      config.guardrails?.min && config.guardrails.min > 0
        ? config.guardrails.min
        : platformTarget.min,
    max:
      config.guardrails?.max && config.guardrails.max > 0
  ...
  const CHAR_LABELS: Record<string, Record<AppLanguage, string>> = {
    ...
    noCharacters: {
      KO: "등록된 캐릭터 없음",
      EN: "No characters registered",
      JP: "キャラクター未登録",
      CN: "未注册角色",
    },
  };
  const cl = (key: string) =>
    CHAR_LABELS[key]?.[language] ?? CHAR_LABELS[key]?.EN ?? key;

  const characterDNA =
    injectedCharacters.length > 0
      ? (config.characters.length > MAX_CHARACTERS
  ...
  const charRelations =
    filteredRelations.length > 0
      ? filteredRelations
          .map((r) => {
            const fromName =
              injectedCharacters.find((c) => c.id === r.from)?.name || r.from;
            const toName =
              injectedCharacters.find((c) => c.id === r.to)?.name || r.to;
            const label =
              REL_LABELS[r.type]?.[language] ??
              REL_LABELS[r.type]?.EN ??
              r.type;
  ...
  if (sd) {
    ...
    if (sd.goguma && sd.goguma.length > 0) {
      ...
      sd.goguma.forEach((g) => {
        parts.push(
          `  - ${g.type === "goguma" ? t("pipeline.goguma") : t("pipeline.cider")} (${g.intensity}): ${g.desc}`,
        );
      });
    }
    ...
    if (sd.emotionTargets && sd.emotionTargets.length > 0) {
      ...
      sd.emotionTargets.forEach((e) => {
        parts.push(
          `  - ${e.emotion}: ${t("pipeline.intensity")} ${e.intensity}%`,
        );
      });
    }
    ...
    if (sd.cliffhanger) {
      parts.push(
        `[${t("pipeline.cliffhangerLabel")}] ${t("pipeline.cliffType")}: ${sd.cliffhanger.cliffType} — ${sd.cliffhanger.desc}`,
      );
    }
    if (sd.plotStructure) {
    ...
    if (sd.foreshadows && sd.foreshadows.length > 0) {
      ...
      sd.foreshadows.forEach((f) => {
        const status = f.resolved
          ...
          : t("pipeline.pending");
        parts.push(
          `  - EP${f.episode}: ${f.planted} → ${f.payoff} (${status})`,
        );
      });
    }
    ...
    if (sd.tensionCurve && sd.tensionCurve.length > 0) {
      ...
      sd.tensionCurve.forEach((tc) => {
        parts.push(
          `  - ${tc.label}: ${t("pipeline.position")} ${tc.position}%, ${t("pipeline.level")} ${tc.level}%`,
        );
      });
    }
  ...
  if (simRef) {
    if (simRef.worldConsistency)
      simParts.push(`- ${t("pipeline.worldConsistency")}`);
    if (simRef.genreLevel && simRef.ruleLevel)
      simParts.push(
        `- ${t("pipeline.genreLevelRules")}: Lv${simRef.ruleLevel}`,
      );
    if (simRef.genreSelections && simRef.genreSelections.length > 0) {
      const genreStr = simRef.genreSelections
        .map((s) => `${s.genre} Lv${s.level}`)
        .join(" + ");
      simParts.push(
        `- ${t("pipeline.genreBlend")}: ${genreStr} (${simRef.genreSelections.length}${t("pipeline.genreBlendSuffix")})`,
      );
    }
    if (
      simRef.civRelations &&
      simRef.civRelationSummary &&
      simRef.civRelationSummary.length > 0
    ) {
  ...
  if (worldSim) {
    ...
    if (worldSim.ruleLevel) {
      simParts.push(
        `- [UNIVERSE MODE] Rule Intensity: Lv${worldSim.ruleLevel}`,
      );
    }
    if (worldSim.civs && worldSim.civs.length > 0) {
      ...
      worldSim.civs.forEach((c) => {
        const traits =
          c.traits && c.traits.length > 0 ? c.traits.join(", ") : "No traits";
  ...
  {
    const wParts: string[] = [];
    if (config.corePremise)
      wParts.push(`- ${t("pipeline.corePremise")}: ${config.corePremise}`);
    if (config.powerStructure)
      wParts.push(
        `- ${t("pipeline.powerStructure")}: ${config.powerStructure}`,
      );
    if (config.currentConflict)
      wParts.push(
        `- ${t("pipeline.currentConflict")}: ${config.currentConflict}`,
      );
    if (config.worldHistory)
      wParts.push(`- ${t("pipeline.history")}: ${config.worldHistory}`);
    if (config.magicTechSystem)
      wParts.push(`- ${t("pipeline.magicTech")}: ${config.magicTechSystem}`);
    if (config.socialSystem)
      wParts.push(`- ${t("pipeline.socialSystem")}: ${config.socialSystem}`);
    if (config.factionRelations)
      wParts.push(
        `- ${t("pipeline.factionRelations")}: ${config.factionRelations}`,
      );
    if (config.dailyLife)
      wParts.push(`- ${t("pipeline.dailyLife")}: ${config.dailyLife}`);
    if (config.truthVsBeliefs)
      wParts.push(
        `- ${t("pipeline.beliefsVsTruth")}: ${config.truthVsBeliefs}`,
      );
    if (wParts.length > 0) {
  ...
  const subGenreBlock =
    config.useSubGenrePrompt && config.subGenres && config.subGenres.length > 0
  ...
  const publishPlatformBlock = buildPublishPlatformBlock(
...
${
  config.narrativeIntensity === "iron"
    ? `[NARRATIVE INTENSITY: IRON — 서사 강도 강]
...
`
    : config.narrativeIntensity === "soft"
      ? ""
      : `[NARRATIVE INTENSITY: STANDARD — 서사 강도 중]
...
export function postProcessResponse(
  ...
  if (jsonMatch) {
    try {
      ...
      /* JSON parse advisory — world_updates extraction is optional */
    }
  } else {
    ...
    try {
      ...
      if (gradeIndex !== -1) {
        for (
          let braceIndex = text.lastIndexOf("{", gradeIndex);
          braceIndex >= 0;
      ...
      /* JSON fallback parse advisory — non-blocking */
    }
  }
...
function stripTrailingReportJson(text: string): string {
  ...
  const mdMatch = text.match(
    /```(?:json|JSON)?\s*\{[\s\S]*?"grade"\s*:\s*[\s\S]*?\}\s*```\s*$/,
  );
  if (mdMatch) {
  ...
  for (
    let braceIndex = text.lastIndexOf("{", scanStart);
    braceIndex >= 0;
    braceIndex = text.lastIndexOf("{", braceIndex - 1)
  ) {
    const candidate = text.slice(braceIndex).trim();
    ...
    try {
      ...
      if (
        ...
        ("grade" in parsed || "metrics" in parsed || "world_updates" in parsed)
      ) {
...
export function stripEngineArtifacts(text: string): string {
  let clean = text
    .replace(
      /\{[^{}]*(?:\{[^{}]*\}[^{}]*)*"(?:grade|metrics|critique|tension|eos(?:_score|Score)?|pacing|immersion)"[^{}]*(?:\{[^{}]*\}[^{}]*)*\}/g,
      "",
    )
    .replace(
    ...
    .replace(
      /^\s*"(?:grade|metrics|tension|pacing|immersion|eos)"[\s:].*/gm,
      "",
    );
  ...
  clean = clean
    .replace(
      /^(?:알겠습니다[,.]?\s*작가님[.!]?\s*|네[,.]?\s*(?:이어서|계속|작성|시작)\s*(?:하겠습니다|합니다|할게요)[.!]?\s*|(?:Sure|Okay|Got it)[,.]?\s*(?:I'll|Let me)\s*(?:continue|start|write)[.!]?\s*)/i,
      "",
    )
    .replace(/\n{3,}/g, "\n\n")
    .trim();
