...
export function buildTurnSignal(text: string): TurnSignal {
  ...
  return {
    ...
    humorLevel: Math.min(
      1,
      HUMOR_MARKERS.filter((m) => lower.includes(m)).length * 0.3,
    ),
    connectiveDensity: Math.min(
      1,
      CONNECTIVES.filter((c) => lower.includes(c)).length /
        Math.max(1, text.length / 100),
    ),
...
export function verdictToPromptModifier(
  verdict: HFCPVerdict,
  nrg: NRGStrategy,
  isKO: boolean,
): string {
...
export function processHFCPTurn(
  state: HFCPState,
  userInput: string,
): {
  mode: InputMode;
