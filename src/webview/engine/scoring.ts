...
const EMOTION_KEYWORD_REGEXES = EMOTION_KEYWORDS_KO.map(
  (kw) => new RegExp(kw, "g"),
);
const SENSORY_KEYWORD_REGEXES = SENSORY_KEYWORDS_KO.map(
  (kw) => new RegExp(kw, "g"),
);

export function calculateEOSScore(
  text: string,
  precomputedSentenceCount?: number,
): number {
...
export function calculateGrade(
  ...
  const tensionDelta =
    tensionTarget != null && actualTension != null
...
const TENSION_KEYWORD_REGEXES = TENSION_KEYWORDS.map(
  (kw) => new RegExp(kw, "gi"),
);
...
export function analyzeMetrics(
  ...
  const sentences =
    precomputedSentences ?? text.split(/[.!?。]+/).filter((s) => s.trim());
  const sentenceCount =
    precomputedCount ??
    (sentences.length > 0
      ? sentences.length
  ...
  const shortSentenceRatio =
    sentences.filter((s) => s.trim().length < 20).length / sentenceCount;
  const tension = Math.min(
    100,
    Math.round((tensionHits / sentenceCount) * 200 + shortSentenceRatio * 50),
  );
...
export function generateEngineReport(
  ...
  const tensionTarget = Math.round(
  ...
  const sentenceCount =
    sentences.length <= 1 && text.length > 100
      ? Math.ceil(text.length / 200)
      : sentences.length || 1;
