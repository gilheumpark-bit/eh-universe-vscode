...
describe("prism-builder", () => {
  let buildPrismBlock:
    | ((config: StoryConfig, isKO: boolean) => string)
    | undefined;
...
describe("platform-builder (pipeline.ts)", () => {
  ...
  beforeAll(async () => {
    try {
      ...
      // 향후 분리 시 여기서 import
      const mod = (await import("../builders/platform-builder").catch(
        () => null,
      )) as Record<string, unknown> | null;
      if (mod && typeof mod.buildPlatformBlock === "function")
        buildPlatformBlock = mod.buildPlatformBlock as (p: string) => string;
    } catch {
      // skip
    }
  });
