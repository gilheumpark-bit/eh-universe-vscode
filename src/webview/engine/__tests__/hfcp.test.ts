...
describe("scoring", () => {
  ...
  it("rich input affects score", () => {
    ...
    updateScore(
      state,
      buildTurnSignal(
        "왜 이런 구조야? 하지만 이건 좀 다르지 않아? 어떻게 생각해? 그래서 뭐가 좋아?",
      ),
    );
...
describe("NRG", () => {
  ...
  it("디테일하게 회수 마다 다른 각도 검증 (detailed verification of different angles per iteration)", () => {
    ...
    expect(
      verdictToPromptModifier("normal_free", "light_variation", true),
    ).toContain("이전과 다른 구조로");
    ...
    expect(
      verdictToPromptModifier("normal_free", "frame_shift", true),
    ).toContain("다른 관점에서");
    ...
    expect(
      verdictToPromptModifier("normal_free", "perspective_shift", true),
    ).toContain("비평적 시점");
    ...
    expect(verdictToPromptModifier("normal_free", "meta_ack", true)).toContain(
      "다른 각도에서",
    );
  });
});
