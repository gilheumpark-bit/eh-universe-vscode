...
describe("evaluateQuality", () => {
  ...
  it("grade_below reason appears when grade is too low", () => {
    ...
    const result = evaluateQuality(SAMPLE_TEXT, makeConfig(), strict);
    const gradeReason = result.failReasons.find((r) =>
      r.startsWith("grade_below"),
    );
  ...
  it("director_below reason appears when director score threshold is high", () => {
    ...
    const result = evaluateQuality(SAMPLE_TEXT, makeConfig(), strict);
    const reason = result.failReasons.find((r) =>
      r.startsWith("director_below"),
    );
  ...
  it("different episodes affect tension target", () => {
    const thresholds = getDefaultThresholds("beginner");
    const earlyEp = evaluateQuality(
      SAMPLE_TEXT,
      makeConfig({ episode: 1 }),
      thresholds,
    );
    const climaxEp = evaluateQuality(
      SAMPLE_TEXT,
      makeConfig({ episode: 20 }),
      thresholds,
    );
...
describe("buildRetryHint", () => {
  ...
  it("tension_misaligned produces tension hint", () => {
    const result = makeResult({
      failReasons: ["tension_misaligned: delta=35 > 25"],
    });
    const hint = buildRetryHint(result, 1, true);
