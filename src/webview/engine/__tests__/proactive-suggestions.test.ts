...
function makeConfig(
  overrides: Partial<SuggestionConfig> = {},
): SuggestionConfig {
  return {
    enabled: true,
    maxPerGeneration: 5,
...
describe("generateSuggestions", () => {
  ...
  describe("character_drift", () => {
    ...
    it("does not fire when character appeared recently", () => {
      ...
      const results = generateSuggestions(ctx, makeConfig());
      expect(
        results.find((s) => s.category === "character_drift"),
      ).toBeUndefined();
    });
  ...
  describe("tension_mismatch", () => {
    it("fires when tension delta exceeds 30", () => {
      ...
      const ctx = makeCtx({
        currentEpisode: 10,
        recentMetrics: [
          { tension: 10, pacing: 50, immersion: 50, eos: 50, grade: "A" },
        ],
        config: makeStoryConfig({ totalEpisodes: 25 }),
      });
      const results = generateSuggestions(ctx, makeConfig());
      expect(
        results.find((s) => s.category === "tension_mismatch"),
      ).toBeDefined();
    });
  ...
  describe("emotion_flat", () => {
    ...
    it("does not fire when any of the last 3 has EOS >= 30", () => {
      ...
      const results = generateSuggestions(ctx, makeConfig());
      expect(
        results.find((s) => s.category === "emotion_flat"),
      ).toBeUndefined();
    });
  ...
  describe("pacing_anomaly", () => {
    it("fires warning when pacing < 30", () => {
      const ctx = makeCtx({
        recentMetrics: [
          { tension: 50, pacing: 20, immersion: 50, eos: 50, grade: "A" },
        ],
      });
      const results = generateSuggestions(ctx, makeConfig());
    ...
    it("fires info when pacing > 90", () => {
      const ctx = makeCtx({
        recentMetrics: [
          { tension: 50, pacing: 95, immersion: 50, eos: 50, grade: "A" },
        ],
      });
      const results = generateSuggestions(ctx, makeConfig());
      const pacing = results.find((s) => s.category === "pacing_anomaly");
    ...
    it("does not fire for normal pacing (30-90)", () => {
      const ctx = makeCtx({
        recentMetrics: [
          { tension: 50, pacing: 60, immersion: 50, eos: 50, grade: "A" },
        ],
      });
      const results = generateSuggestions(ctx, makeConfig());
      expect(
        results.find((s) => s.category === "pacing_anomaly"),
      ).toBeUndefined();
    });
  });
  ...
  describe("ai_tone_creep", () => {
    ...
    it("does not fire when grades are decent", () => {
      ...
      const results = generateSuggestions(ctx, makeConfig());
      expect(
        results.find((s) => s.category === "ai_tone_creep"),
      ).toBeUndefined();
    });
