...
function makeFullConfig(overrides: Partial<StoryConfig> = {}): StoryConfig {
  return {
    ...
    totalEpisodes: 20,
    synopsis:
      "AI가 지배하는 도시에서 인간이 감정을 되찾는 이야기. 주인공은 기억을 잃었다.",
      ...
      characters: [
        {
          ...
          name: "노역할",
          role: "",
          traits: "충분히 긴 트레이트 설명입니다 이것은 충분합니다",
          appearance: "",
          dna: 50,
        },
      ],
...
describe("executePipeline — edge cases", () => {
  ...
  it("direction_setup with no guardrails object", () => {
    ...
    const exec = executePipeline(
      {
        config: makeFullConfig({
          guardrails: undefined as unknown as { min: number; max: number },
        }),
        currentEpisode: 1,
      },
      cfg,
