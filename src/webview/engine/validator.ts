...
export function validateAITone(text: string): {
  score: number;
  fixes: FixRecord[];
} {
...
export function validateQuality(text: string): {
  showTellIssues: FixRecord[];
  repetitionIssues: FixRecord[];
  score: number;
} {
  // ReDoS prevention
...
function isCompoundWord(
  ...
  const after =
    position + wordLength < text.length ? text[position + wordLength] : "";
  const koreanRange = /[\uAC00-\uD7AF]/;
...
export function validateStatic(text: string): {
  fixes: FixRecord[];
...
export function applyFormattingRules(text: string): {
  formatted: string;
  changes: string[];
} {
  if (text.length > MAX_TEXT_LENGTH) text = text.slice(0, MAX_TEXT_LENGTH);
...
export function validateFormattingIssues(text: string): ValidationIssue[] {
  ...
  if (parenCount > 0) {
    issues.push({
      category: "formatting",
      message: `괄호 ${parenCount}개 잔존`,
      severity: Severity.WARNING,
      suggestion: "괄호 기호를 제거하고 내용만 유지하세요",
    });
  }
  ...
  if (dashCount > 0) {
    issues.push({
      category: "formatting",
      message: `Em dash(—) ${dashCount}개 잔존`,
      severity: Severity.WARNING,
      suggestion: "Em dash를 삭제하세요",
    });
  }
  ...
  if (dotCount > 0) {
    issues.push({
      category: "formatting",
      message: `마침표 3개 이상(${dotCount}건) → 말줄임표(…)로 통일 필요`,
      severity: Severity.INFO,
    });
  }
  ...
  if (inlineDialogue && inlineDialogue.length > 0) {
    issues.push({
      category: "formatting",
      message: `인라인 대화문 ${inlineDialogue.length}건 — 줄 분리 필요`,
      severity: Severity.WARNING,
    });
  }
...
export function calculateCleanTaste(text: string): {
  aiTone: number;
  humanNoise: number;
  balance: number;
} {
...
export function validate10PartStructure(text: string): {
  withinRange: boolean;
  partSizes: number[];
} {
...
export function validateGeneratedContent(
  ...
  if (cleanTaste.balance < 0.5) {
    allIssues.push({
      category: "clean_taste",
      message: `클린테이스트 ${cleanTaste.balance.toFixed(2)} (목표 0.6~0.8) — AI톤 과다`,
      severity: Severity.WARNING,
    });
  }
  if (cleanTaste.balance > 0.85) {
    allIssues.push({
      category: "clean_taste",
      message: `클린테이스트 ${cleanTaste.balance.toFixed(2)} — 과도하게 다듬어짐`,
      severity: Severity.INFO,
    });
  }
...
const TRADEMARK_PATTERNS: {
  ...
  // Games / Entertainment
  {
    pattern: /포켓몬|피카츄|Pokémon|Pokemon|Pikachu/gi,
    replacement: "○○몬",
    category: "game",
  },
  {
    pattern: /마리오|Mario|루이지|Luigi/gi,
    replacement: "○○오",
    category: "game",
  },
  {
    pattern: /젤다|Zelda|링크|Link(?=가|를|의|은|는)/gi,
    replacement: "○○다",
    category: "game",
  },
  {
    pattern: /마인크래프트|Minecraft/gi,
    replacement: "블록월드",
    category: "game",
  },
  {
    pattern: /리그 오브 레전드|League of Legends|LOL|롤/gi,
    replacement: "○○ 리그",
    category: "game",
  },
  { pattern: /오버워치|Overwatch/gi, replacement: "○○워치", category: "game" },
  {
    pattern: /포트나이트|Fortnite/gi,
    replacement: "○○나이트",
    category: "game",
  },
  ...
  {
    pattern: /진격의 거인|Attack on Titan/gi,
    replacement: "○○의 거인",
    category: "anime",
  },
  {
    pattern: /귀멸의 칼날|Demon Slayer/gi,
    replacement: "○○의 칼날",
    category: "anime",
  },
  {
    pattern: /해리\s?포터|Harry\s?Potter/gi,
    replacement: "○○ 포터",
    category: "novel",
  },
  {
    pattern: /반지의 제왕|Lord of the Rings/gi,
    replacement: "○○의 제왕",
    category: "novel",
  },
  ...
  {
    pattern: /갤럭시|Galaxy(?=\s|S|노트|폰)/gi,
    replacement: "스마트폰",
    category: "brand",
  },
  { pattern: /구글|Google/gi, replacement: "검색엔진", category: "brand" },
  {
    pattern: /애플|Apple(?=\s|의|이|은|는|을)/gi,
    replacement: "IT기업",
    category: "brand",
  },
  { pattern: /테슬라|Tesla/gi, replacement: "전기차", category: "brand" },
  {
    pattern: /페이스북|Facebook|메타|Meta(?=버스)/gi,
    replacement: "소셜플랫폼",
    category: "brand",
  },
  {
    pattern: /인스타그램|Instagram/gi,
    replacement: "사진앱",
    category: "brand",
  },
  {
    pattern: /유튜브|YouTube/gi,
    replacement: "동영상플랫폼",
    category: "brand",
  },
  {
    pattern: /트위터|Twitter/gi,
    replacement: "마이크로블로그",
    category: "brand",
  },
...
const _KOREAN_CHAR_RE =
  /[\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uD7B0-\uD7FF]/;

// Common Korean particles/postpositions that can follow a trademark word
// without making it part of a longer compound word.
const _KOREAN_PARTICLES_RE =
  /^[을를이가은는의에와과도로으며면서까지부터만보다라]|^으로|^에서|^까지|^부터|^처럼|^마저|^조차/;

function isKoreanWordBoundary(
  text: string,
  matchStart: number,
  matchEnd: number,
): boolean {
  ...
  for (const target of [text, normalized]) {
    for (const { pattern, replacement, category } of TRADEMARK_PATTERNS) {
      ...
      while ((m = regex.exec(target)) !== null) {
        ...
        if (target === normalized && matches.some((x) => x.original === m![0]))
          continue;
        // Korean word boundary check: skip if the match is a substring of a longer Korean word
        if (!isKoreanWordBoundary(target, m.index, m.index + m[0].length))
          continue;
        matches.push({
          original: m[0],
          replacement,
...
export function filterTrademarks(text: string): {
  filtered: string;
...
function validateTrademarkIP(text: string): {
  fixes: FixRecord[];
  issues: ValidationIssue[];
} {
  const fixes: FixRecord[] = [];
