...
export function isExperienceScore(
  s: ChunkScoreDetail,
): s is ExperienceScoreDetail {
 ...
 */
function buildExperienceDirective(
  band: number,
  targetLang: TranslationTarget,
): string {
  const b = clampBand(band);
...
function buildCountrySpecificDirective(
  targetLang: TranslationTarget,
  mode: TranslationMode,
): string {
...
export function buildTranslationSystemPrompt(
  ...
  const safeConfig: TranslationConfig = {
    ...
    band: safeBand,
    contextBridge:
      config.contextBridge?.slice(0, MAX_CONTEXT_BRIDGE_CHARS) ?? "",
  };
...
Output ONLY the recreated text, nothing else.`);
  ...
  if (isMode1) {
    ...
    parts.push(
      buildExperienceDirective(safeConfig.band, safeConfig.targetLang),
    );
  }
...
${glossaryLines.join("\n")}`);
  ...
  const countryDirective = buildCountrySpecificDirective(
    safeConfig.targetLang,
    safeConfig.mode,
  );
  if (countryDirective) {
  ...
  // 장르 프리셋 주입 (PART 13)
  if (safeConfig.genre) {
    const genreDirective = buildGenreTranslationDirective(
      safeConfig.genre,
      safeConfig.targetLang,
    );
  ...
  // 캐릭터 레지스터 주입 (PART 14)
  if (
    safeConfig.characterRegisters &&
    safeConfig.characterRegisters.length > 0
  ) {
    const registerDirective = buildCharacterRegisterDirective(
      safeConfig.characterRegisters,
      safeConfig.targetLang,
    );
  ...
  if (safeConfig.translatorProfile) {
    const profileHint = buildTranslatorProfileHint(
      safeConfig.translatorProfile,
    );
  ...
  for (let i = 0; i < paragraph.length; i++) {
    ...
    if (QUOTE_OPENERS.has(ch)) {
      ...
      if (ch === '"') {
        if (
          quoteStack.length > 0 &&
          quoteStack[quoteStack.length - 1] === '"'
        ) {
          quoteStack.pop();
        } else {
          quoteStack.push(ch);
    ...
    if (ch === "." && i >= 1) {
      ...
      if (prev >= "A" && prev <= "Z" && next && next >= "A" && next <= "Z")
        continue;
    }
...
function parseFidelityScore(raw: string): FidelityScoreDetail {
  ...
  try {
    ...
    const overall = (1 - t) * 0.35 + f * 0.3 + n * 0.25 + c * 0.1;
    return {
...
function parseExperienceScore(raw: string): ExperienceScoreDetail {
  const fallback: ExperienceScoreDetail = {
    overall: 0.5,
    immersion: 0.5,
    emotionResonance: 0.5,
    culturalFit: 0.5,
    consistency: 1.0,
    groundedness: 0.5,
    voiceInvisibility: 0.5,
  };
  try {
    const jsonMatch = raw.match(/\{[\s\S]*?\}/);
    if (!jsonMatch) return fallback;
    ...
    const overall =
      im * 0.22 + er * 0.22 + cf * 0.16 + co * 0.1 + gr * 0.15 + vi * 0.15;
    return {
      overall: round3(overall),
      immersion: im,
      emotionResonance: er,
...
export function buildRecreatePrompt(
  ...
  if (mode === "fidelity" && isFidelityScore(scoreDetail)) {
    if (scoreDetail.translationese > 0.4)
      issues.push(
        "Reads like a translation (번역투). Rewrite to sound native.",
      );
    if (scoreDetail.fidelity < 0.6)
      issues.push("Strayed too far from source meaning or structure.");
    if (scoreDetail.naturalness < 0.6)
      issues.push("Unnatural phrasing or rhythm.");
    if (scoreDetail.consistency < 0.8)
      issues.push("Glossary terms inconsistent.");
  } else if (mode === "experience" && isExperienceScore(scoreDetail)) {
    if (scoreDetail.immersion < 0.6)
      issues.push(
        "Reader would pause — not immersive enough. Sounds translated.",
      );
    if (scoreDetail.emotionResonance < 0.6)
      issues.push(
        "Emotional impact lost or distorted. The feeling doesn't match the source.",
      );
    if (scoreDetail.culturalFit < 0.6)
      issues.push("Cultural references confuse the target reader.");
    if (scoreDetail.consistency < 0.8)
      issues.push("Names, terms, or tone inconsistent.");
    if (scoreDetail.groundedness < 0.7)
      issues.push(
        "Groundless additions detected: words/phrases/implications not in the source were inserted. Remove ALL added time markers, emotional hedging, interpretive additions, and atmospheric padding.",
      );
    if (scoreDetail.voiceInvisibility < 0.7)
      issues.push(
        "Translator's literary voice is showing. Simplify crafted sentences back to the source's own register. The translator must be invisible.",
      );
  }
...
export function modeDescription(
  ...
  if (mode === "fidelity") {
    return isKO
      ? {
          title: "MODE 1 — 원문 보존",
          desc: "원문의 구조, 문장 형식, 리듬을 최대한 유지하며 번역합니다.",
        }
      : {
          title: "MODE 1 — Source Preservation",
          desc: "Preserves the original structure, sentence form, and rhythm.",
        };
  }
  return isKO
    ? {
        title: "MODE 2 — 독자 경험",
        desc: "감정선을 보존하되, 타겟 독자의 몰입을 위해 리듬 재구성과 문화 적응을 허용합니다. 무근거 보강과 번역자 세공은 차단됩니다.",
      }
    : {
        title: "MODE 2 — Reader Experience",
        desc: "Preserves emotional arc while allowing rhythm restructuring and cultural adaptation. Groundless additions and translator literary polish are blocked.",
      };
}
...
export function staticValidate(
  ...
  const aiToneScore = translatedIsKorean
    ? validateAITone(translatedText).score
    : 0;
  ...
  const translatedEOS = translatedIsKorean
    ? calculateEOSScore(translatedText)
    : sourceEOS;
...
export function buildAutoBridge(
  ...
  const characters = (glossary ?? []).filter(
    (g) =>
      g.context?.includes("인물") ||
      g.context?.includes("character") ||
      g.context?.includes("이름") ||
  ...
  const charEntries =
    characters.length > 0
      ? characters
      : (glossary ?? []).filter(
          (g) => g.locked && /^[가-힣]{2,4}$/.test(g.source),
        );
  if (charEntries.length > 0) {
    lines.push(
      `[Active characters] ${charEntries.map((c) => `${c.source}→${c.target}`).join(", ")}`,
    );
  }

  // 번역 모드/밴드 (일관성 유지)
  lines.push(
    `[Prev translation: MODE=${prevResult.mode}, band=${prevResult.band.toFixed(3)}]`,
  );
...
export function buildGenreTranslationDirective(
  ...
  const lang = langName(targetLang);
  const pacingGuide =
    PACING_TO_TRANSLATION[preset.pacing] ?? "Maintain the source text pacing.";
  const tensionGuide =
    preset.tensionBase >= 0.7
...
const PACING_TO_TRANSLATION: Record<string, string> = {
  slow_burn_with_spikes:
    "Preserve slow, building rhythm. Sentences should breathe. Spike moments get abrupt, short sentences.",
  fast_spikes:
    "Keep rapid pacing. Short sentences, active verbs. Action sequences must feel fast even in translation.",
  epic_waves:
    "Wave-like rhythm: build slowly, crest intensely, trough for reflection. Mirror this in sentence length variation.",
  steady_rise_with_reversals:
    "Steady escalation with sharp turns. Each reversal moment needs maximum clarity and impact.",
  slow_build_to_spike:
    "Extended atmospheric buildup. Long, creeping sentences → sudden short impact. The contrast IS the genre.",
  layered_accumulation:
    "Layer upon layer of meaning. Each sentence adds another dimension. Maintain this density in translation.",
};
...
export function buildTranslatorProfileHint(profile: TranslatorProfile): string {
  ...
  if (topErrors.length > 0) {
    hints.push(
      `[Translator Pattern Correction] Recurring errors in past translations: ${topErrors.join(", ")}. Actively avoid these patterns.`,
    );
  }
  ...
  if (profile.termConsistencyRate < 0.85) {
    hints.push(
      "[Terminology Alert] Past translations showed terminology inconsistency. Double-check all glossary terms.",
    );
  }

  // 톤 정합이 낮으면 경고
  if (profile.toneAlignmentRate < 0.8) {
    hints.push(
      "[Tone Alignment Alert] Past translations had tone drift. Match the source text tone precisely.",
    );
  }
...
export function updateTranslatorProfile(
  ...
  p.termConsistencyRate = isFirst
    ? termConsistency
    : alpha * termConsistency + (1 - alpha) * p.termConsistencyRate;
  p.toneAlignmentRate = isFirst
    ? toneAlignment
    : alpha * toneAlignment + (1 - alpha) * p.toneAlignmentRate;
...
/** 빈 번역 프로필 생성 */
export function createEmptyTranslatorProfile(
  id: string = "default",
): TranslatorProfile {
  return {
    id,
...
export function adaptiveChunkSize(
  ...
  const sampleTokens = sampleSentences
    .slice(0, 10)
    .map((s) => estimateTokens(s));
  const avgTokensPerSentence =
    sampleTokens.reduce((a, b) => a + b, 0) / sampleTokens.length;
  ...
  const optimal = Math.max(
    1,
    Math.floor(maxTokensPerChunk / avgTokensPerSentence),
  );
...
/** 언어쌍별 예상 길이 확장 비율 */
const LENGTH_RATIO_RANGES: Record<
  TranslationTarget,
  { min: number; max: number }
> = {
  EN: { min: 1.1, max: 1.6 }, // KO→EN: 보통 1.2~1.4x
  JP: { min: 0.85, max: 1.2 }, // KO→JP: 비슷하거나 약간 짧음
  CN: { min: 0.8, max: 1.15 }, // KO→CN: 한자 압축으로 짧을 수 있음
  KO: { min: 0.9, max: 1.1 }, // KO→KO: 거의 동일
};
...
export function verifyLength(
  ...
  if (ratio < range.min) {
    issues.push(
      `length_too_short: ratio=${ratio.toFixed(2)} < expected_min=${range.min}`,
    );
  }
  if (ratio > range.max) {
    issues.push(
      `length_too_long: ratio=${ratio.toFixed(2)} > expected_max=${range.max}`,
    );
  }
  ...
  const tolerance =
    mode === "fidelity" ? 2 : Math.max(3, Math.ceil(srcSentences * 0.3));

  if (Math.abs(delta) > tolerance) {
    issues.push(
      `sentence_count_mismatch: source=${srcSentences}, translated=${tgtSentences}, delta=${delta}`,
    );
  }
...
export function hasCriticalAxisFailure(
  score: ChunkScoreDetail,
  mode: TranslationMode,
): boolean {
  if (mode === "fidelity" && isFidelityScore(score)) {
    // 번역투가 너무 높으면 무조건 실패
    if (score.translationese > 0.6) return true;
    // 충실도가 너무 낮으면 무조건 실패
    if (score.fidelity < 0.4) return true;
    return false;
  }
  if (mode === "experience" && isExperienceScore(score)) {
    ...
    // 몰입도가 바닥이면 무조건 실패
    if (score.immersion < 0.4) return true;
    return false;
  }
  return false;
}
