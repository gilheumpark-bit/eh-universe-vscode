...
export function getDefaultGateConfig(
  level: SkillLevel,
  tierMaxRetries?: number,
): QualityGateConfig {
  const baseRetries = level === "advanced" ? 5 : 3;
  return {
    enabled: true,
    maxRetries:
      tierMaxRetries !== undefined
        ? Math.min(baseRetries, tierMaxRetries)
        : baseRetries,
...
export function evaluateQuality(
  ...
  const avgScore =
    (report.metrics.tension +
      report.metrics.pacing +
      report.metrics.immersion) /
    3;
  ...
  const tag = calculateQualityTag(
    director,
    config.narrativeIntensity ?? "standard",
  );
  ...
  if (director.score < thresholds.minDirectorScore) {
    failReasons.push(
      `director_below: ${director.score} < ${thresholds.minDirectorScore}`,
    );
  }
  ...
  if (tensionDelta > thresholds.minTensionAlignment) {
    failReasons.push(
      `tension_misaligned: delta=${Math.round(tensionDelta)} > ${thresholds.minTensionAlignment}`,
    );
  }

  // 5. AI tone check
  const aiToneFindings = director.findings.filter(
    (f) => f.kind === "AI_TONE" || f.kind === "ESCAPE",
  );
  const aiTonePercent =
    text.length > 0 ? (aiToneFindings.length / (text.length / 1000)) * 100 : 0;
  if (aiTonePercent > thresholds.maxAITonePercent) {
    failReasons.push(
      `ai_tone_high: ${Math.round(aiTonePercent)}% > ${thresholds.maxAITonePercent}%`,
    );
  }
...
export function buildRetryHint(
  ...
  for (const reason of result.failReasons) {
    if (reason.startsWith("grade_below")) {
      hints.push(
        isKO
          ? "전체 품질을 높여주세요. 감정 묘사와 감각 표현을 강화하세요."
          : "Improve overall quality. Enhance emotional and sensory descriptions.",
      );
    } else if (reason.startsWith("director_below")) {
      hints.push(
        isKO
          ? "인과관계를 명확히 하고, AI 요약 문투를 피하세요."
          : "Clarify cause-effect and avoid AI summary tone.",
      );
    } else if (reason.startsWith("eos_below")) {
      hints.push(
        isKO
          ? "감정 키워드와 내면 독백을 더 넣어주세요."
          : "Add more emotion keywords and inner monologue.",
      );
    } else if (reason.startsWith("tension_misaligned")) {
      hints.push(
        isKO
          ? "긴장감을 목표 수준에 맞춰주세요. 갈등과 위기감을 조절하세요."
          : "Align tension to target level. Adjust conflict and urgency.",
      );
    } else if (reason.startsWith("ai_tone")) {
      hints.push(
        isKO
          ? '"요약하자면", "결론적으로" 같은 AI 문투를 제거하세요.'
          : 'Remove AI-like phrases such as "in summary", "in conclusion".',
      );
    }
  }
  ...
  if (attempt >= 3) {
    hints.push(
      isKO
        ? "이전 시도의 문제를 반드시 해결한 뒤 작성하세요."
        : "You MUST fix the issues from previous attempts.",
    );
  }
