...
export function updateProfile(
  ...
  const avgLen =
    sentences.reduce((s, sent) => s + sent.length, 0) /
    Math.max(sentences.length, 1);
  const dialogueLines = metrics.text
    .split("\n")
    .filter((l) => /^[「『"']/.test(l.trim()) || /^[""']/.test(l.trim()));
  const dlgRatio =
    dialogueLines.length / Math.max(metrics.text.split("\n").length, 1);
  ...
  p.avgEpisodeLength = isFirst
    ? metrics.text.length
    : ema(p.avgEpisodeLength, metrics.text.length);
  p.pacingPreference = isFirst
    ? metrics.pacing
    : ema(p.pacingPreference, metrics.pacing);
  p.avgGrade = isFirst
    ? (GRADE_NUMERIC[metrics.grade] ?? 70)
    : ema(p.avgGrade, GRADE_NUMERIC[metrics.grade] ?? 70);
  p.avgDirectorScore = isFirst
    ? metrics.directorScore
    : ema(p.avgDirectorScore, metrics.directorScore);
  p.avgEOS = isFirst ? metrics.eosScore : ema(p.avgEOS, metrics.eosScore);

  // 감정 밀도 — EOS 기반
  p.emotionDensity = isFirst
    ? metrics.eosScore / 100
    : ema(p.emotionDensity, metrics.eosScore / 100);
...
function determineLevel(p: WriterProfile): SkillLevel {
  if (p.episodeCount >= 30 && p.avgGrade >= 80 && p.overrideRate >= 0.2)
    return "advanced";
...
export function buildProfileHint(
  ...
  if (profile.dialogueRatio > 0.6) {
    hints.push(
      isKO
        ? "이 작가는 대화를 선호합니다. 대화 중심으로 작성하되, 묘사도 균형있게 넣으세요."
        : "This writer prefers dialogue. Write dialogue-heavy but balance with description.",
    );
  } else if (profile.dialogueRatio < 0.2) {
    hints.push(
      isKO
        ? "이 작가는 서술을 선호합니다. 내면 묘사와 장면 서술에 집중하세요."
        : "This writer prefers narration. Focus on inner thoughts and scene description.",
    );
  }
