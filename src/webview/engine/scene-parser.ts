...
const DESCRIPTION_KEYWORDS =
  /^(?:.*?(?:하늘|바다|산|숲|건물|거리|방|복도|창문|문|빛|어둠|바람|비|눈|해|달|별|노을|안개|연기|냄새|소리|향기|공기|온도|계절|봄|여름|가을|겨울|아침|낮|밤|새벽|저녁|오후|dawn|dusk|night|morning|rain|snow|wind|sky|sea|forest|building|street|room))/i;

/** 행동 키워드 */
const ACTION_KEYWORDS =
  /(?:걸었다|뛰었다|멈추었다|돌아보았다|손을|고개를|일어섰다|앉았다|눈을|입을|주먹을|달려|잡았다|놓았다|열었다|닫았다|올려|내려|walked|ran|stopped|turned|grabbed|opened|closed|stood|sat)/;
...
function estimateEmotion(text: string): Emotion {
  ...
  for (const [key, patterns] of Object.entries(EMOTION_WORDS)) {
    ...
    (result as unknown as Record<string, number>)[key] = Math.min(
      1,
      count / (len / 200),
    );
  }
...
function estimateTempo(text: string): Tempo {
  const avgSentenceLen =
    text.length / Math.max(1, (text.match(/[.!?。]+/g) || []).length);
...
const LOCATION_KEYWORDS =
  /(?:으?로\s*(?:돌아|향|갔|왔|이동)|에\s*(?:도착|들어섰|나왔)|에서\s|밖으로|안으로|(?:went|arrived|entered|left|returned)\s+(?:to|at|from))/i;
const TIME_KEYWORDS =
  /(?:시간이?\s*(?:지나|흘러)|후에?|뒤에?|다음\s*날|이튿날|아침|저녁|새벽|밤|later|next\s*(?:day|morning)|hours?\s*(?:later|passed))/i;
...
const LOCATION_EXTRACT =
  /(?:에서|에|로|으로|안|밖|위|아래|앞|뒤|속|at|in|on|inside|outside|near)\s/;
const TIME_EXTRACT =
  /(?:아침|낮|오후|저녁|밤|새벽|해질녘|dawn|morning|noon|afternoon|evening|night|dusk|midnight)/i;
...
function extractSceneMeta(beats: SceneBeat[]): {
  ...
  const location =
    firstDesc.slice(0, 30).replace(/[.!?,。]+$/, "") || undefined;
  ...
  const promptParts: string[] = [
    "anime style illustration, novel scene background",
  ];
...
const TENSION_WORDS =
  /(?:위험|급|갑자기|폭발|비명|긴장|전투|충돌|죽|피|칼|총|도망|쫓|danger|explosion|scream|fight|blood|death|chase|crash)/gi;
...
function calculateSceneTension(beats: SceneBeat[]): number {
  ...
  const shortSentenceRatio =
    beats.filter((b) => b.text.length < 20).length / Math.max(1, beats.length);
  const dialogueRatio =
...
function estimateBeatDuration(beat: SceneBeat): number {
  ...
  switch (beat.type) {
    case "dialogue":
      return Math.max(2, charCount / 8); // 대사: 초당 8자 (음성 속도)
    case "thought":
      return Math.max(2, charCount / 6); // 독백: 약간 느림
    case "description":
      return Math.max(3, charCount / 10); // 묘사: 읽기 시간
    case "action":
      return Math.max(1.5, charCount / 12); // 행동: 빠르게
    case "narration":
      return Math.max(2, charCount / 10); // 서술: 보통
    default:
      return Math.max(2, charCount / 10); // P1#9: 안전 폴백
  }
}
...
export function parseManuscript(
  ...
  for (let si = 0; si < boundaries.length; si++) {
    ...
    const endLine =
      si + 1 < boundaries.length ? boundaries[si + 1].lineIndex : lines.length;
    ...
    for (let li = 0; li < sceneLines.length; li++) {
      ...
      if (SCENE_BREAK_MARKERS.test(line.trim())) {
        prevLine = line;
        continue;
      }
  ...
  if (scenes.length === 1 && text.length > 3000)
    warnings.push("장면 구분이 없습니다. *** 또는 빈 줄을 추가해보세요.");

  for (const scene of scenes) {
    const dialogueCount = scene.beats.filter(
      (b) => b.type === "dialogue",
    ).length;
    const totalCount = scene.beats.length;
    if (totalCount > 8 && dialogueCount === 0) {
      warnings.push(
        `장면 ${scene.index + 1}: 대사 없이 서술만 ${totalCount}비트`,
      );
    }
    const noSpeaker = scene.beats.filter(
      (b) => b.type === "dialogue" && !b.speaker,
    ).length;
    if (noSpeaker > 0) {
  ...
  const totalDuration = Math.round(
    allBeats.reduce((s, b) => s + estimateBeatDuration(b), 0),
  );
...
export function generateVoiceMappings(characters: Character[]): VoiceMapping[] {
  ...
  for (const char of characters) {
    const traits =
      `${char.traits ?? ""} ${char.personality ?? ""} ${char.speechStyle ?? ""}`.toLowerCase();
...
export function adjustVoiceForEmotion(
  ...
  const dominant = (Object.entries(emotion) as [keyof Emotion, number][]).sort(
    (a, b) => b[1] - a[1],
  )[0];
    ...
    speak(text: string, voice: VoiceMapping, emotion?: Emotion): Promise<void> {
      return new Promise((resolve, reject) => {
        ...
        const adjusted = emotion
          ? adjustVoiceForEmotion(voice, emotion)
          : voice;
