...
export function APIKeySlotManager({ onClose }: Props) {
  const { slots, addSlot, removeSlot, toggleSlot, enabledSlots } =
    useUnifiedSettings();
  const { lang } = useLang();
  ...
  const T = (v: { ko: string; en: string; ja?: string; zh?: string }) =>
    L4(lang, v);
    ...
    >
      <div className="bg-bg-primary border border-border rounded-2xl w-full max-w-lg flex flex-col shadow-luxury mx-4">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-text-tertiary">
                {T({
                  ko: "프로바이더를 선택하고 키를 입력하세요",
                  en: "Select a provider and enter your key",
                })}
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-bg-tertiary text-text-tertiary transition-colors"
          >
            <X size={18} />
          </button>
        </div>
          ...
          <div className="px-4 pt-3 pb-1 space-y-1.5">
            ...
            {slots.map((slot) => {
              ...
              return (
                <div
                  key={slot.id}
                  className={`flex items-center gap-2.5 px-3 py-2 rounded-xl border transition-all ${slot.enabled ? "border-border bg-bg-secondary/60" : "border-transparent bg-bg-tertiary/30 opacity-50"}`}
                >
                  <div
                  ...
                  >
                    {slot.enabled ? (
                      ...
                      <ToggleLeft size={16} className="text-text-tertiary" />
                    )}
                  </button>
                  <button
                    onClick={() => removeSlot(slot.id)}
                    className="p-1 rounded hover:bg-bg-tertiary transition-colors text-text-tertiary hover:text-accent-red"
                    title="Delete"
                  >
                    <Trash2 size={13} />
                  </button>
              ...
              return (
                ...
                >
                  ...
                  {isRegistered && (
                    <Check
                      size={10}
                      className="absolute top-1 right-1.5 text-accent-green"
                    />
                  )}
                </button>
              );
              ...
              <div className="flex items-center gap-2 bg-bg-primary border border-border rounded-lg px-3 py-2">
                ...
                >
                  {testing ? (
                    ...
                    <X size={10} className="text-accent-red" />
                  ) : null}
                  {T({ ko: "검증", en: "Test" })}
                </button>
              </div>
        ...
        <div className="border-t border-border px-5 py-3 flex items-center justify-between">
          <div className="text-[11px] text-text-tertiary">
            {T({
              ko: `활성: ${enabledSlots.length}개`,
              en: `Active: ${enabledSlots.length}`,
            })}
          </div>
          <div className="text-[10px] text-text-tertiary">
            {T({
              ko: "모든 스튜디오에서 공유됩니다",
              en: "Shared across all studios",
            })}
          </div>
        </div>
      </div>
