...
export default function UnifiedSettingsBar() {
  ...
  return (
    <>
      <div className="flex items-center gap-2 flex-wrap justify-center">
        ...
        {storageInfo && (
          <>
            <div className="w-px h-5 bg-border/30" />
            <div
              className="inline-flex items-center gap-1.5 px-3 py-2 rounded-full border border-border/50 bg-bg-secondary/60 backdrop-blur-sm text-text-tertiary text-xs"
            ...
            >
              ...
              <div className="w-12 h-1.5 rounded-full bg-border/30 overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${storageInfo.percent > 80 ? "bg-accent-red" : storageInfo.percent > 50 ? "bg-accent-amber" : "bg-accent-green"}`}
      ...
      {showApiKeys && (
        <APIKeySlotManager onClose={() => setShowApiKeys(false)} />
      )}
    </>
  );
}
