  ...
  const navigateChange = useCallback(
    (dir: "next" | "prev") => {
      ...
      setCurrentChangeIndex((idx) =>
        dir === "next"
          ? (idx + 1) % hunks.length
          : (idx - 1 + hunks.length) % hunks.length,
      );
    },
    [hunks],
  );
          ...
          {hunks.length > 0 && (
            <div className="flex items-center gap-1">
              <button
                ...
                className="px-2 py-1 text-xs bg-white/5 text-white/50 hover:text-white rounded"
              >
                <ChevronUp size={12} />
              </button>
              <span className="text-[10px] text-white/50 min-w-[3rem] text-center">
                {currentChangeIndex + 1}/{hunks.length}
              </span>
              <button
      ...
      {!readOnly && hunks.length > 0 && (
        <div className="border-b border-white/8 bg-[#0a0e17] max-h-28 overflow-y-auto px-3 py-1">
          <div className="text-[10px] text-white/50 mb-1">
            Changes ({hunks.filter((h) => h.status === "pending").length}{" "}
            pending):
          </div>
          {hunks.map((hunk, idx) => (
            <div
              key={hunk.id}
              className={`flex items-center gap-2 text-[10px] px-1 py-0.5 rounded ${idx === currentChangeIndex ? "bg-white/5" : ""}`}
            >
              <span className="text-white/60 min-w-[4rem]">
              ...
              {hunk.status === "pending" ? (
                <div className="flex items-center gap-1 ml-auto">
                  <button
                    ...
                    className="flex items-center gap-0.5 px-1.5 py-0.5 text-green-400 hover:bg-green-500/20 rounded"
                  >
                    <ArrowRight size={10} /> Accept
                  </button>
                  <button
                    onClick={() => handleRejectHunk(hunk.id)}
                    className="flex items-center gap-0.5 px-1.5 py-0.5 text-red-400 hover:bg-red-500/20 rounded"
                  >
                    <ArrowLeft size={10} /> Reject
                  </button>
                </div>
              ) : (
                <span
                  className={`ml-auto text-[10px] ${hunk.status === "accepted" ? "text-green-400" : "text-red-400"}`}
                >
        ...
        {renderSideBySide ? (
          <div className="flex h-full">
            <div className="flex-1 overflow-auto border-r border-white/8 p-2">
              ...
              {origLines.map((line, i) => (
                <div key={i} className="flex">
                  <span className="w-8 text-right pr-2 text-white/60 select-none">
                    {i + 1}
                  </span>
            ...
            <div className="flex-1 overflow-auto p-2">
              ...
              {modLines.map((line, i) => {
                const isChanged =
                  i < origLines.length ? origLines[i] !== line : true;
                return (
                  <div
                    key={i}
                    className={`flex ${isChanged ? "bg-green-500/10" : ""}`}
                  >
                    <span className="w-8 text-right pr-2 text-white/60 select-none">
                      {i + 1}
                    </span>
                    <span
                      className={isChanged ? "text-green-400" : "text-white/60"}
                    >
                      {line}
                    </span>
                  </div>
