...
export interface CodeStudioPanelManagerProps {
  ...
  onSetDiffState: (
    state: { original: string; modified: string; fileName: string } | null,
  ) => void;
...
function ActivityBar({
  ...
  return (
    <div className="w-12 shrink-0 border-r border-white/8 bg-bg-primary flex flex-col items-center py-2 gap-1">
        ...
        return (
          <button
            ...
            onClick={() => {
              if (item.isAction) {
                ...
                onSetRightPanel(
                  rightPanel === item.id ? null : (item.id as RightPanel),
                );
              }
            }}
          ...
          >
            ...
            {item.id === "pipeline" && bugReports.length > 0 && (
              <span className="absolute -top-0.5 -right-0.5 h-3.5 w-3.5 rounded-full bg-accent-red text-[8px] text-white flex items-center justify-center">
                {bugReports.length}
              </span>
            )}
          </button>
          ...
          .map((p) => {
            ...
            return (
              <button
                ...
                onClick={() =>
                  onSetRightPanel(
                    rightPanel === p.id ? null : (p.id as RightPanel),
                  )
                }
                className="relative w-10 h-10 flex items-center justify-center rounded-lg transition-all duration-150 hover:bg-white/6 group"
                title={lbl}
              >
                <span
                  className={`absolute left-0 top-1/2 -translate-y-1/2 w-[2px] rounded-r bg-accent-purple transition-all duration-200 ${rightPanel === p.id ? "h-5 opacity-100" : "h-0 opacity-0"}`}
                />
                {Icon ? (
                  <Icon
                    className={`h-[18px] w-[18px] transition-colors ${rightPanel === p.id ? "text-text-primary" : "text-text-tertiary group-hover:text-text-secondary"}`}
                  />
                ) : (
      ...
      >
        <ChevronRight
          className={`h-[18px] w-[18px] text-text-tertiary transition-transform ${showAdvancedPanels ? "rotate-90" : ""}`}
        />
      </button>
      <button
      ...
      >
        <Settings
          className={`h-[18px] w-[18px] ${showSettings ? "text-accent-amber" : "text-text-tertiary hover:text-text-secondary"}`}
        />
      </button>
    </div>
  );
...
function RightPanelContent(props: CodeStudioPanelManagerProps) {
  ...
  const panelPropsMap: Record<string, () => React.ReactNode> = {
    ...
    git: () => (
      <PI.GitPanelComponent
        files={files}
        openFiles={openFiles}
        onRestore={(fid: string, content: string) => {
          onSetOpenFiles((prev) =>
            prev.map((f) =>
              f.id === fid ? { ...f, content, isDirty: true } : f,
            ),
          );
          fsUpdateContent(fid, content);
        }}
        onClearDirty={() =>
          onSetOpenFiles((prev) => prev.map((f) => ({ ...f, isDirty: false })))
        }
      />
    ),
      ...
      <PI.AutopilotPanelComponent
        ...
        onComplete={(result) => {
          if (result && result.files?.length > 0) {
            ...
            toast(
              `Autopilot applied to ${result.files.length} file(s)`,
              "success",
            );
          }
        }}
    ...
    composer: () => (
      <PI.ComposerPanelComponent
        ...
        onApplyChanges={(
          changes: Array<{ fileId: string; modified: string }>,
        ) => {
          ...
          for (const c of changes) {
            onSetOpenFiles((prev) =>
              prev.map((f) =>
                f.id === c.fileId
                  ? { ...f, content: c.modified, isDirty: true }
                  : f,
              ),
            );
    ...
    review: () => {
      const effectiveScore =
        verificationResult?.finalScore ?? pipelineScore ?? 0;
            ...
            findings: [
              {
                ...
                message: "Self-repair fix staged for review",
                line: 0,
              },
            ],
          }))}
    ...
    preview: () => (
      <PI.PreviewPanelComponent
        files={files}
        visible={rightPanel === "preview"}
      />
    ),
    outline: () => (
    ...
    evaluation: () => (
      <PI.EvaluationPanelComponent
        files={files}
        onClose={() => onSetRightPanel(null)}
      />
    ),
    collab: () => (
      <PI.CollabPanelComponent onClose={() => onSetRightPanel(null)} />
    ),
    creator: () => (
    ...
    database: () => (
      <PI.DatabasePanelComponent
        connections={panels.dbConnections}
        onConnect={panels.handleDbConnect}
        onExecuteQuery={panels.handleDbQuery}
        tables={panels.dbTables}
      />
    ),
    "diff-editor": () => (
      <PI.DiffEditorPanelComponent original="" modified="" />
    ),
    "git-graph": () => (
      <PI.GitGraphComponent commits={[]} branches={[]} currentBranch="main" />
    ),
    "ai-hub": () => (
      <PI.AIHubComponent
        features={panels.aiFeatures}
        onToggleFeature={panels.toggleAiFeature}
        onConfigureProvider={() => onSetRightPanel("api-config" as RightPanel)}
      />
    ),
    "ai-workspace": () => (
      <PI.AIWorkspaceComponent
        threads={panels.wsThreads}
        sharedMemory={panels.wsSharedMemory}
        onSendMessage={panels.sendWsMessage}
        onCreateThread={panels.createWsThread}
        onDeleteThread={panels.deleteWsThread}
      />
    ),
    canvas: () => {
      panels.initCanvas();
      return (
        <PI.CanvasPanelComponent
          nodes={panels.canvasNodes}
          connections={panels.canvasConnections}
          onNodesChange={panels.setCanvasNodes}
          onConnectionsChange={panels.setCanvasConnections}
        />
      );
    },
    progress: () => {
      const status: "pass" | "warn" | "fail" | undefined = pipelineScore
        ? pipelineScore >= 80
          ? "pass"
          : pipelineScore >= 60
            ? "warn"
        ...
        : undefined;
      return (
        <PI.ProgressDashboardComponent
          pipelineScore={pipelineScore ?? undefined}
          pipelineStatus={status}
          stressReport={stressReport}
          onRunStress={handleRunStressTest}
          isStressTesting={isStressTesting}
          verificationScore={verificationScore ?? undefined}
          onRunVerification={handleRunVerification}
          isVerifying={isVerifying}
          verificationResult={verificationResult}
          currentVerifyRound={currentVerifyRound}
        />
      );
    },
    onboarding: () => (
      <PI.OnboardingGuideComponent
        onComplete={() => onSetRightPanel(null)}
        onSkip={() => onSetRightPanel(null)}
      />
    ),
    "merge-conflict": () => (
      <PI.MergeConflictEditorComponent
        fileName={activeFile?.name ?? ""}
        conflicts={panels.mergeConflictsWithResolutions}
        onResolve={(
          conflictId: string,
          resolution: "ours" | "theirs" | "both" | "manual" | undefined,
          content?: string,
        ) => {
    ...
    "project-switcher": () => (
      <PI.ProjectSwitcherComponent onClose={() => onSetRightPanel(null)} />
    ),
    "recent-files": () => (
      <PI.RecentFilesComponent
        files={panels.recentFiles}
        onOpen={(fileId: string) => {
          const found = findFileNodeByName(files, fileId);
          if (found) onFileSelect(found);
        }}
        onClear={() => {
          panels.clearRecentFiles();
          toast("Recent files cleared", "info");
        }}
      />
    ),
    "symbol-palette": () => (
      <PI.SymbolPaletteComponent
    ...
    keybindings: () => (
      <PI.KeybindingsPanelComponent onClose={() => onSetRightPanel(null)} />
    ),
    "api-config": () => (
      <PI.APIKeyConfigComponent onClose={() => onSetRightPanel(null)} />
    ),
    "network-inspector": () => (
      <PI.PreviewNetworkTabComponent
        visible={rightPanel === "network-inspector"}
        onClose={() => onSetRightPanel(null)}
      />
    ),
    "code-actions": () => (
      <PI.QuickActionsComponent
        selectedText={panels.editorSelection.text}
        position={{
          top: panels.editorSelection.top,
          left: panels.editorSelection.left,
        }}
        language={activeFile?.language ?? "plaintext"}
        onAction={async (actionId: string, contextPrompt?: string) => {
          ...
          if (activeFile && contextPrompt) {
            try {
              ...
              if (actionId === "explain")
                ...
                result = await generateDocstring(
                  activeFile.content,
                  activeFile.language,
                );
    ...
    audit: () => (
      <PI.AuditPanelComponent
        files={files.flatMap(function flatFiles(
          n: (typeof files)[number],
        ...
        onRunAudit={() => {
          import("@/lib/code-studio/audit/audit-engine").then(
            ({ runProjectAudit }) => {
              const ctx = {
                files: files.flatMap(function flatFiles(
                  n: (typeof files)[number],
              ...
              toast(
                `Audit: ${report.totalScore}/100 (${report.totalGrade}) — ${report.totalFindings} findings`,
                report.hardGateFail ? "error" : "success",
              );
            },
          );
        }}
    ...
    "cognitive-load": () => (
      <PI.CognitiveLoadPanelComponent code={activeFile?.content ?? ""} />
    ),
    adr: () => (
      <PI.ADRPanelComponent
        files={files.flatMap(function flatFiles(
          n: (typeof files)[number],
        ): string[] {
    ...
    "code-rhythm": () => (
      <PI.RhythmPanelComponent code={activeFile?.content ?? ""} />
    ),
    "migration-audit": () => <PI.MigrationAuditPanelComponent />,
    "snippet-market": () => (
      <PI.SnippetMarketComponent onImportToEditor={undefined} />
    ),
  };
    ...
    <div className="border-t border-white/8 max-h-[40vh] overflow-hidden flex flex-col">
      <div className="flex items-center gap-1 border-b border-white/8 px-2 py-0.5 bg-bg-primary shrink-0">
        <button
          ...
          title={tcs.consoleTooltip}
          className={`px-2 py-0.5 rounded text-[10px] font-mono transition-colors duration-150 ${showTerminal ? "text-accent-green bg-accent-green/10" : "text-text-tertiary hover:text-text-secondary"}`}
        >
          {tcs.console}
        </button>
        <button
          onClick={onToggleProblems}
          className={`px-2 py-0.5 rounded text-[10px] font-mono transition-colors duration-150 ${showProblems ? "text-accent-red bg-accent-red/10" : "text-text-tertiary hover:text-text-secondary"}`}
        >
          Problems {bugReports.length > 0 ? `(${bugReports.length})` : ""}
        </button>
        <button
          onClick={onTogglePipelineBottom}
          className={`px-2 py-0.5 rounded text-[10px] font-mono transition-colors duration-150 ${showPipelineBottom ? "text-accent-blue bg-accent-blue/10" : "text-text-tertiary hover:text-text-secondary"}`}
        >
          Pipeline
        </button>
        <button
          onClick={onCloseAllBottom}
          aria-label="하단 패널 닫기"
          className="ml-auto rounded p-0.5 text-text-tertiary hover:text-text-primary transition-colors duration-150"
        >
          <X className="h-3 w-3" />
        </button>
      </div>
      ...
      {showPipelineBottom && pipelineStages.length > 0 && (
        <div className="h-40 overflow-auto p-2">
          {pipelineStages.map((s) => (
            ...
            >
              <span
                className={`w-2 h-2 rounded-full ${s.status === "pass" ? "bg-accent-green" : s.status === "warn" ? "bg-accent-amber" : s.status === "fail" ? "bg-accent-red" : "bg-white/20"}`}
              />
