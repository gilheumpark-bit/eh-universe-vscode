...
export function KeybindingsPanel({ onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-[#0f1419] border border-white/10 rounded-xl shadow-2xl w-[500px] max-h-[500px] overflow-hidden"
        ...
        <div className="overflow-y-auto max-h-[380px] p-2">
          {categories.map((cat) => (
            <div key={cat} className="mb-3">
              <p className="text-[10px] font-semibold text-white/60 uppercase tracking-wider px-2 mb-1">
                {cat}
              </p>
              {filtered
                ...
                .map((k) => (
                  <div
                    key={k.id}
                    className="flex items-center justify-between px-2 py-1 text-xs text-white/70 hover:bg-white/5 rounded"
                  >
                    <span>{k.label}</span>
                    <kbd className="text-[10px] px-1.5 py-0.5 bg-[#0a0e17] border border-white/10 rounded font-mono text-white/50">
                      {k.shortcut}
                    </kbd>
                  </div>
                ))}
