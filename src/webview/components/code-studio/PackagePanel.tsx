...
function parsePackageJson(
  content: string,
): { deps: Record<string, string>; devDeps: Record<string, string> } | null {
  try {
...
export function PackagePanel({ files, onFilesChange }: Props) {
  ...
  const handleSearch = async () => {
    ...
    setSearching(true);
    try {
      setSearchResults(await searchNpm(searchQuery.trim()));
      setShowResults(true);
    } catch {
      setSearchResults([]);
    }
    ...
    <div className="flex flex-col h-full bg-[#0f1419] text-xs">
      ...
      <div className="flex-1 overflow-y-auto p-2 space-y-3">
            ...
            {showResults && (
              <div className="ml-2 mt-1 space-y-1">
                {searchResults.map((pkg) => (
                  <div
                    key={pkg.name}
                    className="flex items-start gap-2 p-1.5 rounded hover:bg-white/5 group"
                  >
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-1 text-white">
                        ...
                        <span className="text-[9px] px-1 py-0.5 rounded bg-amber-900/30 text-amber-400 flex-shrink-0">
                          v{pkg.version}
                        </span>
                      </div>
                      {pkg.description && (
            ...
            {showInstalled && (
              <div className="ml-2 mt-1 space-y-0.5">
                {Object.entries(installed.deps).map(([name, version]) => (
                  <div
                    key={name}
                    className="flex items-center gap-1 group py-0.5 text-white/70"
                  >
                    <Package
                      size={10}
                    ...
                    <span className="text-[9px] px-1 py-0.5 rounded bg-amber-900/30 text-amber-400">
                      {version}
                    </span>
                    <button
        ...
        {installedCount === 0 && searchResults.length === 0 && (
          <p className="text-center text-white/50 py-4">
            <Package size={14} className="inline opacity-30" /> package.json을
            찾을 수 없습니다
          </p>
        )}
      </div>
      {showTerminal && (
        <div className="border-t border-white/8">
          ...
          <div
            ref={terminalRef}
            className="h-32 overflow-y-auto p-2 bg-[#0a0e17] font-mono text-[10px] text-green-400"
          >
