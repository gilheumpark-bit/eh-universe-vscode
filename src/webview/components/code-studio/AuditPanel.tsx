...
import type {
  ...
  AuditAreaResult,
  AuditSeverity,
} from "@/lib/code-studio/audit/audit-types";
import {
  AUDIT_AREA_LABELS,
  CATEGORY_LABELS,
} from "@/lib/code-studio/audit/audit-types";
...
function CategoryCard({
  ...
  const catLabel =
    lang === "ko"
      ? CATEGORY_LABELS[cat.category]?.ko
      : CATEGORY_LABELS[cat.category]?.en;
  ...
  return (
    <div className="border border-white/8 rounded-lg p-3 bg-white/[0.02]">
      ...
      >
        ...
        <span
          className={`text-[13px] font-mono font-bold ${GRADE_COLORS[cat.grade] ?? ""}`}
        >
          {cat.score}/100
        </span>
      </button>
      {expanded && (
        <div className="mt-2 space-y-0.5">
          {cat.areas.map((area) => {
            const areaLabel =
              lang === "ko"
                ? AUDIT_AREA_LABELS[area.area]?.ko
                : AUDIT_AREA_LABELS[area.area]?.en;
            return (
              <button
                key={area.area}
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        ...
        <span
          className={`text-[14px] font-mono font-bold ${GRADE_COLORS[area.grade] ?? ""}`}
        >
      ...
      {area.findings.length > 0 && (
        <div className="space-y-1 mt-2 max-h-[300px] overflow-y-auto">
          {area.findings.map((f) => (
            <div
              key={f.id}
              className="flex items-start gap-1.5 text-[10px] py-1 border-b border-white/5"
            >
      ...
      {area.findings.length === 0 && (
        <div className="text-[10px] text-accent-green py-2">
          {lang === "ko" ? "이슈 없음" : "No issues found"}
        </div>
      )}
    </div>
  );
        ...
        <>
          ...
          <div className="px-3 py-2 border-b border-white/8 bg-white/[0.02]">
            <div className="flex items-center justify-between">
              <div>
                <span
                  className={`text-[22px] font-mono font-bold ${GRADE_COLORS[auditResult.totalGrade] ?? ""}`}
                >
                  {auditResult.totalScore}
                </span>
                <span className="text-[11px] text-text-tertiary">/100</span>
                <span
                  className={`ml-2 text-[14px] font-bold ${GRADE_COLORS[auditResult.totalGrade] ?? ""}`}
                >
                      ...
                      >
                        <ScoreBar
                          ...
                          label={
                            (isKo
                              ? AUDIT_AREA_LABELS[area.area]?.ko
                              : AUDIT_AREA_LABELS[area.area]?.en) ?? area.area
                          }
                        />
                      </button>
                    ))}
                ...
                {auditResult.urgent.map((item) => (
                  ...
                  >
                    ...
                    <div className="min-w-0 text-[10px]">
                      ...
                      <span className="text-text-tertiary ml-1 text-[9px]">
                        [
                        {(isKo
                          ? AUDIT_AREA_LABELS[item.area]?.ko
                          : item.area) ?? item.area}
                        ]
                      </span>
                      {item.file && (
