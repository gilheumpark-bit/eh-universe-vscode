...
function loadSessionStats(): {
  ...
  try {
    ...
    return { fileEdits: 0, aiCalls: 0, tokens: 0 };
  }
}
...
function TeamProgressBar({ team }: { team: TeamProgress }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <span className="w-20 text-[10px] text-text-tertiary truncate">
        {team.name}
      </span>
...
function ActionIcon({ type }: { type: RecentAction["type"] }) {
  switch (type) {
    case "ai":
      return <Brain size={10} className="text-amber-400 shrink-0" />;
    case "edit":
      return <FileText size={10} className="text-blue-400 shrink-0" />;
    case "pipeline":
      return <Zap size={10} className="text-accent-amber shrink-0" />;
    default:
      return <Activity size={10} className="text-text-tertiary shrink-0" />;
  }
}
...
function GradeBadge({ grade }: { grade: string }) {
  return (
    <span
      ...
      style={{
        backgroundColor: `${GRADE_COLOR[grade] ?? "#8b949e"}22`,
        color: GRADE_COLOR[grade] ?? "#8b949e",
      }}
    >
      {grade}
    </span>
    ...
    <div className="flex-1 h-1.5 rounded-full bg-bg-tertiary overflow-hidden">
      <div
        ...
        style={{
          ...
          backgroundColor: GRADE_COLOR[grade] ?? "#8b949e",
        }}
      />
    </div>
  );
...
export function ProgressDashboard({
  ...
  const etaMs =
    runningTeams > 0
      ? Math.round((avgEstMs / runningTeams) * (1 - overallProgress / 100))
      : 0;

  return (
    <div
      className="h-full flex flex-col bg-bg-secondary border-l border-border overflow-hidden"
      style={{ minWidth: 320 }}
    >
      {/* Header */}
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4 text-xs">
        ...
        {teams && teams.length > 0 && (
          <section>
            <h3 className="flex items-center gap-1.5 font-semibold text-text-primary mb-2">
              <BarChart3 size={12} className="text-amber-400" /> Pipeline
              Progress
            </h3>
            <div className="space-y-1 mb-2">
              <div className="flex items-center justify-between text-[10px] text-text-tertiary">
                <span>
                  {completedTeams}/{totalTeams} teams complete
                </span>
            ...
            <div className="space-y-1.5">
              {teams.map((team) => (
                <TeamProgressBar key={team.name} team={team} />
              ))}
            </div>
          </section>
        )}
        ...
        <section>
          ...
          <div className="grid grid-cols-3 gap-2">
            <StatCard
              label="File Edits"
              value={`${sessionStats.fileEdits}`}
              icon={<FileText size={12} />}
            />
            <StatCard
              label="AI Calls"
              value={`${sessionStats.aiCalls}`}
              icon={<Brain size={12} />}
            />
            <StatCard
              label="Tokens"
              value={formatTokens(sessionStats.tokens)}
              icon={<Trophy size={12} />}
            />
          </div>
        </section>
          ...
          <div className="space-y-1">
            {recentActions.length === 0 ? (
              ...
              recentActions.map((action, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 py-1 px-2 rounded hover:bg-bg-tertiary"
                >
        ...
        <section>
          <h3 className="flex items-center gap-1.5 font-semibold text-text-primary mb-2">
            <Gauge size={12} className="text-orange-400" /> AI-Predicted
            Performance
          </h3>
          ...
          {onRunStress && (
            ...
            >
              {isStressTesting ? (
                <>
                  <Loader2 size={12} className="animate-spin" /> Running Stress
                  Test...
                </>
              ) : (
                <>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3 p-2 rounded-lg bg-bg-tertiary">
                <div className="text-center">
                  <div
                    className="text-lg font-bold"
                    style={{ color: GRADE_COLOR[stressReport.grade] }}
                  >
                    {stressReport.grade}
                  </div>
                ...
                <div className="flex-1">
                  ...
                  <ScoreBar
                    score={stressReport.overallScore}
                    grade={stressReport.grade}
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                {stressReport.scenarios.map((r) => {
                  ...
                  return (
                    ...
                    >
                      <span
                        className="w-24 text-[10px] text-text-tertiary truncate"
                        title={`${r.scenario.name} (${r.scenario.virtualUsers}u)`}
                      >
              ...
              <div className="flex items-start gap-1.5 pt-1 text-[9px] text-text-tertiary">
                <AlertTriangle
                  size={10}
                  className="shrink-0 mt-px text-accent-amber"
                />
          ...
          {isVerifying &&
            ...
            currentVerifyRound > 0 && (
              <div className="flex items-center gap-2 py-1">
                ...
                />
                <span className="text-[10px] text-text-tertiary">
                  Round {currentVerifyRound}/3 — auto-fixing...
                </span>
              </div>
            )}
          ...
          {verificationResult && (
            <div className="space-y-1 mt-1">
              ...
              {verificationResult.scoreDelta !== 0 && (
                <div className="text-[9px] text-text-tertiary">
                  Score delta: {verificationResult.scoreDelta > 0 ? "+" : ""}
                  {verificationResult.scoreDelta}
                </div>
              )}
              {verificationResult.hardGateFailures.length > 0 && (
                <div className="flex items-center gap-1 text-[9px] text-red-400">
                  ...
                  {verificationResult.hardGateFailures.join(", ")}
                </div>
              )}
            </div>
          )}
