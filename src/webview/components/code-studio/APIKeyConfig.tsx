...
const PROVIDERS = [
  ...
  },
  {
    ...
    models: ["openai/gpt-oss-20b", "qwen/qwen3-30b-a3b-2507", "qwen/qwen3-14b"],
  },
];
...
export function APIKeyConfig({ onClose }: Props) {
  ...
  const autoLabel = useMemo(
    () => `${provider.name} - ${ROLE_LABELS[formRole]}`,
    [provider, formRole],
  );
  ...
  function handleStartEdit(slot: APIKeySlot) {
    ...
    setFormKey(slot.apiKey);
    setFormModel(slot.model);
    setFormRole(slot.role);
    setFormCustom(slot.customPerspective ?? "");
  ...
  const modalContent = (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="API 키 슬롯 관리"
    >
      <div
        className="bg-[#0f1419] border border-white/10 rounded-xl w-full max-w-2xl max-h-[85vh] flex flex-col"
        style={{ minHeight: 320 }}
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-5 py-3 space-y-2">
          ...
          {slots.map((slot) => {
            ...
            return (
              <div
                key={slot.id}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-lg border transition-colors ${slot.enabled ? "border-white/10 bg-white/5" : "border-transparent bg-white/3 opacity-50"}`}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      style={{
                        backgroundColor: ROLE_COLORS[slot.role] + "22",
        ...
        <div className="border-t border-white/8 px-5 py-3 flex items-center justify-between">
          <div className="text-[11px] text-white/60">
            등록: {slots.length}개 | 활성: {enabledSlots.length}개
          </div>
          {!showForm && (
            <button
              onClick={() => {
