  ...
  return (
    <div className="relative group">
      ...
      >
        <GripVertical
          size={10}
          className="text-text-tertiary opacity-0 group-hover:opacity-50 shrink-0 cursor-grab"
        />
        {isFolder ? (
          expanded ? (
        ...
        {isModified && (
          <span
            ...
            title={L4(lang, { ko: "수정됨", en: "Modified" })}
          />
        )}
        {isRenaming ? (
          <input
        ...
        {isFolder && folderFileCount > 0 && (
          <span className="text-[8px] text-text-tertiary shrink-0 ml-1 opacity-60">
            ({folderFileCount})
          </span>
        )}
        {fileSize && (
      ...
      {showPreview && filePreview && (
        <div className="absolute left-full top-0 ml-2 z-50 bg-bg-primary border border-white/8 rounded shadow-lg p-2 max-w-xs pointer-events-none">
          <pre className="text-[9px] text-text-tertiary whitespace-pre-wrap font-mono leading-tight">
            {filePreview}
          </pre>
        </div>
      )}
  ...
  const handleContextAction = useCallback(
    (action: string) => {
      ...
      if (action === "new-file")
        setDialog({
          type: "new-file",
          parentId: node?.type === "folder" ? node.id : null,
        });
  ...
  const handleDialogConfirm = useCallback(
    (value: string) => {
      if (dialog?.type === "new-file")
        onCreateFile?.(dialog.parentId, value, "file");
      else if (dialog?.type === "new-folder")
        onCreateFile?.(dialog.parentId, value, "folder");
      else if (dialog?.type === "rename" && dialog.nodeId)
        onRenameFile?.(dialog.nodeId, value);
      setDialog(null);
    },
    ...
    <div className="text-xs" onContextMenu={(e) => handleContextMenu(e, null)}>
        ...
        <div className="flex items-center gap-0.5">
          {selectedIds.size > 0 && (
            <button
              ...
              title={L4(lang, {
                ko: `${selectedIds.size}개 선택 항목 삭제`,
                en: `Delete ${selectedIds.size} selected`,
              })}
            >
          ...
          <button
            onClick={() =>
              setSortMethod((m) =>
                m === "name-asc" ? "type" : m === "type" ? "size" : "name-asc",
              )
            }
      ...
      {selectedIds.size > 0 && (
        <div className="text-[9px] text-text-tertiary px-3 mb-1">
          {selectedIds.size}{" "}
          {L4(lang, {
            ko: "개 선택됨 (Shift+클릭으로 범위 선택)",
            en: "selected (Shift+Click for range)",
          })}
        </div>
      )}
      {sortedFiles.length === 0 && (
        <div className="mx-3 my-4 px-3 py-8 text-center text-xs text-text-tertiary rounded-lg border-2 border-dashed border-white/8">
          ...
          <p>
            {L4(lang, {
              ko: "아직 파일이 없습니다. 파일을 생성하여 시작하세요.",
              en: "No files yet. Create one to get started.",
            })}
          </p>
        </div>
      )}
      ...
      {dialog && (
        <InputDialog
          title={
            dialog.type === "rename"
              ? L4(lang, { ko: "이름 바꾸기", en: "Rename" })
              : dialog.type === "new-folder"
                ? L4(lang, { ko: "새 폴더", en: "New Folder" })
                : L4(lang, { ko: "새 파일", en: "New File" })
          }
          placeholder={
            dialog.type === "rename"
              ? L4(lang, { ko: "새 이름 입력", en: "Enter new name" })
              : dialog.type === "new-folder"
                ? L4(lang, { ko: "폴더 이름", en: "Folder name" })
                : L4(lang, { ko: "파일 이름", en: "File name" })
          }
