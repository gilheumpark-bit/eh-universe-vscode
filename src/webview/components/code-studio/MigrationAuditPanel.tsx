...
function SigBadge({
  ...
  variant: "lost" | "new" | "matched";
}) {
  const color =
    variant === "lost"
      ? "text-text-danger bg-red-500/10 border-red-500/30"
      : variant === "new"
  ...
  return (
    <div className={`px-2 py-1 rounded border text-xs font-mono ${color}`}>
      ...
      {sig.isExported && (
        <span className="ml-1 text-text-tertiary">[export]</span>
      )}
...
export function MigrationAuditPanel() {
  ...
  return (
    <div className="flex flex-col h-full text-text-primary text-sm">
      ...
      {/* Input panes */}
      <div
        className="grid grid-rows-2 gap-0 border-b border-border"
        style={{ height: result ? "200px" : "300px" }}
      >
        <div className="flex flex-col border-b border-border">
          <div className="flex items-center justify-between px-3 py-1 bg-bg-tertiary">
            ...
            <button
              ...
              className="text-xs text-accent-blue hover:underline focus-visible:ring-2 ring-accent-blue rounded px-1"
            >
              Paste
            </button>
          </div>
          ...
          {result.lostFunctions.length > 0 && (
            <div className="px-3 py-2 border-b border-border">
              <p className="text-xs font-semibold text-text-danger flex items-center gap-1 mb-1.5">
                <XCircle className="w-3 h-3" /> Lost Functions (
                {result.lostFunctions.length})
              </p>
              <div className="space-y-1">
                {result.lostFunctions.map((sig) => (
                  <SigBadge
                    key={`${sig.name}-${sig.lineNumber}`}
                    sig={sig}
                    variant="lost"
                  />
                ))}
          ...
          {result.newFunctions.length > 0 && (
            <div className="px-3 py-2 border-b border-border">
              <p className="text-xs font-semibold text-accent-blue flex items-center gap-1 mb-1.5">
                <Plus className="w-3 h-3" /> New Functions (
                {result.newFunctions.length})
              </p>
              <div className="space-y-1">
              ...
              <div className="space-y-1.5">
                {result.matched.map((m) => (
                  ...
                  >
                    ...
                    <span
                      className={`ml-auto ${m.confidence >= 90 ? "text-accent-green" : "text-accent-amber"}`}
                    >
                      {m.confidence}%
                    </span>
                    {!m.paramMatch && (
