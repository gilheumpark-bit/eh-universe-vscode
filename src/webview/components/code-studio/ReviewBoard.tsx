...
const REVIEWER_PERSONAS: ReviewerPersona[] = [
  {
    ...
    systemPrompt:
      "You are a senior React expert. Review this code for: hooks correctness, composition patterns, unnecessary re-renders, unstable references, missing cleanup, and component architecture issues.",
  },
  {
    id: "security-auditor",
    ...
    systemPrompt:
      "You are a security auditor. Scan for: XSS vectors (innerHTML, dangerouslySetInnerHTML), injection risks, hardcoded secrets, auth token mishandling, CSRF gaps, and OWASP Top 10 violations.",
  },
  {
    id: "a11y-inspector",
    ...
    nameKo: "???????????? ??????",
    description:
      "ARIA, keyboard nav, contrast, screen reader, focus management",
    icon: <Accessibility className="w-4 h-4" />,
    color: "text-accent-green",
    systemPrompt:
      "You are an accessibility expert. Check for: missing ARIA attributes, keyboard navigation gaps, focus traps, color contrast violations, missing alt text, and screen reader compatibility.",
  },
  {
    id: "ts-purist",
    ...
    systemPrompt:
      "You are a TypeScript purist. Flag: any/unknown without narrowing, missing return types on exported functions, loose generic constraints, type assertions (as) without validation, and missing discriminated unions.",
  },
  {
    id: "perf-engineer",
    ...
    systemPrompt:
      "You are a performance engineer. Identify: missing memoization on expensive paths, large bundle imports, synchronous heavy computation, missing code splitting, N+1 patterns, and memory leaks from uncleaned subscriptions.",
  },
];
...
function generatePlaceholderResult(personaId: string): ReviewerResult {
  const templates: Record<
    string,
    { score: number; findings: ReviewFinding[] }
  > = {
    "react-expert": {
      score: 82,
      findings: [
        {
          severity: "warning",
          message:
            "useState with object literal recreated every render; extract to useMemo or useRef",
        },
        {
          severity: "info",
          message: "Consider extracting form logic into a custom useForm hook",
        },
        {
          severity: "warning",
          message:
            "Missing useCallback on event handler passed to memoized child",
        },
      ],
    },
    "security-auditor": {
      ...
      findings: [
        ...
        },
        {
          severity: "warning",
          message:
            "API key referenced in client-side constant; move to server environment variable",
        },
      ],
    },
    "a11y-inspector": {
      ...
      findings: [
        {
          severity: "error",
          message: 'Interactive div without role="button" and keyboard handler',
        },
        {
          severity: "warning",
        ...
        {
          severity: "warning",
          message: "Color-only status indicator; add icon or text label",
        },
      ...
      findings: [
        {
          severity: "error",
          message:
            "Type assertion `as any` bypasses type safety on API response",
        },
        {
          severity: "warning",
        ...
        {
          severity: "warning",
          message: "Generic <T> without constraint allows unexpected types",
        },
      ],
    },
    "perf-engineer": {
      ...
      findings: [
        {
          severity: "warning",
          message:
            "Large library imported at top level; consider dynamic import()",
        },
        {
          severity: "info",
          message: "List of 100+ items rendered without virtualization",
        },
      ],
    },
  };
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {status === "done" && result != null && (
            <>
              <span
                className={`text-lg font-black ${scoreColor(result.score)}`}
              >
                {result.score}
              </span>
      ...
      {status === "done" && result != null && result.findings.length > 0 && (
        <ul className="space-y-1.5">
          {result.findings.map((f, i) => (
            ...
            >
              <AlertTriangle
                className={`w-3 h-3 mt-0.5 flex-shrink-0 ${SEVERITY_COLORS[f.severity]}`}
              />
...
export function ReviewBoard({
  code,
  language = "en",
}: ReviewBoardProps): React.JSX.Element {
  ...
  const totalFindings = completedResults.reduce(
    ...
    <div className="flex flex-col h-full bg-bg-primary">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-bg-secondary">
        <div className="flex items-center gap-2">
          ...
          {avgScore != null && (
            <span
              className={`text-xs font-black px-2 py-0.5 rounded-full bg-bg-primary ${scoreColor(avgScore)}`}
            >
              {avgScore}/100
            </span>
          )}
      ...
      {completedResults.length > 0 && (
        <div className="flex items-center gap-4 px-4 py-2 border-b border-border bg-bg-tertiary text-[10px]">
          ...
          <span className="text-text-secondary">{totalFindings} findings</span>
          {errorCount > 0 && (
      ...
      <div className="flex-1 overflow-y-auto px-4 py-3 space-y-3">
        {code.trim().length < 10 ? (
          <div className="flex items-center justify-center h-32 text-text-tertiary text-xs">
            {isKo
              ? "?????? ?????? ?????? ????????? ?????? ???????????????."
              : "Paste or select code to review."}
          </div>
        ) : (
