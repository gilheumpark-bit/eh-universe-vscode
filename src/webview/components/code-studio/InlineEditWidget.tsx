...
function loadHistory(): string[] {
  try {
    const raw = localStorage.getItem(INLINE_EDIT_HISTORY_KEY);
...
export function InlineEditWidget({
  ...
  const contextSize = useMemo(() => {
    const totalChars =
      fullCode.slice(0, 2000).length + selectedText.length + prompt.length;
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === "Enter" && !e.shiftKey) {
      e.preventDefault();
      if (preview && !loading) onApply(preview);
      else handleGenerate();
    }
  ...
  return (
    <div
      className="absolute z-50 bg-[#0f1419] border border-amber-700/45 rounded-lg shadow-2xl p-2 w-[400px]"
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <input
          ...
          className="flex-1 bg-white/5 text-xs text-white px-2 py-1.5 rounded outline-none placeholder:text-white/50"
          disabled={loading}
        />
        {history.length > 0 && (
          <button
        ...
        <div className="mb-2 bg-white/5 rounded p-1 max-h-24 overflow-y-auto">
          {history.map((h, i) => (
            <button
              ...
              className="block w-full text-left text-[10px] px-2 py-1 hover:bg-white/10 rounded truncate text-white/60"
            >
              {h}
            </button>
          ))}
      ...
      <div className="flex items-center justify-between text-[9px] text-white/50 mb-1 px-1">
        <span>
          선택{isMultiLine ? ` (${selectionLineCount}줄)` : ""}:{" "}
      ...
      {preview && diffLines.length > 0 && (
        <div className="mb-2">
          <pre
            ref={previewRef}
            className="text-[10px] font-mono bg-[#0a0e17] p-2 rounded max-h-32 overflow-y-auto whitespace-pre-wrap"
          >
      ...
      {preview && diffLines.length === 0 && (
        <div className="mb-2">
          <pre className="text-[10px] font-mono bg-[#0a0e17] p-2 rounded max-h-32 overflow-y-auto text-green-400 whitespace-pre-wrap">
            {preview}
          </pre>
        </div>
      )}
      {error && (
        <div className="mb-2 px-2 py-1 text-[10px] text-red-400 bg-red-500/10 rounded">
          {error}
        </div>
      )}
      {preview && !loading && (
        <div className="flex items-center justify-end gap-2">
          <button
            onClick={onCancel}
            className="flex items-center gap-1 px-2 py-1 text-[10px] text-white/60 hover:text-red-400"
          >
            <X size={10} /> 거절
          </button>
          <button
            onClick={() => onApply(preview)}
            className="flex items-center gap-1 px-2 py-1 text-[10px] bg-green-500/20 text-green-400 rounded hover:bg-green-500/30"
          >
