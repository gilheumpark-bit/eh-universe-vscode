...
const DEMO_ENTRIES: NetworkEntry[] = [
  ...
  },
  {
    ...
    responsePreview: '{"error":"rate limited"}',
  },
];
  ...
  return (
    <div className="flex flex-col h-full border-t border-white/8 bg-[#0f1419]">
        ...
        <div className="flex items-center gap-0.5">
          ...
          {FILTER_OPTIONS.map((opt) => (
            <button
              ...
              className={`px-1.5 py-0.5 rounded text-[10px] ${filterType === opt.value ? "bg-amber-900/30 text-amber-400" : "hover:bg-white/5 text-white/60"}`}
            >
              {opt.label}
            </button>
          ))}
      ...
      <div className="flex-1 flex overflow-hidden">
        <div
          className={`flex-1 flex flex-col overflow-hidden ${selectedEntry ? "w-1/2" : "w-full"}`}
        >
          <div className="flex items-center border-b border-white/8 text-[10px] text-white/60 font-semibold select-none">
            <div className="w-14 px-1 py-1">Status</div>
            <div className="w-14 px-1 py-1">Method</div>
            <div className="flex-1 px-1 py-1">URL</div>
            <div className="w-12 px-1 py-1">Type</div>
            <div className="w-16 px-1 py-1 text-right">Size</div>
            <div className="w-16 px-1 py-1 text-right">Time</div>
          </div>
          <div className="flex-1 overflow-y-auto">
            {filteredEntries.length === 0 ? (
              ...
              filteredEntries.map((entry) => (
                ...
                >
                  <div
                    className={`w-14 px-1 py-0.5 font-mono ${statusColor(entry.status)}`}
                  >
                    {entry.status || "--"}
                  </div>
                  <div className="w-14 px-1 py-0.5 font-mono text-white/60">
                    {entry.method}
                  </div>
                  <div
                    className="flex-1 px-1 py-0.5 truncate font-mono text-white/70"
                    title={entry.url}
                  >
                    {truncateUrl(entry.url)}
                  </div>
                  <div className="w-12 px-1 py-0.5 text-white/60">
                    {entry.type}
                  </div>
                  <div className="w-16 px-1 py-0.5 text-right font-mono text-white/60">
        ...
        {selectedEntry && (
          <div className="w-1/2 border-l border-white/8 flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-2 py-1 border-b border-white/8 text-xs font-semibold text-white">
              <span>
                {selectedEntry.method} {truncateUrl(selectedEntry.url, 40)}
              </span>
              <button
            ...
            <div className="flex-1 overflow-y-auto p-2 text-[10px] space-y-3 text-white/60">
              <DetailSection title="General">
                ...
                <DetailRow
                  label="Status"
                  value={`${selectedEntry.status} ${selectedEntry.statusText}`}
                />
                <DetailRow
                  label="Size"
                ...
                <DetailRow
                  label="Time"
                  value={formatDuration(selectedEntry.duration)}
                />
              </DetailSection>
              {selectedEntry.requestBody && (
                <DetailSection title="Request Body">
                  <pre className="bg-[#0a0e17] p-1.5 rounded text-[9px] font-mono overflow-x-auto whitespace-pre-wrap break-all text-white/50">
                    {selectedEntry.requestBody}
                  </pre>
                </DetailSection>
              )}
              {selectedEntry.responsePreview && (
                <DetailSection title="Response">
                  <pre className="bg-[#0a0e17] p-1.5 rounded text-[9px] font-mono overflow-x-auto whitespace-pre-wrap break-all max-h-40 text-white/50">
                    {selectedEntry.responsePreview}
                  </pre>
                </DetailSection>
              )}
...
function DetailSection({
  ...
  return (
    <div>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex items-center gap-1 text-[10px] font-semibold text-white mb-1 hover:text-amber-400"
      >
...
function DetailRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex gap-2">
      <span className="text-white/50 flex-shrink-0 min-w-[80px]">{label}:</span>
