...
import type {
  ...
  OpenFile,
  CodeStudioSettings,
} from "@/lib/code-studio/core/types";
import {
  DEFAULT_SETTINGS,
  detectLanguage,
  fileIconColor,
...
import {
  findBugsStatic,
  type BugReport,
} from "@/lib/code-studio/pipeline/bugfinder";
import {
  runStressReport,
  type StressReport,
} from "@/lib/code-studio/pipeline/stress-test";
import {
  runVerificationLoop,
...
import {
  ...
  type PanelGroup,
  type PanelDef,
} from "@/lib/code-studio/core/panel-registry";
import {
  useSessionRestore,
...
import {
  findFilePathById,
  toMonacoModelPath,
} from "@/lib/code-studio/editor/model-path";
import {
  attachEditorSurfaceContextMenu,
  runEditorSurfaceMenuAction,
} from "@/lib/code-studio/editor/editor-surface-context-menu";
import {
  ContextMenu,
  buildEditorSurfaceMenu,
} from "@/components/code-studio/ContextMenu";
...
import {
  ...
  BottomPanels,
  type PipelineStage,
} from "@/components/code-studio/CodeStudioPanelManager";
...
const TouchGesturesComponent = dynamic(
  () =>
    import("@/components/code-studio/TouchGestures").then((m) => ({
      default: m.TouchGestures,
    })),
  { ssr: false },
);
const BreadcrumbComponent = dynamic(
  () =>
    import("@/components/code-studio/Breadcrumb").then((m) => ({
...
const ConfirmDialog = dynamic(
  () =>
    import("@/components/code-studio/ConfirmDialog").then((m) => ({
    ...
    <div>
      ...
      >
        ...
        >
          {isFolder ? (
            open ? (
              <FolderOpen className="h-3.5 w-3.5 shrink-0 text-accent-amber" />
            ) : (
              <Folder className="h-3.5 w-3.5 shrink-0 text-accent-amber" />
            )
          ) : (
            <FileText
              className={`h-3.5 w-3.5 shrink-0 ${fileIconColor(node.name)}`}
            />
          )}
          {editing ? (
            <input
        ...
        {!isFolder && node.id !== "root" && (
          <div className="flex gap-0.5 opacity-0 group-hover:opacity-100">
            ...
            <button
              ...
              aria-label="삭제"
              className="rounded p-0.5 hover:bg-white/10 text-accent-red"
            >
...
function CodeStudioShellInner() {
  ...
  const {
    tree: files,
    setTree: setFiles,
    deleteNode: fsDeleteNode,
    renameNode: fsRenameNode,
    updateContent: fsUpdateContent,
    undo: fsUndo,
    redo: fsRedo,
    canUndo: fsCanUndo,
    canRedo: fsCanRedo,
    persist: fsPersist,
    load: fsLoad,
  } = useCodeStudioFileSystem(DEMO_FILES);

  // ── Core State ──
  const [openFiles, setOpenFiles] = useState<OpenFile[]>([]);
  const [activeFileId, setActiveFileId] = useState<string | null>(null);
  const [settings, setSettings] =
    useState<CodeStudioSettings>(DEFAULT_SETTINGS);
  ...
  const [mobileEditorSurfaceMenu, setMobileEditorSurfaceMenu] = useState<{
    x: number;
    y: number;
  } | null>(null);
  const mobileEditorSurfaceTargetRef =
    useRef<MonacoNS.editor.IStandaloneCodeEditor | null>(null);
  ...
  type VerifyAction =
    | Partial<VerifyState>
    | ((prev: VerifyState) => Partial<VerifyState>);
  ...
  const activeMobileEditorPath = activeFile
    ? toMonacoModelPath(
        findFilePathById(files, activeFile.id),
        activeFile.id,
        activeFile.name,
      )
    : undefined;
  const pipelineScore =
    pipelineStages.length > 0
      ? Math.round(
          pipelineStages.reduce((sum, s) => sum + (s.score ?? 0), 0) /
  ...
  useCodeStudioKeyboard({
    ...
    bindings: [
      ...
      {
        keys: "ctrl+shift+f",
        handler: () => setRightPanel((v) => (v === "search" ? null : "search")),
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        if (state.showTerminal !== undefined)
          setShowTerminal(state.showTerminal);
        if (state.showProblems !== undefined)
          setShowProblems(state.showProblems);
        if (state.showPipelineBottom !== undefined)
          setShowPipelineBottom(state.showPipelineBottom);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (mod && e.key === "z" && !e.shiftKey && fsCanUndo) {
        e.preventDefault();
        fsUndo();
      }
      if (
      ...
      if (mod && e.key === "-") {
        e.preventDefault();
        setSettings((s) => ({ ...s, fontSize: Math.max(10, s.fontSize - 1) }));
      }
      if (mod && e.key === "`") {
        e.preventDefault();
        setShowTerminal((v) => !v);
        ...
        if (activeFileId) {
          ...
          if (af?.isDirty) {
            setConfirmState({
              ...
              message: L4(lang, {
                ko: "저장하지 않은 변경사항이 손실됩니다. 닫으시겠습니까?",
                en: "Unsaved changes will be lost. Close anyway?",
              }),
              onConfirm: () => {
                setOpenFiles((prev) => {
                  const next = prev.filter((f) => f.id !== activeFileId);
                  setActiveFileId(
    ...
    const processCommand = (
      ...
      switch (command) {
        ...
        case "clear":
          ...
          break;
        case "date":
          t.writeln("  " + new Date().toLocaleString());
          break;
        case "whoami":
          t.writeln("  \x1b[33meh-developer\x1b[0m");
          break;
        case "pwd":
          t.writeln("  /project/src");
          break;
        case "echo":
          t.writeln("  " + args.join(" "));
          break;
        case "ls": {
          const flatFiles = (nodes: FileNode[]): string[] => {
            ...
            for (const n of nodes) {
              if (n.type === "folder") {
                result.push("\x1b[34m" + n.name + "/\x1b[0m");
                if (n.children)
                  result.push(...flatFiles(n.children).map((f) => "  " + f));
        ...
        case "cat": {
          ...
          if (file) {
            ...
            t.writeln(
              "  \x1b[31mFile not found: " + (args[0] ?? "") + "\x1b[0m",
            );
          break;
        }
        case "pipeline": {
          ...
          if (af) {
            t.writeln(
              "  \x1b[36mRunning pipeline on " + af.name + "...\x1b[0m",
            );
            ...
            t.writeln(
              `  \x1b[36mOverall: ${result.overallScore}/100 (${result.overallStatus})\x1b[0m`,
            );
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      toast(
        `Pipeline: ${passed}/${result.stages.length} passed`,
        passed === result.stages.length ? "success" : "info",
      );
    }, 1000);
  ...
  const handleNewFile = useCallback(() => {
    ...
    setFiles((prev) => {
      ...
      const targetId =
        findFirstFolder(prev.flatMap((n) => n.children ?? [])) ??
        findFirstFolder(prev);
  ...
  const handleDelete = useCallback(
    (id: string) => {
      const node = files
        .flatMap(function walk(n: FileNode): FileNode[] {
      ...
      setConfirmState({
        ...
        message: L4(lang, {
          ko: `"${name}"을(를) 삭제하시겠습니까? 되돌릴 수 없습니다.`,
  ...
  const handleApplyCode = useCallback(
    (code: string, fileName?: string) => {
      const targetFileId = fileName
        ? openFiles.find((f) => f.name === fileName)?.id ||
          findFileNodeByName(files, fileName)?.id
        : activeFileId;
      ...
      setOpenFiles((prev) => {
        ...
        if (exists) {
          return prev.map((f) =>
            f.id === targetFileId ? { ...f, content: code, isDirty: true } : f,
          );
        }
  ...
  const handleOpenDemo = useCallback(() => {
    ...
    const indexFile: FileNode = {
      ...
      name: "index.ts",
      type: "file",
      content: DEMO_FILES[0]?.children?.[0]?.children?.[0]?.content ?? "",
    };
    setOpenFiles([
      {
        id: indexFile.id,
        ...
        language: detectLanguage(indexFile.name),
      },
    ]);
  ...
  const handleBlankProject = useCallback(() => {
    ...
    const describeStr = L4(lang, {
      ko: "프로젝트 설명을 작성하세요.",
      en: "Describe your project here.",
    });
    ...
    const blankFiles: FileNode[] = [
      {
        ...
        type: "folder",
        children: [
          { id: "readme", name: "README.md", type: "file", content: mdContent },
        ],
      },
    ];
    setFiles(blankFiles);
  ...
  const handleResumeProject = useCallback(async () => {
    try {
      ...
      if (tree && tree.length > 0) {
        setFiles(tree);
        const firstFile = tree.flatMap(function findFiles(
          n: FileNode,
        ): FileNode[] {
          return n.type === "file"
            ? [n]
            : (n.children ?? []).flatMap(findFiles);
        })[0];
        if (firstFile) {
          setOpenFiles([
            {
        ...
        setHasEverOpened(true);
        toast(
          L4(lang, { ko: "프로젝트 복원됨", en: "Project resumed" }),
          "success",
        );
      } else {
        handleOpenDemo();
  ...
  const handleRunStressTest = useCallback(async () => {
    ...
    try {
      ...
      toast(
        `Stress Test: ${report.grade} (${report.overallScore}/100)`,
        report.grade === "F" ? "error" : "success",
      );
    } catch {
      toast("Stress test failed", "error");
    } finally {
      dispatchVerify({ isStressTesting: false });
    }
  ...
  const handleRunVerification = useCallback(async () => {
    ...
    try {
      const result = await runVerificationLoop(
        ...
        (iteration) => {
          dispatchVerify((prev) => ({
            ...
            pipelineStages: prev.pipelineStages.map((s, i) => ({
              ...s,
      ...
      if (
        result.totalFixesApplied > 0 &&
        result.finalCode !== result.originalCode
      ) {
        setStagedFiles((prev) => ({
          ...prev,
          [activeFile.name]: result.finalCode,
        }));
        setRightPanel("review");
        toast(
          `Verification: ${result.finalStatus.toUpperCase()} (${result.finalScore}/100) — ${result.totalFixesApplied} fixes staged. Open Review Center to apply.`,
      ...
      dispatchVerify({ isVerifying: false });
    }
  ...
  const handleApproveFile = useCallback(
    (fileName: string) => {
      ...
      const fileNode =
        ...
        files
          .flatMap(function walk(
            n: import("@/lib/code-studio/core/types").FileNode,
          ): import("@/lib/code-studio/core/types").FileNode[] {
            return [n, ...(n.children ?? []).flatMap(walk)];
          })
          .find((n) => n.name === fileName);
      const targetFileId = fileNode?.id;
      if (targetFileId) {
        setPreApplySnapshot((prev) => ({
          ...prev,
          [fileName]: fileNode.content ?? "",
        }));
        fsUpdateContent(targetFileId, code);
        setOpenFiles((prev) =>
          prev.map((f) =>
      ...
      toast(
        L4(lang, {
          ko: `${fileName}의 변경사항을 승인했습니다`,
          en: `Approved fixes for ${fileName}`,
        }),
        "success",
      );
    },
  ...
  const handleRejectFile = useCallback(
    (fileName: string) => {
      ...
      toast(
        L4(lang, {
          ko: `${fileName}의 변경사항을 거절했습니다`,
          en: `Rejected fixes for ${fileName}`,
        }),
        "info",
      );
    },
    ...
    (fileName: string) => {
      ...
      if (targetFileId) {
        ...
        setOpenFiles((prev) =>
          prev.map((f) =>
            f.id === targetFileId
              ? { ...f, content: snapshot, isDirty: true }
              : f,
          ),
        );
        setPreApplySnapshot((prev) => {
        ...
        toast(
          L4(lang, {
            ko: `${fileName}을(를) 검증 이전 상태로 되돌렸습니다`,
            en: `Rolled back ${fileName} to pre-verification state`,
          }),
          "info",
        );
      }
  ...
  const explorerPanel = (
    <div className="flex h-full flex-col bg-bg-secondary">
      <div className="flex items-center gap-2 border-b border-white/8 px-3 py-2">
        <Link
          href="/"
          className="rounded p-1 text-text-tertiary hover:bg-white/8 hover:text-accent-amber transition-colors"
          title="Home"
        >
        ...
        <button
          ...
          className="ml-auto rounded p-1 text-text-tertiary hover:bg-white/8 hover:text-text-primary"
          title="New File"
        >
    ...
    <>
        ...
        <div className="flex-1 min-h-0">
          {activeFile ? (
            <MonacoEditor
              ...
              path={activeMobileEditorPath}
              value={activeFile.content}
              onChange={handleEditorChange}
              theme="vs-dark"
              options={{
                fontSize: isMobile ? 13 : settings.fontSize,
              ...
              onMount={(editor, monaco) => {
                ...
                const ctxSub = attachEditorSurfaceContextMenu(
                  ed,
                  (pos, target) => {
                ...
                import("@/lib/code-studio/editor/monaco-setup").then(
                  ({ setupMonaco }) =>
                    setupMonaco(monaco, editor, { theme: "dark" }),
                );
                import("@/lib/code-studio/editor/editor-features").then(
                  ({ registerEditorFeatures }) =>
                    registerEditorFeatures(monaco, editor),
                );
                import("@/lib/code-studio/ai/ghost").then(
            ...
            <WelcomeScreen
              ...
              onQuickVerify={() => setRightPanel("quick-verify" as RightPanel)}
            />
          ) : (
            <div className="flex h-full items-center justify-center">
      ...
      {mobileEditorSurfaceMenu && (
        <ContextMenu
          ...
          onSelect={(id) => {
            runEditorSurfaceMenuAction(
              mobileEditorSurfaceTargetRef.current,
              id,
    ...
    const pipelineResult =
      pipelineStages.length > 0
        ? {
            ...
            overallStatus: ((pipelineScore ?? 0) >= 80
              ? "pass"
              : (pipelineScore ?? 0) >= 60
                ? "warn"
  ...
  const panelManagerProps = {
    rightPanel,
    onSetRightPanel: setRightPanel as (p: RightPanel | null) => void,
    showAdvancedPanels,
    ...
    onComposerTransition: composer.transitionMode,
    panels,
    onFileSelect: handleFileSelect,
    onApplyCode: handleApplyCode,
    onSetDiffState: setDiffState,
    fsUpdateContent,
    onSetOpenFiles: setOpenFiles,
    onSetFiles: setFiles,
    handleRunStressTest,
    handleRunVerification,
    editorNavigateToLine,
    ...
    return (
      ...
      >
        <PI.MobileLayoutComponent
          ...
          editor={mobileEditorPanel}
          chat={chatPanel}
          terminal={terminalPanel}
          pipeline={pipelinePanelMobile}
  ...
  return (
    <div className="flex h-full w-full flex-col bg-bg-primary text-text-primary">
      <a
        href="#main-editor"
        className="sr-only focus:not-sr-only focus:absolute focus:top-2 focus:left-2 focus:z-50 focus:bg-accent-purple focus:text-white focus:px-3 focus:py-1 focus:rounded"
      >
        Skip to Editor
      </a>
      <div className="flex flex-1 min-h-0">
        ...
        {sidebarVisible && (
          <div
            className="flex shrink-0 flex-col border-r border-white/8 bg-bg-secondary"
        ...
        <CodeStudioEditor
          ...
          isMobile={false}
          useEditorGroup={useEditorGroup}
          onToggleEditorGroup={() => setUseEditorGroup((v) => !v)}
          showSettings={showSettings}
          onToggleSettings={() => setShowSettings((s) => !s)}
          showMultiKey={showMultiKey}
          onCloseMultiKey={() => setShowMultiKey(false)}
          onCursorChange={(line, col) => setCursorPos({ line, col })}
          diffState={diffState}
          onDiffAccept={(content) => {
            handleApplyCode(content);
            setDiffState(null);
          }}
          onDiffReject={() => setDiffState(null)}
          onFileSelect={handleFileSelect}
          ...
          onWelcomeNewFile={handleWelcomeNewFile}
          onOpenDemo={handleOpenDemo}
          onBlankProject={handleBlankProject}
          onResumeProject={handleResumeProject}
          onQuickVerify={() => setRightPanel("quick-verify" as RightPanel)}
          ...
          onToggleTerminal={() => setShowTerminal((v) => !v)}
          onTogglePipeline={() =>
            setRightPanel(rightPanel === "pipeline" ? null : "pipeline")
          }
          onToggleAgent={() =>
            setRightPanel(rightPanel === "agents" ? null : "agents")
          }
          onToggleSearch={() =>
            setRightPanel(rightPanel === "search" ? null : "search")
          }
          onNewFile={() => setShowNewFile(true)}
          onToggleProblems={() =>
            setRightPanel(rightPanel === "bugs" ? null : "bugs")
          }
          onRunBugFinder={() =>
            setRightPanel(rightPanel === "bugs" ? null : "bugs")
          }
          onDeploy={() =>
            setRightPanel(rightPanel === "deploy" ? null : "deploy")
          }
          ...
          onSettingsSaved={() =>
            toast(
              L4(lang, { ko: "설정 저장됨", en: "Settings saved" }),
              "success",
            )
          }
        ...
        >
          ...
          {rightPanel && rightPanel !== "api-config" && (
            <>
              ...
              />
              <div
                className="shrink-0 flex flex-col border-l border-white/8 bg-bg-secondary overflow-hidden"
                style={{ width: rightPanelWidth }}
              >
                <RightPanelContent
                  {...(panelManagerProps as Parameters<
                    typeof RightPanelContent
                  >[0])}
                />
              </div>
            </>
        ...
        {/* Rollback Banner */}
        {Object.keys(preApplySnapshot).length > 0 &&
          Object.keys(stagedFiles).length === 0 && (
            <div className="border-t border-accent-purple/30 bg-accent-purple/5 px-4 py-2 flex items-center justify-between animate-[fadeSlideDown_0.2s_ease-out]">
              <span className="font-mono text-[11px] text-accent-purple">
                {L4(lang, {
                  ko: "검증 수정 사항 적용됨 — 롤백 가능",
                  en: "Verification fixes applied — Rollback available",
                })}
              </span>
              <button
                onClick={() => {
                  Object.keys(preApplySnapshot).forEach((f) =>
                    handleRollback(f),
                  );
                }}
                className="rounded border border-accent-purple/30 bg-accent-purple/10 px-3 py-1 text-[11px] text-accent-purple hover:bg-accent-purple/20"
              >
                {L4(lang, { ko: "모두 롤백", en: "Rollback All" })}
              </button>
            </div>
          )}
        ...
        <BottomPanels
          ...
          showPipelineBottom={showPipelineBottom}
          onToggleTerminal={() => setShowTerminal((v) => !v)}
          onToggleProblems={() => setShowProblems((v) => !v)}
          onTogglePipelineBottom={() => setShowPipelineBottom((v) => !v)}
          onCloseAllBottom={() => {
        ...
        {showCommandPalette && (
          <CommandPalette
            ...
            searchPlaceholder={L4(lang, {
              ko: "명령어 검색…",
              en: "Search commands…",
            })}
            noResultsText={L4(lang, {
              ko: "일치하는 명령이 없습니다",
              en: "No matching commands",
            })}
            formatFoundCount={(n) =>
              L4(lang, { ko: `${n}개`, en: `${n} found` })
            }
            ...
            onExecute={(cmdId) => {
              ...
              if (cmdId === "toggle-settings") {
                setShowSettings((v) => !v);
                return;
              }
              if (cmdId === "run-stress-test") {
                handleRunStressTest();
                return;
              }
              if (cmdId === "run-verification") {
                handleRunVerification();
                return;
              }
              ...
              if (PANEL_REGISTRY.some((p) => p.id === panelId)) {
                setRightPanel((v) =>
                  v === panelId ? null : (panelId as RightPanel),
                );
              }
            }}
            commands={[
              ...
              {
                id: "toggle-terminal",
                label: L4(lang, { ko: "콘솔 토글", en: "Toggle Console" }),
                shortcut: "Ctrl+`",
                category: "View",
              },
              ...Object.entries(
                PANEL_REGISTRY.reduce<Record<string, readonly PanelDef[]>>(
                  (acc, p) => {
                    const g = p.group;
              ...
              },
              {
                ...
                label: L4(lang, {
                  ko: "스트레스 테스트 실행",
                  en: "Run Stress Test (AI-Predicted)",
                }),
                category: "Tools",
              },
              {
                ...
                label: L4(lang, {
                  ko: "통합 검증 실행",
                  en: "Run Full Verification (Pipeline + Bugs + Stress)",
                }),
                category: "Tools",
              },
            ]}
...
export default function CodeStudioShell() {
  return (
    <ErrorBoundary
      variant="panel"
      fallbackMessage="Code Studio encountered an error"
    >
      <ToastProvider>
