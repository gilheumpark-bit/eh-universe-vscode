...
export function ModelSwitcher({ compact = false }: Props) {
  ...
  const [activeProviderId, setActiveProviderId] =
    useState<ProviderId>(getActiveProvider());
  ...
  return (
    <div ref={dropdownRef} className="relative inline-block">
      ...
      >
        ...
        />
        <span
          className={`truncate ${compact ? "max-w-[100px]" : "max-w-[140px]"}`}
        >
          {activeModel}
        </span>
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 z-50 min-w-[280px] max-h-[420px] overflow-y-auto rounded-lg border border-white/10 bg-[#0f1419] shadow-xl">
          ...
          {PROVIDERS.map((provider) => {
            const hasKey =
              getApiKey(provider.id as ProviderId).trim().length > 0;
            return (
              <div key={provider.id}>
                <div className="flex items-center gap-2 px-3 py-1.5 bg-white/3">
                  ...
                  <span className="ml-auto">
                    <Key
                      className={`w-3 h-3 ${hasKey ? "text-green-400" : "text-white/20"}`}
                    />
                  </span>
                </div>
                {provider.models.map((model) => {
                  ...
                  return (
                    ...
                    >
                      {isActive ? (
                        <Check className="w-3 h-3 flex-shrink-0 text-amber-400" />
                      ) : (
