...
export default function CollabPanel({ onClose }: Props) {
  ...
  const [localUser] = useState<CollabUser>(() => ({
    id: generateUserId(),
    name: userName,
    color: generateColor(),
  }));
  ...
  const attachListeners = useCallback((mgr: CollaborationManager) => {
    ...
    mgr.onChatReceived((userId: string, message: string) => {
      const resolvedName =
        peerNamesRef.current[userId] ?? `Peer ${userId.slice(0, 6)}`;
      const hue =
        userId.split("").reduce((a, c) => a + c.charCodeAt(0), 0) % 360;
      ...
      <div className="flex flex-col h-full bg-[#0a0e17] text-white text-[13px]">
        <div className="flex items-center justify-between px-3 py-2 border-b border-white/8 bg-[#0f1419] text-[12px] font-semibold uppercase tracking-wider">
          <span className="flex items-center gap-1.5">
            <Users size={14} /> 협업{" "}
            <span className="ml-1 rounded bg-amber-900/60 px-1.5 py-0.5 text-[9px] font-normal normal-case tracking-normal text-amber-300">
              (시뮬레이션)
            </span>
          </span>
          {onClose && (
        ...
        <div className="p-3 border-b border-white/8">
          <button
            onClick={createRoom}
            className="w-full flex items-center justify-center gap-1.5 py-2 bg-amber-800 text-stone-100 text-xs rounded hover:bg-amber-700 transition-colors"
          >
            <Plus size={14} /> 방 만들기
          </button>
        </div>
        <div className="p-3 border-b border-white/8">
          ...
          <div className="flex gap-1">
            ...
            <button
              onClick={joinRoom}
              className="flex items-center gap-1 px-3 py-1.5 bg-amber-800 text-stone-100 text-xs rounded hover:bg-amber-700"
            >
              <LogIn size={14} /> 참가
            </button>
          </div>
    ...
    <div className="flex flex-col h-full bg-[#0a0e17] text-white text-[13px]">
      ...
      <div className="p-3 border-b border-white/8">
        <div className="flex items-center justify-between mb-1.5">
          ...
          <button
            onClick={leaveRoom}
            className="flex items-center gap-1 text-[10px] text-white/50 hover:text-red-400 border border-white/10 rounded px-2 py-0.5"
          >
            <LogOut size={12} /> 나가기
          </button>
        </div>
        <div className="flex gap-1 items-center">
          <code className="flex-1 bg-[#161b22] px-2 py-1 rounded text-xs font-mono text-amber-400">
            {roomId}
          </code>
          <button
            onClick={copyShareUrl}
            className="flex items-center gap-1 text-[10px] border border-white/10 rounded px-2 py-1 text-white/50 hover:text-white"
          >
            <Copy size={12} />
            {copied ? " 복사됨" : ""}
          </button>
        ...
        <div className="text-[10px] text-white/50 mt-1 flex items-center gap-1">
          ...
          /code-studio?room={roomId}
        </div>
        <p className="text-[10px] text-amber-200/70 mt-2 leading-snug">
          같은 브라우저의 탭 간만 동기화됩니다(BroadcastChannel). 다른
          기기·브라우저와는 연결되지 않습니다.
        </p>
      </div>
      ...
      <div className="p-3 border-b border-white/8">
        <button
          ...
          className="flex items-center gap-1 text-[12px] font-semibold text-white mb-1.5"
        >
        ...
        {showUsers &&
          allUsers.map((user) => (
            <div
              key={user.id}
              className={`flex items-center gap-2 px-1.5 py-1 rounded ${user.id === localUser.id ? "bg-white/5" : ""}`}
            >
              <Circle size={10} fill={user.color} stroke={user.color} />
              <span className="flex-1 text-xs">
                {user.name}
                {user.id === localUser.id && (
                  <span className="text-white/60 text-[10px] ml-1">(나)</span>
                )}
              </span>
              {user.cursor && (
      ...
      <div className="flex-1 flex flex-col min-h-0 border-t border-white/8">
        <button
          ...
          className="flex items-center gap-1 px-3 py-2 text-[12px] font-semibold text-white"
        >
          ...
          <>
            <div className="flex-1 overflow-y-auto px-3 py-1 space-y-1 min-h-[100px]">
              {chatMessages.length === 0 && (
                <div className="text-center text-[11px] text-white/50 py-4">
                  메시지가 없습니다
                </div>
              )}
              {chatMessages.map((msg) => (
                <div key={msg.id} className="text-xs">
                  ...
                  <div className="text-white/80 mt-0.5 pl-0.5">
                    {msg.message}
                  </div>
                </div>
              ))}
            ...
            <div className="flex gap-1 px-3 py-1.5 border-t border-white/8">
              <input
                ...
                placeholder="메시지 입력..."
                className="flex-1 bg-[#0a0e17] border border-white/10 rounded px-2 py-1 text-xs text-white outline-none"
              />
              <button
                onClick={sendChat}
                disabled={!chatInput.trim()}
                className="px-2 py-1 bg-amber-800 text-stone-100 rounded text-xs hover:bg-amber-700 disabled:opacity-30"
              >
                <Send size={12} />
              </button>
            </div>
