...
const SAMPLE_KO = {
  ...
  dialogue:
    '"이건 네가 할 일이 아니야." 그녀가 말했다. "알아. 그래도 해야 해." 그는 대답했다. 침묵이 둘 사이를 채웠다.',
  introspection:
    "왜 이 길을 선택했는지 더 이상 기억나지 않았다. 분명 처음에는 이유가 있었다. 지금은 그저 관성이다. 멈추면 무너질 것 같았다.",
  description:
    "창밖으로 비가 내렸다. 도시의 불빛이 빗줄기에 번져 흐릿했다. 가로등 아래 고인 물웅덩이가 하늘을 거꾸로 비추고 있었다.",
};
...
const SAMPLE_EN = {
  ...
  dialogue:
    '"This isn\'t your job." she said. "I know. But I have to." he replied. Silence filled the space between them.',
  introspection:
    "He couldn't remember why he chose this path. There had been a reason once. Now it was just inertia. Stopping felt like collapsing.",
  description:
    "Rain fell outside the window. City lights blurred through the streaks. A puddle beneath the streetlamp reflected the sky upside down.",
};
  ...
  if (s2 >= 4) {
    ...
    if (parts.length > midPoint) {
      parts.splice(
        midPoint,
        0,
        emotionMarkers[midPoint % emotionMarkers.length],
      );
    }
    result = parts.join(" ");
  }
...
function RadarChart({
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={isKO ? "스타일 레이더 차트" : "Style radar chart"}
    >
      {/* Grid */}
...
export default function StylePreview({ profile, language }: Props) {
  ...
  const selectedArchs = STYLE_ARCHETYPES.filter((a) =>
    compareArchs.includes(a.id),
  );
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-[9px] font-bold border transition-all ${
                    active
                      ? "text-white"
                      : "text-text-tertiary border-border hover:border-white/20"
                  }`}
                  style={
                    active
                      ? {
