...
> = {
  KO: {
    ...
    quickStart: "쾌속 시작",
    quickStartDesc:
      "장르와 한 줄 프롬프트만 입력하면 세계관·캐릭터·첫 장면 초안이 생성됩니다.",
    manual: "직접 설정",
  ...
  EN: {
    ...
    quickStartDesc:
      "Enter a genre and one-line prompt. World, characters, and first scene drafts are generated.",
    manual: "Manual Setup",
  ...
  JP: {
    ...
    quickStart: "クイックスタート",
    quickStartDesc:
      "ジャンルとプロンプトを入力するだけ。世界観・キャラ・冒頭が自動生成されます。",
    manual: "手動設定",
...
export default function OnboardingGuide({
  ...
  const cards: {
  ...
  }[] = [
    {
      ...
      title: L.quickStart,
      desc: L.quickStartDesc,
      accent: true,
      badge: showQuickStartLock ? L.byok : undefined,
    },
    {
      key: "manual",
  ...
  return (
    <div
      className={`mx-auto w-full max-w-2xl transition-all duration-700 ease-out ${visible ? "translate-y-0 opacity-100" : "translate-y-8 opacity-0"}`}
    >
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-3">
            {cards.map(({ key, icon, title, desc, accent, badge }, idx) => (
              ...
              >
                ...
                {/* Title */}
                <span
                  className={`text-sm font-black uppercase tracking-wider font-mono ${
                ...
                {/* Arrow indicator */}
                <span
                  className={`mt-auto text-[10px] uppercase tracking-wider font-bold transition-all duration-300 ${
                    accent
          ...
          <div className="mt-6 space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {DEMO_PRESETS.map((preset) => {
                const icons = [
                  <Sword key="s" className="h-5 w-5" />,
                  <BookOpen key="b" className="h-5 w-5" />,
                  <RotateCcw key="r" className="h-5 w-5" />,
                ];
                const colors = [
                ...
                return (
                  <button
                    key={preset.id}
                    type="button"
                    onClick={() => handleDemoSelect(preset.id)}
                  ...
                  >
                    ...
                    <div className="flex flex-wrap justify-center gap-1 mt-1">
                      {preset.tags.slice(0, 3).map((tag) => (
                        <span
                          key={tag}
                          className="rounded-full bg-white/5 px-2 py-0.5 text-[9px] text-text-tertiary"
                        >
                          {tag}
                        </span>
                      ))}
