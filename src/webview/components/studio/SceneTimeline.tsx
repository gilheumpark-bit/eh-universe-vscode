...
function detectTimelineWarnings(scenes: ParsedScene[]): TimelineWarning[] {
  ...
  for (let si = 0; si < scenes.length; si++) {
    ...
    if (beats.length > 8 && dialogueCount === 0) {
      warnings.push({
        sceneIndex: si,
        message: `대사 없이 서술만 ${beats.length}비트`,
        severity: "warning",
      });
    }
      ...
      if (beats[bi].type === beats[bi - 1].type) {
        ...
        if (streak >= 5) {
          warnings.push({
            sceneIndex: si,
            beatIndex: bi,
            message: `${BEAT_LABELS[beats[bi].type]} ${streak}연속 — 단조로움`,
            severity: "warning",
          });
          break;
        }
    ...
    if (si > 0) {
      ...
      if (drop >= 30) {
        warnings.push({
          sceneIndex: si,
          message: `텐션 급락 ${prevTension}→${scene.tension} (-${drop})`,
          severity: "critical",
        });
      }
    }
    ...
    // 화자 미확인 대사
    const noSpeaker = beats.filter(
      (b) => b.type === "dialogue" && !b.speaker,
    ).length;
    if (noSpeaker > 0) {
      warnings.push({
        sceneIndex: si,
        message: `화자 미확인 대사 ${noSpeaker}건`,
        severity: "info",
      });
    }
  }
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          ...
          {beat.speaker && (
            <span className="text-[9px] font-mono text-accent-green truncate">
              {beat.speaker}
            </span>
          )}
...
function SceneLane({
  ...
  return (
    <div
      className={`border rounded-xl ${hasWarnings ? "border-accent-amber/30" : "border-border/20"} bg-bg-secondary/30`}
    >
      {/* 장면 헤더 */}
      <div
      ...
      >
        ...
        <div className="flex-1 flex items-center gap-1.5 mx-2">
          ...
          <span className="text-[9px] text-text-tertiary w-5 text-right">
            {scene.tension}
          </span>
        </div>
        ...
        {onPlayFrom && (
          <button
            onClick={(e) => {
              e.stopPropagation();
              onPlayFrom(sceneIndex, 0);
            }}
            className="p-1 rounded hover:bg-accent-purple/20 transition-colors"
            aria-label="이 장면부터 재생"
          >
      ...
      {!collapsed && (
        <div className="px-2 pb-2 space-y-1">
          {scene.beats.map((beat, bi) => {
            ...
            return (
              <BeatBlock
                ...
                beatIndex={bi}
                isSelected={
                  selectedBeat?.sceneIndex === sceneIndex &&
                  selectedBeat?.beatIndex === bi
                }
                warning={beatWarning}
          ...
          {/* 장면 레벨 경고 */}
          {sceneWarnings
            .filter((w) => w.beatIndex === undefined)
            .map((w, i) => (
              <div
                key={i}
                className="flex items-center gap-1.5 px-2 py-1 text-[10px] text-accent-amber bg-accent-amber/5 rounded-lg"
              >
  ...
  for (let si = 0; si < scenes.length; si++) {
    ...
    for (const beat of scenes[si].beats) {
      ...
      switch (beat.type) {
        case "dialogue":
          parts.push(
            beat.speaker ? `${beat.speaker}: "${beat.text}"` : `"${beat.text}"`,
          );
          break;
        case "thought":
...
export default function SceneTimeline({
    ...
    (si: number, bi: number) => {
      ...
      setScenes((prev) => {
        const next = prev.map((s, i) =>
          i === si ? { ...s, beats: s.beats.filter((_, j) => j !== bi) } : s,
        );
        onScenesChange(next);
        return next;
      });
  ...
  const handleDrop = useCallback(
    (targetSi: number, targetBi: number) => {
  ...
  const typeDistribution = useMemo(() => {
    const dist: Record<BeatType, number> = {
      dialogue: 0,
      narration: 0,
      action: 0,
      thought: 0,
    ...
    for (const scene of scenes)
      for (const beat of scene.beats) dist[beat.type]++;
    return dist;
  }, [scenes]);
    ...
    <div className="flex flex-col h-full bg-bg-primary">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b border-border/20">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[9px] text-text-tertiary">
            {scenes.length}장면 · {totalBeats}비트
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className="p-1.5 rounded-lg hover:bg-white/5 disabled:opacity-40 transition-colors text-text-secondary"
            aria-label="되돌리기"
          >
      ...
      <div className="flex h-2 mx-3 mt-2 rounded-full overflow-hidden bg-bg-tertiary/30">
        {(
          ...
          <div
            ...
            style={{
              width: `${(typeDistribution[type] / Math.max(1, totalBeats)) * 100}%`,
            }}
      ...
      <div className="flex items-center gap-3 px-3 py-1">
        {(
          ...
          <span
            key={type}
            className="flex items-center gap-1 text-[8px] text-text-tertiary"
          >
      ...
      {timelineWarnings.filter((w) => w.severity === "critical").length > 0 && (
        <div className="mx-3 mt-1 px-3 py-1.5 bg-accent-red/10 border border-accent-red/20 rounded-lg flex items-center gap-2 text-[10px] text-accent-red">
          ...
          {isKO
            ? "텐션 급락 감지 — 독자 이탈 위험 구간이 있습니다"
            : "Tension drop detected — risk of reader dropout"}
        </div>
      )}
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        {scenes.map((scene, si) => (
          <SceneLane
            ...
            onSelectBeat={(si2, bi) =>
              setSelectedBeat({ sceneIndex: si2, beatIndex: bi })
            }
            onDeleteBeat={deleteBeat}
            onPlayFrom={onPlayFrom}
            onDragStart={(si2, bi) =>
              setDragSource({ sceneIndex: si2, beatIndex: bi })
            }
