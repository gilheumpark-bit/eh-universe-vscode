...
const OUTPUT_MODES: {
  key: AdvancedWritingSettings["outputMode"];
  tKey: string;
}[] = [
  ...
  {
    key: "description-boost",
    tKey: "advancedWritingExtra.modeDescriptionBoost",
  },
    ...
    <div className="space-y-3">
      ...
      <Section title={t("advancedWriting.narrativeConstraints")} icon={Sliders}>
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-text-tertiary w-14">
            {t("advancedWriting.pov")}
          </span>
          <div className="flex gap-1">
            {[
              { v: "1st" as const, tKey: "advancedWritingExtra.pov1st" },
              {
                v: "3rd-limited" as const,
                tKey: "advancedWritingExtra.pov3rdLimited",
              },
              {
                v: "3rd-omni" as const,
                tKey: "advancedWritingExtra.pov3rdOmni",
              },
            ].map((p) => (
              <button
                key={p.v}
                onClick={() => updateConstraint("pov", p.v)}
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-text-tertiary w-14">
            {t("advancedWriting.dialogue")}
          </span>
          <input
            type="range"
            min={10}
            max={90}
            value={s.constraints.dialogueRatio}
            onChange={(e) =>
              updateConstraint("dialogueRatio", Number(e.target.value))
            }
            className="flex-1 h-1 accent-accent-purple"
          />
          <span className="text-[9px] text-text-tertiary w-8 text-right">
            {s.constraints.dialogueRatio}%
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex gap-1">
            {[
              ...
              {
                v: "stable" as const,
                tKey: "advancedWritingExtra.tempoStable",
              },
              { v: "slow" as const, tKey: "advancedWritingExtra.tempoSlow" },
            ].map((ti) => (
              <button
                key={ti.v}
                onClick={() => updateConstraint("tempo", ti.v)}
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-text-tertiary w-14">
            {t("advancedWriting.sentenceLen")}
          </span>
          <div className="flex gap-1">
            {[
              {
                v: "short" as const,
                tKey: "advancedWritingExtra.sentenceShort",
              },
              {
                v: "normal" as const,
                tKey: "advancedWritingExtra.sentenceNormal",
              },
              { v: "long" as const, tKey: "advancedWritingExtra.sentenceLong" },
            ].map((sl) => (
              <button
                key={sl.v}
                onClick={() => updateConstraint("sentenceLen", sl.v)}
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] text-text-tertiary w-14">
            {t("advancedWriting.emotion")}
          </span>
          <div className="flex gap-1">
            {[
              {
                v: "restrained" as const,
                tKey: "advancedWritingExtra.emotionRestrained",
              },
              {
                v: "normal" as const,
                tKey: "advancedWritingExtra.emotionNormal",
              },
              {
                v: "intense" as const,
                tKey: "advancedWritingExtra.emotionIntense",
              },
            ].map((em) => (
              <button
                key={em.v}
                onClick={() => updateConstraint("emotionExposure", em.v)}
      ...
      <Section title={t("advancedWriting.referenceScope")} icon={BookOpen}>
        <div className="flex items-center gap-2 mb-2">
          <span className="text-[9px] text-text-tertiary">
            {t("advancedWriting.prevEpisodes")}
          </span>
          <input
            type="range"
            min={0}
            max={25}
            value={s.references.prevEpisodes}
            onChange={(e) => updateRef("prevEpisodes", Number(e.target.value))}
            className="flex-1 h-1 accent-accent-purple"
          />
          <span className="text-[9px] text-text-tertiary w-8 text-right">
            {s.references.prevEpisodes}
            {t("advancedWriting.ep")}
          </span>
        </div>
        <div className="flex flex-wrap gap-1.5">
          {[
            {
              key: "characterCards" as const,
              tKey: "advancedWritingExtra.refCharacterCards",
            },
            {
              key: "worldSetting" as const,
              tKey: "advancedWritingExtra.refWorldSetting",
            },
            {
              key: "styleProfile" as const,
              tKey: "advancedWritingExtra.refStyleProfile",
            },
            {
              key: "sceneSheet" as const,
              tKey: "advancedWritingExtra.refSceneSheet",
            },
            {
              key: "platformPreset" as const,
              tKey: "advancedWritingExtra.refPlatformPreset",
            },
          ].map((r) => (
            <button
              key={r.key}
              onClick={() => updateRef(r.key, !s.references[r.key])}
      ...
      {/* 4. 설정 잠금 */}
      <Section
        title={t("advancedWriting.settingLocks")}
        icon={Lock}
        defaultOpen={false}
      >
        <div className="flex flex-wrap gap-1.5">
          {[
            {
              key: "speechStyle" as const,
              tKey: "advancedWritingExtra.lockSpeechStyle",
            },
            {
              key: "worldRules" as const,
              tKey: "advancedWritingExtra.lockWorldRules",
            },
            {
              key: "charRelations" as const,
              tKey: "advancedWritingExtra.lockCharRelations",
            },
            {
              key: "bannedWords" as const,
              tKey: "advancedWritingExtra.lockBannedWords",
            },
          ].map((lk) => (
            <button
              key={lk.key}
              onClick={() => updateLock(lk.key, !s.locks[lk.key])}
              className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-bold transition-all ${
                s.locks[lk.key]
      ...
      <Section title={t("advancedWriting.outputMode")} icon={FileOutput}>
        <div className="flex flex-wrap gap-1.5">
          {OUTPUT_MODES.map((m) => (
            <button
              key={m.key}
              onClick={() => update("outputMode", m.key)}
              className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${
                s.outputMode === m.key
                  ? "bg-accent-purple text-white"
