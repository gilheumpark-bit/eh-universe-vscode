...
import {
  setNarrativeDepth as narrativeDepthSetter,
  getNarrativeDepth,
} from "@/lib/noa/lora-swap";
...
const SettingsView: React.FC<SettingsViewProps> = ({
  ...
  const [defaultPlatform, setDefaultPlatform] = useState<string>(
    () =>
      (typeof window !== "undefined"
        ? localStorage.getItem("noa_default_platform")
        : null) || "MOBILE",
  );
  const [defaultEpisodes, setDefaultEpisodes] = useState<number>(() =>
    parseInt(
      (typeof window !== "undefined"
        ? localStorage.getItem("noa_default_episodes")
        : null) || "25",
    ),
  );
  const [temperature, setTemperature] = useState<number>(() =>
    parseFloat(
      (typeof window !== "undefined"
        ? localStorage.getItem("noa_temperature")
        : null) || "0.9",
    ),
  );
  ...
  const activeProvider =
    typeof window !== "undefined" ? getActiveProvider() : "gemini";
  ...
  const apiProvider =
    typeof window !== "undefined" ? getActiveProvider() : activeProvider;
  void apiKeyRefresh;
  const hasPersonalApiKey =
    typeof window !== "undefined" && hasStoredApiKey(apiProvider);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-black/40 p-4 rounded-xl border border-border space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-xs text-text-secondary">
                  {language === "KO" ? "로컬 저장 용량" : "Local Storage"}
                </span>
                {storageEstimate && storageEstimate.quota > 0 ? (
                  (() => {
                    ...
                    return (
                      <span
                        className={`text-xs font-black ${pct > 80 ? "text-red-400" : pct > 50 ? "text-accent-amber" : "text-green-500"}`}
                      >
                  ...
                  >
                    {(getStorageUsageBytes() / 1024 / 1024).toFixed(1)} MB / 5
                    MB
                  </span>
                )}
              </div>
              <div className="h-1.5 bg-bg-tertiary rounded-full overflow-hidden">
                {storageEstimate && storageEstimate.quota > 0 ? (
                  ...
                  <div
                    className={`h-full rounded-full transition-all ${(() => {
                      const pct =
                        (getStorageUsageBytes() / (5 * 1024 * 1024)) * 100;
                      return pct > 80
                        ? "bg-red-500"
                        : pct > 50
                          ? "bg-accent-amber"
                    ...
                    style={{
                      width: `${Math.min(100, (getStorageUsageBytes() / (5 * 1024 * 1024)) * 100)}%`,
                    }}
                  />
                )}
              </div>
              {storageEstimate &&
                ...
                storageEstimate.usage / storageEstimate.quota > 0.8 && (
                  <p className="text-[10px] text-red-400">
                    {language === "KO"
                      ? "용량이 부족합니다. 오래된 세션을 삭제하거나 백업 후 정리하세요."
                      : "Storage nearly full. Delete old sessions or export a backup."}
                  </p>
                )}
            </div>
          </div>
        ...
        <div className="md:col-span-2 ds-card-lg">
          ...
          <div className="space-y-2">
            ...
            >
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <div className="p-2 md:p-3 bg-bg-secondary rounded-2xl shrink-0">
                  <Key className="w-4 h-4 md:w-5 md:h-5 text-text-tertiary" />
                </div>
              ...
              <div className="flex flex-col items-end gap-0.5 shrink-0">
                <div
                  ...
                  className="text-[9px] md:text-[10px] font-black text-blue-500 uppercase"
                >
                  {hasPersonalApiKey
            ...
            >
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <div className="p-2 md:p-3 bg-bg-secondary rounded-2xl shrink-0">
                  <Bell className="w-4 h-4 md:w-5 md:h-5 text-text-tertiary" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs md:text-sm font-bold truncate">
              ...
              <div
                className={`relative w-10 h-6 rounded-full flex items-center transition-colors duration-300 shrink-0 ${notificationsOn ? "bg-blue-600 justify-end" : "bg-bg-tertiary justify-start"}`}
              >
            ...
            >
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <div className="p-2 md:p-3 bg-bg-secondary rounded-2xl group-hover:bg-red-500/20 transition-colors shrink-0">
          ...
          <div className="md:col-span-2 ds-card-lg">
            <div className="flex items-center justify-between mb-6">
              ...
              {onRefreshBackups && (
                <button
                  ...
                  className="text-[10px] text-text-tertiary hover:text-text-primary font-mono uppercase tracking-wider transition-colors"
                  title={language === "KO" ? "목록 새로고침" : "Refresh list"}
                >
            ...
            {versionedBackups.length === 0 ? (
              <div className="text-sm text-text-tertiary py-4 text-center">
                {language === "KO"
                  ? "저장된 백업이 없습니다. 10분 후 자동 백업됩니다."
                  : "No backups yet. Auto-backup runs every 10 minutes."}
              </div>
            ) : (
              <div className="space-y-2">
                {versionedBackups.map((b) => (
                  <div
                    key={b.timestamp}
                    className="flex items-center justify-between p-4 bg-black/40 rounded-xl border border-border"
                  >
                    <div>
        ...
        <div className="md:col-span-2 ds-card-lg">
          <h3 className="text-[10px] font-black text-text-tertiary uppercase tracking-widest mb-8 flex items-center gap-2">
            ...
            {t("settingsEngine.engineSettings")}
          </h3>
          <div className="space-y-2">
            {/* Default Platform */}
              ...
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <div className="p-2 md:p-3 bg-bg-secondary rounded-2xl shrink-0">
                  {defaultPlatform === "MOBILE" ? (
                    <Smartphone className="w-4 h-4 md:w-5 md:h-5 text-text-tertiary" />
                  ) : (
                    <Monitor className="w-4 h-4 md:w-5 md:h-5 text-text-tertiary" />
                  )}
                </div>
                <div className="min-w-0">
                  <div className="text-xs md:text-sm font-bold truncate">
                    {t("settingsEngine.defaultPlatform")}
                  </div>
                  <div className="text-[10px] md:text-[11px] text-text-tertiary hidden sm:block">
                    {t("settingsEngine.defaultPlatformDesc")}
                  </div>
                </div>
              </div>
                ...
                {(["MOBILE", "WEB"] as const).map((p) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      localStorage.setItem("noa_default_platform", p);
                    }}
                  ...
                  >
                    {p === "MOBILE"
                      ? t("settingsEngine.mobile")
                      : t("settingsEngine.web")}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-4 md:p-6 rounded-3xl border border-transparent">
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <div className="p-2 md:p-3 bg-bg-secondary rounded-2xl shrink-0">
                  <Hash className="w-4 h-4 md:w-5 md:h-5 text-text-tertiary" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs md:text-sm font-bold truncate">
                    {t("settingsEngine.defaultEpisodes")}
                  </div>
                  <div className="text-[10px] md:text-[11px] text-text-tertiary hidden sm:block">
                    {t("settingsEngine.defaultEpisodesDesc")}
                  </div>
                </div>
              </div>
              <input
                ...
                onChange={(e) => {
                  ...
                  localStorage.setItem("noa_default_episodes", String(v));
                }}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 p-4 md:p-6 rounded-3xl border border-transparent">
              <div className="flex items-center gap-3 md:gap-4 min-w-0">
                <div className="p-2 md:p-3 bg-bg-secondary rounded-2xl shrink-0">
                  <Thermometer className="w-4 h-4 md:w-5 md:h-5 text-text-tertiary" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs md:text-sm font-bold truncate">
                    {t("settingsEngine.temperature")}
                  </div>
                  <div className="text-[10px] md:text-[11px] text-text-tertiary hidden sm:block">
                    {t("settingsEngine.temperatureDesc")}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2 md:gap-3 shrink-0">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setTemperature(v);
                    localStorage.setItem("noa_temperature", String(v));
                  }}
                  className="w-20 md:w-24 accent-blue-600 h-1.5 bg-bg-tertiary rounded-full appearance-none cursor-pointer"
                />
                <span className="text-xs md:text-sm font-black text-blue-400 w-7 md:w-8 text-right">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 md:gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="text-xs md:text-sm font-bold truncate">
                    {language === "KO" ? "서사 깊이" : "Narrative Depth"}
                  </div>
                  <div className="text-[10px] md:text-[11px] text-text-tertiary hidden sm:block">
                    {narrativeDepth <= 0.9
                      ? language === "KO"
                        ? "평작 — 가독성 우선"
                        : "Light — Readability first"
                      : narrativeDepth <= 1.0
                        ? language === "KO"
                          ? "기본 — 장르 균형"
                          : "Standard — Genre balance"
                        : narrativeDepth <= 1.2
                          ? language === "KO"
                            ? "심화 — 비유/상징 활용"
                            : "Deep — Metaphor/symbolism"
                          : language === "KO"
                            ? "최대 — 문예 수준 밀도"
                            : "Maximum — Literary density"}
                  </div>
                </div>
              </div>
              <div className="flex items-center gap-2 md:gap-3 shrink-0">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    localStorage.setItem("noa_narrative_depth", String(v));
                ...
                />
                <span className="text-xs md:text-sm font-black text-accent-purple w-7 md:w-8 text-right">
                  {narrativeDepth.toFixed(1)}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="md:col-span-2 flex flex-col gap-4 md:flex-row justify-between items-center px-2 md:px-10">
          ...
          <div className="flex gap-6 text-[9px] font-black text-text-tertiary uppercase tracking-widest">
            ...
            <a
              href="https://github.com/gilheumpark-bit/eh-universe-web/issues/new"
              target="_blank"
              rel="noopener"
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-black text-base md:text-lg">
              {user.displayName || t("settings.writer")}
            </h3>
