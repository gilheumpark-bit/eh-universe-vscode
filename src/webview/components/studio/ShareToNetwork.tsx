...
export default function ShareToNetwork({
  language,
  config,
  messages,
  onClose,
    ...
    try {
      ...
      void ingestAgent(
        ...
        if (!ok) {
          logger.warn(
            "ShareToNetwork",
            "network-agent ingest failed after publish; post kept",
          );
          setIngestWarning(
            isKO
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div
        className="w-full max-w-lg bg-bg-primary border border-border rounded-2xl p-6 space-y-5 animate-in zoom-in-95 duration-200"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
          ...
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              >
                <span
                  className={
                    selectedPlanet ? "text-text-primary" : "text-text-tertiary"
                  }
                >
                  {planetsLoading
                    ? isKO
              ...
              {showPlanetDropdown && (
                <div className="absolute z-10 mt-1 w-full max-h-40 overflow-y-auto bg-bg-primary border border-border rounded-xl shadow-lg">
                  ...
                  {planets.map((p) => (
                    ...
                    >
                      {p.name}{" "}
                      <span className="text-text-tertiary">· {p.genre}</span>
                    </button>
                  ))}
                </div>
              )}
        ...
        <div className="text-[10px] text-text-tertiary bg-bg-secondary/50 border border-border/50 rounded-xl px-4 py-2.5">
          {shareType === "episode" && (
            <>
              {isKO
                ? `${episodes.length}개 에피소드가 공유됩니다.`
                : `${episodes.length} episodes will be shared.`}
            </>
          )}
          {shareType === "character_sheet" && (
            <>
              {isKO
                ? `${config.characters.length}명의 캐릭터 시트가 공유됩니다.`
                : `${config.characters.length} character sheets will be shared.`}
            </>
          )}
          {shareType === "world_bible" && (
            <>
              {isKO
                ? "세계관 설정의 읽기 전용 스냅샷이 공유됩니다."
                : "A read-only snapshot of world settings will be shared."}
            </>
          )}
          {shareType === "style_profile" && (
            <>
              {isKO
                ? "현재 문체 프로필이 공유됩니다."
                : "Current style profile will be shared."}
            </>
          )}
        </div>
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl text-[10px] font-bold border border-border text-text-tertiary hover:text-white transition-all"
          >
            {isKO ? "취소" : "Cancel"}
          </button>
          {!user ? (
