...
const ACTION_LABEL: Record<
  string,
  { label: Record<AppLanguage, string>; color: string }
> = {
  rewrite: {
  ...
  expand: {
    ...
    color: "text-green-400 bg-green-600/10 border-green-500/20",
  },
  compress: {
  ...
  tension: {
    label: { KO: "긴장감", EN: "Tension", JP: "緊張感", CN: "紧张感" },
    color: "text-red-400 bg-red-600/10 border-red-500/20",
  },
  dialogue: {
    label: { KO: "대사", EN: "Dialogue", JP: "セリフ", CN: "台词" },
    color: "text-pink-400 bg-pink-600/10 border-pink-500/20",
  },
  insert_after: {
    label: { KO: "삽입", EN: "Insert", JP: "挿入", CN: "插入" },
    color: "text-purple-400 bg-purple-600/10 border-purple-500/20",
  },
};

const AutoRefiner: React.FC<AutoRefinerProps> = ({
  content,
  language,
  context,
  onApply,
  ...
  const [phase, setPhase] = useState<"idle" | "analyzing" | "ready" | "fixing">(
    "idle",
  );
  ...
  const startAnalysis = useCallback(async () => {
    ...
    try {
      await streamChat({
        systemInstruction: {
          KO: "소설 편집자. JSON 배열만 출력. 다른 텍스트 절대 금지.",
          EN: "Fiction editor. Output JSON array only. No other text.",
          JP: "小説編集者。JSON配列のみ出力。他のテキスト禁止。",
          CN: "小说编辑。仅输出JSON数组。禁止其他文本。",
        }[language],
        messages,
        temperature: 0.3,
      ...
      const parsed: { p: number; issue: string; action: string }[] = JSON.parse(
        jsonMatch[0],
      );
  ...
  const fixSuggestion = useCallback(
    async (idx: number) => {
      ...
      if (!apiKey) {
        alert(
          language === "KO"
            ? "API 키가 필요합니다. 설정에서 등록해주세요."
            : "API key required. Please add one in Settings.",
        );
        return;
      }
      ...
      try {
        await streamChat({
          systemInstruction: {
            KO: "소설 텍스트 리라이터. 순수 소설 텍스트만 출력. 설명 금지.",
            EN: "Fiction rewriter. Output pure fiction text only. No explanations.",
            JP: "小説テキストリライター。純粋な小説テキストのみ出力。説明禁止。",
            CN: "小说文本改写器。仅输出纯小说文本。禁止解释。",
          }[language],
          messages,
          temperature: 0.85,
          signal: controller.signal,
  ...
  const applySuggestion = useCallback(
    (idx: number) => {
      ...
      setSuggestions((prev) =>
        prev.map((s, i) => (i === idx ? { ...s, status: "applied" } : s)),
      );
    },
  ...
  const undoLast = useCallback(() => {
    ...
    setSuggestions((prevSugs) => {
      const lastApplied = [...prevSugs]
        .reverse()
        .findIndex((s) => s.status === "applied");
      if (lastApplied === -1) return prevSugs;
      const realIdx = prevSugs.length - 1 - lastApplied;
      return prevSugs.map((s, i) =>
        i === realIdx ? { ...s, status: "ready" } : s,
      );
    });
  }, [undoStack]);
    ...
    <div className="border border-accent-purple/20 bg-accent-purple/5 rounded-2xl overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          {phase === "idle" && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-accent-purple text-white rounded-lg text-[10px] font-bold font-mono hover:opacity-80 transition-opacity"
            >
          ...
          {phase === "analyzing" && (
            <>
              <StreamingIndicator
                charCount={streamingChars}
                language={language}
              />
              <button
                onClick={cancel}
                className="flex items-center gap-1 px-2 py-1 bg-red-600/20 border border-red-500/30 text-red-400 rounded-lg text-[10px] font-bold font-mono hover:bg-red-600/30 transition-colors"
              >
          ...
          {phase === "ready" && pendingCount > 0 && (
            <button
              onClick={runAllFixes}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-blue-600/20 border border-blue-500/30 text-blue-400 rounded-lg text-[10px] font-bold font-mono hover:bg-blue-600/30 transition-colors"
            >
          ...
          {undoStack.length > 0 && (
            <button
              onClick={undoLast}
              className="flex items-center gap-1.5 px-2 py-1.5 bg-bg-tertiary/30 border border-zinc-600/30 text-text-secondary rounded-lg text-[10px] font-bold font-mono hover:bg-bg-tertiary/50 transition-colors"
              title={t("ui.undo")}
            >
          ...
          {appliedCount > 0 && (
            <button
              onClick={applyAll}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-green-600/20 border border-green-500/30 text-green-400 rounded-lg text-[10px] font-bold font-mono hover:bg-green-600/30 transition-colors"
            >
          ...
          {expanded ? (
            <ChevronUp
              className="w-3.5 h-3.5 text-text-tertiary cursor-pointer"
          ...
          {suggestions.map((sug, idx) => {
            ...
            return (
              <div
                ...
                className={`border rounded-xl p-3 space-y-2 transition-all ${
                  sug.status === "applied"
                    ...
                    : sug.status === "skipped"
                      ? "border-border/30 bg-bg-secondary/30 opacity-40"
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-bold text-text-tertiary font-mono">
                      P{sug.paragraphIndex + 1}
                    </span>
                    <span
                      className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${actionInfo.color} font-mono`}
                    >
                  ...
                  <div className="flex items-center gap-1">
                    {sug.status === "pending" && (
                      <>
                        <button
                          onClick={() => fixSuggestion(idx)}
                          className="p-1 rounded hover:bg-blue-900/20 text-blue-500/50 hover:text-blue-400 transition-colors"
                          title={t("ui.generate")}
                        >
                    ...
                    {sug.status === "ready" && (
                      <>
                        <button
                          ...
                          className="p-1 rounded hover:bg-green-900/20 text-green-500/50 hover:text-green-400 transition-colors"
                          title={t("ui.apply")}
                        >
                ...
                {/* Original (truncated) */}
                <p className="text-[10px] text-text-tertiary font-serif leading-relaxed line-clamp-2">
                  {sug.original}
                </p>
      ...
      {refinerError !== null && (
        <ErrorToast
          error={refinerError}
          language={language}
          onDismiss={() => setRefinerError(null)}
          onRetry={() => {
