...
export default function TranslationPanel({
  ...
  // Advanced Settings
  const [contractionLevel, setContractionLevel] = useState<
  ...
  const {
      ...
      if (p.status === "scoring") {
        setLogs((prev) => [
          ...prev.slice(-49),
          {
            id: Date.now(),
            type: "info",
            text: `Analyzing metrics for chunk ${p.currentChunk + 1}...`,
          },
        ]);
        ...
        setLogs((prev) => [
          ...prev.slice(-49),
          {
            id: Date.now(),
            type: "warn",
            text: `Quality Gate failed. Re-creating chunk ${p.currentChunk + 1} (Attempt ${p.recreateCount})...`,
          },
        ]);
      }
    },
    ...
    onError: (err) => {
      ...
      setLogs((prev) => [
        ...prev.slice(-49),
        { id: Date.now(), type: "error", text: `Error: ${err}` },
      ]);
    },
    onSave: (entry) => {
      setConfig((prev) => ({
      ...
      setLogs((prev) => [
        ...prev,
        {
          id: Date.now(),
          type: "success",
          text: `Episode ${entry.episode} finalization completed and saved to manuscript.`,
        },
      ]);
      // 완료 알림 (탭 비활성 시)
      import("@/lib/browser")
        .then(({ notifyTranslationComplete }) => {
  ...
  const handleGeneralTranslate = useCallback(async () => {
    ...
    setLogs([
      {
        id: Date.now(),
        type: "info",
        text: `General translation: ${generalDomain} mode, ${targetLang}...`,
      },
    ]);
    ...
    try {
      const {
        ...
        applyPassthrough,
        restorePassthrough,
        GENERAL_DOMAIN_PRESETS,
      } = await import("@/lib/translation");
      const preset =
        GENERAL_DOMAIN_PRESETS[
          generalDomain as keyof typeof GENERAL_DOMAIN_PRESETS
        ] ?? GENERAL_DOMAIN_PRESETS.general;

      // 패스스루: 수식/코드/인용 보호
      const { filtered, map } = applyPassthrough(
        generalText,
        preset.passthroughPatterns,
      );
      ...
      for (let stage = 1; stage <= 4; stage++) {
        setLogs((prev) => [
          ...prev,
          { id: Date.now(), type: "info", text: `Stage ${stage}/4...` },
        ]);
      ...
      // 세그먼트 뷰 생성 (문장 정렬)
      const { createSegments, addBatchToTM, extractTermsRuleBased } =
        await import("@/lib/translation");
      ...
      // TM에 자동 저장
      const pairs = segs
        .filter((s: { source: string; target: string }) => s.source && s.target)
        .map((s: { source: string; target: string }) => ({
          source: s.source,
          target: s.target,
        }));
      ...
      setLogs((prev) => [
        ...prev,
        {
          id: Date.now(),
          type: "success",
          text: `Translation complete. ${segs.length} segments, ${pairs.length} TM entries saved, ${candidates.length} terms detected.`,
        },
      ]);
      // 완료 알림 + 뱃지
      ...
      browser
        .cacheResponse(
          "translate",
          generalDomain,
          [{ role: "user", content: generalText.slice(0, 500) }],
          0.1,
          final,
        )
        .catch(() => {});
  ...
  const handleTranslate = useCallback(async () => {
      ...
      setLogs([
        {
          id: Date.now(),
          type: "error",
          text: "Taint Policy: novel → translation transfer blocked",
        },
      ]);
      return;
    }
    if (batchMode && manuscripts.length > 0) {
      ...
      setLogs([
        {
          id: Date.now(),
          type: "info",
          text: `Batch mode: ${manuscripts.length} episodes queued for translation...`,
        },
      ]);
      let completed = 0;
        ...
        try {
          ...
          setLogs((prev) => [
            ...prev,
            {
              id: Date.now(),
              type: "info",
              text: `EP.${ms.episode} (${completed + 1}/${manuscripts.length}) translating...`,
            },
          ]);
          ...
          setLogs((prev) => [
            ...prev,
            {
              id: Date.now(),
              type: "success",
              text: `EP.${ms.episode} complete (${completed}/${manuscripts.length})`,
            },
          ]);
        } catch (err) {
          setLogs((prev) => [
            ...prev,
            {
              id: Date.now(),
              type: "error",
              text: `EP.${ms.episode} failed: ${err}. Continuing to next...`,
            },
          ]);
          // 실패해도 다음 에피소드로 계속 진행
        }
      ...
      setLogs((prev) => [
        ...
        {
          id: Date.now(),
          type: completed === manuscripts.length ? "success" : "warn",
          text: `Batch complete: ${completed}/${manuscripts.length} succeeded`,
        },
      ]);
    } else {
      // 단일 에피소드
      ...
      setLogs([
        {
          id: Date.now(),
          type: "info",
          text: `Initialization: Parsing episode ${selectedEpisode} (${ms.charCount} chars) into syntax chunks...`,
        },
      ]);
      try {
        await translateEpisode(ms, translationConfig);
        ...
        setLogs((prev) => [
          ...prev,
          { id: Date.now(), type: "error", text: `Translation failed: ${err}` },
        ]);
      }
    }
  }, [
    ...
    <div className="mx-auto max-w-5xl px-4 py-8 space-y-6 animate-fade-in overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        ...
        >
          ...
          <ChevronRight
            className={`h-3.5 w-3.5 transition-transform duration-300 ${showAdvanced ? "rotate-90" : ""}`}
          />
        </button>
      </div>
      ...
      <div className="flex items-center gap-2 p-1 rounded-xl bg-black/30 border border-white/5 w-fit">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-mono text-[11px] font-bold uppercase tracking-wider transition-all ${scope === "novel" ? "bg-[rgba(184,149,92,0.15)] text-[rgba(228,215,190,0.95)] shadow-[inset_0_0_0_1px_rgba(184,149,92,0.3)]" : "text-text-tertiary hover:text-text-secondary"}`}
        >
          {isKO ? "소설 번역" : "Novel"}
        </button>
        <button
          onClick={() => setScope("general")}
          className={`px-4 py-2 rounded-lg font-mono text-[11px] font-bold uppercase tracking-wider transition-all ${scope === "general" ? "bg-[rgba(184,149,92,0.15)] text-[rgba(228,215,190,0.95)] shadow-[inset_0_0_0_1px_rgba(184,149,92,0.3)]" : "text-text-tertiary hover:text-text-secondary"}`}
        >
          {isKO ? "일반 번역" : "General"}
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {[
              ...
              return (
                <button
                  key={d}
                  onClick={() => setGeneralDomain(d)}
                  className={`px-3 py-2 rounded-xl font-mono text-[10px] font-bold uppercase tracking-wider border transition-all ${generalDomain === d ? "border-[rgba(184,149,92,0.4)] bg-[rgba(184,149,92,0.12)] text-[rgba(228,215,190,0.95)]" : "border-white/8 text-text-tertiary hover:border-white/15"}`}
                >
                  {labels[d] || d}
                </button>
              );
          ...
          <div className="flex gap-2 bg-black/30 p-1 rounded-xl border border-white/5 w-fit">
            {(["EN", "JP", "CN"] as const).map((l) => (
              <button
                ...
                className={`px-4 py-2 rounded-lg font-mono text-[11px] font-bold tracking-wider transition-all ${targetLang === l ? "bg-[rgba(184,149,92,0.15)] text-[rgba(228,215,190,0.95)] shadow-[inset_0_0_0_1px_rgba(184,149,92,0.3)]" : "text-text-tertiary hover:text-text-secondary"}`}
              >
                {l}
              </button>
            ))}
          ...
          <textarea
            ...
            onPaste={async (e) => {
              ...
              try {
                const { isUrl, extractTextFromUrl } =
                  await import("@/lib/web-features");
                if (isUrl(pasted)) {
                  e.preventDefault();
                  setGeneralText(
                    `(${isKO ? "URL에서 텍스트 추출 중" : "Extracting from URL"}...)`,
                  );
                  setLogs((prev) => [
                    ...prev,
                    {
                  ...
                  if (result) {
                    ...
                    setLogs((prev) => [
                      ...prev,
                      {
                        id: Date.now(),
                        type: "success",
                        text: `Extracted: "${result.title}" (${result.wordCount} words, ${result.language})`,
                      },
                    ]);
                  } else {
                    ...
                    setLogs((prev) => [
                      ...prev,
                      {
                        id: Date.now(),
                        type: "warn",
                        text: "URL extraction failed, using raw URL",
                      },
                    ]);
                  }
                }
            ...
            placeholder={
              isKO
                ? "번역할 텍스트 또는 URL을 붙여넣으세요..."
                : "Paste text or URL to translate..."
            }
          ...
          {generalResult && (
            <div className="relative rounded-xl border border-[rgba(184,149,92,0.2)] bg-black/30 p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="font-mono text-[10px] font-bold uppercase tracking-wider text-[rgba(184,149,92,0.7)]">
                  {isKO ? "번역 결과" : "Result"}
                </span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => setShowSegmentView(!showSegmentView)}
                    className="font-mono text-[10px] text-text-tertiary hover:text-text-secondary px-2 py-1 rounded border border-white/10 hover:border-white/20 transition-all"
                  >
                    {showSegmentView
                      ? isKO
                        ? "전체 보기"
                ...
                <div className="space-y-1 max-h-[400px] overflow-y-auto">
                  {segments.map((seg: TranslationSegment, idx: number) => (
                    <div key={seg.id} className="flex gap-2 group">
                      <span className="shrink-0 w-6 text-[10px] text-text-tertiary text-right pt-2">
                        {idx + 1}
                      </span>
                      <div className="flex-1 grid grid-cols-2 gap-2 rounded-lg border border-white/5 bg-white/2 p-2 hover:border-[rgba(184,149,92,0.2)] transition-colors">
                        ...
                        <input
                          ...
                          onChange={async (e) => {
                            ...
                            setSegments((prev: typeof segments) =>
                              prev.map((s: typeof seg) =>
                                s.id === seg.id ? updated : s,
                              ),
                            );
                          }}
                      ...
                      <div className="shrink-0 flex flex-col gap-0.5">
                        <button
                          ...
                          onClick={async () => {
                            const { buildPartialRetranslatePrompt } =
                              await import("@/lib/translation");
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setSegments((prev: typeof segments) =>
                              prev.map((s: typeof seg) =>
                                s.id === seg.id ? confirmSegment(s) : s,
                              ),
                            );
                          }}
          ...
          {glossaryCandidates.length > 0 && (
            <details className="rounded-xl border border-white/8 bg-black/20 p-3">
              <summary className="font-mono text-[10px] font-bold uppercase tracking-wider text-text-tertiary cursor-pointer">
                {isKO
                  ? `추출된 용어 (${glossaryCandidates.length}개)`
                  : `Extracted terms (${glossaryCandidates.length})`}
              </summary>
              <div className="mt-2 flex flex-wrap gap-1.5">
                {glossaryCandidates.map(
                  (
                    c: { term: string; type: string; confidence: number },
                    i: number,
                  ) => (
                    <button
                      key={i}
                      onClick={() => {
                        setGlossaryTerm(c.term);
                      }}
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-[rgba(184,149,92,0.08)] border border-[rgba(184,149,92,0.15)] font-mono text-[10px] text-[rgba(228,215,190,0.9)] hover:bg-[rgba(184,149,92,0.15)] transition-colors"
                    >
                      {c.term}{" "}
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button
                onClick={async () => {
                  ...
                  a.href = url;
                  a.download = `translation_${targetLang}.xlf`;
                  a.click();
                  URL.revokeObjectURL(url);
                }}
                className="font-mono text-[10px] px-2 py-1 rounded border border-white/10 text-text-tertiary hover:text-text-secondary hover:border-white/20 transition-all"
              >
                XLIFF
              </button>
              {/* TMX */}
              ...
              <button
                onClick={() => {
                  const blob = new Blob([generalResult], {
                    type: "text/plain",
                  });
                  const url = URL.createObjectURL(blob);
                  const a = document.createElement("a");
        ...
        <>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <span
                        className={`flex h-2 w-2 rounded-full ${isSelected ? "bg-[rgba(184,149,92,0.9)] shadow-[0_0_8px_rgba(184,149,92,0.8)]" : "bg-white/20"}`}
                      />
                      <div
                        className={`font-mono text-[12px] font-bold uppercase tracking-[0.15em] ${isSelected ? "text-[rgba(200,225,255,0.95)]" : "text-text-secondary"}`}
                      >
                        {info.title}
                      </div>
                    </div>
            ...
            <div className="flex-1 w-full space-y-3 relative z-10">
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 font-mono text-[10px] font-semibold uppercase tracking-[0.15em] text-text-tertiary">
                  ...
                  {isKO
                    ? "번역 밴드 (정확도 ↔ 자연스러움)"
                    : "Translation Band (Accuracy ↔ Naturalness)"}
                </label>
              ...
              <div className="relative pt-1">
                ...
                <div
                  ...
                  style={{
                    width: `${((band - BAND_META.min) / (BAND_META.max - BAND_META.min)) * 100}%`,
                  }}
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 p-5 rounded-[1.25rem] border border-accent-amber/20 bg-bg-secondary animate-in slide-in-from-top-2 duration-300">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) =>
                      setScoreThreshold(parseFloat(e.target.value) || 0.75)
                    }
                    className="w-full rounded-xl border border-white/10 bg-bg-tertiary px-3 py-2.5 font-mono text-[11px] text-[rgba(235,230,215,0.9)] outline-none focus:border-[rgba(184,149,92,0.5)] focus:ring-1 focus:ring-[rgba(184,149,92,0.3)] transition-all"
                  />
                </div>
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setContractionLevel(
                      e.target.value as typeof contractionLevel,
                    )
                  }
          ...
          <details className="rounded-[1.25rem] border border-white/8 bg-black/20 backdrop-blur-md">
            <summary className="flex items-center gap-3 px-5 py-3 cursor-pointer font-mono text-[11px] font-bold uppercase tracking-[0.15em] text-text-tertiary hover:text-text-secondary transition-colors">
              ...
              {isKO
                ? `용어집 (${Object.keys(glossary).length}개)`
            ...
            <div className="px-5 pb-4 space-y-3">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={isKO ? "원문 용어" : "Source term"}
                  className="flex-1 rounded-lg border border-white/10 bg-bg-tertiary px-3 py-2 font-mono text-[11px] text-text-primary outline-none"
                />
                <input
                  value={glossaryTranslation}
                  onChange={(e) => setGlossaryTranslation(e.target.value)}
                  placeholder={isKO ? "번역" : "Translation"}
                  className="flex-1 rounded-lg border border-white/10 bg-bg-tertiary px-3 py-2 font-mono text-[11px] text-text-primary outline-none"
                />
                <button
                  onClick={() => {
                    if (glossaryTerm.trim() && glossaryTranslation.trim()) {
                      saveGlossary({
                        ...glossary,
                        [glossaryTerm.trim()]: glossaryTranslation.trim(),
                      });
                      setGlossaryTerm("");
                      setGlossaryTranslation("");
                    }
                  }}
                  className="px-3 py-2 rounded-lg bg-[rgba(184,149,92,0.15)] text-[rgba(228,215,190,0.95)] font-mono text-[10px] font-bold hover:bg-[rgba(184,149,92,0.25)] transition-colors"
                >
                  +
                </button>
              </div>
              {Object.entries(glossary).length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                  {Object.entries(glossary).map(([k, v]) => (
                    <span
                      key={k}
                      className="inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-[rgba(184,149,92,0.1)] border border-[rgba(184,149,92,0.2)] font-mono text-[10px] text-[rgba(228,215,190,0.9)]"
                    >
                      {k} → {v}
                      <button
                        onClick={() => {
                        ...
                        className="ml-1 text-[rgba(255,100,100,0.6)] hover:text-[rgba(255,100,100,1)]"
                      >
                        &times;
                      </button>
                    </span>
          ...
          <div className="flex flex-col sm:flex-row items-end gap-4 p-5 rounded-[1.25rem] border border-white/8 bg-black/20 backdrop-blur-md">
            <div className="flex-1 w-full space-y-2">
              <div className="flex items-center gap-3">
                ...
                <button
                  ...
                  className={`font-mono text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-md border transition-all ${batchMode ? "border-[rgba(184,149,92,0.4)] bg-[rgba(184,149,92,0.15)] text-[rgba(228,215,190,0.95)]" : "border-white/10 text-text-tertiary hover:border-white/20"}`}
                >
                  {isKO
                    ? batchMode
                      ? "배치 ON"
              ...
              <div className="relative group">
                ...
                >
                  <option value="">
                    {isKO
                      ? "/// 번역 대기 큐에서 선택 ///"
                      : "/// Select queued episode ///"}
                  </option>
                  {manuscripts.map((m) => (
                    <option key={m.episode} value={m.episode}>
                      EP {m.episode} — {m.title || `Episode ${m.episode}`} [{" "}
                      {m.content.length.toLocaleString()} BYTE ]
                    </option>
                  ))}
                </select>
            ...
            {isTranslating ? (
              ...
              <button
                ...
                disabled={
                  batchMode
                    ...
                    : selectedEpisode === null || manuscripts.length === 0
                }
              ...
              >
                ...
                {batchMode
                  ? isKO
                    ? `배치 번역 (${manuscripts.length}화)`
                    : `BATCH (${manuscripts.length})`
                  : isKO
                    ? "번역 연결 (INIT)"
                    : "INIT"}
              </button>
          ...
          {(progress.status !== "idle" || logs.length > 0) && (
            <div className="rounded-[1.25rem] border border-[rgba(184,149,92,0.2)] bg-bg-secondary overflow-hidden flex flex-col h-[320px] shadow-[0_10px_40px_rgba(0,0,0,0.5),inset_0_0_20px_rgba(184,149,92,0.03)] relative">
              ...
              <div className="flex items-center justify-between px-5 py-3 border-b border-[rgba(184,149,92,0.15)] bg-[rgba(184,149,92,0.05)] relative z-20">
                <div
                  className={`flex items-center gap-3 font-mono text-[11px] font-bold uppercase tracking-[0.2em] ${progress.status === "idle" ? "text-[rgba(184,149,92,0.5)]" : "text-[rgba(235,220,190,0.9)]"}`}
                >
                  {(progress.status === "translating" ||
                    progress.status === "scoring") && (
                    <Loader2 className="h-3 w-3 animate-spin text-[rgba(184,149,92,0.8)]" />
                  )}
                  {progress.status === "recreating" && (
                  ...
                  {progress.recreateCount > 0 && (
                    <span className="text-accent-amber bg-accent-amber/10 px-1.5 rounded">
                      RETRY:{progress.recreateCount}
                    </span>
                  )}
                </div>
                <div className="flex items-center gap-4">
                  <span className="font-mono text-[10px] font-bold tracking-widest text-[rgba(184,149,92,0.6)]">
                    BLK[{progress.completedChunks.toString().padStart(3, "0")}/
                    {progress.totalChunks.toString().padStart(3, "0")}]
                  </span>
                  <div className="w-32 h-1.5 bg-black/60 rounded-full overflow-hidden border border-white/5 relative">
                    <div
                      ...
                      style={{
                        width: `${progress.totalChunks > 0 ? (progress.completedChunks / progress.totalChunks) * 100 : 0}%`,
                      }}
                    />
                  </div>
                </div>
                ...
                <div className="space-y-3">
                  {logs.map((log) => (
                    <div key={log.id} className="flex gap-4 items-start group">
                      <span className="text-[rgba(184,149,92,0.4)] shrink-0 font-bold group-hover:text-[rgba(184,149,92,0.6)] transition-colors">
                        [{new Date(log.id).toISOString().substring(11, 23)}]
                      </span>
                      <span
                        className={`shrink-0 w-12 text-center uppercase text-[9px] font-black tracking-widest py-0.5 rounded-sm border ${
                          log.type === "error"
                            ? "text-[#ff6b6b] border-[#ff6b6b]/30 bg-[#ff6b6b]/10"
                            : log.type === "success"
                              ? "text-[#38d9a9] border-[#38d9a9]/30 bg-[#38d9a9]/10"
                              : log.type === "warn"
                                ? "text-[#fcc419] border-[#fcc419]/30 bg-[#fcc419]/10"
                  ...
                  {progress.status !== "idle" && progress.status !== "done" && (
                    <div className="flex gap-4 animate-pulse">
                      <span className="text-[rgba(184,149,92,0.4)] shrink-0 font-bold">
              ...
              <style jsx>{`
                .custom-scrollbar-archive::-webkit-scrollbar {
                  width: 6px;
                }
                .custom-scrollbar-archive::-webkit-scrollbar-track {
                  background: transparent;
                }
                .custom-scrollbar-archive::-webkit-scrollbar-thumb {
                  background: rgba(184, 149, 92, 0.2);
                  border-radius: 10px;
                }
                .custom-scrollbar-archive::-webkit-scrollbar-thumb:hover {
                  background: rgba(184, 149, 92, 0.4);
                }
              `}</style>
            </div>
          )}
            ...
            <div className="pt-6 border-t border-[rgba(184,149,92,0.15)] space-y-4">
              <div className="flex items-center gap-3 text-[rgba(235,220,190,0.8)]">
                ...
                <h3 className="font-mono text-[13px] font-bold uppercase tracking-[0.2em]">
                  {isKO ? "현지화 완료 데이터베이스" : "Localized Database"}{" "}
                  <span className="text-[rgba(184,149,92,0.5)]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {translatedList.map((t, i) => (
                  <div
                    key={i}
                    className="group relative flex flex-col gap-2.5 rounded-2xl border border-[rgba(184,149,92,0.15)] bg-bg-tertiary backdrop-blur-md p-4 transition-all duration-300 hover:border-[rgba(184,149,92,0.4)] hover:bg-[rgba(184,149,92,0.05)] hover:-translate-y-1 shadow-lg hover:shadow-[0_10px_30px_rgba(184,149,92,0.1)]"
                  >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="font-mono text-[13px] font-bold text-white/90 truncate">
                        EP_{t.episode.toString().padStart(2, "0")} {"//"}{" "}
                        {t.translatedTitle || `Episode ${t.episode}`}
                      </div>
                    </div>
                    <div className="flex items-center gap-2 font-mono text-[10px] text-[rgba(235,220,190,0.7)] ml-11">
                      <span className="bg-bg-tertiary border border-white/10 px-2 py-1 rounded-[0.5rem] tracking-wider font-bold shadow-inner">
                        {t.sourceLang}{" "}
                        <span className="text-[rgba(184,149,92,0.5)]">→</span>{" "}
                        {t.targetLang}
                      </span>
                      ...
                      <span className="text-emerald-400 font-bold ml-auto pr-1">
                        Q_GATE: {(t.avgScore * 100).toFixed(1)}%
                      </span>
                    </div>
                  </div>
                ))}
          ...
          {manuscripts.length === 0 && (
            <div className="flex flex-col items-center justify-center rounded-[1.25rem] border border-dashed border-[rgba(184,149,92,0.3)] bg-black/20 backdrop-blur-md p-12 text-center shadow-[inset_0_0_50px_rgba(0,0,0,0.5)]">
              ...
              <p className="font-mono text-[12px] tracking-widest text-[rgba(235,220,190,0.7)]">
                {isKO
                  ? "대기열(Queue)이 비어 있습니다. 집필 모드에서 에피소드를 작성하십시오."
                  : "Translation Queue is empty. Write episodes in Zenith Canvas."}
              </p>
            </div>
          )}
        </>
