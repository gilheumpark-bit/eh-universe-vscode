...
const SUB_TAB_ORDER: WorldSubTab[] = [
  "design",
  "simulator",
  "analysis",
  "timeline",
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-10 pt-6">
        <div
          className="relative flex flex-wrap items-center gap-2 p-2 bg-[linear-gradient(to_bottom,rgba(255,200,50,0.05),rgba(0,0,0,0.4))] backdrop-blur-xl border border-[rgba(255,200,50,0.2)] rounded-2xl shadow-[0_10px_30px_rgba(0,0,0,0.5),inset_0_0_20px_rgba(255,200,50,0.02)] overflow-hidden"
          role="tablist"
        >
          ...
          {SUB_TAB_ORDER.map((tab) => {
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  const sc = document.querySelector(
                    '[data-testid="studio-content"] .overflow-y-auto',
                  );
              ...
              >
                <Icon
                  className={`w-4 h-4 transition-transform duration-300 ${active ? "scale-110 drop-shadow-[0_0_8px_rgba(255,200,50,0.8)]" : "group-hover:scale-110 group-hover:drop-shadow-[0_0_5px_rgba(255,200,50,0.4)]"}`}
                />
                <span
                  className={
                    active ? "drop-shadow-[0_0_10px_rgba(255,200,50,0.5)]" : ""
                  }
                >
                  {labels[tab]}
                </span>
      ...
      {subTab === "design" && (
        <>
          ...
          {config.worldSimData?._latestUpdates &&
            config.worldSimData._latestUpdates.length > 0 && (
                ...
                <div className="flex flex-col gap-2 p-4 bg-[linear-gradient(to_right,rgba(255,200,50,0.05),transparent)] border-l-4 border-l-[rgba(255,200,50,0.8)] border border-y-[rgba(255,200,50,0.2)] border-r-[rgba(255,200,50,0.2)] rounded-r-xl rounded-l-sm backdrop-blur-sm shadow-[0_5px_15px_rgba(0,0,0,0.2)]">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[11px] font-bold text-[rgba(255,200,50,0.9)] uppercase tracking-widest font-mono">
                      {language === "KO"
                        ? "AI 스튜디오 궤도 동기화 (ORBITAL SYNC)"
                        : "A.I. Orbital Sync Completed"}
                    </span>
                  </div>
                  <ul className="list-disc list-inside text-[11px] text-[rgba(220,200,150,0.8)] font-mono space-y-1 ml-9">
                    {config.worldSimData._latestUpdates.map(
                      (update: string, i: number) => (
                        <li key={i}>{update}</li>
                      ),
                    )}
                  </ul>
                  <div className="text-[9px] text-[rgba(255,200,50,0.4)] mt-1 ml-9 font-mono tracking-widest">
                    {language === "KO"
                      ? "에피소드 데이터 변경분이 항성계 데이터베이스(World Database)에 머지되었습니다."
                      : "Story modifications merged into Atlas Database."}
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-10 pt-3">
              <div className="flex items-center gap-3 px-5 py-4 bg-[linear-gradient(45deg,rgba(255,200,50,0.05),transparent)] border border-[rgba(255,200,50,0.2)] rounded-xl shadow-[0_0_20px_rgba(255,200,50,0.05)]">
                ...
                <span className="text-xs text-[rgba(220,200,150,0.9)] font-mono tracking-wide">
                  {language === "EN"
                    ? "Enter a title and synopsis below — AI will forge your Universe"
                    : language === "JP"
                      ? "タイトルとシノプシスを入力すると、AIが世界を設計します"
                      : language === "CN"
                        ? "输入标题和大纲，AI将设计您的世界"
                        : "아래에 제목과 시놉시스를 입력하면 AI 컨스트럭트가 우주를 설계합니다"}
                </span>
              </div>
            </div>
          )}
          <PlanningView
            ...
            hasAiAccess={
              ...
              Object.values(hostedProviders).some(Boolean)
            }
          />
          ...
          <div className="max-w-6xl mx-auto px-4 pb-8 flex gap-3 justify-end">
            ...
            {config.title || config.synopsis ? (
              <>
                <button
                  ...
                  className="group flex items-center justify-center gap-2 px-6 py-3 rounded-xl border border-accent-amber/30 bg-bg-secondary/60 text-[12px] font-black uppercase tracking-widest font-mono transition-all hover:bg-accent-amber/10 hover:border-accent-amber/60 hover:shadow-[0_0_20px_rgba(255,200,50,0.15)] text-accent-amber hover:text-text-primary"
                >
                  <Cpu className="w-4 h-4 group-hover:text-[rgba(255,200,50,0.9)] transition-colors" />{" "}
                  {language === "EN"
                    ? "ATLAS SIMULATOR"
                    : language === "JP"
                      ? "シミュレーター"
                ...
                >
                  <Compass className="w-4 h-4 drop-shadow-[0_0_5px_rgba(255,255,255,0.8)]" />{" "}
                  {startLabel ?? t("planning.commence")}
                </button>
              </>
            ) : null}
            <button
            ...
            >
              {saveFlash ? (
                ...
                <div className="w-1.5 h-1.5 rounded-full bg-[rgba(255,220,100,0.9)] shadow-[0_0_8px_rgba(255,200,50,1)]" />
              )}
              <span>
                {saveFlash
                  ? t("worldStudio.saved")
                  : t("worldStudio.saveSettings")}
              </span>
            </button>
          </div>
        </>
