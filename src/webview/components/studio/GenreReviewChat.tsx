...
const AspectBar: React.FC<{ aspect: AspectResult; lang: "ko" | "en" }> = ({
  aspect,
  lang,
}) => {
  ...
  return (
    <div
      className={`${style.bg} border ${style.border} rounded-lg p-3 space-y-1.5`}
    >
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-bold">{aspect.label[lang]}</span>
        <span className={`text-[10px] font-bold ${style.text}`}>
          {aspect.value}
          {aspect.unit === "%" ? "%" : ` ${aspect.unit}`}
        </span>
      </div>
      {/* Benchmark bar */}
      <div className="relative h-3 bg-bg-primary rounded-full overflow-visible">
        ...
        {/* Benchmark labels */}
        <span
          className="absolute text-[7px] text-text-tertiary"
          style={{ left: `${bmLeft}%`, top: "-12px" }}
        >
          {bmLeft}
        </span>
        <span
          className="absolute text-[7px] text-text-tertiary"
          style={{ left: `${bmRight}%`, top: "-12px" }}
        >
          {bmRight}
        </span>
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-3">
        <span
          className={`text-3xl font-black ${GRADE_COLOR[review.overallGrade] ?? "text-text-primary"}`}
        >
          {review.overallGrade}
        </span>
        <div>
          ...
          <p className="text-[10px] text-text-tertiary">
            {genreLabel} · {review.levelMeta.desc[lang]}
          </p>
        </div>
      </div>
      ...
      <button
        ...
        className="flex items-center gap-1 text-[10px] text-accent-purple font-bold"
      >
        <ChevronDown
        ...
        {(() => {
          ...
          return expanded
            ? tw("genreReview.collapseDetail")
            : tw("genreReview.expandDetail");
        })()}
      </button>
...
const GenreReviewChat: React.FC<GenreReviewChatProps> = ({
  language,
  config,
  manuscriptText,
}) => {
  ...
  const requestReview = () => {
    ...
    const levelMeta =
      READER_LEVELS.find((l) => l.level === selectedLevel) ?? READER_LEVELS[0];
  ...
  const runAllLevels = () => {
    ...
    setMessages((prev) => [
      ...prev,
      {
        ...
        role: "user",
        content: {
          KO: `${genreLabel} 장르 / 전체 레벨 (Lv.1~4) 풀 리뷰`,
          EN: `Full review: ${genreLabel} / All levels (Lv.1~4)`,
          JP: `${genreLabel}ジャンル / 全レベル (Lv.1~4) フルレビュー`,
          CN: `${genreLabel}类型 / 全等级 (Lv.1~4) 完整审阅`,
        }[language],
        timestamp: Date.now(),
      },
    ]);
    ...
    setTimeout(() => {
      const reviews: GenreLevelReview[] = ([1, 2, 3, 4] as ReaderLevel[]).map(
        (lv) => runGenreLevelReview(manuscriptText, genre, lv),
      );
    ...
    <div className="flex flex-col h-[600px] bg-bg-primary border border-border rounded-2xl overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-bg-secondary">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] text-text-tertiary px-2 py-0.5 bg-bg-primary rounded-full">
            {genreLabel}
          </span>
        </div>
      </div>
