...
const EngineDashboard: React.FC<EngineDashboardProps> = ({
  config,
  report,
  isGenerating,
  language,
    ...
    <div className="h-full bg-bg-primary border-l border-border flex flex-col w-80 text-xs font-mono overflow-y-auto custom-scrollbar">
      <div className="p-6 border-b border-border bg-bg-secondary/30">
        ...
        <div className="flex items-center gap-2 mt-3">
          <div
            className={`w-2 h-2 rounded-full ${isGenerating ? "bg-blue-500 animate-ping" : "bg-bg-tertiary"}`}
          />
                  ...
                  <div
                    className={`absolute bottom-0 w-full rounded-t-sm transition-all duration-300 ${
                      isCurrentEp
                        ...
                        : "bg-gradient-to-t from-blue-600/60 to-indigo-400/40"
                    }`}
                    style={{ height: `${height}%` }}
                  />
                  {isCurrentEp && (
            ...
            <div className="bg-bg-secondary/50 rounded-2xl p-4 border border-border/50 space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 text-center p-3 bg-black/40 rounded-xl">
                  ...
                  <div
                    className={`text-lg font-black ${report.eosScore >= 40 ? "text-green-400" : "text-red-400"}`}
                  >
                    {report.eosScore}
                  </div>
                </div>
              ...
              <div className="space-y-1">
                ...
                <div
                  className="h-1 bg-bg-tertiary rounded-full overflow-hidden relative"
                  role="progressbar"
                  aria-valuenow={Math.round(
                    Math.min(
                      100,
                      (report.serialization.byteSize /
                        report.serialization.targetRange.max) *
                        100,
                    ),
                  )}
                  aria-valuemin={0}
                  aria-valuemax={100}
                  aria-label={`Byte size ${(report.serialization.byteSize / 1024).toFixed(1)}KB`}
                >
                  <div
                    className={`h-full transition-all duration-500 ${report.serialization.withinRange ? "bg-green-600" : "bg-amber-600"}`}
                    style={{
                      width: `${Math.min(100, (report.serialization.byteSize / report.serialization.targetRange.max) * 100)}%`,
                    }}
                  />
                </div>
                <div className="flex justify-between text-[7px] text-text-tertiary">
                  <span>
                    {(report.serialization.targetRange.min / 1024).toFixed(1)}KB
                  </span>
                  <span>
                    {(report.serialization.targetRange.max / 1024).toFixed(1)}KB
                  </span>
                </div>
              </div>
              ...
              {(() => {
                ...
                const charInRange =
                  chars >= charRange.min && chars <= charRange.max;
                return (
                  <div className="space-y-1">
                    <div className="flex justify-between text-[10px] font-black text-text-tertiary uppercase">
                      <span>CHARS</span>
                      <span
                        className={
                          charInRange ? "text-green-500" : "text-amber-500"
                        }
                      >
                    ...
                    <div
                      ...
                      role="progressbar"
                      aria-valuenow={Math.round(
                        Math.min(100, (chars / charRange.max) * 100),
                      )}
                      aria-valuemin={0}
                      aria-valuemax={100}
                      aria-label={`Character count ${chars.toLocaleString()}`}
                    >
                      <div
                        className={`h-full transition-all duration-500 ${charInRange ? "bg-green-600" : "bg-amber-600"}`}
                        style={{
                          width: `${Math.min(100, (chars / charRange.max) * 100)}%`,
                        }}
                      />
                    </div>
                    <div className="flex justify-between text-[7px] text-text-tertiary">
              ...
              <div className="flex justify-between items-center">
                ...
                <span
                  className={`text-[9px] font-black ${report.aiTonePercent <= 10 ? "text-green-500" : report.aiTonePercent <= 30 ? "text-amber-500" : "text-red-500"}`}
                >
