    ...
    <details className="group" open={report.findings.length > 0}>
      ...
      <div className="mt-2 pl-1 space-y-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[9px] font-black ${gradeColor}`}>
            {report.score}
          </span>
        </div>
            ...
            {findingsToShow.map((f, i) => (
              <div key={i} className="flex items-start gap-1.5">
                ...
                <div className="min-w-0">
                  <div
                    className={`text-[10px] font-bold ${SEV_COLORS[f.severity] || "text-text-tertiary"}`}
                  >
                    {KIND_LABELS[f.kind]?.[language] || f.kind}
                    {f.lineNo ? ` L${f.lineNo}` : ""}
                  </div>
                  <div className="text-[9px] text-text-tertiary break-words">
                    {f.message}
                  </div>
                  {f.excerpt && (
                    <div className="text-[9px] text-text-tertiary italic truncate">
                      &quot;{f.excerpt}&quot;
                    </div>
                  )}
                </div>
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1 pt-1 border-t border-border/50 text-[10px] text-text-tertiary font-mono">
          {report.stats.ending_mono > 0 && (
            <span>
              {{ KO: "종결", EN: "End", JP: "語尾", CN: "结尾" }[language]}
              {report.stats.ending_mono}%
            </span>
          )}
          {report.stats.blur > 0 && (
            <span>
              {{ KO: "흐림", EN: "Blur", JP: "ぼかし", CN: "模糊" }[language]}
              {report.stats.blur}
            </span>
          )}
          {report.stats.gain_no_cost > 0 && (
            <span>
              {
                { KO: "무대가", EN: "NoCost", JP: "無対価", CN: "无代价" }[
                  language
                ]
              }
              {report.stats.gain_no_cost}
            </span>
          )}
          {report.stats.ai_tone > 0 && (
            <span>
              {{ KO: "AI톤", EN: "AI", JP: "AI", CN: "AI" }[language]}
              {report.stats.ai_tone}
            </span>
          )}
          {report.stats.typo > 0 && (
            <span>
              {{ KO: "오타", EN: "Typo", JP: "誤字", CN: "错字" }[language]}
              {report.stats.typo}
            </span>
          )}
          {report.stats.similar_context > 0 && (
            <span>
              {{ KO: "반복", EN: "Repeat", JP: "繰返", CN: "重复" }[language]}
              {report.stats.similar_context}
            </span>
          )}
        </div>
      </div>
