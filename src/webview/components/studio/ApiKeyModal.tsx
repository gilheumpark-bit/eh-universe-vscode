...
const ApiKeyModal: React.FC<ApiKeyModalProps> = ({
  ...
  const [keys, setKeys] = useState<Record<ProviderId, string>>(() => {
    ...
    for (const p of PROVIDER_LIST_UI) {
      ...
      loaded[pid] = getApiKey(pid);
    }
    return loaded;
  });
  const [selectedModel, setSelectedModel] = useState(getActiveModel());
  const [testStatus, setTestStatus] = useState<
    Record<ProviderId, "idle" | "testing" | "success" | "error">
  >(() => {
    const s = {} as Record<
      ProviderId,
      ...
      >
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 id="api-key-modal-title" className="font-black text-base">
                {t("apiKeyModal.title")}
              </h3>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            >
              ...
              <ChevronDown
                className={`w-4 h-4 text-text-tertiary transition-transform ${showModels ? "rotate-180" : ""}`}
              />
            </button>
            {showModels && (
                  ...
                  >
                    ...
                    {m === currentProvider.defaultModel && (
                      <span className="ml-2 text-[10px] text-text-tertiary uppercase">
                        {t("apiKeyModal.defaultModel")}
                      </span>
                    )}
                    {isPreviewModel(m) && (
                      <span className="ml-2 text-[10px] text-accent-amber uppercase">
                        ⚠ Preview
                      </span>
                    )}
                  </button>
        ...
        <div className="flex gap-2">
          <button
            ...
            onClick={handleTest}
            disabled={
              (!currentKey.trim() && !hasStoredApiKey(activeId)) ||
              status === "testing"
            }
              ...
              >
                <div className="flex items-center gap-2">
                  <span
                    ...
                    style={{
                      background: hasKey ? p.color : "var(--color-border)",
                    }}
                  />
                  <span
                    className={
