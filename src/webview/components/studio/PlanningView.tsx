    ...
    <div className="space-y-2">
        ...
        {subGenres.length > 0 && (
          <label className="flex items-center gap-1.5 cursor-pointer">
            ...
            <span className="text-[10px] text-text-tertiary">
              {isKO ? "AI 프롬프트에 반영" : "Apply to AI prompt"}
            </span>
          </label>
        )}
      </div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {subGenres.map((tag) => (
          <span
            key={tag}
            className="inline-flex items-center gap-1 px-2.5 py-1 bg-accent-amber/10 border border-accent-amber/20 rounded-full text-[11px] font-bold text-accent-amber"
          >
            #{tag}
            <button
              onClick={() => onChange(subGenres.filter((t) => t !== tag))}
              className="text-accent-amber/50 hover:text-accent-amber text-xs"
            >
              &times;
            </button>
          </span>
        ...
        {subGenres.length === 0 && (
          <span className="text-[11px] text-text-tertiary italic">
            {isKO
              ? "태그를 추가하면 AI 설계에 반영됩니다"
              : "Tags will be injected into AI prompts"}
          </span>
        )}
      </div>
      {/* Input */}
...
const PlanningView: React.FC<PlanningViewProps> = ({
  ...
  const tensionData = useMemo(
    () => generateTensionCurveData(totalEpisodes, config.genre),
    [totalEpisodes, config.genre],
  );
  ...
  const [advancedMode, setAdvancedMode] = useState(() => {
    if (typeof window !== "undefined")
      return localStorage.getItem("noa_planning_advanced") === "true";
    return false;
  });
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      setConfig((prev: StoryConfig) => ({
        ...
        survivalEnvironment:
          result.survivalEnvironment || prev.survivalEnvironment,
        // Tier 3
      ...
      if (msg.includes("401") || msg.includes("credentials")) {
        ...
        showAlert(
          tl("planningExtra.aiFailed") + (msg ? ` (${msg.slice(0, 80)})` : ""),
        );
      }
    } finally {
    ...
    <div className="max-w-6xl mx-auto p-4 sm:p-6 md:px-10 md:pt-4 md:pb-10 space-y-8 animate-in fade-in duration-700 pb-32">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <select
            value={autoGenGenre}
            onChange={(e) => setAutoGenGenre(e.target.value as Genre)}
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-accent-amber/10 text-accent-amber border border-accent-amber/40 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-accent-amber/20 transition-all active:scale-95 shadow-[0_0_10px_rgba(255,200,50,0.1)]"
            >
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="text-[9px] mt-0.5 opacity-70">
                        {data.pov} · {data.emotion}
                      </div>
                    </button>
                  );
                })}
                {(!AUTO_PRESETS[autoGenGenre] ||
                  AUTO_PRESETS[autoGenGenre].length === 0) && (
                  <div className="px-4 py-3 text-[11px] text-text-tertiary">
                    {tl("planningExtra.noPreset")}
                  </div>
                )}
              </div>
            )}
          ...
          >
            ...
            {aiGenerating
              ? tl("planningExtra.aiGenerating")
              : tl("planningExtra.aiGenerate")}
          </button>
        </div>
      </div>
      ...
      <div className="ds-card-lg space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className="space-y-2">
            ...
            <select
              ...
              value={config.genre}
              onChange={(e) =>
                setConfig({ ...config, genre: e.target.value as Genre })
              }
            >
              {Object.values(Genre).map((g) => (
                <option key={g} value={g}>
        ...
        {advancedMode && (
          <SubGenreTagInput
            ...
            onTogglePrompt={(val) =>
              setConfig({ ...config, useSubGenrePrompt: val })
            }
          />
        )}
            ...
            {(totalEpisodes < 1 || totalEpisodes > 500) && (
              <p className="text-[10px] font-bold text-red-400 px-1">
                {totalEpisodes < 1
                  ...
                  : language === "KO"
                    ? "500화 초과는 시스템 부하를 유발할 수 있습니다."
                    : "Over 500 episodes may cause performance issues."}
              </p>
            )}
          </div>
          <div className="space-y-2">
            ...
            <div className="flex gap-3">
              <button
                onClick={() =>
                  setConfig({ ...config, platform: PlatformType.MOBILE })
                }
              ...
              <button
                onClick={() =>
                  setConfig({ ...config, platform: PlatformType.WEB })
                }
                className={`flex-1 flex items-center justify-center gap-2 py-4 rounded-xl border text-xs font-black uppercase tracking-widest transition-all ${
      ...
      {!advancedMode && (
        <div className="flex flex-wrap gap-2 items-center">
          ...
          {(["OFF", "FREE", "ALL", "T15", "M18"] as const).map((mode) => (
            <button
              key={mode}
              onClick={() => setConfig({ ...config, prismMode: mode })}
      ...
      <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
        ...
        <button
          onClick={() => {
            ...
            const encoded = btoa(
              unescape(encodeURIComponent(JSON.stringify(worldPayload))),
            );
            ...
            navigator.clipboard
              ...
              .catch(() => {
                ...
                window.prompt(
                  language === "KO"
                    ? "클립보드 접근이 거부됐습니다. 아래 링크를 직접 복사하세요:"
                    : "Clipboard access denied. Copy the link below:",
                  url,
                );
              });
          }}
        ...
        <button
          onClick={() => {
            ...
            const encoded = btoa(
              unescape(encodeURIComponent(JSON.stringify(planetPayload))),
            );
