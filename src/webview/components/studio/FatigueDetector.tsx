...
function extractSnapshots(messages: Message[]): MetricSnapshot[] {
  ...
  for (const msg of messages) {
    ...
    const dialogueRatio =
      content.length > 0
        ? Math.round((dialogueChars / content.length) * 100)
        : 0;
...
function detectFatigue(
  ...
  for (let i = 0; i <= snapshots.length - 3; i++) {
    ...
    if (
      tensionBand[0] === tensionBand[1] &&
      tensionBand[1] === tensionBand[2]
    ) {
      alerts.push({
        id: `tension-flat-${i}`,
        level: "warning",
  ...
  for (let i = 0; i <= snapshots.length - 3; i++) {
    ...
    if (
      window[0].grade !== "—" &&
      window[0].grade === window[1].grade &&
      window[1].grade === window[2].grade
    ) {
  ...
  if (snapshots.length >= 4) {
    ...
    const maxDev = Math.max(
      ...lengths.map((l) => Math.abs(l - avgLen) / avgLen),
    );
  ...
  if (snapshots.length >= 4) {
    ...
    const declining = last4.every(
      (s, i) => i === 0 || s.pacing <= last4[i - 1].pacing,
    );
  ...
  if (snapshots.length < 3) {
    return (
      <div className="text-center py-4 text-text-tertiary text-[10px]">
        {isKO
          ? "피로도 분석에는 최소 3개 챕터가 필요합니다."
          : "Need at least 3 chapters for fatigue analysis."}
      </div>
    );
  }
    ...
    return (
      <div className="bg-bg-secondary border border-border rounded-xl p-4">
        ...
        <div className="text-[10px] text-green-400">
          ✓{" "}
          {isKO
            ? "반복 패턴이 감지되지 않았습니다. 건강한 흐름입니다."
            : "No repetitive patterns detected. Healthy flow."}
        </div>
      </div>
    );
  }
