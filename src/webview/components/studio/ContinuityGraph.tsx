...
} from "@/engine/continuity-tracker";
import type {
  AppLanguage,
  StoryConfig,
  EpisodeManuscript,
      ...
      {episodes.map((ep) => {
        ...
        return (
          ...
          >
            ...
            />
            <span className="text-[7px] text-text-tertiary mt-0.5">
              {ep.episode}
            </span>
...
const WarningList: React.FC<{ ep: EpisodeSnapshot; lang: "ko" | "en" }> = ({
  ep,
  lang,
}) => {
...
const CharPresenceRow: React.FC<{ ep: EpisodeSnapshot; lang: "ko" | "en" }> = ({
  ep,
  lang: _lang,
}) => {
  ...
  return (
    <div className="flex flex-wrap gap-1">
      {activeChars.map((c) => (
        ...
        >
          {c.name}
          {c.stateFlags.includes("부상") && (
            <span className="ml-0.5 text-red-400">🩹</span>
          )}
          {c.stateFlags.includes("사망") && (
            <span className="ml-0.5 text-red-500">💀</span>
          )}
          {c.stateFlags.includes("분노") && (
            <span className="ml-0.5 text-amber-400">🔥</span>
          )}
          {c.stateFlags.includes("슬픔") && (
...
const ContinuityGraph: React.FC<ContinuityGraphProps> = ({
  language,
  config,
}) => {
  const lang: "ko" | "en" =
    language === "KO" || language === "JP" ? "ko" : "en";
  ...
  const selectedEp =
    report.episodes.find((e) => e.episode === selectedEpNum) ?? null;
            ...
            <div className="bg-bg-primary border border-border rounded-lg p-3 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span
                  ...
                  style={{ color: SCORE_COLOR(selectedEp.continuityScore) }}
                >
                  {selectedEp.continuityScore}
                  {t("continuity.point")}
                </span>
              </div>
