...
const RARITY_CONFIG: Record<
  ItemRarity,
  { tKey: string; color: string; bg: string }
> = {
  common: {
    tKey: "itemStudio.rarityCommon",
    color: "#9ca3af",
    bg: "bg-gray-500/10",
  },
  uncommon: {
    tKey: "itemStudio.rarityUncommon",
    color: "#22c55e",
    bg: "bg-green-500/10",
  },
  rare: {
    tKey: "itemStudio.rarityRare",
    color: "#3b82f6",
    bg: "bg-blue-500/10",
  },
  epic: {
    tKey: "itemStudio.rarityEpic",
    color: "#a855f7",
    bg: "bg-purple-500/10",
  },
  legendary: {
    tKey: "itemStudio.rarityLegendary",
    color: "#f59e0b",
    bg: "bg-amber-500/10",
  },
  mythic: {
    tKey: "itemStudio.rarityMythic",
    color: "#ef4444",
    bg: "bg-red-500/10",
  },
};
...
const CATEGORY_CONFIG: Record<
  ItemCategory,
  { tKey: string; icon: React.ElementType }
> = {
...
const PRESET_MAGIC_SYSTEMS: MagicSystem[] = [
  {
    ...
    rules:
      "체내 존재력을 압축·방출하여 전투에 활용. 수련과 전투 경험으로 성장. 검술(내부 밀도 강화)과 마법(외부 마나 조작)으로 분기.",
    limitations:
      "과다 사용 시 의식 상실. 4파동 후반부터 불안정, 5파동 미완성 시 폭주 위험(주변 파괴). GH 달성 후 안정화.",
    ranks: [
      "1파동 — 일반인",
      "2파동 — 숙련 전사",
      "3파동 — 상위 기사",
  ...
  },
  {
    ...
    rules:
      "감정 상태와 직결. 사랑·신뢰로 증가, 공포·분노로 감소. 치유·정화·결계 발현. 권위(Authority) 능력은 세계 법칙 예외 코드.",
    limitations:
      "과다 사용 시 인간성 감소·감정 둔화. 분노 극한 시 성력 폭주. 권위 명령은 마족에게 무효.",
    ranks: [
      "1급 — 기본 치유 (10m)",
      "2급 — 집단 치유 (50m)",
      "3급 — 정화 능력",
  ...
  },
  {
    ...
    rules:
      "측정 가능하나 역산·재현 불가. 은하계 기축통화(단위: Hart). Type A(감정형/높은 강도), Type B(윤리형/장기 지속), Type C(존재형/극희귀).",
    limitations:
      "복제 지구 유지비가 EH 수입 초과 시 파산. 일부 개체 매매 금지. NOA 안드로이드는 EH=0 환경에서 부팅 불가.",
    ranks: [
      "Type A — 순수 감정형",
      "Type B — 윤리적",
      "Type C — 존재형·창의적",
  ...
  },
  {
    ...
    rules:
      "원소(화·수·풍·토), 강화, 정신계, 소환, 특수의 5분류. 외부 마나 조작이 핵심. 코어 등급에 따라 성장 속도 차이.",
    limitations:
      "검술과 상호배타(마나 외부 방출 vs 존재력 내부 밀도). 비마법 체계(검술)와 공존하나 동시 사용 불가.",
    ranks: [
      "1급 — 초급",
      "3급 — 중급",
      "5급 — 상급",
...
const PRESET_SKILLS: Skill[] = [
  {
    ...
    owner: "민시영",
    description:
      "존재력을 직선으로 극압축 후 방출하는 대형 베기. 보스급 몬스터 전용 기술.",
    cost: "존재력 대량 소모",
    cooldown: "장기전 불가",
    rank: "4파동+",
  },
  {
    ...
    owner: "민시영",
    description:
      "체력계 존재력을 극압축하여 초고속 이동. 잔상이 남을 정도의 속도.",
    cost: "체력계 존재력",
    cooldown: "연속 3회 제한",
    rank: "3파동+",
  },
  {
    ...
    owner: "민시영",
    description:
      "상대 파동의 역위상을 포착하여 붕괴시키는 카운터 기술. 파동 자체를 무력화.",
    cost: "정밀 타이밍 필요",
  ...
  },
  {
    ...
    owner: "민시영",
    description:
      "5파동 미완성 상태에서 공간을 압박하는 위험 기술. 부작용 있음.",
    cost: "5파동 불안정 소모",
    cooldown: "1회성 (부작용 위험)",
    rank: "5파동 미완성",
  },
  {
    ...
    owner: "민시영",
    description:
      "아크 형태 변환 + 5파동 완전 공명. 존재를 원래 자리로 재정렬(파괴가 아닌 안정화).",
    cost: "GH 완전 각성 필요",
    cooldown: "최종 비기",
    rank: "GH (5파동 완성)",
  },
...
const PRESET_ITEMS: Item[] = [
  {
    ...
    rarity: "legendary",
    description:
      '아크 형태 변환이 가능한 민시영의 주력 검. 5파동 완전 공명 시 "완결" 발동 매체.',
  ...
  },
  {
    ...
    rarity: "mythic",
    description:
      "루미나 대성당 지하의 성녀 소환용 대형 마법진. 활성화에 3개월 소요.",
  ...
  },
  {
    ...
    rarity: "rare",
    description:
      "마법사의 체내에서 형성되는 마나 결정체. 등급에 따라 성장 속도 결정.",
...
function analyzeBalance(
  ...
  if (skills.length > 0) {
    ...
    if (avgPerChar > 5) {
      warnings.push(
        t("itemStudio.warningSkillComplexity").replace(
          "${avg}",
          avgPerChar.toFixed(1),
        ),
      );
    }
  }
...
const ItemStudioView: React.FC<ItemStudioViewProps> = ({
  language,
  config,
  setConfig,
}) => {
  ...
  const [tierExpanded, setTierExpanded] = useState<
    Record<string, { t2?: boolean; t3?: boolean }>
  >({});
  ...
  const setMagicSystems = useCallback(
    (fn: (prev: MagicSystem[]) => MagicSystem[]) =>
      setConfig((prev) => ({
        ...prev,
        magicSystems: fn(prev.magicSystems ?? []),
      })),
    [setConfig],
  );
  ...
  const loadPreset = (category: PresetCategory) => {
    ...
    if (category === "all" || category === "magic") {
      const newMagics = PRESET_MAGIC_SYSTEMS.filter(
        (m) => !existingIds.has(m.id),
      );
      if (newMagics.length > 0)
        setMagicSystems((prev) => [...prev, ...newMagics]);
    }
  ...
  const handleAIGenerate = async () => {
    if (!activeSupportsStructured()) {
      showAlert(
        language === "KO"
          ? "현재 프로바이더는 구조화 생성을 지원하지 않습니다. Gemini를 사용해주세요."
          : "Current provider does not support structured generation. Please use Gemini.",
      );
      return;
    }
    if (!config.synopsis) {
    ...
    if (subTab === "balance") {
      showAlert(
        language === "KO"
          ? "밸런스 탭에서는 생성할 수 없습니다. 아이템/스킬/상성 탭을 선택해주세요."
          : "Cannot generate in balance tab. Select Items, Skills, or Systems tab.",
      );
      return;
    }
      ...
      const targetName =
        subTab === "skills"
          ...
          : subTab === "magic"
            ...
            : "Item";
      const msg = {
        KO: `${targetName} 생성에 실패했습니다. API 키를 확인하세요.`,
        EN: `${targetName} generation failed. Check API key.`,
        JP: "生成に失敗しました。",
        CN: "生成失败。",
      }[language];
      showAlert(msg);
            ...
            {showPresetMenu && (
              <div className="absolute right-0 top-full mt-2 z-50 bg-bg-secondary border border-border rounded-xl shadow-2xl p-2 min-w-[200px]">
                <button
                  ...
                  className="w-full text-left px-3 py-2 rounded-lg text-xs font-bold hover:bg-cyan-500/20 transition-colors"
                >
                  🌐 {t("itemStudio.loadAll")}
                </button>
                <button
                  onClick={() => loadPreset("magic")}
                  className="w-full text-left px-3 py-2 rounded-lg text-xs hover:bg-cyan-500/20 transition-colors"
                >
                  🔮{" "}
                  {`${t("itemStudio.magicSystemsCount")} (${getPresetSummary().magic})`}
                </button>
                <button
                  onClick={() => loadPreset("skills")}
                  className="w-full text-left px-3 py-2 rounded-lg text-xs hover:bg-cyan-500/20 transition-colors"
                >
                  ⚡{" "}
                ...
                <button
                  onClick={() => loadPreset("items")}
                  className="w-full text-left px-3 py-2 rounded-lg text-xs hover:bg-cyan-500/20 transition-colors"
                >
                  ⚔️{" "}
                  {`${t("itemStudio.itemsCount")} (${getPresetSummary().items})`}
                </button>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-2 flex-wrap">
            <button
              ...
              className={`px-3 py-1.5 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] text-[10px] font-bold border ${itemFilter === "all" ? "bg-cyan-500/30 text-cyan-300 border-cyan-500/50 shadow-[0_0_8px_rgba(6,182,212,0.4)]" : "bg-emerald-900/20 border-emerald-500/20 text-emerald-100/50 hover:text-cyan-200 hover:border-cyan-500/50"}`}
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredItems.map((item) => {
              ...
              const cCfg =
                CATEGORY_CONFIG[item.category] || CATEGORY_CONFIG.misc;
              const CatIcon = cCfg.icon;
              return (
                <div
                  key={item.id}
                  className={`${rCfg.bg} border border-emerald-500/30 backdrop-blur-md p-4 space-y-2 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] transition-all hover:shadow-[0_0_20px_rgba(6,182,212,0.15)]`}
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <CatIcon
                  ...
                  {item.effect && (
                    <p className="text-[10px] text-cyan-400 font-bold tracking-wider relative z-10">
                      ✦ {item.effect}
                    </p>
                  )}
                  {item.obtainedFrom && (
                  ...
                  <div className="space-y-1.5 pt-2 border-t border-border/50">
                    <input
                      value={item.purpose ?? ""}
                      onChange={(e) =>
                        updateItemField(item.id, "purpose", e.target.value)
                      }
                      placeholder={t("itemStudio.purposePlaceholder")}
                    ...
                    <input
                      ...
                      placeholder={t("itemStudio.activationCondPlaceholder")}
                      className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] px-2.5 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] outline-none py-1.5 text-[10px] outline-none focus:border-cyan-400"
                    />
                    <input
                      value={item.costWeakness ?? ""}
                      onChange={(e) =>
                        updateItemField(item.id, "costWeakness", e.target.value)
                      }
                      placeholder={t("itemStudio.costWeaknessPlaceholder")}
                      className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] px-2.5 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] outline-none py-1.5 text-[10px] outline-none focus:border-cyan-400"
                    />
                    <input
                      value={item.storyFunction ?? ""}
                      onChange={(e) =>
                        updateItemField(
                          item.id,
                          "storyFunction",
                      ...
                      placeholder={t("itemStudio.storyFunctionPlaceholder")}
                      className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] px-2.5 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] outline-none py-1.5 text-[10px] outline-none focus:border-cyan-400"
                    />
                  </div>
                  {/* 2단계 — 작동 */}
                    ...
                    {tierExpanded[item.id]?.t2 && (
                      <div className="space-y-1.5 pt-1.5">
                        <input
                          ...
                          placeholder={t(
                            "itemStudio.worldConnectionPlaceholder",
                          )}
                          className="w-full bg-emerald-950/40 border border-cyan-500/20 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-cyan-50 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.misuse ?? ""}
                          onChange={(e) =>
                            updateItemField(item.id, "misuse", e.target.value)
                          }
                          placeholder={t("itemStudio.misusePlaceholder")}
                          className="w-full bg-emerald-950/40 border border-cyan-500/20 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-cyan-50 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.lore ?? ""}
                          onChange={(e) =>
                            updateItemField(item.id, "lore", e.target.value)
                          }
                          placeholder={t("itemStudio.lorePlaceholder")}
                          className="w-full bg-emerald-950/40 border border-cyan-500/20 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-cyan-50 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.material ?? ""}
                          ...
                          placeholder={t("itemStudio.materialPlaceholder")}
                          className="w-full bg-emerald-950/40 border border-cyan-500/20 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-cyan-50 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.craftMethod ?? ""}
                          onChange={(e) =>
                            updateItemField(
                              item.id,
                              "craftMethod",
                          ...
                          placeholder={t("itemStudio.craftMethodPlaceholder")}
                          className="w-full bg-emerald-950/40 border border-cyan-500/20 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-cyan-50 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.valueRarity ?? ""}
                          onChange={(e) =>
                            updateItemField(
                              item.id,
                              "valueRarity",
                          ...
                          placeholder={t("itemStudio.valueRarityPlaceholder")}
                          className="w-full bg-emerald-950/40 border border-cyan-500/20 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-cyan-50 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.whoTargets ?? ""}
                          onChange={(e) =>
                            updateItemField(
                              item.id,
                              "whoTargets",
                          ...
                          placeholder={t("itemStudio.whoTargetsPlaceholder")}
                          className="w-full bg-emerald-950/40 border border-cyan-500/20 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-cyan-50 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                      </div>
                    )}
                    ...
                    {tierExpanded[item.id]?.t3 && (
                      <div className="space-y-1.5 pt-1.5">
                        <input
                          ...
                          placeholder={t("itemStudio.appearancePlaceholder")}
                          className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-emerald-100 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.symbolism ?? ""}
                          onChange={(e) =>
                            updateItemField(
                              item.id,
                              "symbolism",
                          ...
                          placeholder={t("itemStudio.symbolismPlaceholder")}
                          className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-emerald-100 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          ...
                          placeholder={t(
                            "itemStudio.currentLocationPlaceholder",
                          )}
                          className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-emerald-100 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          value={item.ownershipCond ?? ""}
                          onChange={(e) =>
                            updateItemField(
                              item.id,
                              "ownershipCond",
                        ...
                        <input
                          ...
                          placeholder={t("itemStudio.durabilityPlaceholder")}
                          className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-emerald-100 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          ...
                          placeholder={t("itemStudio.evolutionPlaceholder")}
                          className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-emerald-100 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                        <input
                          ...
                          placeholder={t("itemStudio.maintenancePlaceholder")}
                          className="w-full bg-emerald-950/40 border border-emerald-500/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)] text-emerald-100 focus:border-cyan-400 focus:bg-cyan-900/20 px-2.5 py-1.5 text-[10px] outline-none focus:border-cyan-400"
                        />
                      </div>
                    )}
                    ...
                    <div className="pt-2 border-t border-border/50">
                      <p className="text-[9px] text-text-tertiary italic leading-relaxed">
                        {t("itemStudio.summaryTemplate")
                          ...
                          .replace(
                            "${purpose}",
                            item.purpose || item.description || "___",
                          )
          ...
          <div className="relative overflow-hidden bg-[#040f14]/80 backdrop-blur-md border border-emerald-500/30 p-4 space-y-3 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] shadow-[0_0_15px_rgba(16,185,129,0.05)]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              <input
                ...
                placeholder={t("itemStudio.namePlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
              <select
              ...
              <input
                ...
                onChange={(e) =>
                  setNewItem((p) => ({ ...p, effect: e.target.value }))
                }
                placeholder={t("itemStudio.effectPlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              <input
                ...
                placeholder={t("itemStudio.descriptionPlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
              <input
                value={newItem.obtainedFrom ?? ""}
                onChange={(e) =>
                  setNewItem((p) => ({ ...p, obtainedFrom: e.target.value }))
                }
                placeholder={t("itemStudio.obtainedFromPlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
              <button
                onClick={addItem}
                disabled={!newItem.name}
                className="flex items-center justify-center gap-2 bg-cyan-500/20 text-cyan-300 border border-cyan-500/50 [clip-path:polygon(8px_0,100%_0,100%_calc(100%-8px),calc(100%-8px)_100%,0_100%,0_8px)] px-4 py-2 text-xs font-bold disabled:opacity-40 hover:bg-cyan-500/40 hover:shadow-[0_0_15px_rgba(6,182,212,0.4)]"
              >
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {skills.map((skill) => (
              <div
                key={skill.id}
                className="relative overflow-hidden bg-[#040f14]/80 backdrop-blur-md border border-emerald-500/30 p-4 space-y-2 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] shadow-[0_0_15px_rgba(16,185,129,0.05)] transition-all hover:bg-emerald-950/20 hover:shadow-[0_0_20px_rgba(6,182,212,0.15)]"
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <Zap
                      className={`w-4 h-4 ${skill.type === "ultimate" ? "text-cyan-400" : skill.type === "passive" ? "text-green-400" : "text-blue-400"}`}
                    />
                    <span className="font-bold text-sm">{skill.name}</span>
                    <span className="text-[9px] font-bold px-2 py-0.5 rounded-full bg-bg-primary text-text-tertiary">
                      {(() => {
                        const st = SKILL_TYPES.find(
                          (s) => s.value === skill.type,
                        );
                        return st ? t(st.tKey) : skill.type;
                      })()}
                    </span>
                  </div>
                  <button
          ...
          <div className="relative overflow-hidden bg-[#040f14]/80 backdrop-blur-md border border-emerald-500/30 p-4 space-y-3 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] shadow-[0_0_15px_rgba(16,185,129,0.05)]">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
              <input
                ...
                placeholder={t("itemStudio.skillNamePlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
              <select
                value={newSkill.type}
              ...
              <input
                value={newSkill.owner ?? ""}
                onChange={(e) =>
                  setNewSkill((p) => ({ ...p, owner: e.target.value }))
                }
                placeholder={t("itemStudio.ownerPlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
              <input
                value={newSkill.rank ?? ""}
                onChange={(e) =>
                  setNewSkill((p) => ({ ...p, rank: e.target.value }))
                }
                placeholder={t("itemStudio.rankPlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              <input
                ...
                placeholder={t("itemStudio.descriptionPlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs col-span-2"
              />
              <input
                value={newSkill.cost ?? ""}
                onChange={(e) =>
                  setNewSkill((p) => ({ ...p, cost: e.target.value }))
                }
                placeholder={t("itemStudio.costPlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
              <button
                onClick={addSkill}
        ...
        <div className="space-y-4">
            ...
            <MagicSystemCard
              ...
              onRemoveRank={(idx) =>
                setMagicSystems((prev) =>
                  prev.map((m) =>
                    m.id === magic.id
                      ? { ...m, ranks: m.ranks.filter((_, i) => i !== idx) }
                      : m,
                  ),
                )
              }
          ...
          <div className="relative overflow-hidden bg-[#040f14]/80 backdrop-blur-md border border-emerald-500/30 p-4 space-y-3 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] shadow-[0_0_15px_rgba(16,185,129,0.05)]">
            <h4 className="text-xs font-bold">
              {t("itemStudio.addMagicSystem")}
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <input
                value={newMagic.name ?? ""}
                onChange={(e) =>
                  setNewMagic((p) => ({ ...p, name: e.target.value }))
                }
                placeholder={t("itemStudio.magicNamePlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
              <input
                value={newMagic.source ?? ""}
                onChange={(e) =>
                  setNewMagic((p) => ({ ...p, source: e.target.value }))
                }
                placeholder={t("itemStudio.magicSourcePlaceholder")}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs"
              />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <textarea
                value={newMagic.rules ?? ""}
                onChange={(e) =>
                  setNewMagic((p) => ({ ...p, rules: e.target.value }))
                }
                placeholder={t("itemStudio.rulesPlaceholder")}
                rows={2}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs resize-none"
              />
              <textarea
                value={newMagic.limitations ?? ""}
                onChange={(e) =>
                  setNewMagic((p) => ({ ...p, limitations: e.target.value }))
                }
                placeholder={t("itemStudio.limitationsPlaceholder")}
                rows={2}
                className="bg-black/40 border border-emerald-500/30 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)] px-3 text-cyan-100 focus:border-cyan-400 focus:bg-cyan-900/20 focus:shadow-[0_0_10px_rgba(6,182,212,0.2)] py-2 text-xs resize-none"
              />
            </div>
            <button
              onClick={addMagic}
              disabled={!newMagic.name}
              className="flex items-center gap-2 bg-cyan-500/20 text-cyan-300 border border-cyan-500/50 [clip-path:polygon(8px_0,100%_0,100%_calc(100%-8px),calc(100%-8px)_100%,0_100%,0_8px)] px-4 py-2 text-xs font-bold disabled:opacity-40 hover:bg-cyan-500/40 hover:shadow-[0_0_15px_rgba(6,182,212,0.4)]"
            >
              <Plus className="w-3.5 h-3.5" /> {t("itemStudio.add")}
            </button>
          </div>
        ...
        <div className="space-y-6">
          {items.length === 0 && skills.length === 0 ? (
            ...
            <>
              ...
              <div className="relative overflow-hidden bg-[#040f14]/80 backdrop-blur-md border border-emerald-500/30 p-4 space-y-3 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] shadow-[0_0_15px_rgba(16,185,129,0.05)]">
                <h4 className="text-xs font-bold">
                  {t("itemStudio.rarityDistribution")}
                </h4>
              ...
              {skills.length > 0 && (
                <div className="relative overflow-hidden bg-[#040f14]/80 backdrop-blur-md border border-emerald-500/30 p-4 space-y-3 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] shadow-[0_0_15px_rgba(16,185,129,0.05)]">
                  <h4 className="text-xs font-bold">
                    {t("itemStudio.skillTypeDistribution")}
                  </h4>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  {
                    label: t("itemStudio.totalItems"),
                    value: items.length,
                    icon: Sword,
                  },
                  {
                    label: t("itemStudio.totalSkills"),
                    value: skills.length,
                    icon: Zap,
                  },
                  {
                    label: t("itemStudio.magicSystems"),
                    value: magicSystems.length,
                    icon: Wand2,
                  },
                  {
                    label: t("itemStudio.warningCount"),
                    value: balance.warnings.length,
                    icon: BarChart3,
                  },
    ...
    <div className="relative overflow-hidden bg-[#040f14]/80 backdrop-blur-md border border-emerald-500/30 p-4 space-y-3 [clip-path:polygon(12px_0,100%_0,100%_calc(100%-12px),calc(100%-12px)_100%,0_100%,0_12px)] shadow-[0_0_15px_rgba(16,185,129,0.05)] transition-all hover:bg-emerald-950/20 hover:shadow-[0_0_20px_rgba(6,182,212,0.15)]">
      ...
      {expanded && (
        <>
          {magic.source && (
            <p className="text-xs text-text-secondary">
              🔮 {t("itemStudio.source")}: {magic.source}
            </p>
          )}
          {magic.rules && (
            <p className="text-xs text-text-secondary">
              📜 {t("itemStudio.rules")}: {magic.rules}
            </p>
          )}
          {magic.limitations && (
            <p className="text-xs text-red-400/80">
              ⛔ {t("itemStudio.limits")}: {magic.limitations}
            </p>
          )}

          {/* Ranks */}
          <div className="space-y-2">
            <h5 className="text-[10px] font-bold text-text-tertiary uppercase">
              {t("itemStudio.rankSystem")}
            </h5>
            <div className="flex flex-wrap gap-1.5">
              {magic.ranks.map((rank, i) => (
                <span
                  key={i}
                  className="flex items-center gap-1 px-2 py-1 bg-bg-primary rounded-lg text-[10px] font-bold"
                >
                  {i + 1}. {rank}
                  <button
                    onClick={() => onRemoveRank(i)}
                    className="text-text-tertiary hover:text-red-400 ml-1"
                  >
                    ×
                  </button>
                </span>
