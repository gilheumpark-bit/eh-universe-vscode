...
function FieldRow({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3">
      <label className="text-[10px] font-bold text-text-tertiary uppercase tracking-wider font-mono shrink-0 w-24 pt-1.5">
        {label}
      </label>
...
function CopyButton({
  ...
  return (
    <button
      onClick={handleCopy}
      aria-label="복사"
      className="p-1.5 rounded bg-bg-tertiary/50 text-text-tertiary hover:text-accent-green transition-colors"
      title={t("chapterAnalysis.copy")}
    >
      {copied ? (
...
export default function ChapterAnalysisView({
  ...
  const [characters, setCharacters] = useState<CharacterStateEntry[]>(
    analysis?.characterState ?? [],
  );
  const [background, setBackground] = useState<BackgroundState>(
    analysis?.backgroundState ?? { ...EMPTY_BACKGROUND },
  );
  const [scene, setScene] = useState<SceneAnalysisState>(
    analysis?.sceneState ?? { ...EMPTY_SCENE },
  );
  const [sound, setSound] = useState<SoundState>(
    analysis?.soundState ?? { ...EMPTY_SOUND },
  );
  const [imagePrompt, setImagePrompt] = useState<ImagePromptPack>(
    analysis?.imagePromptPack ?? { ...EMPTY_IMAGE_PROMPT },
  );
  const [musicPrompt, setMusicPrompt] = useState<MusicPromptPack>(
    analysis?.musicPromptPack ?? { ...EMPTY_MUSIC_PROMPT },
  );
  ...
  const updateCharacter = useCallback(
    (index: number, patch: Partial<CharacterStateEntry>) => {
      setCharacters((prev) =>
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="flex gap-1.5">
          ...
          >
            ...
            {analyzing
              ? t("chapterAnalysis.analyzing")
              : t("chapterAnalysis.aiAutoAnalyze")}
          </button>
          <button
        ...
        {openSections.character && (
          <div className="p-4 pt-0 space-y-4">
            {characters.map((char, idx) => (
              <div
                key={idx}
                className="bg-bg-primary border border-border rounded-lg p-3 space-y-2"
              >
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="text-[9px] text-text-tertiary hover:text-accent-red transition-colors"
                  >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <FieldRow label={t("chapterAnalysis.role")}>
                    <TextInput
                      value={char.sceneRole}
                      onChange={(v) => updateCharacter(idx, { sceneRole: v })}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.emotion")}>
                    <TextInput
                      value={char.emotion.primary}
                      onChange={(v) =>
                        updateCharacter(idx, {
                          emotion: { ...char.emotion, primary: v },
                        })
                      }
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.intensity")}>
                    <IntensitySelect
                      value={char.emotion.intensity}
                      onChange={(v) =>
                        updateCharacter(idx, {
                          emotion: { ...char.emotion, intensity: v },
                        })
                      }
                      language={language}
                    />
                  ...
                  <FieldRow label={t("chapterAnalysis.expression")}>
                    <TextInput
                      ...
                      onChange={(v) => updateCharacter(idx, { expression: v })}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.gaze")}>
                    <TextInput
                      value={char.gaze.direction}
                      onChange={(v) =>
                        updateCharacter(idx, {
                          gaze: { ...char.gaze, direction: v },
                        })
                      }
                      placeholder={t("chapterAnalysis.gazeDirection")}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.gazeTarget")}>
                    <TextInput
                      value={char.gaze.target}
                      onChange={(v) =>
                        updateCharacter(idx, {
                          gaze: { ...char.gaze, target: v },
                        })
                      }
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.pose")}>
                    <TextInput
                      value={char.pose}
                      onChange={(v) => updateCharacter(idx, { pose: v })}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.action")}>
                    <TextInput
                      value={char.actionState}
                      onChange={(v) => updateCharacter(idx, { actionState: v })}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.relation")}>
                    <TextInput
                      value={char.relationContext}
                      onChange={(v) =>
                        updateCharacter(idx, { relationContext: v })
                      }
                    />
                  </FieldRow>
                </div>
                <div className="space-y-2">
                  <FieldRow label={t("chapterAnalysis.body")}>
                    <ArrayInput
                      value={char.bodyState}
                      onChange={(v) => updateCharacter(idx, { bodyState: v })}
                      placeholder={t("chapterAnalysis.commaSeparated")}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.outfit")}>
                    <ArrayInput
                      value={char.outfitDelta}
                      onChange={(v) => updateCharacter(idx, { outfitDelta: v })}
                      placeholder={t("chapterAnalysis.commaSeparated")}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.items")}>
                    <ArrayInput
                      value={char.heldItem}
                      onChange={(v) => updateCharacter(idx, { heldItem: v })}
                      placeholder={t("chapterAnalysis.commaSeparated")}
                    />
                  </FieldRow>
                  <FieldRow label={t("chapterAnalysis.aura")}>
                    <ArrayInput
                      value={char.aura}
                      onChange={(v) => updateCharacter(idx, { aura: v })}
                      placeholder={t("chapterAnalysis.commaSeparated")}
                    />
                  </FieldRow>
                </div>
          ...
          <div className="p-4 pt-0 space-y-2">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <FieldRow label={t("chapterAnalysis.location")}>
                <TextInput
                  ...
                  onChange={(v) =>
                    setBackground((p) => ({ ...p, location: v }))
                  }
                />
              </FieldRow>
              <FieldRow label={t("chapterAnalysis.space")}>
                <TextInput
                  value={background.spaceType}
                  onChange={(v) =>
                    setBackground((p) => ({ ...p, spaceType: v }))
                  }
                />
              </FieldRow>
              <FieldRow label={t("chapterAnalysis.time")}>
                <TextInput
                  ...
                  onChange={(v) => setBackground((p) => ({ ...p, time: v }))}
                />
              </FieldRow>
              <FieldRow label={t("chapterAnalysis.weather")}>
                <TextInput
                  value={background.weather}
                  onChange={(v) => setBackground((p) => ({ ...p, weather: v }))}
                />
              </FieldRow>
              <FieldRow label={t("chapterAnalysis.lighting")}>
                <TextInput
                  value={background.lighting}
                  onChange={(v) =>
                    setBackground((p) => ({ ...p, lighting: v }))
                  }
                />
              </FieldRow>
            </div>
            <FieldRow label={t("chapterAnalysis.mood")}>
              <ArrayInput
                ...
                onChange={(v) => setBackground((p) => ({ ...p, mood: v }))}
                placeholder={t("chapterAnalysis.commaSeparated")}
              />
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.keyObjects")}>
              <ArrayInput
                value={background.keyObjects}
                onChange={(v) =>
                  setBackground((p) => ({ ...p, keyObjects: v }))
                }
                placeholder={t("chapterAnalysis.commaSeparated")}
              />
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.condition")}>
              <ArrayInput
                value={background.environmentCondition}
                onChange={(v) =>
                  setBackground((p) => ({ ...p, environmentCondition: v }))
                }
                placeholder={t("chapterAnalysis.commaSeparated")}
              />
            </FieldRow>
        ...
        {openSections.scene && (
          <div className="p-4 pt-0 space-y-2">
            <FieldRow label={t("chapterAnalysis.summary")}>
              <TextInput
                value={scene.summary}
                onChange={(v) => setScene((p) => ({ ...p, summary: v }))}
              />
            </FieldRow>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <FieldRow label={t("chapterAnalysis.phase")}>
                <TextInput
                  value={scene.phase}
                  onChange={(v) => setScene((p) => ({ ...p, phase: v }))}
                />
              </FieldRow>
              <FieldRow label={t("chapterAnalysis.tension")}>
                <IntensitySelect
                  value={scene.tension}
                  onChange={(v) => setScene((p) => ({ ...p, tension: v }))}
                  language={language}
                />
              </FieldRow>
              <FieldRow label={t("chapterAnalysis.goal")}>
                <TextInput
                  value={scene.characterGoal}
                  onChange={(v) =>
                    setScene((p) => ({ ...p, characterGoal: v }))
                  }
                />
              </FieldRow>
              <FieldRow label={t("chapterAnalysis.obstacle")}>
                <TextInput
                  ...
                  onChange={(v) => setScene((p) => ({ ...p, obstacle: v }))}
                />
              </FieldRow>
            </div>
            <FieldRow label={t("chapterAnalysis.conflict")}>
              <ArrayInput
                value={scene.conflictType}
                onChange={(v) => setScene((p) => ({ ...p, conflictType: v }))}
              />
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.turning")}>
              <TextInput
                value={scene.turningPoint}
                onChange={(v) => setScene((p) => ({ ...p, turningPoint: v }))}
              />
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.symbols")}>
              <ArrayInput
                value={scene.symbolicTags}
                onChange={(v) => setScene((p) => ({ ...p, symbolicTags: v }))}
              />
            </FieldRow>
          </div>
        )}
        ...
        {openSections.sound && (
          <div className="p-4 pt-0 space-y-2">
            <FieldRow label={t("chapterAnalysis.ambient")}>
              <ArrayInput
                value={sound.ambient}
                onChange={(v) => setSound((p) => ({ ...p, ambient: v }))}
              />
            </FieldRow>
            ...
            <FieldRow label={t("chapterAnalysis.voice")}>
              <ArrayInput
                value={sound.voiceTone}
                onChange={(v) => setSound((p) => ({ ...p, voiceTone: v }))}
              />
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.audioMood")}>
              <ArrayInput
                value={sound.audioMood}
                onChange={(v) => setSound((p) => ({ ...p, audioMood: v }))}
              />
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.bgmTags")}>
              <ArrayInput
                value={sound.bgmTags}
                onChange={(v) => setSound((p) => ({ ...p, bgmTags: v }))}
              />
            </FieldRow>
          </div>
        )}
          ...
          <div className="p-4 pt-0 space-y-2">
            <FieldRow label={t("chapterAnalysis.character")}>
              <div className="flex gap-1">
                ...
                />
                <CopyButton
                  text={imagePrompt.characterFocus}
                  language={language}
                />
              </div>
            </FieldRow>
              ...
              <div className="flex gap-1">
                <textarea
                  ...
                  onChange={(e) =>
                    setImagePrompt((p) => ({
                      ...p,
                      backgroundFocus: e.target.value,
                    }))
                  }
                  rows={2}
                  className="flex-1 bg-bg-primary border border-border rounded-lg px-3 py-1.5 text-xs outline-none focus:border-accent-purple transition-colors resize-y"
                />
                <CopyButton
                  text={imagePrompt.backgroundFocus}
                  language={language}
                />
              </div>
            </FieldRow>
            ...
            <FieldRow label={t("chapterAnalysis.style")}>
              <ArrayInput
                value={imagePrompt.styleHints}
                onChange={(v) =>
                  setImagePrompt((p) => ({ ...p, styleHints: v }))
                }
              />
            </FieldRow>
            {/* Combined prompt for quick copy */}
            {(imagePrompt.characterFocus ||
              imagePrompt.backgroundFocus ||
              imagePrompt.sceneFocus) && (
              <div className="mt-2 p-3 bg-bg-primary border border-accent-purple/20 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[9px] font-bold text-accent-purple font-mono uppercase">
                    {t("chapterAnalysis.combinedPrompt")}
                  </span>
                  <CopyButton
                    text={[
                      imagePrompt.characterFocus,
        ...
        {openSections.music && (
          <div className="p-4 pt-0 space-y-2">
            <FieldRow label={t("chapterAnalysis.mood")}>
              <div className="flex gap-1">
                <TextInput
                  ...
                  onChange={(v) => setMusicPrompt((p) => ({ ...p, mood: v }))}
                />
            ...
            <FieldRow label={t("chapterAnalysis.emotionFlow")}>
              <div className="flex gap-1">
                <TextInput
                  value={musicPrompt.emotionFlow}
                  onChange={(v) =>
                    setMusicPrompt((p) => ({ ...p, emotionFlow: v }))
                  }
                />
                <CopyButton
                  text={musicPrompt.emotionFlow}
                  language={language}
                />
              </div>
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.sound")}>
              <ArrayInput
                value={musicPrompt.soundKeywords}
                onChange={(v) =>
                  setMusicPrompt((p) => ({ ...p, soundKeywords: v }))
                }
              />
            </FieldRow>
            <FieldRow label={t("chapterAnalysis.musicStyle")}>
              <ArrayInput
                value={musicPrompt.musicStyle}
                onChange={(v) =>
                  setMusicPrompt((p) => ({ ...p, musicStyle: v }))
                }
              />
            </FieldRow>
            ...
            {(musicPrompt.mood || musicPrompt.emotionFlow) && (
              <div className="mt-2 p-3 bg-bg-primary border border-accent-blue/20 rounded-lg">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[9px] font-bold text-accent-blue font-mono uppercase">
                    {t("chapterAnalysis.combinedPrompt")}
                  </span>
                  <CopyButton
                    text={[
                      musicPrompt.mood,
                      musicPrompt.emotionFlow,
                      musicPrompt.soundKeywords.join(", "),
                      musicPrompt.musicStyle.join(", "),
                    ]
                      .filter(Boolean)
                      .join(". ")}
                    language={language}
