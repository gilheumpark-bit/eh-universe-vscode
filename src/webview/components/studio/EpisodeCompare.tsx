...
function extractEpMetrics(messages: Message[]): EpMetric[] {
  ...
  for (const msg of messages) {
    ...
    const dialogueRatio =
      lines.length > 0
        ? Math.round((dialogueLines.length / lines.length) * 100)
        : 0;
  ...
  for (const msg of messages) {
    ...
    const paragraphs: ParaAnalysis[] = rawParas.map((p) => {
      ...
      const variance =
        lengths.length > 1
          ? Math.sqrt(
              lengths.reduce((s, l) => s + (l - avgLen) ** 2, 0) /
                lengths.length,
            )
          : 0;
      ...
      const tension = Math.min(
        100,
        Math.round((variance / Math.max(avgLen, 1)) * 100),
      );
  ...
  return (
    <div className="bg-black/20 border border-border/30 rounded-xl p-3 space-y-2">
      ...
      <svg
        ...
        role="img"
        aria-label="Full trend sparkline"
      >
        ...
        {[25, 50, 75].map((v) => (
          <line
            key={v}
            x1={padX}
            y1={padY + chartH - (v / 100) * chartH}
            x2={w - padX}
            y2={padY + chartH - (v / 100) * chartH}
        ...
        {metrics
          .reduce<Array<{ m: EpMetric; i: number }>>((acc, m, i) => {
            if (
              i === 0 ||
              i === metrics.length - 1 ||
              (i + 1) % Math.max(1, Math.ceil(metrics.length / 10)) === 0
            ) {
              acc.push({ m, i });
            }
            return acc;
  ...
  return (
    <div className="space-y-2">
      <p className="text-[9px] text-text-tertiary uppercase tracking-widest font-bold">
        {isKO
          ? `비교할 에피소드 선택 (최대 ${MAX_COMPARE})`
          : `Select episodes to compare (max ${MAX_COMPARE})`}
      </p>
      <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-1.5">
        {metrics.map((m) => {
          ...
          const color = isSelected
            ? COMPARE_PALETTE[selIdx % COMPARE_PALETTE.length]
            : undefined;
          ...
          return (
            <button
              ...
              style={
                isSelected
                  ? { background: `${color}15`, borderColor: color }
                  : undefined
              }
            >
              {/* Delta badge */}
              ...
              <div className="flex gap-px mt-1 h-1.5">
                <div
                  className="flex-1 rounded-sm bg-red-500/40"
                  style={{ height: `${m.tension * 1.5}%` }}
                />
                <div
                  className="flex-1 rounded-sm bg-blue-500/40"
                  style={{ height: `${m.pacing * 1.5}%` }}
                />
                <div
                  className="flex-1 rounded-sm bg-green-500/40"
                  style={{ height: `${m.immersion * 1.5}%` }}
                />
              </div>
            </button>
          );
  ...
  return (
    <div className="space-y-2">
      <svg
        ...
        role="img"
        aria-label="Episode comparison chart"
      >
        {/* Y-axis grid */}
        {[25, 50, 75, 100].map((v) => (
          <g key={v}>
            ...
            <g key={ep.index}>
              ...
              <text
                ...
                y={
                  padY +
                  chartH -
                  (ep[metricLabels[metricLabels.length - 1]] / 100) * chartH +
                  3
                }
                fill={color}
                fontSize="8"
          ...
          <thead>
            <tr className="text-text-tertiary uppercase tracking-wider">
              <th className="text-left py-1 px-2">
                {language === "KO" ? "지표" : "Metric"}
              </th>
              {episodes.map((ep, i) => (
                <th
                  key={ep.index}
                  className="text-center py-1 px-2 cursor-pointer hover:underline"
          ...
          <tbody>
            {(
              [
                ...
                [
                  "avgSentenceLen",
                  language === "KO" ? "평균문장길이" : "Avg Sent Len",
                ],
              ...
              return (
                <tr key={key} className="border-t border-border/30">
                  <td className="py-1.5 px-2 text-text-tertiary font-bold">
                    {label}
                  </td>
                  {episodes.map((ep, i) => {
                    ...
                    return (
                      <td
                        key={ep.index}
                        className={`py-1.5 px-2 text-center font-bold ${isMax ? "text-green-400" : isMin ? "text-red-400" : "text-text-secondary"}`}
                      >
                        {key === "charCount"
                          ? val.toLocaleString()
    ...
    <div className="bg-bg-secondary border border-border rounded-xl p-4 space-y-4 animate-in fade-in duration-200">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-xs font-bold text-white flex items-center gap-2">
          ...
          {isKO
            ? `에피소드 #${data.index} 상세`
            : `Episode #${data.index} Drilldown`}
        </h3>
        <button
          onClick={onClose}
          className="text-[9px] text-text-tertiary hover:text-white transition-colors uppercase tracking-wider font-bold"
        >
          {isKO ? "닫기" : "Close"}
        </button>
      </div>
      ...
      <div className="space-y-1">
          ...
          {data.paragraphs.map((p, i) => (
            <div
              ...
              className={`flex items-center gap-2 p-1.5 rounded-md text-[8px] ${
                i === data.highDialogueParagraphIdx
                  ? "bg-purple-500/10 border border-purple-500/20"
                  : "bg-black/10"
              }`}
            >
              <span className="text-text-tertiary font-mono w-5 shrink-0 text-right">
                P{i + 1}
              </span>
              {/* Tension bar */}
              <div
                className="flex-1 h-2 bg-black/20 rounded-full overflow-hidden"
                title={`Tension: ${p.tension}`}
              >
                <div
              ...
              <div
                className="flex-1 h-2 bg-black/20 rounded-full overflow-hidden"
                title={`Pacing: ${p.pacing}`}
              >
                <div
                  className="h-full rounded-full transition-all"
                  style={{
                    width: `${p.pacing}%`,
                    background: METRIC_COLORS.pacing,
                  }}
                />
              </div>
              {p.isDialogue && (
                <span className="text-[7px] text-purple-400 font-bold shrink-0">
                  DLG
                </span>
              )}
            </div>
        ...
        <div className="flex gap-4 text-[7px] text-text-tertiary">
          <span className="flex items-center gap-1">
            <span
              className="inline-block w-2 h-1 rounded-full"
              style={{ background: METRIC_COLORS.tension }}
            />{" "}
            {isKO ? "긴장감" : "Tension"}
          </span>
          <span className="flex items-center gap-1">
            <span
              className="inline-block w-2 h-1 rounded-full"
              style={{ background: METRIC_COLORS.pacing }}
            />{" "}
            {isKO ? "호흡" : "Pacing"}
          </span>
          <span className="flex items-center gap-1">
  ...
  return (
    <div className="bg-black/20 border border-border/30 rounded-xl p-3 space-y-2">
      <h4 className="text-[9px] font-bold text-text-tertiary uppercase tracking-widest flex items-center gap-2">
        ...
        {isKO
          ? `최근 ${windowSize}화 vs 이전 ${previous.length}화`
          : `Last ${windowSize} vs prev ${previous.length}`}
      </h4>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
        {METRIC_KEYS.map((key) => {
          ...
          return (
            <div
              key={key}
              className="flex items-center gap-2 bg-bg-secondary/50 rounded-lg px-3 py-2"
            >
              <div
              ...
              <div className="min-w-0">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span
                    className={`text-[8px] font-bold ${diff > 0 ? "text-green-400" : diff < 0 ? "text-red-400" : "text-text-tertiary"}`}
                  >
                    {diff > 0 ? "+" : ""}
                    {diff}
                  </span>
...
export default function EpisodeCompare({ messages, language }: Props) {
  ...
  const selectedEpisodes = useMemo(
    () =>
      selected
        .map((idx) => metrics.find((m) => m.index === idx))
        .filter((m): m is EpMetric => !!m),
    [selected, metrics],
  );
  ...
  if (metrics.length < 2) {
    return (
      <div className="text-center py-8 text-text-tertiary text-xs">
        {isKO
          ? "비교하려면 최소 2개 챕터가 필요합니다."
          : "Need at least 2 chapters to compare."}
      </div>
    );
  }
    ...
    <div className="space-y-4">
        ...
        <div className="bg-bg-secondary border border-border rounded-xl p-4 space-y-3">
          <h3 className="text-[10px] font-bold text-text-tertiary uppercase tracking-wider font-mono">
            ...
            <span className="ml-2 text-[8px] text-text-tertiary font-normal">
              {isKO
                ? "(헤더 클릭 시 상세 분석)"
                : "(click header to drill down)"}
            </span>
          </h3>
          <OverlayChart
            episodes={selectedEpisodes}
            language={language}
            onEpisodeClick={handleDrilldown}
          />
        </div>
      )}
      ...
      {selectedEpisodes.length === 0 && (
        <div className="text-center py-6 text-text-tertiary text-[10px] border border-border/30 border-dashed rounded-xl">
          {isKO
            ? "위에서 에피소드를 선택하면 비교 차트가 표시됩니다"
            : "Select episodes above to see comparison chart"}
        </div>
      )}
