...
function analyzeRhythm(messages: Message[], isKO: boolean): RhythmStats | null {
  ...
  const avgLength = Math.round(
    lengths.reduce((s, l) => s + l, 0) / totalSentences,
  );
  ...
  if (!stats) {
    return (
      <div className="text-center py-4 text-text-tertiary text-[10px]">
        {isKO
          ? "리듬 분석에 충분한 텍스트가 없습니다."
          : "Not enough text for rhythm analysis."}
      </div>
    );
  }
  ...
  return (
    <div className="bg-bg-secondary border border-border rounded-xl p-4 space-y-4">
      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <div className="bg-black/30 rounded-lg p-2 text-center">
          <div
            className={`text-sm font-black ${stats.rhythmScore >= 60 ? "text-green-400" : stats.rhythmScore >= 40 ? "text-amber-400" : "text-red-400"}`}
          >
            {stats.rhythmScore}
          </div>
          <div className="text-[8px] text-text-tertiary uppercase">
        ...
        {stats.buckets.map((bucket) => {
          ...
          const ratio =
            stats.totalSentences > 0
              ? Math.round((bucket.count / stats.totalSentences) * 100)
              : 0;
          return (
        ...
        <div className="text-[10px] text-text-secondary leading-relaxed space-y-1">
          <div>
            ...
            <span className="font-bold text-text-primary">
              {stats.dominantPattern}
            </span>
          </div>
          {stats.shortRatio > 50 && (
            <div className="text-blue-400">
              {isKO
                ? "💡 단문 비율이 높습니다. 속도감은 좋지만, 장문으로 호흡을 늘리면 깊이가 생깁니다."
          ...
          {stats.longRatio > 40 && (
            <div className="text-amber-400">
              {isKO
                ? "💡 장문 비율이 높습니다. 짧은 문장을 중간에 끊어 넣으면 리듬감이 살아납니다."
                : "💡 High long-sentence ratio. Injecting short sentences creates better rhythm."}
            </div>
          )}
          {stats.rhythmScore >= 60 &&
            stats.shortRatio <= 50 &&
            stats.longRatio <= 40 && (
              <div className="text-green-400">
                {isKO
                  ? "✓ 단문·장문이 잘 섞여 있어 리듬이 건강합니다."
