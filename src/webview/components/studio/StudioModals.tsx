      ...
      >
        ...
        <div className="mt-5 pt-4 border-t border-white/[0.06] text-center">
          <p className="text-[10px] text-text-tertiary">
            {isKO
              ? "F12를 눌러 언제든지 이 창을 열 수 있습니다"
              : "Press F12 anytime to open this panel"}
          </p>
        </div>
      </div>
    </div>
...
export function MoveSessionModal({
  ...
  return (
    ...
    >
      <div
        className="bg-bg-primary border border-border rounded-2xl p-6 w-full max-w-sm mx-4 space-y-4"
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
      >
        ...
        <select
          ...
          onChange={(e) => {
            if (e.target.value) {
              onMove(data.sessionId, e.target.value);
              onClose();
            }
          }}
        >
...
export function SaveSlotModal({
  ...
  return (
    ...
    >
      <div
        className="bg-bg-primary border border-border rounded-2xl p-6 w-full max-w-sm mx-4 space-y-4 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
      >
        <h3 className="text-sm font-bold text-text-primary">
          {t("saveSlot.enterSaveName")}
        </h3>
        <input
          autoFocus
          type="text"
