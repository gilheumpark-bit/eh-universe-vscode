...
export default function MobileTabBar({
  ...
  const isMoreActive =
    !isGuided && (moreOpen || MORE_TABS.some((tab) => tab.key === activeTab));
  ...
  useEffect(() => {
    if (moreOpen && moreScrollRef.current) {
      const activeBtn = moreScrollRef.current.querySelector(
        '[data-active="true"]',
      );
      if (activeBtn)
      ...
      >
        ...
        >
          ...
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="flex flex-col items-center gap-1 py-2.5 px-4 min-w-[52px]"
              suppressHydrationWarning
            >
        ...
        {primaryTabs.map(({ key, icon: Icon }) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`relative z-10 transition-transform duration-200 ${active ? "scale-110" : ""}`}
              >
              ...
              {/* Label */}
              <span
                className={`relative z-10 transition-colors duration-200 ${active ? "text-accent-purple font-semibold" : ""}`}
              >
                {getTabLabel(key)}
              </span>
