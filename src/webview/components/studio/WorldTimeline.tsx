...
function buildTracks(simData: WorldSimData): {
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="overflow-x-auto">
        <svg
          viewBox={`0 0 ${w} ${h}`}
          className="min-w-[500px] h-auto"
          style={{ fontFamily: "var(--font-mono, monospace)" }}
          role="img"
          aria-label={isKO ? "문명 타임라인" : "Civilization timeline"}
        >
          {/* Background */}
          <rect
            width={w}
            height={h}
            fill="rgba(15,10,0,0.6)"
            rx="8"
            opacity="0.8"
          />
              ...
              >
                ...
                {isSelected && (
                  <rect
                    ...
                    y={PAD.top - 10}
                    width={ERA_W}
                    height={h - PAD.top - PAD.bottom + 10}
                ...
                <line
                  ...
                  y2={h - PAD.bottom}
                  stroke={
                    isSelected ? "rgba(255,200,50,0.4)" : "rgba(255,200,50,0.1)"
                  }
                ...
                <text
                  ...
                  y={PAD.top - 18}
                  fill={
                    isSelected ? "rgba(255,220,100,1)" : "rgba(255,200,50,0.6)"
                  }
                  fontSize="8"
                  textAnchor="middle"
                  fontWeight="bold"
          ...
          {tracks.map((track, i) => (
            <text
              ...
              fontWeight="bold"
            >
              {track.civName.length > 8
                ? track.civName.slice(0, 8) + "…"
                : track.civName}
            </text>
          ))}
              ...
              <g key={`band-${track.civName}`}>
                ...
                {track.eras.map((era) => (
                  <circle
                    ...
                    r="4"
                    fill={track.color}
                    opacity={selectedEra && selectedEra !== era ? 0.3 : 0.8}
                    style={{ cursor: onSelectEra ? "pointer" : undefined }}
                    onClick={(e) => {
                      e.stopPropagation();
                      onSelectEra?.(era);
      ...
      <div className="flex flex-wrap gap-3 text-[9px] mt-4 p-3 border border-[rgba(255,200,50,0.15)] bg-[rgba(255,200,50,0.02)] rounded-xl backdrop-blur-sm">
        {tracks.map((track) => (
          <span
            key={track.civName}
            className="flex items-center gap-1.5 px-2 py-1 bg-[rgba(0,0,0,0.2)] rounded-md border border-[rgba(255,255,255,0.05)]"
          >
            <span
              className="w-2.5 h-2.5 rounded-full inline-block shadow-[0_0_5px_currentColor]"
              style={{ background: track.color, color: track.color }}
            />
            <span className="text-[rgba(230,220,180,0.9)] font-mono">
              {track.civName}
            </span>
            <span className="text-[rgba(255,200,50,0.5)] font-mono">
              ({track.eras.length}
              {isKO ? "시대" : "Era"})
            </span>
          </span>
