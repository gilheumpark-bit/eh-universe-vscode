...
const HistoryTab: React.FC<HistoryTabProps> = ({
  ...
  const hasWorldData = allSessions.some(
    (s) => s.config.worldSimData?.civs?.length,
  );
  ...
  const filtered = allSessions
    .filter((s) => {
      ...
      if (archiveFilter === "WORLD")
        return (s.config.worldSimData?.civs?.length ?? 0) > 0;
        ...
        {projects.length > 1 && (
          <div className="flex gap-1.5">
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest font-mono border transition-colors ${archiveScope === "project" ? "bg-accent-purple/20 border-accent-purple/30 text-accent-purple" : "bg-bg-secondary border-border text-text-tertiary hover:text-text-primary"}`}
            >
              {t("archive.currentProject")}
            </button>
            <button
              onClick={() => setArchiveScope("all")}
              className={`px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest font-mono border transition-colors ${archiveScope === "all" ? "bg-accent-purple/20 border-accent-purple/30 text-accent-purple" : "bg-bg-secondary border-border text-text-tertiary hover:text-text-primary"}`}
            >
        ...
        <div className="flex gap-1.5 flex-wrap">
          {categories.map((cat) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-md text-[9px] font-black uppercase tracking-widest font-mono border transition-colors ${archiveFilter === cat.key ? "bg-blue-600/15 border-blue-500/30 text-blue-400" : "bg-bg-secondary border-border text-text-tertiary hover:text-text-primary"}`}
            >
              {cat.label}
              <span className="ml-1 text-[8px] opacity-50">
                {cat.key === "ALL"
                  ...
                  : cat.key === "WORLD"
                    ? allSessions.filter(
                        (s) => (s.config.worldSimData?.civs?.length ?? 0) > 0,
                      ).length
                    : allSessions.filter((s) => s.config.genre === cat.key)
                        .length}
              </span>
            </button>
          ))}
        ...
        {filtered.length === 0 ? (
          ...
          filtered.map((s) => (
            <div
              key={s.id}
              onClick={() => {
                if (s._projectId && s._projectId !== currentProjectId)
                  setCurrentProjectId(s._projectId);
            ...
            >
              <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 z-10">
                <button
                  ...
                  aria-label="이름 변경"
                  className="p-1.5 bg-bg-tertiary/50 rounded-full text-text-tertiary hover:text-accent-purple transition-all"
                >
                  ...
                  <button
                    onClick={(e) => {
                      ...
                      const others = projects.filter(
                        (p) => p.id !== (s._projectId || currentProjectId),
                      );
                      if (others.length === 1) {
                ...
                <button
                  ...
                  aria-label="인쇄"
                  className="p-1.5 bg-bg-tertiary/50 rounded-full text-text-tertiary hover:text-text-primary transition-all"
                >
                ...
                <button
                  ...
                  aria-label="삭제"
                  className="p-1.5 bg-bg-tertiary/50 rounded-full text-text-tertiary hover:text-accent-red transition-all"
                >
                  <X className="w-3 h-3" />
                </button>
              </div>
              {renamingSessionId === s.id ? (
                ...
                <h4 className="font-black text-sm mb-2 pr-16 truncate">
                  {s.title}
                </h4>
              )}
              <div className="flex flex-wrap gap-1.5 mt-1">
                ...
                {(s.config.worldSimData?.civs?.length ?? 0) > 0 && (
                  <span className="px-1.5 py-0.5 bg-emerald-900/30 border border-emerald-500/20 rounded text-[8px] font-bold text-emerald-400 font-mono">
                    {t("archive.worldLabel")} ·{" "}
                    {s.config.worldSimData!.civs!.length}
                  </span>
                )}
