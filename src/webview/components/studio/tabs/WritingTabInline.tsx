...
const DynSkeleton = () => (
  <div className="h-8 rounded-lg bg-bg-secondary/50 animate-pulse" />
);
...
const WritingToolbar = dynamic(
  () =>
    import("@/components/studio/WritingToolbar").then((m) => ({
      default: m.WritingToolbar,
    })),
  { ssr: false, loading: DynSkeleton },
);
...
export default function WritingTabInline(props: Props) {
  ...
  const { chatMessages, sendChat, chatLoading, abortChat, clearChat } =
    useWritingChat({
      ...
      characters: currentSession.config.characters
        ?.map((c) => c.name)
        .join(", "),
      currentChapter: currentSession.messages
        .slice(-2)
        .map((m) => m.content)
        .join("\n")
        .slice(0, 2000),
          ...
          {(currentSession.messages.length > 0 || writingMode !== "ai") && (
            <details className="w-full group border border-border rounded-xl bg-bg-secondary/50 overflow-hidden">
              <summary className="flex items-center justify-between px-4 py-3 cursor-pointer hover:bg-bg-secondary transition-colors">
                ...
                <span className="text-[11px] text-text-tertiary group-open:rotate-180 transition-transform">
                  ▼
                </span>
              </summary>
              <div className="px-4 pb-4 space-y-3 text-[10px] border-t border-border pt-3">
                <div className="flex flex-wrap gap-x-4 gap-y-1">
                  <span className="text-text-tertiary font-bold uppercase w-16">
                    {t("applied.genre")}
                  </span>
                ...
                {currentSession.messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center text-center space-y-6 py-12 md:py-20 px-2">
                    ...
                    <div className="space-y-3 max-w-md">
                      ...
                      <p className="text-text-tertiary text-[11px] leading-relaxed pt-1">
                        {t("writingMode.emptyStateManualHint")}
                      </p>
                      <button
                        type="button"
                        onClick={() => {
            ...
            {writingMode === "canvas" && (
              <div className="flex-1 space-y-4">
                <div className="bg-accent-green/5 border border-accent-green/20 rounded-xl p-6">
                  <h3 className="text-sm font-bold text-accent-green mb-2">
                    {isKO ? "3단계 캔버스 모드" : "Three-Step Canvas"}
                  </h3>
                  <p className="text-xs text-text-secondary mb-4">
                    {isKO
                      ? "구조 → 초안 → 다듬기 3단계로 글을 완성합니다."
                      : "Complete your writing in 3 steps: Structure → Draft → Polish."}
                  </p>
                  <div className="flex items-center gap-2 text-xs text-text-tertiary">
                    <span
                      className={`px-2 py-1 rounded ${props.canvasPass >= 1 ? "bg-accent-green/20 text-accent-green" : "bg-bg-secondary"}`}
                    >
                      {isKO ? "1. 구조" : "1. Structure"}
                    </span>
                    <span className="text-text-quaternary">&rarr;</span>
                    <span
                      className={`px-2 py-1 rounded ${props.canvasPass >= 2 ? "bg-accent-green/20 text-accent-green" : "bg-bg-secondary"}`}
                    >
                      {isKO ? "2. 초안" : "2. Draft"}
                    </span>
                    <span className="text-text-quaternary">&rarr;</span>
                    <span
                      className={`px-2 py-1 rounded ${props.canvasPass >= 3 ? "bg-accent-green/20 text-accent-green" : "bg-bg-secondary"}`}
                    >
                      {isKO ? "3. 다듬기" : "3. Polish"}
                    </span>
                  </div>
                ...
                <textarea
                  ...
                  placeholder={
                    isKO
                      ? "캔버스에 구조를 작성하세요..."
                      : "Write your structure on the canvas..."
                  }
                />
              </div>
            )}
            ...
            {writingMode === "refine" && (
              <div className="flex-1 space-y-4">
                <div className="bg-accent-blue/5 border border-accent-blue/20 rounded-xl p-6">
                  <h3 className="text-sm font-bold text-accent-blue mb-2">
                    {isKO ? "자동 30% 다듬기" : "Auto 30% Refine"}
                  </h3>
                  <p className="text-xs text-text-secondary mb-3">
                    {isKO
                      ? "AI가 현재 원고를 분석하고 약 30%를 자동으로 개선합니다."
                      : "AI analyzes your manuscript and automatically improves ~30%."}
                  </p>
                  {props.promptDirective && (
                    <p className="text-xs text-accent-blue font-mono bg-accent-blue/5 rounded px-3 py-2">
                      {isKO ? "지시:" : "Directive:"}{" "}
                      {props.promptDirective}
                    </p>
                  )}
                ...
                <textarea
                  ...
                  placeholder={
                    isKO
                      ? "다듬을 원고를 붙여넣으세요..."
                      : "Paste your manuscript to refine..."
                  }
                />
              </div>
            )}
            ...
            {writingMode === "advanced" && (
              <div className="flex-1 space-y-4">
                <div className="bg-accent-red/5 border border-accent-red/20 rounded-xl p-6">
                  <h3 className="text-sm font-bold text-accent-red mb-2">
                    {isKO ? "고급 모드" : "Advanced Mode"}
                  </h3>
                  <p className="text-xs text-text-secondary">
                    {isKO
                      ? "엔진 파라미터, 장르 프리셋, HFCP 설정을 직접 제어합니다."
                      : "Direct control over engine parameters, genre presets, and HFCP settings."}
                  </p>
                </div>
                <textarea
                  ref={editDraftRef}
                  ...
                  placeholder={
                    isKO
                      ? "고급 모드에서 직접 작성하세요..."
                      : "Write directly in advanced mode..."
                  }
                />
              </div>
            )}
          ...
          <div className="p-4 md:p-6 bg-linear-to-t from-bg-primary via-bg-primary/95 to-transparent sticky bottom-0 z-20">
            {showAiLock && (
              <div className="mb-3 flex items-center gap-2 rounded-xl border border-accent-amber/30 bg-accent-amber/10 px-4 py-2.5 text-xs text-accent-amber">
                ...
                <span>
                  {isKO
                    ? "스플래시 화면에서 API 키를 등록하면 AI 생성을 사용할 수 있습니다."
                    : "Register an API key from the splash screen to use AI generation."}
                </span>
              </div>
            )}
            ...
            <p className="mt-2 text-[10px] text-text-tertiary text-center font-mono uppercase tracking-[0.2em] opacity-40">
              {isKO
                ? "Narrative Origin Writer — 소설 생성 엔진"
                : "Narrative Origin Writer — Story Engine"}
            </p>
          </div>
        )}
      </div>
