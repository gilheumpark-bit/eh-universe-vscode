...
const ContinuityGraph = dynamic(
  () => import("@/components/studio/ContinuityGraph"),
  { ssr: false, loading: () => null },
);
const EngineStatusBar = dynamic(
  () => import("@/components/studio/EngineStatusBar"),
  { ssr: false, loading: () => null },
);
const ChatMessage = dynamic(() => import("@/components/studio/ChatMessage"), {
  ssr: false,
  loading: () => null,
});
const VersionDiff = dynamic(() => import("@/components/studio/VersionDiff"), {
  ssr: false,
  loading: () => null,
});
const TypoPanel = dynamic(() => import("@/components/studio/TypoPanel"), {
  ssr: false,
  loading: () => null,
});
const InlineRewriter = dynamic(
  () => import("@/components/studio/InlineRewriter"),
  { ssr: false, loading: () => null },
);
const AutoRefiner = dynamic(() => import("@/components/studio/AutoRefiner"), {
  ssr: false,
  loading: () => null,
});
const AdvancedWritingPanel = dynamic(
  () => import("@/components/studio/AdvancedWritingPanel"),
  { ssr: false, loading: () => null },
);
const DirectorPanel = dynamic(
  () => import("@/components/studio/DirectorPanel"),
  { ssr: false, loading: () => null },
);
const EngineDashboard = dynamic(
  () => import("@/components/studio/EngineDashboard"),
  { ssr: false, loading: () => null },
);
const EpisodeScenePanel = dynamic(
  () => import("@/components/studio/EpisodeScenePanel"),
  { ssr: false, loading: () => null },
);
...
interface WritingTabProps {
  ...
  setWritingMode: (
    mode: "ai" | "edit" | "canvas" | "refine" | "advanced",
  ) => void;
  editDraft: string;
  ...
  editDraftRef?: React.RefObject<HTMLTextAreaElement>;
  showAiLock?: boolean;
...
const WritingTab: React.FC<WritingTabProps> = ({
  ...
  searchQuery,
  filteredMessages,
  searchMatchesEditDraft: _searchMatchesEditDraft,
  hasApiKey,
  setShowApiKeyModal,
  setActiveTab,
  advancedSettings,
    ...
    updateCurrentSession({
      messages: [
        ...currentSession.messages,
        {
          id: `u-edit-${now + 1}`,
          role: "user",
          content: t("writingMode.inlineEditComplete"),
          timestamp: now + 1,
        },
        editMsg,
      ],
    ...
    <div className="flex-1 flex flex-col overflow-hidden relative">
      <div className="flex-1 flex overflow-hidden">
        <div className="flex-1 overflow-y-auto">
          <div
            className={`max-w-6xl w-full mx-auto px-4 md:px-8 lg:px-12 flex flex-col ${currentSession.messages.length === 0 && writingMode === "ai" ? "h-full justify-center items-center" : "py-6 md:py-8 space-y-6 min-h-full"}`}
          >
            ...
            {(currentSession.messages.length > 0 || writingMode !== "ai") && (
              <details className="group border border-border rounded-2xl bg-gradient-to-br from-bg-secondary/30 to-transparent backdrop-blur-sm overflow-hidden">
                <summary className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-bg-secondary/50 transition-colors">
                  ...
                  <span className="text-xs text-text-tertiary group-open:rotate-180 transition-transform duration-200">
                    ▼
                  </span>
                </summary>
                <div className="px-5 pb-5 space-y-4 border-t border-border pt-4">
                  ...
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    <span className="px-3 py-1.5 rounded-lg bg-bg-secondary border border-border text-text-secondary text-sm font-mono">
                      EP.{currentSession.config.episode}/
                  ...
                  {currentSession.config.characters.length > 0 && (
                    <div className="space-y-2">
                      <span className="text-[10px] font-bold uppercase tracking-wider text-text-tertiary">
                        {t("applied.characters")}
                      </span>
                      <div className="flex flex-wrap gap-2">
                        {currentSession.config.characters.map((c) => (
                          <span
                            key={c.id}
                            className="flex items-center gap-2 px-3 py-1.5 bg-bg-primary border border-border rounded-lg text-xs"
                          >
                  ...
                  <div className="flex flex-wrap gap-2 pt-2">
                    <button
                      ...
                      className="px-4 py-2 bg-bg-secondary hover:bg-accent-purple/10 border border-border hover:border-accent-purple/30 rounded-xl text-[11px] font-bold text-text-tertiary hover:text-accent-purple transition-all duration-200"
                    >
                      🌍 {t("applied.editWorld")}
                    </button>
                    <button
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2">
                    ...
                    >
                      ...
                      <span
                        className={`text-[11px] font-bold uppercase tracking-wider ${writingMode === "ai" ? "text-accent-purple" : "text-text-secondary"}`}
                      >
                    ...
                    >
                      ...
                      <span
                        className={`text-[11px] font-bold uppercase tracking-wider ${writingMode === "edit" ? "text-accent-amber" : "text-text-secondary"}`}
                      >
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (!canvasContent) setCanvasPass(0);
                      }}
                      className={`group relative flex flex-col items-center gap-2 px-4 py-4 rounded-2xl border transition-all duration-200 ${
                        writingMode === "canvas"
                    ...
                    >
                      <span className="text-2xl">🎨</span>
                      <span
                        className={`text-[11px] font-bold uppercase tracking-wider ${writingMode === "canvas" ? "text-accent-green" : "text-text-secondary"}`}
                      >
                    ...
                    >
                      <span className="text-2xl">⚡</span>
                      <span
                        className={`text-[11px] font-bold uppercase tracking-wider ${writingMode === "refine" ? "text-accent-blue" : "text-text-secondary"}`}
                      >
                    ...
                    >
                      <span className="text-2xl">🎯</span>
                      <span
                        className={`text-[11px] font-bold uppercase tracking-wider ${writingMode === "advanced" ? "text-amber-400" : "text-text-secondary"}`}
                      >
                  ...
                  <div className="flex gap-3 items-center p-3 rounded-xl bg-bg-secondary/30 border border-border">
                    ...
                    <input
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) =>
                        setPromptDirective(e.target.value)
                      }
                ...
                {currentSession.messages.length === 0 ? (
                  <div className="flex flex-col items-center justify-center text-center space-y-4">
                    ...
                    <p className="text-text-tertiary text-base font-medium">
                      {t("engine.startPrompt")}
                    </p>
                    <div className="flex flex-wrap gap-2 justify-center pt-2 max-w-2xl">
                      {(tObj.presets as string[]).map(
                        (preset: string, i: number) => (
                          <button
                            key={i}
                            onClick={() => handleSend(preset)}
                            className="px-3 py-1.5 bg-bg-secondary/80 border border-border rounded-full text-[10px] text-text-tertiary hover:text-accent-purple transition-all"
                          >
                            {preset}
                          </button>
                        ),
                        ...
                        msg.versions.length > 1 && (
                          <div className="ml-11 md:ml-12">
                            <VersionDiff
                              ...
                              language={language}
                              onSwitch={(idx) =>
                                handleVersionSwitch(msg.id, idx)
                              }
                            />
                          </div>
                        )}
                      {msg.role === "assistant" && msg.content && (
                        <div className="ml-11 md:ml-12">
                          <TypoPanel
                            text={msg.content}
                            language={language}
                            onApplyFix={(idx, orig, sug) =>
                              handleTypoFix(msg.id, idx, orig, sug)
                            }
                          />
                        </div>
                      )}
                ...
                {!editDraft.trim() ? (
                  <div className="text-center py-16 space-y-4">
                    ...
                    <textarea
                      ...
                      className="w-full min-h-[300px] bg-bg-primary border border-border rounded-xl p-4 text-sm text-left outline-none focus:border-accent-purple transition-colors font-mono resize-y"
                    />
                  </div>
                ) : (
                  <InlineRewriter
                    ...
                    context={
                      currentSession.config.genre
                        ? `${currentSession.config.genre} | ${currentSession.config.title || ""}`
                        : undefined
                    }
                    onApply={(newContent: string) => setEditDraft(newContent)}
                  />
                )}
              </div>
            )}
            ...
            {writingMode === "canvas" && (
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  <div
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-bold ${canvasPass >= 1 ? "bg-blue-600/20 text-blue-400 border border-blue-500/30" : "bg-bg-secondary text-text-tertiary"}`}
                  >
                    🦴 {t("canvas.skeleton")}
                  </div>
                  <div
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-bold ${canvasPass >= 2 ? "bg-pink-600/20 text-pink-400 border border-pink-500/30" : "bg-bg-secondary text-text-tertiary"}`}
                  >
                    💓 {t("canvas.emotion")}
                  </div>
                  <div
                    className={`px-3 py-1.5 rounded-lg text-[10px] font-bold ${canvasPass >= 3 ? "bg-amber-600/20 text-amber-400 border border-amber-500/30" : "bg-bg-secondary text-text-tertiary"}`}
                  >
                    👁 {t("canvas.sensory")}
                  </div>
                </div>
                <textarea
                  ...
                  onContextMenu={textMenu.openMenu}
                  className="w-full min-h-[50vh] bg-bg-primary border border-border rounded-xl p-6 text-sm leading-[2] font-serif outline-none focus:border-accent-purple transition-colors resize-y"
                />
              </div>
            )}
        ...
        {writingMode === "ai" && !showDashboard && (
          <aside
            className={`hidden lg:flex shrink-0 flex-col border-l border-border bg-bg-primary transition-all duration-300 ${rightPanelOpen ? "w-80" : "w-10"}`}
          >
            <button
              onClick={() => setRightPanelOpen(!rightPanelOpen)}
              className="w-full py-2 text-[10px] text-text-tertiary hover:text-text-primary border-b border-border"
            >
              {rightPanelOpen ? "▶" : "◀"}
            </button>
            {rightPanelOpen && (
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <details className="group" open>
                  <summary className="text-xs font-bold text-text-tertiary cursor-pointer hover:text-text-secondary">
                    👤 {t("panel.chars")} (
                    {currentSession.config.characters.length})
                  </summary>
                  <div className="mt-2 space-y-1.5">
                    {currentSession.config.characters.map((c) => (
                      <div
                        key={c.id}
                        className="text-[10px] text-text-secondary font-bold break-words"
                      >
                        {c.name} ({c.role})
                      </div>
                    ))}
                ...
                <details className="group">
                  ...
                  <div className="mt-2 pl-2">
                    <EpisodeScenePanel
                      ...
                      episodeSceneSheets={
                        currentSession.config.episodeSceneSheets ?? []
                      }
                      onSave={(sheet) => {
                        const existing =
                          currentSession.config.episodeSceneSheets ?? [];
                        ...
                        setConfig({
                          ...
                          episodeSceneSheets: [...filtered, sheet].sort(
            ...
            <div className="absolute -bottom-10 left-1/2 -translate-x-1/2 md:bottom-auto md:-top-10 md:left-4 md:translate-x-0 flex gap-2 items-center">
              <button
                onClick={() => {
                  ...
                  handleSend(t("engine.nextChapterPrompt"));
                }}
                className={`px-3 py-1.5 bg-bg-secondary border border-border rounded-full text-[10px] font-bold text-text-tertiary hover:text-text-primary transition-all whitespace-nowrap font-mono ${!hasApiKey ? "opacity-50" : ""}`}
              >
              ...
              <button
                onClick={() => {
                  ...
                  handleSend(t("engine.plotTwistPrompt"));
                }}
                className={`px-3 py-1.5 bg-bg-secondary border border-border rounded-full text-[10px] font-bold text-text-tertiary hover:text-text-primary transition-all whitespace-nowrap font-mono ${!hasApiKey ? "opacity-50" : ""}`}
              >
                {t("engine.plotTwist")}
              </button>
              {currentSession.config.episode <
                currentSession.config.totalEpisodes && (
                <button
                  onClick={handleNextEpisode}
                  className="px-3 py-1.5 bg-accent-purple/10 border border-accent-purple/20 rounded-full text-[10px] font-bold text-accent-purple"
                >
                  EP.{currentSession.config.episode} →{" "}
            ...
            <div className="relative bg-bg-secondary border border-border rounded-2xl md:rounded-[2rem] shadow-2xl p-2 pl-4 md:pl-6 flex items-end">
              ...
              {isGenerating ? (
                <button
                  ...
                  aria-label="생성 중지"
                  className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center bg-accent-red text-white shrink-0"
                >
                ...
                <button
                  ...
                  disabled={!input.trim()}
                  aria-label="전송"
                  className={`w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl flex items-center justify-center transition-all shrink-0 ${input.trim() ? "bg-accent-purple text-white" : "bg-bg-tertiary text-text-tertiary"}`}
                >
