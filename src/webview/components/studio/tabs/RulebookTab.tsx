  ...
  return (
    <div className="max-w-5xl mx-auto py-8 px-4 md:py-12 md:px-6">
      <SceneSheet
        ...
        initialDirection={
          config.sceneDirection
            ? {
                goguma: config.sceneDirection.goguma?.map((g, i) => ({
                  id: `r-${i}`,
                  type: g.type as "goguma" | "cider",
                  intensity: g.intensity as "small" | "medium" | "large",
                  desc: g.desc,
                  episode: g.episode || 1,
                })),
                hooks: config.sceneDirection.hooks?.map((h, i) => ({
                  id: `r-${i}`,
                  position: h.position as "opening" | "middle" | "ending",
                  hookType: h.hookType,
                  desc: h.desc,
                })),
                emotions: config.sceneDirection.emotionTargets?.map((e, i) => ({
                  id: `r-${i}`,
                  position: e.position ?? i * 25,
                  emotion: e.emotion,
                  intensity: e.intensity,
                })),
                dialogueRules: config.sceneDirection.dialogueTones?.map(
                  (d, i) => ({
                    id: `r-${i}`,
                ...
                dopamines: config.sceneDirection.dopamineDevices?.map(
                  (dp, i) => ({
                    id: `r-${i}`,
                    scale: dp.scale as "micro" | "medium" | "macro",
                    device: dp.device,
                    desc: dp.desc,
                    resolved: dp.resolved ?? false,
                  }),
                ),
                cliffs: config.sceneDirection.cliffhanger
                  ? [
                      {
                        id: "r-0",
                        cliffType: config.sceneDirection.cliffhanger.cliffType,
                        desc: config.sceneDirection.cliffhanger.desc,
                        episode: config.sceneDirection.cliffhanger.episode || 1,
                      },
                    ]
                  : [],
                foreshadows: config.sceneDirection.foreshadows?.map((f, i) => ({
                  id: `r-${i}`,
                  planted: f.planted,
                  payoff: f.payoff,
                  episode: f.episode,
                ...
                pacings: config.sceneDirection.pacings?.map((p, i) => ({
                  id: `r-${i}`,
                  section: p.section,
                  percent: p.percent,
                  desc: p.desc,
                })),
                tensionPoints: config.sceneDirection.tensionCurve?.map(
                  (t, i) => ({
                    id: `r-${i}`,
                    position: t.position,
                    level: t.level,
                ...
                canons: config.sceneDirection.canonRules?.map((c, i) => ({
                  id: `r-${i}`,
                  character: c.character,
                  rule: c.rule,
                })),
                transitions: config.sceneDirection.sceneTransitions?.map(
                  (t, i) => ({
                    id: `r-${i}`,
          ...
          updateCurrentSession({
            config: {
              ...
              sceneDirection: {
                ...
                emotionTargets: data.emotions.map((e) => ({
                  emotion: e.emotion,
                  intensity: e.intensity,
                  position: e.position,
                })),
                dialogueTones: data.dialogueRules.map((d) => ({
                  character: d.character,
                  tone: d.tone,
                  notes: d.notes,
                })),
                dopamineDevices: data.dopamines.map((dp) => ({
                  scale: dp.scale,
                  device: dp.device,
                  desc: dp.desc,
                ...
                cliffhanger:
                  data.cliffs.length > 0
                    ? {
                        cliffType: data.cliffs[0].cliffType,
                        desc: data.cliffs[0].desc,
                        episode: data.cliffs[0].episode,
                      }
                    : undefined,
                foreshadows: data.foreshadows.map((f) => ({
                  planted: f.planted,
                  payoff: f.payoff,
                  episode: f.episode,
                  resolved: f.resolved,
                })),
                pacings: data.pacings.map((p) => ({
                  section: p.section,
                  percent: p.percent,
                  desc: p.desc,
                })),
                tensionCurve: data.tensionPoints.map((t) => ({
                  position: t.position,
                  level: t.level,
                  label: t.label,
                })),
                canonRules: data.canons.map((c) => ({
                  character: c.character,
                  rule: c.rule,
                })),
                sceneTransitions: data.transitions.map((t) => ({
                  fromScene: t.fromScene,
                  toScene: t.toScene,
                  method: t.method,
                })),
      ...
      <div className="flex justify-end mt-4">
        ...
        >
          <span
            className={`transition-transform duration-200 ${saveFlash ? "animate-icon-pop" : "group-hover:scale-110"}`}
          >
            {saveFlash ? "✓" : "💾"}
          </span>
