...
const DynSkeleton = () => (
  <div className="h-20 w-full animate-pulse bg-white/5 rounded-lg mb-2" />
);
...
const SuggestionPanel = dynamic(
  () => import("@/components/studio/SuggestionPanel"),
  { ssr: false, loading: DynSkeleton },
);
const PipelineProgress = dynamic(
  () => import("@/components/studio/PipelineProgress"),
  { ssr: false, loading: DynSkeleton },
);
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-bg-primary/40 backdrop-blur-md shrink-0">
          <div className="flex items-center gap-2">
            ...
            <h3
              ...
              title={
                backendLabel
                  ? `${t("writingMode.nowWriterBadge")} · ${backendLabel}`
                  : t("writingMode.nowWriterBadge")
              }
            >
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-thin scrollbar-thumb-white/5 pb-6 custom-scrollbar">
          ...
          <div className="border-b border-border bg-bg-primary/20">
            ...
            >
              ...
              <ChevronDown
                className={`w-3.5 h-3.5 text-text-tertiary transition-transform duration-300 ${refOpen ? "rotate-180" : ""}`}
              />
            </button>
                  ...
                  <div className="pl-4 border-l border-white/5 space-y-1.5">
                    {currentSession.config.characters.length > 0 ? (
                      currentSession.config.characters.map((c) => (
                        <div
                          key={c.id}
                          className="text-[10px] text-text-tertiary leading-tight"
                        >
                      ...
                      <p className="text-[10px] text-text-tertiary italic opacity-40">
                        {t("panel.none")}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="pl-4 border-l border-white/5 space-y-2">
                    {currentSession.config.sceneDirection ? (
                      <div className="space-y-1">
                        {currentSession.config.sceneDirection.hooks
                          ?.slice(0, 2)
                          .map((h, i) => (
                            <div
                              key={i}
                              className="text-[10px] text-blue-400/80 truncate opacity-90"
                            >
                              ⚓ {h.desc}
                            </div>
                          ))}
                        {currentSession.config.sceneDirection.cliffhanger && (
                          <div className="text-[10px] text-accent-red/80 truncate opacity-90">
                            🔚{" "}
                            {
                              currentSession.config.sceneDirection.cliffhanger
                                .desc
                ...
                <div className="flex items-center gap-2 pt-2 border-t border-white/5">
                  <div
                    className={`w-2 h-2 rounded-full animate-pulse ${
                      hfcpState.verdict === "engagement"
                        ? "bg-accent-green shadow-[0_0_8px_rgba(34,197,94,0.5)]"
                        : hfcpState.verdict === "normal_free"
                  ...
                  <span className="text-[10px] font-black text-text-secondary font-mono tracking-tighter">
                    ...
                    )[hfcpState.verdict] || hfcpState.verdict}
                  </span>
                  <span className="text-[9px] text-text-tertiary ml-auto opacity-50">
                    SCORE: {Math.round(hfcpState.score)}
                  </span>
                </div>
              </div>
            )}
          ...
          {(directorReport ||
            (pipelineResult && pipelineResult.stages.length > 0)) && (
              ...
              <div className="py-12 flex flex-col items-center justify-center text-center space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[10px] text-text-tertiary italic max-w-[180px] leading-relaxed mx-auto">
                    {isKO
                      ? "생성 중인 소설의 개연성이나 설정을 질문해보세요."
                      : "Ask about consistency or settings of your story."}
                  </p>
                </div>
              </div>
            ) : (
        ...
        <div className="p-5 bg-bg-primary/60 backdrop-blur-xl border-t border-border shrink-0">
          <div className="relative group bg-bg-secondary/80 border border-border rounded-2xl p-1.5 focus-within:border-accent-purple/40 transition-all shadow-[0_4px_24px_rgba(0,0,0,0.2)] focus-within:shadow-[0_8px_32px_rgba(139,92,246,0.15)]">
            <div className="flex items-end gap-2 px-2 py-1.5">
              <textarea
                ...
                placeholder={
                  loading ? t("engine.thinking") : t("ui.askAnything")
                }
              ...
              {loading ? (
                ...
                >
                  <Send
                    className={`w-4 h-4 ${input.trim() ? "animate-in zoom-in duration-300" : ""}`}
                  />
                </button>
              )}
            </div>
          ...
          <div className="mt-3 flex items-center justify-center gap-1.5 opacity-40">
            ...
            <p className="text-[9px] text-text-tertiary font-mono uppercase tracking-widest font-bold">
              {isKO
                ? "독립 어시스턴트 프로토콜"
                : "Independent Assistant Protocol"}
            </p>
          </div>
        </div>
      </aside>
