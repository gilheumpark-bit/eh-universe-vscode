...
const CharacterTab: React.FC<CharacterTabProps> = ({
  ...
  const handleAutoGenerate = async () => {
    if (!activeSupportsStructured()) {
      const msg = {
        KO: "현재 프로바이더는 구조화 생성을 지원하지 않습니다. Gemini를 사용해주세요.",
        EN: "Current provider doesn't support structured generation. Please use Gemini.",
        JP: "現在のプロバイダーは構造化生成に対応していません。Geminiをご利用ください。",
        CN: "当前提供商不支持结构化生成，请使用Gemini。",
      }[language];
    ...
    try {
      ...
      const msg = {
        KO: "캐릭터 생성 중 오류가 발생했습니다.",
        EN: "Error generating characters.",
        JP: "キャラクター生成中にエラーが発生しました。",
        CN: "生成角色时出错。",
      }[language];
      setUxError({ error: new Error(msg) });
    } finally {
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 pt-5 pb-3">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          <div className="flex gap-2 p-1.5 bg-bg-secondary/30 backdrop-blur-sm border border-border rounded-2xl">
            ...
            >
              <span
                className={`text-lg transition-transform duration-200 ${charSubTab === "characters" ? "scale-110" : "group-hover:scale-105"}`}
              >
                👥
              </span>
            ...
            >
              <span
                className={`text-lg transition-transform duration-200 ${charSubTab === "items" ? "scale-110" : "group-hover:scale-105"}`}
              >
                ⚔️
              </span>
              <span className="uppercase tracking-wider">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <span
                className={`text-base transition-transform duration-200 ${saveFlash ? "animate-icon-pop" : "group-hover:scale-110"}`}
              >
                {saveFlash ? "✓" : "💾"}
              </span>
