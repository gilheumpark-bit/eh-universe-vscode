...
const ScenePlayer = dynamic(() => import("@/components/studio/ScenePlayer"), {
  ssr: false,
});
const SceneTimeline = dynamic(
  () => import("@/components/studio/SceneTimeline"),
  { ssr: false },
);
...
const ManuscriptTab: React.FC<ManuscriptTabProps> = ({
  ...
  const [sceneMode, setSceneMode] = useState<
    "off" | "radio" | "visual" | "edit"
  >("off");
  const [parsedScenes, setParsedScenes] = useState<ParsedScene[]>([]);

  // 현재 선택된 에피소드 원고에서 장면 파싱
  const handleSceneMode = useCallback(
    (mode: "radio" | "visual" | "edit") => {
      const manuscripts = config.manuscripts ?? [];
      const latestMs = manuscripts[manuscripts.length - 1];
      if (!latestMs?.content) {
        alert(
          language === "KO"
            ? "원고를 먼저 작성해주세요. 집필 탭에서 에피소드를 생성하세요."
            : "Please write a manuscript first. Create an episode in the Writing tab.",
        );
        return;
      }
      ...
      <div className="max-w-6xl mx-auto px-4 pt-4 flex gap-2">
        ...
        <button
          onClick={() => {
            ...
            if (!showTranslation) setShowDashboard(false);
          }}
        ...
        >
          ...
          {language === "KO" ? "③ 비주얼 노벨" : "③ Visual Novel"}
        </button>
      </div>
      {showShare && (
        <ShareToNetwork
