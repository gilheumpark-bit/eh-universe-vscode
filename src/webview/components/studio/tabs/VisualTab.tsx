...
} from "@/lib/visual-defaults";
import {
  buildFinalVisualPrompt,
  buildNegativePrompt,
...
function SceneGallery({
  ...
  const allImages = useMemo(() => {
    const imgs: Array<
      GeneratedVisualAsset & { cardTitle: string; cardId: string }
    > = [];
    for (const card of cards) {
      for (const img of card.generatedImages ?? []) {
        imgs.push({
          ...img,
          cardTitle: card.title || `EP${card.episode} Card`,
          cardId: card.id,
        });
      }
    }
  ...
  const filtered = useMemo(() => {
    let list = allImages;
    if (filterEp !== "all")
      list = list.filter((i) => i.assignedEpisode === filterEp);
  ...
  const timeline = useMemo(() => {
    const map = new Map<
      number,
      Array<GeneratedVisualAsset & { cardTitle: string }>
    >();
    ...
    return (
      <div className="flex flex-col items-center justify-center h-48 text-center">
        ...
        <p className="text-[12px] text-text-tertiary">
          {isKO
            ? "생성된 이미지가 없습니다. 카드에서 이미지를 생성하세요."
            : "No images yet. Generate images from cards."}
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
            ...
            <button
              ...
              className={`shrink-0 flex flex-col items-center gap-1 px-3 py-2 rounded-xl border transition-all ${
                filterEp === ep
                  ...
                  : "border-border bg-bg-secondary/50 hover:border-border"
              }`}
            >
      ...
      <div className="columns-2 md:columns-3 lg:columns-4 gap-3 space-y-3">
        {filtered.map((img) => (
          <div
            key={img.id}
            className="break-inside-avoid rounded-xl overflow-hidden border border-border/40 bg-bg-secondary/50 group"
          >
            <div className="relative">
                ...
                <button
                  ...
                  className={`p-1.5 rounded-lg transition-colors ${
                    img.favorite
                      ...
                      : "bg-bg-primary/60 text-text-tertiary hover:text-accent-amber"
                  }`}
                >
                  <Star
            ...
            <div className="px-2.5 py-2 space-y-1.5">
              ...
              <div className="flex items-center gap-1.5">
                ...
                {img.favorite && (
                  <Star
                    className="w-2.5 h-2.5 text-accent-amber"
                    fill="currentColor"
                  />
                )}
              </div>
...
function useBatchGeneration(
  ...
  const start = useCallback(async () => {
    ...
    const pending = cards.filter(
      (c) => !(c.generatedImages && c.generatedImages.length > 0),
    );
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      setProgress((p) => ({
        ...p,
        current: i + 1,
        currentCardTitle: card.title || `EP${card.episode} Card`,
      }));
      ...
      if (!prompt) {
        setProgress((p) => ({
          ...p,
          errors: [...p.errors, `${card.title || card.id}: empty prompt`],
        }));
        continue;
      }
      ...
      try {
        ...
        if (result.error) {
          setProgress((p) => ({
            ...p,
            errors: [...p.errors, `${card.title || card.id}: ${result.error}`],
          }));
          ...
          updateCard({
            ...card,
            generatedImages: [asset, ...(card.generatedImages ?? [])].slice(
              0,
              8,
            ),
          });
          setProgress((p) => ({
            ...p,
            completed: [...p.completed, card.title || card.id],
          }));
        }
      } catch {
        setProgress((p) => ({
          ...p,
          errors: [...p.errors, `${card.title || card.id}: network error`],
        }));
      }
    }
...
export default function VisualTab({
  ...
  const deleteGalleryImage = (cardId: string, imageId: string) => {
    ...
    updateCard({
      ...card,
      generatedImages: (card.generatedImages ?? []).filter(
        (img) => img.id !== imageId,
      ),
    });
  };
      ...
      <div className="w-full lg:w-64 shrink-0 space-y-4">
        ...
        <div className="ds-card flex gap-1 p-1">
          ...
          >
            ...
            {totalImages > 0 && (
              <span className="ml-0.5 px-1.5 py-0.5 rounded-full bg-purple-600/30 text-[8px] text-purple-300">
                {totalImages}
              </span>
            )}
          </button>
        ...
        <div className="ds-card">
          ...
          {analyzedEpisodes.length === 0 ? (
            <p className="text-[11px] text-text-tertiary">
              {isKO
                ? "원고탭에서 챕터 분석을 먼저 실행하세요"
                : "Run chapter analysis in Manuscript tab first"}
            </p>
          ) : (
              ...
              return (
                ...
                >
                  <div className="font-semibold truncate flex items-center gap-1.5">
                    ...
                    {imgCount > 0 && (
                      <span className="text-[8px] px-1 py-0.5 rounded bg-green-600/20 text-green-400">
                        {imgCount}
                      </span>
                    )}
                  </div>
            ...
            {batch.progress.running ? (
              <div className="space-y-2">
                ...
                <div className="w-full bg-black/50 rounded-full h-1.5">
                  <div
                    ...
                    style={{
                      width: `${(batch.progress.current / Math.max(batch.progress.total, 1)) * 100}%`,
                    }}
                  />
                </div>
                <button
              ...
              <>
                ...
                >
                  ...
                  {isKO
                    ? `이미지 없는 카드 전체 생성 (${cardsWithoutImages})`
                    : `Generate All (${cardsWithoutImages} pending)`}
                </button>
                {batch.progress.completed.length > 0 && (
                  <p className="text-[9px] text-green-400">
                    {isKO
                      ? `완료: ${batch.progress.completed.length}건`
                      : `Done: ${batch.progress.completed.length}`}
                  </p>
                )}
                {batch.progress.errors.length > 0 && (
                  <div className="text-[9px] text-red-400 space-y-0.5">
                    {batch.progress.errors.slice(0, 3).map((e, i) => (
                      <p key={i}>{e}</p>
                    ))}
                  </div>
                )}
        ...
        <div className="ds-card">
          <button
            onClick={() => setShowImgSettings(!showImgSettings)}
          ...
          {showImgSettings && (
            <div className="mt-3 space-y-2">
              <select
                value={imgProvider}
                onChange={(e) =>
                  setImgProvider(e.target.value as ImageGenProvider)
                }
              ...
              {imgApiKey && (
                <p className="text-[9px] text-green-400">
                  {isKO
                    ? '키 설정됨 — 카드 편집기에서 "이미지 생성" 버튼 사용 가능'
                    : 'Key set — "Generate" button available in card editor'}
                </p>
              )}
            </div>
          )}
        ...
        {activeView === "gallery" ? (
          ...
          <div className="flex flex-col items-center justify-center h-64 text-center">
            ...
            <h3 className="text-lg font-black text-text-tertiary mb-2">
              {isKO
                ? "비주얼 카드를 선택하거나 생성하세요"
                : "Select or create a visual card"}
            </h3>
            ...
            >
              <Plus className="w-4 h-4" />{" "}
              {isKO ? "빈 카드 만들기" : "Create Empty Card"}
            </button>
          </div>
        )}
      </div>
