...
function WorldMap({ simData, language, onChange, highlightEra }: Props) {
  ...
  const civColors = Object.fromEntries(
    (simData.civs || []).map((c) => [c.name, c.color || "#6b7280"]),
  );
  ...
  const activeCivNames = useMemo(() => {
    ...
    (simData.civs || []).forEach((c) => {
      if (c.era === highlightEra) names.add(c.name);
    });
    return names;
  ...
  const addTerritory = () => {
    const id = `t-${Date.now()}`;
    const name = isKO
      ? `영역 ${territories.length + 1}`
      : `Region ${territories.length + 1}`;
    ...
    if (linkMode) {
      if (!linkFrom) {
        ...
        const exists = links.some(
          (l) =>
            (l.from === linkFrom && l.to === id) ||
            (l.from === id && l.to === linkFrom),
        );
        if (!exists) {
          onChange({
            territoryLinks: [
              ...links,
              { from: linkFrom, to: id, type: linkType },
            ],
          });
        }
        setLinkFrom(null);
    ...
    <div className="space-y-3">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest border transition-all active:scale-95 ${
              linkMode
                ? "bg-[linear-gradient(135deg,rgba(255,100,50,0.2),rgba(200,50,20,0.1))] text-[rgba(255,150,100,0.9)] border-[rgba(255,100,50,0.5)] shadow-[0_0_15px_rgba(255,100,50,0.2)]"
                : "bg-[rgba(255,200,50,0.05)] text-[rgba(255,220,100,0.7)] border-[rgba(255,200,50,0.2)] hover:border-[rgba(255,200,50,0.5)] hover:text-[rgba(255,220,100,0.95)]"
            }`}
          >
      ...
      {linkMode && (
        <div className="flex flex-wrap gap-1 items-center bg-[rgba(255,200,50,0.02)] border border-[rgba(255,200,50,0.15)] rounded-xl p-2 backdrop-blur-sm">
          <span className="text-[9px] text-[rgba(255,200,50,0.6)] font-mono px-1">
            {isKO ? "연결 유형:" : "Link type:"}
          </span>
            ...
            <button
              ...
              className={`px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${
                linkType === type
                  ...
                  : "text-[rgba(255,200,50,0.5)] border-[rgba(255,200,50,0.2)] hover:text-[rgba(255,200,50,0.8)] hover:bg-[rgba(255,200,50,0.05)]"
              }`}
              style={
                linkType === type
                  ? {
                      background: LINK_COLORS[type],
          ...
          {linkFrom && (
            <span className="text-[9px] text-amber-400 ml-2">
              {isKO
                ? `"${getT(linkFrom)?.name}" 에서 → 대상 클릭`
                : `From "${getT(linkFrom)?.name}" → click target`}
            </span>
          )}
        </div>
      )}
      ...
      <svg
        ...
        className="w-full border border-[rgba(255,200,50,0.3)] rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.5),inset_0_0_20px_rgba(255,200,50,0.05)]"
        style={{
          touchAction: "none",
        ...
        role="img"
        aria-label={isKO ? "세계관 영토 지도" : "World territory map"}
      >
        {/* Grid + Animations */}
        <defs>
          <pattern
          ...
          return (
            <g key={`link-${i}`}>
              <line
                ...
                strokeWidth="1.5"
                strokeDasharray={link.type === "border" ? "4,4" : undefined}
                opacity="0.6"
              />
              <text
                x={(from.x + to.x) / 2}
        ...
        {territories.map((t) => {
          ...
          const isEraActive =
            activeCivNames == null || activeCivNames.has(t.civName);
          ...
          return (
            ...
            >
              ...
              <circle
                ...
                opacity="0.1"
                stroke={color}
                strokeWidth={isLinkTarget ? 3 : 1.5}
                strokeDasharray={isLinkTarget ? "4,2" : undefined}
                className={
                  isEraActive && activeCivNames != null
                    ? "era-active-border"
                    : undefined
                }
              />
        ...
        {territories.length === 0 && (
          <g>
            ...
            >
              {isKO
                ? '"영역 추가" 버튼을 눌러 지도를 구성하세요'
                : 'Click "Add Region" to build your map'}
            </text>
          </g>
        )}
      </svg>
      ...
      {territories.length > 0 && (
        <div className="space-y-1">
          {territories.map((t) => (
            <div
              key={t.id}
              className="flex items-center gap-2 bg-[linear-gradient(135deg,rgba(255,200,50,0.02),rgba(0,0,0,0.3))] border border-[rgba(255,200,50,0.15)] rounded-lg px-3 py-1.5 backdrop-blur-sm transition-all hover:bg-[rgba(255,200,50,0.05)] hover:border-[rgba(255,200,50,0.3)]"
            >
              <span
                className="w-2.5 h-2.5 rounded-full shrink-0 shadow-[0_0_5px_currentColor]"
                style={{
                  background: t.color || civColors[t.civName] || "#6b7280",
                  color: t.color || civColors[t.civName] || "#6b7280",
                }}
              />
              <input
                value={t.name}
                onChange={(e) =>
                  updateTerritory(t.id, { name: e.target.value })
                }
              ...
              <select
                value={t.civName}
                onChange={(e) =>
                  updateTerritory(t.id, {
                    civName: e.target.value,
                    color: civColors[e.target.value],
                  })
                }
              ...
              <button
                ...
                className="text-[rgba(255,200,50,0.5)] hover:text-red-400 hover:drop-shadow-[0_0_5px_rgba(255,50,50,0.5)] transition-all"
              >
      ...
      <div className="flex flex-wrap gap-3 text-[9px] bg-[rgba(255,200,50,0.02)] border border-[rgba(255,200,50,0.1)] p-2 rounded-lg backdrop-blur-sm">
        {(Object.entries(LINK_COLORS) as [TerritoryLink["type"], string][]).map(
          ([type, color]) => (
            <span key={type} className="flex items-center gap-1.5">
              <span
                className="w-4 h-0.5 inline-block rounded shadow-[0_0_5px_currentColor]"
                style={{ background: color, color: color }}
              />
              <span className="text-[rgba(230,220,180,0.8)] font-mono">
                {type}
              </span>
            </span>
          ),
