...
const ChatMarkdownBlock = dynamic(
  ...
  {
    ssr: false,
    loading: () => <span className="text-text-tertiary text-xs">…</span>,
  },
);
...
const ChatMessage: React.FC<ChatMessageProps> = ({
  ...
  return (
    <div
      className={`flex w-full ${isCompact ? "gap-2" : "gap-3 md:gap-4"} group ${isUser ? "flex-row-reverse" : "flex-row"}`}
    >
      <div
        className={`shrink-0 ${isCompact ? "w-6 h-6" : "w-8 h-8"} rounded-lg flex items-center justify-center border shadow-lg ${
          isUser
            ? "bg-bg-tertiary border-border"
            : "bg-linear-to-br from-blue-600 to-blue-800 border-blue-500"
        }`}
      >
        {isUser ? (
          <User
          ...
          <Bot
            className={isCompact ? "w-3 h-3 text-white" : "w-4 h-4 text-white"}
          />
        )}
      </div>
      ...
      >
        ...
        >
          {isUser ? (
            <p
              className={`${isCompact ? "text-[11px]" : "text-sm"} leading-relaxed whitespace-pre-wrap`}
            >
              {mainContent}
            </p>
          ) : (
            <div
              className={`prose ${isCompact ? "prose-xs" : "prose-sm sm:prose-base"} prose-invert max-w-none break-words prose-p:font-serif prose-p:text-text-secondary prose-p:leading-[1.8]`}
            >
              <ChatMarkdownBlock
                mainContent={mainContent}
                isCompact={isCompact}
          ...
          {!isUser && report && !isCompact && (
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <span
                className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black border ${
                  report.eosScore >= 40
              ...
              <span className="inline-flex items-center gap-1 px-2 py-1 bg-bg-secondary/50 border border-border/50 rounded-lg text-[9px] font-black text-text-tertiary">
                <Activity className="w-2.5 h-2.5" /> {report.actPosition.act}
                {language === "KO" ? "막" : ""}
              </span>
              <span
                className={`inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-black border ${
          ...
          {!isUser && report && (
            <div className="mt-3">
              <button
                ...
                className="flex items-center gap-1 text-[9px] text-text-tertiary hover:text-text-secondary transition-colors font-mono uppercase tracking-wider"
              >
                <ChevronDown
              ...
              {showDetail && (
                <div className="mt-2 p-3 bg-bg-secondary/50 border border-border/50 rounded-xl space-y-2 text-[10px] font-mono animate-in fade-in duration-300">
                  ...
                  <div className="flex justify-between text-text-tertiary">
                    <span>
                      {language === "KO" ? "텐션 타겟" : "Tension Target"}
                    </span>
                    <span>
                      {report.tensionTarget}% → {report.metrics.tension}%
                    </span>
                  </div>
                  {report.fixes.length > 0 && (
                    <div className="border-t border-border/50 pt-2 space-y-1">
                      ...
                      {report.fixes.length > 3 && (
                        <div className="text-text-tertiary">
                          +{report.fixes.length - 3}
                          {language === "KO" ? "건 더" : " more"}
                        </div>
                      )}
                    </div>
                  )}
                  ...
                  {report.fixes.length > 0 && onAutoFix && (
                    <button
                      onClick={() => onAutoFix(message.id)}
                      className="mt-2 w-full flex items-center justify-center gap-1.5 py-2 bg-accent-purple/10 border border-accent-purple/30 rounded-lg text-accent-purple text-[10px] font-bold uppercase tracking-wider hover:bg-accent-purple/20 transition-colors"
                    >
                      <Wrench className="w-3 h-3" />{" "}
                      {language === "KO" ? "자동 수정 적용" : "Apply Auto-Fix"}
                    </button>
                  )}
                </div>
              )}
              ...
              >
                ...
                {(message.meta.qualityFindings?.length ?? 0) > 0 && (
                  <span className="text-[8px] opacity-70">
                    ({message.meta.qualityFindings?.length})
                  </span>
                )}
              </button>
              {showDetail &&
                message.meta.qualityFindings &&
                message.meta.qualityFindings.length > 0 && (
            ...
            <div className="mt-8 p-4 md:p-6 bg-bg-secondary/50 border border-border rounded-2xl space-y-4 animate-in fade-in duration-500">
              <div className="flex justify-between items-center text-[9px] font-black text-text-tertiary uppercase tracking-widest">
                <div className="flex items-center gap-2">
                  <Activity className="w-3 h-3 text-blue-500" /> Engine Report
                </div>
                {displayGrade && (
                  <div className="text-blue-500">{displayGrade} Grade</div>
                )}
              </div>
              {displayMetrics && (
                <div className="grid grid-cols-3 gap-4">
                  {Object.entries(displayMetrics as Record<string, number>).map(
                    ([k, v]) => (
                      <div key={k} className="space-y-1">
                        <div className="flex justify-between text-[7px] font-black text-text-tertiary uppercase">
                          <span>{k}</span>
                          <span>{v}%</span>
                        </div>
          ...
          <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-all">
            <ActionBar
              ...
              onFeedback={(positive) => {
                logger.info(
                  "feedback",
                  `AI message ${message.id}: ${positive ? "positive" : "negative"}`,
                );
              }}
            />
            {onRegenerate && (
