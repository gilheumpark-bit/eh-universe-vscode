...
function extractEmotionArc(messages: Message[]): EmotionPoint[] {
  ...
  for (const msg of messages) {
    ...
    const emotionScore = Math.round(
      tension * 0.4 + immersion * 0.35 + pacing * 0.25,
    );
...
function findPeaksAndValleys(points: EmotionPoint[]): {
  peaks: number[];
  valleys: number[];
} {
  const peaks: number[] = [];
...
function EmotionArcChart({ messages, language }: Props) {
  ...
  const { peaks, valleys } = useMemo(
    () => findPeaksAndValleys(points),
    [points],
  );
  ...
  if (points.length < 2) {
    return (
      <div className="text-center py-6 text-text-tertiary text-[10px]">
        {language === "KO"
          ? "감정 아크를 표시하려면 최소 2개 챕터가 필요합니다."
          : "Need at least 2 chapters for emotion arc."}
      </div>
    );
  }
  ...
  const linePoints = points
    .map((p, i) => `${toX(i)},${toY(p.emotionScore)}`)
    .join(" ");
  // 영역 채움
  ...
  const avgScore = Math.round(
    points.reduce((s, p) => s + p.emotionScore, 0) / points.length,
  );
  const variance = Math.round(
    Math.sqrt(
      points.reduce((s, p) => s + (p.emotionScore - avgScore) ** 2, 0) /
        points.length,
    ),
  );
    ...
    <div className="bg-bg-secondary border border-border rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3 text-[8px] text-text-tertiary">
          <span>
            AVG:{" "}
            <span className="text-text-secondary font-bold">{avgScore}%</span>
          </span>
          <span>
            {L4(language, { ko: "변동", en: "Var", ja: "変動", zh: "变动" })}:{" "}
      ...
      >
        ...
        {valleys.map((i) => (
          <g key={`valley-${i}`}>
            ...
            <text
              x={toX(i)}
              y={toY(points[i].emotionScore) + 12}
              fill="#3b82f6"
              fontSize="7"
              textAnchor="middle"
            >
      ...
      <div className="flex flex-wrap gap-4 text-[8px]">
        ...
        <span className="flex items-center gap-1 opacity-50">
          <span
            className="w-4 h-0.5 bg-red-500 inline-block rounded"
            style={{
              backgroundImage:
                "repeating-linear-gradient(90deg, #ef4444 0 3px, transparent 3px 6px)",
            }}
          />
          {L4(language, {
            ko: "긴장감",
        ...
        <span className="flex items-center gap-1 opacity-50">
          <span
            className="w-4 h-0.5 bg-green-500 inline-block rounded"
            style={{
              backgroundImage:
                "repeating-linear-gradient(90deg, #22c55e 0 3px, transparent 3px 6px)",
            }}
          />
          {L4(language, {
            ko: "몰입도",
        ...
        <span className="text-red-400">
          ▲ {L4(language, { ko: "피크", en: "Peak", ja: "ピーク", zh: "峰值" })}
        </span>
        <span className="text-blue-400">
          ▼ {L4(language, { ko: "밸리", en: "Valley", ja: "谷", zh: "谷值" })}
        </span>
      </div>
