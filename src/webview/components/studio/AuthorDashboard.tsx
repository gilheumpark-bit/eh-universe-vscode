...
function MetricBar({
  ...
  return (
    <div
      ...
      title={`${label}: ${value}% (max ${max}%)`}
    >
      <span className="text-[9px] text-text-tertiary font-mono w-16 shrink-0 uppercase">
        {label}
      </span>
      <div
...
function AuthorDashboard({ messages, language }: Props) {
  ...
  useEffect(() => {
    if (!flags.EPISODE_COMPARE && subTab === "compare")
      queueMicrotask(() => setSubTab("overview"));
  }, [flags.EPISODE_COMPARE, subTab]);

  const subTabEntries = useMemo(() => {
    const rows: [DashSubTab, string][] = [
  ...
  const avg = {
    tension: Math.round(
      metrics.reduce((s, m) => s + m.tension, 0) / metrics.length,
    ),
    pacing: Math.round(
      metrics.reduce((s, m) => s + m.pacing, 0) / metrics.length,
    ),
    immersion: Math.round(
      metrics.reduce((s, m) => s + m.immersion, 0) / metrics.length,
    ),
    eos: Math.round(metrics.reduce((s, m) => s + m.eos, 0) / metrics.length),
    charCount: Math.round(
      metrics.reduce((s, m) => s + m.charCount, 0) / metrics.length,
    ),
  };
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            ...
            <div className="bg-bg-secondary border border-border rounded-xl p-3 text-center">
              ...
              <div className="text-[9px] text-text-tertiary font-mono uppercase">
                {language === "KO" ? "총 글자수" : "Total chars"}
              </div>
            </div>
            <div className="bg-bg-secondary border border-border rounded-xl p-3 text-center">
              <div className="text-lg font-black text-text-primary">
                {avg.charCount.toLocaleString()}
              </div>
              <div className="text-[9px] text-text-tertiary font-mono uppercase">
                {language === "KO" ? "평균 글자수" : "Avg chars"}
              </div>
            </div>
            <div className="bg-bg-secondary border border-border rounded-xl p-3 text-center">
              <div
                className={`text-lg font-black ${GRADE_COLORS[metrics[metrics.length - 1]?.grade] || "text-text-primary"}`}
              >
                {metrics[metrics.length - 1]?.grade || "—"}
              </div>
              <div className="text-[9px] text-text-tertiary font-mono uppercase">
                {language === "KO" ? "최근 등급" : "Last grade"}
              </div>
            </div>
          </div>
          ...
          <div className="bg-bg-secondary border border-border rounded-xl p-4">
            ...
            {metrics.length >= 2 ? (
              <svg
                viewBox={`0 0 ${Math.max(200, metrics.length * 30)} 100`}
                ...
                aria-label={
                  language === "KO"
                    ? "챕터별 메트릭 추세 차트"
                    : "Per-chapter metric trend chart"
                }
              >
                {/* Grid lines */}
                {[25, 50, 75].map((y) => (
                ...
                <polyline
                  ...
                  strokeDasharray="3,3"
                  points={metrics
                    .map((m, i) => `${i * 30 + 15},${100 - m.immersion}`)
                    .join(" ")}
                />
                {/* Grade dots */}
                {metrics.map((m, i) => (
                  <circle
                    ...
                    fill={
                      GRADE_COLORS[m.grade]
                        ...
                        .replace("-400", "") === "yellow"
                        ? "#facc15"
                        : GRADE_COLORS[m.grade]?.includes("green")
                          ? "#22c55e"
                          : GRADE_COLORS[m.grade]?.includes("blue")
                            ? "#3b82f6"
                            : GRADE_COLORS[m.grade]?.includes("red")
                              ? "#ef4444"
                              : "#a855f7"
                    }
                  >
                    <title>
                      #{m.index} {m.grade} T:{m.tension}% P:{m.pacing}% I:
                      {m.immersion}%
                    </title>
                  </circle>
                ))}
                ...
                {metrics.map((m, i) => {
                  ...
                  return (
                    <div
                      ...
                      title={`#${m.index} ${m.grade} T:${m.tension}%`}
                    >
                      <div
                        className={`w-full rounded-t ${GRADE_COLORS[m.grade]?.replace("text-", "bg-") || "bg-text-tertiary"}`}
                        style={{ height: `${h}px` }}
                      />
            ...
            <div className="flex justify-between mt-1">
              ...
              <span className="text-[8px] text-text-tertiary">
                {metrics.length} {language === "KO" ? "챕터" : "chapters"}
              </span>
            </div>
          </div>
        </>
      ...
      <button
        onClick={() => {
          const lines = [
            ...
            ...metrics.map(
              (m) =>
                `| ${m.index} | ${m.grade} | ${m.tension}% | ${m.pacing}% | ${m.immersion}% | ${m.charCount.toLocaleString()} |`,
            ),
          ];
