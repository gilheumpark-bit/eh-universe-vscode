...
function useFindReplace(
  ...
  const clampedIndex =
    matches.length > 0 ? Math.min(matchIndex, matches.length - 1) : 0;
  ...
  const replaceOne = useCallback(() => {
    ...
    onChange(
      value.slice(0, pos) + replaceText + value.slice(pos + findText.length),
    );
...
export function WritingToolbar({
  ...
  const { wrapSelection, adjustIndent } = useTextOps(
    textareaRef,
    value,
    onChange,
  );
  ...
  const btn =
    "p-1.5 rounded hover:bg-bg-primary text-text-secondary hover:text-text-primary transition-colors select-none";
    ...
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-0.5 px-2 py-1 bg-bg-secondary border border-border rounded-lg flex-wrap gap-y-1">
        ...
        <button
          ...
          title={isKO ? "강조 (괄호)" : "Emphasis"}
          className={`${btn} text-xs font-black w-6 h-6 flex items-center justify-center`}
        >
          「」
        </button>
        <button
          onClick={() => insertAtCursor("\n\n* * *\n\n")}
          title={isKO ? "장면 전환" : "Scene Break"}
          className={`${btn} text-[10px] font-black font-mono px-1.5`}
        >
          ***
        </button>
        <button
          onClick={() => insertAtCursor("\n\n────────────────\n\n")}
          title={isKO ? "구분선" : "Divider"}
          className={`${btn} text-[10px] font-mono px-1.5`}
        >
          ──
        </button>
        ...
        <button
          ...
          title="내어쓰기"
          className={`${btn} text-sm font-mono w-6 h-6 flex items-center justify-center`}
        >
          ⇤
        </button>
        <button
        ...
        <div className="ml-auto flex items-center gap-2 text-[10px] text-text-tertiary font-mono flex-wrap">
          ...
          {stats.paras > 1 && (
            <>
              <span className="text-border">|</span>
              <span title={isKO ? "단락수" : "paragraphs"}>
                {stats.paras}
                {isKO ? "단락" : "P"}
              </span>
            </>
        ...
        <div className="px-2 py-1">
          <div className="flex items-center gap-2">
            <div className="flex-1 h-1.5 bg-bg-tertiary/50 rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${
                  stats.charsNoSpace >= targetMin
                    ? "bg-accent-green"
                    : stats.charsNoSpace >= targetMin * 0.5
                      ? "bg-accent-purple"
                      : "bg-border"
                }`}
                style={{
                  width: `${Math.min(100, (stats.charsNoSpace / (targetMax || targetMin)) * 100)}%`,
                }}
              />
            </div>
            <span className="text-[9px] font-mono text-text-tertiary shrink-0">
              ...
              {targetMax ? `~${targetMax.toLocaleString()}` : ""}
            </span>
          </div>
        </div>
      )}
      ...
      {showFind && (
        <div className="flex flex-wrap gap-2 items-center px-3 py-2 bg-bg-secondary border border-border rounded-lg">
          ...
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") navigateTo(matchIndex + 1);
            }}
