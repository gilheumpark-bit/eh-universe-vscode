...
export const ConfirmModal: React.FC<ConfirmModalProps> = ({
  ...
  return (
    ...
    >
      <div
        className="bg-bg-primary border border-border rounded-xl p-6 max-w-sm w-full mx-4 shadow-2xl"
        onClick={(e) => e.stopPropagation()}
        role="dialog"
        aria-modal="true"
      >
        <div
          className={`flex items-start gap-3 p-3 rounded-lg mb-4 ${colors[variant]}`}
        >
        ...
        <div className="flex justify-end gap-2">
          <button
            onClick={onCancel}
            className="px-4 py-2 text-xs text-text-tertiary border border-border rounded-lg hover:bg-bg-secondary transition-colors"
          >
            {cancelLabel}
          </button>
          <button
            onClick={onConfirm}
            className={`px-4 py-2 text-xs text-white rounded-lg transition-colors ${btnColors[variant]}`}
          >
            {confirmLabel}
          </button>
        </div>
...
function classifyError(err: unknown, language: AppLanguage): ErrorInfo {
  ...
  if (
    ...
    return {
      ...
      message: L4(language, {
        ko: "AI 서버가 일시적으로 응답하지 않습니다. 잠시 후 다시 시도해주세요.",
        en: "AI server is temporarily unavailable. Please try again shortly.",
        ja: "AIサーバーが一時的に応答していません。しばらくしてから再試行してください。",
        zh: "AI服务器暂时无法响应，请稍后重试。",
      }),
    };
  }
  if (lower.includes("404") || lower.includes("not found")) {
    return {
      ...
      message: L4(language, {
        ko: "요청한 API 경로를 찾을 수 없습니다. 새로고침 후 다시 시도해주세요.",
        en: "API endpoint not found. Please refresh and try again.",
        ja: "リクエストしたAPIパスが見つかりません。ページを更新してから再試行してください。",
        zh: "找不到请求的API路径，请刷新页面后重试。",
      }),
    };
  }
  if (
    ...
    <div className="fixed bottom-4 right-4 z-[9990] max-w-sm w-full mx-4 animate-in slide-in-from-bottom-4">
      <div className="bg-red-950/90 border border-red-500/30 rounded-xl p-4 shadow-2xl backdrop-blur-sm">
        ...
        {onRetry && info.retryable !== false && (
          <button
            onClick={onRetry}
            className="mt-3 w-full px-3 py-1.5 bg-red-500/20 border border-red-500/30 rounded-lg text-xs text-red-300 hover:bg-red-500/30 transition-colors"
          >
...
export const EmptyStateCompact: React.FC<
  Omit<EmptyStateProps, "secondaryLabel" | "onSecondary">
> = ({
  variant = "generic",
  title,
  description,
...
export const ProgressBar: React.FC<ProgressBarProps> = ({
  ...
  const gradients = {
    ...
    gradient:
      "bg-gradient-to-r from-accent-purple via-accent-blue to-accent-green",
  ...
  return (
    <div className={`w-full ${className}`}>
      ...
      >
        {indeterminate ? (
          <div
            className={`${height} w-1/3 rounded-full ${gradients[variant]} animate-[progress-slide_1.5s_ease-in-out_infinite]`}
          />
        ) : (
          <div
...
export const FeedbackBadge: React.FC<FeedbackBadgeProps> = ({
  ...
  const styles = {
    ...
    error:
      "bg-accent-red/15 border-accent-red/30 text-accent-red animate-error-shake",
...
export const LoadingSpinner: React.FC<{
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div
        className={`${sizes[size]} border-2 border-accent-purple/30 border-t-accent-purple rounded-full animate-spin`}
      />
      {message && (
