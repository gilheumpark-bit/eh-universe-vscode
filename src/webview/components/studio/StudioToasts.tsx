    ...
    <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[200] flex flex-col gap-2 w-full max-w-md px-4">
        ...
        <ToastCard variant="info" onClose={() => setShowSyncReminder(false)}>
          ...
          <button
            onClick={() => {
              ...
              user ? handleSync() : signInWithGoogle();
            }}
      ...
      {fallbackNotice && (
        ...
        >
          ...
          <p className="flex-1 text-sm font-medium text-text-primary">
            {isKO
              ? `AI 제공자 자동 전환: ${fallbackNotice}`
              : `Provider auto-switched: ${fallbackNotice}`}
          </p>
        </AutoDismissToast>
      )}
      ...
      {exportDoneFormat && (
        <AutoDismissToast
          variant="success"
          duration={3000}
          onDismiss={() => setExportDoneFormat(null)}
        >
      ...
      {worldImportBanner && (
        <AutoDismissToast
          variant="success"
          duration={4000}
          onDismiss={() => setWorldImportBanner(false)}
        >
          <Globe className="w-5 h-5 text-accent-green shrink-0" />
          <p className="flex-1 text-sm font-medium text-text-primary">
            {isKO
              ? "Network에서 세계관을 불러왔습니다"
              : "World imported from Network"}
          </p>
        </AutoDismissToast>
      )}
