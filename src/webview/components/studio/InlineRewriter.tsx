...
const QUICK_ACTIONS: QuickAction[] = [
  {
    ...
    promptKo:
      "아래 선택된 텍스트를 같은 의미이지만 더 나은 문장으로 다시 써줘. 앞뒤 문맥에 자연스럽게 이어져야 함. 순수 소설 텍스트만 출력, 설명 없이.",
    promptEn:
      "Rewrite the selected text with better prose while keeping the same meaning. Must flow naturally with surrounding context. Output only the rewritten text, no explanation.",
  },
  {
    id: "expand",
    ...
    promptKo:
      "아래 선택된 텍스트에 감각 묘사, 내면 독백, 환경 묘사를 추가해서 2~3배로 늘려줘. 원래 사건과 대사는 유지. 순수 소설 텍스트만 출력.",
    promptEn:
      "Expand the selected text 2-3x by adding sensory details, inner monologue, and environmental description. Keep original events/dialogue. Output only the expanded text.",
  },
  {
    id: "compress",
    ...
    promptKo:
      "아래 선택된 텍스트를 핵심만 남기고 절반 이하로 압축해줘. 불필요한 수식어, 반복, 과잉 묘사 제거. 순수 소설 텍스트만 출력.",
    promptEn:
      "Compress the selected text to half or less. Remove unnecessary modifiers, repetition, and excessive description. Output only the compressed text.",
  },
  {
    id: "tension",
    ...
    promptKo:
      "아래 선택된 텍스트의 긴장감을 높여줘. 짧은 문장, 급박한 호흡, 위기감 있는 묘사로. 원래 사건은 유지. 순수 소설 텍스트만 출력.",
    promptEn:
      "Increase tension in the selected text. Use shorter sentences, urgent pacing, and crisis atmosphere. Keep original events. Output only the rewritten text.",
  },
  {
    id: "action",
    ...
    promptKo:
      "아래 선택된 텍스트의 액션/전투 묘사를 강화해줘. 동적인 동사, 타격감, 속도감 추가. 순수 소설 텍스트만 출력.",
    promptEn:
      "Enhance action/combat in the selected text. Add dynamic verbs, impact, and speed. Output only the rewritten text.",
  },
  {
    id: "dialogue",
    ...
    promptKo:
      "아래 선택된 텍스트의 대사를 캐릭터 성격에 맞게 더 자연스럽고 개성있게 다듬어줘. 지문도 자연스럽게. 순수 소설 텍스트만 출력.",
    promptEn:
      "Polish the dialogue in the selected text to be more natural and characteristic. Improve dialogue tags too. Output only the rewritten text.",
  },
  {
    id: "insert_after",
    ...
    promptKo:
      "아래 선택된 텍스트 바로 뒤에 이어지는 전환 장면이나 묘사를 새로 써줘. 약 200~400자. 앞 내용에서 자연스럽게 이어져야 함. 순수 소설 텍스트만 출력.",
    promptEn:
      "Write a new transition scene or description to follow the selected text. About 200-400 chars. Must flow naturally. Output only the new text.",
  },
];
...
const InlineRewriter: React.FC<InlineRewriterProps> = ({
  content,
  language,
  context,
  onApply,
  ...
  const executeAction = useCallback(
    async (action: QuickAction) => {
      ...
      const before = editableContent.slice(
        Math.max(0, selection.start - 300),
        selection.start,
      );
  ...
  const applyPreview = useCallback(() => {
    ...
    if (isInsertAfter) {
      newContent =
        editableContent.slice(0, selection.end) +
        "\n" +
        preview +
        editableContent.slice(selection.end);
    } else {
      newContent =
        editableContent.slice(0, selection.start) +
        preview +
        editableContent.slice(selection.end);
    }
  ...
  return (
    <div className="space-y-3">
        ...
        <div className="bg-bg-secondary/80 border border-border/50 rounded-xl p-3 space-y-2 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-black uppercase tracking-widest text-accent-purple font-mono">
              {t("inlineRewriter.selected").replace(
                "{n}",
                String(selection.text.length),
              )}
            </span>
            <button
              onClick={() => {
              ...
              aria-label="닫기"
              className="p-1 hover:bg-bg-tertiary rounded text-text-tertiary hover:text-text-secondary"
            >
              <X className="w-3 h-3" />
            </button>
          </div>
          ...
          <div className="flex gap-1.5">
            ...
            <button
              ...
              className="px-3 py-1.5 bg-accent-purple text-white rounded-lg text-[9px] font-bold disabled:opacity-30 font-mono"
            >
        ...
        <div className="bg-bg-primary border border-blue-500/20 rounded-xl p-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-[9px] font-black uppercase tracking-widest text-blue-400 font-mono">
              {isStreaming
                ? t("inlineRewriter.generating")
                : t("inlineRewriter.preview")}
            </span>
            <div className="flex gap-1.5">
              {!isStreaming && preview && !showApplyConfirm && (
                <button
                  onClick={applyPreview}
                  className="flex items-center gap-1 px-2.5 py-1 bg-green-600/20 border border-green-500/30 rounded-lg text-[9px] font-bold text-green-400 hover:bg-green-600/30 font-mono"
                >
              ...
              {showApplyConfirm && (
                <span className="flex items-center gap-1.5">
                  <span className="text-[10px] text-amber-400 font-mono">
                    {t("inlineRewriter.replaceConfirm")}
                  </span>
                  <button
                    onClick={applyPreview}
                    className="px-2 py-1 bg-green-600/30 border border-green-500/40 rounded text-[9px] font-bold text-green-400 hover:bg-green-600/40"
                  >
                    {t("inlineRewriter.confirmYes")}
                  </button>
                  <button
                    onClick={() => setShowApplyConfirm(false)}
                    className="px-2 py-1 bg-bg-tertiary/30 border border-zinc-600/40 rounded text-[9px] text-text-secondary hover:bg-bg-tertiary/50"
                  >
              ...
              <button
                ...
                className="flex items-center gap-1 px-2.5 py-1 bg-red-600/10 border border-red-500/20 rounded-lg text-[9px] font-bold text-red-400 hover:bg-red-600/20 font-mono"
              >
                {isStreaming ? (
                  <X className="w-3 h-3" />
                ) : (
                ...
                {isStreaming
                  ? t("inlineRewriter.stop")
                  : t("inlineRewriter.cancel")}
              </button>
            </div>
          </div>
          <p className="text-[12px] leading-relaxed text-text-secondary font-serif whitespace-pre-wrap">
            {preview || "..."}
          </p>
        </div>
      )}
      ...
      <div className="flex justify-between items-center text-[9px] text-text-tertiary font-mono">
        <span>
          {editableContent.length.toLocaleString()}
          {t("inlineRewriter.charCount")}
        </span>
