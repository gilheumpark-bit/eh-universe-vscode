...
const WorldAnalysisView: React.FC<WorldAnalysisViewProps> = ({
  language,
  config,
}) => {
  ...
  const loadFromConfig = useCallback(() => {
    ...
    if (config.charRelations?.length) {
      parts.push(`[관계도]`);
      config.charRelations.forEach((r) =>
  ...
  const handleAnalyze = useCallback(async () => {
    ...
    try {
      const messages: ChatMsg[] = [{ role: "user", content: trimmed }];
      ...
      if ((err as Error).name !== "AbortError") {
        ...
        const isKeyIssue =
          /401|403|api.?key|not.?configured|unauthorized/i.test(msg);
        setError(
          isKeyIssue
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {analyzing
                ? t("worldAnalysis.analyzing")
                : t("worldAnalysis.analyze")}
            </button>
          </div>
        </div>
      </div>
      ...
      {result && (
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(Object.keys(SECTION_ICONS) as (keyof typeof SECTION_ICONS)[]).map(
              (key) => {
                ...
                let cardBg =
                  "bg-[linear-gradient(135deg,rgba(255,200,50,0.05),rgba(0,0,0,0.4))]";
                let cardBorder = "border-[rgba(255,200,50,0.15)]";
                let cardShadow =
                  "hover:shadow-[0_10px_30px_rgba(255,200,50,0.1),inset_0_0_15px_rgba(255,200,50,0.05)]";
                let iconColor = "text-[rgba(255,200,50,0.8)]";
                let titleColor = "text-[rgba(255,220,100,0.9)]";

                if (key === "summary") {
                  cardBg =
                    "bg-[linear-gradient(135deg,rgba(255,200,50,0.1),rgba(255,150,0,0.05))]";
                  cardBorder = "border-[rgba(255,200,50,0.4)]";
                  cardShadow =
                    "shadow-[0_10px_30px_rgba(255,200,50,0.1),inset_0_0_20px_rgba(255,200,50,0.05)]";
                  iconColor =
                    "text-[rgba(255,220,100,1)] drop-shadow-[0_0_8px_rgba(255,200,50,0.8)]";
                  titleColor =
                    "text-[rgba(255,220,100,1)] drop-shadow-[0_0_5px_rgba(255,200,50,0.5)]";
                } else if (isWarning) {
                  cardBg =
                    "bg-[linear-gradient(135deg,rgba(255,50,50,0.05),rgba(0,0,0,0.4))]";
                  cardBorder = "border-[rgba(255,50,50,0.3)]";
                  iconColor =
                    "text-[rgba(255,100,100,0.9)] drop-shadow-[0_0_5px_rgba(255,50,50,0.5)]";
                  titleColor = "text-[rgba(255,100,100,0.9)]";
                  cardShadow =
                    "hover:shadow-[0_10px_30px_rgba(255,50,50,0.1),inset_0_0_15px_rgba(255,50,50,0.05)]";
                }
                ...
                return (
                  ...
                  >
                    ...
                    <p className="relative z-10 text-sm leading-relaxed whitespace-pre-wrap text-[rgba(230,220,200,0.9)] font-serif">
                      {value}
                    </p>
                  </div>
                );
