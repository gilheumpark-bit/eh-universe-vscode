  ...
  return (
    <>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-3">
          {[
            {
              value: "iron" as const,
              label: isKO ? "강 (Iron)" : "Iron",
              activeClass: "bg-red-600/10 border-red-500/30 text-red-400",
              desc: isKO
                ? "인과 필수, 모든 경고 표시"
                : "Strict causality, all warnings",
            },
            {
              value: "standard" as const,
              label: isKO ? "중 (Standard)" : "Standard",
              activeClass: "bg-blue-600/10 border-blue-500/30 text-blue-400",
              desc: isKO ? "주요 경고만 표시" : "Major warnings only",
            },
            {
              value: "soft" as const,
              label: isKO ? "약 (Soft)" : "Soft",
              activeClass:
                "bg-text-tertiary/10 border-zinc-500/30 text-text-secondary",
              desc: isKO
                ? "자유 창작, 오타만 표시"
                : "Free creation, typos only",
            },
          ].map(({ value, label, activeClass, desc }) => (
            <button
              key={value}
              onClick={() =>
                setConfig({ ...config, narrativeIntensity: value })
              }
            ...
            >
              <span>{label}</span>
              <span className="text-[8px] font-normal normal-case tracking-normal text-text-tertiary">
                {desc}
              </span>
            </button>
          ))}
      ...
      <div className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            ...
            ...(PLATFORM_BY_LANG[language] ||
              Object.values(PublishPlatform).filter((p) => p !== "NONE")),
          ].map((pp) => {
            ...
            const selected =
              (config.publishPlatform || PublishPlatform.NONE) === pp;
            ...
            return (
              <button
                ...
                onClick={() => {
                  ...
                  if (preset)
                    updates.guardrails = {
                      min: preset.episodeLength.min,
                      max: preset.episodeLength.max,
                    };
        ...
        {config.publishPlatform &&
          config.publishPlatform !== PublishPlatform.NONE &&
          PLATFORM_PRESETS[config.publishPlatform] && (
            <div className="mt-2 p-3 bg-bg-secondary/50 border border-border rounded-xl text-[10px] text-text-tertiary space-y-1">
              <div>
                <span className="text-text-secondary font-bold">
                  {tl("planningExtra.target")}:
                </span>{" "}
                {PLATFORM_PRESETS[config.publishPlatform].targetReader}
              </div>
              <div>
                <span className="text-text-secondary font-bold">
                  {tl("planningExtra.length")}:
                </span>{" "}
                {PLATFORM_PRESETS[
                  config.publishPlatform
                ].episodeLength.min.toLocaleString()}
                ~
                {PLATFORM_PRESETS[
                  config.publishPlatform
                ].episodeLength.max.toLocaleString()}
                {tl("planningExtra.chars")}
              </div>
              <div>
                <span className="text-text-secondary font-bold">
                  {tl("planningExtra.pace")}:
                </span>{" "}
                {PLATFORM_PRESETS[config.publishPlatform].pace}
              </div>
            </div>
          )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        <div className="space-y-2">
          <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest">
            {tl("planningExtra.povCharacter")}
          </label>
          <input
            className="w-full bg-bg-tertiary border border-border rounded-xl p-4 text-sm font-bold text-text-primary placeholder:text-text-tertiary focus:border-blue-600 outline-none transition-all"
            placeholder={tl("planningExtra.povPlaceholder")}
            maxLength={100}
            value={config.povCharacter}
            onChange={(e) =>
              setConfig({ ...config, povCharacter: e.target.value })
            }
          />
        </div>
        <div className="space-y-2">
          <label className="text-[10px] font-black text-text-tertiary uppercase tracking-widest">
            {tl("planningExtra.settingLabel")}
          </label>
          <input
            className="w-full bg-bg-tertiary border border-border rounded-xl p-4 text-sm font-bold text-text-primary placeholder:text-text-tertiary focus:border-blue-600 outline-none transition-all"
        ...
        <div className="space-y-2">
          ...
          <input
            ...
            onChange={(e) =>
              setConfig({ ...config, primaryEmotion: e.target.value })
            }
          />
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <textarea
          className="w-full bg-bg-tertiary border border-border rounded-2xl p-6 text-sm h-64 resize-none text-text-primary placeholder:text-text-tertiary focus:border-blue-600 outline-none font-serif leading-relaxed"
      ...
      <div className="space-y-4 pt-6 border-t border-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {(
            [
              ...
              {
                key: "powerStructure",
                label: t.powerStructure,
                ph: t.powerStructurePH,
              },
              {
                key: "currentConflict",
                label: t.currentConflict,
                ph: t.currentConflictPH,
              },
            ] as const
          ).map((f) => (
            <div key={f.key} className="space-y-2">
              ...
              <textarea
                className="w-full bg-bg-tertiary border border-border rounded-xl p-4 text-sm h-24 resize-none text-text-primary placeholder:text-text-tertiary focus:border-blue-600 outline-none leading-relaxed"
                placeholder={f.ph}
        ...
        {(() => {
          ...
          const s = calcCompletionScore(w, 11);
          return (
            <div className="space-y-2 mt-4">
              <CompletionBar score={s} language={language} />
      ...
      <div className="space-y-4 pt-6 border-t border-border">
        <button
          ...
          className="text-[10px] font-black text-text-tertiary uppercase tracking-widest cursor-pointer flex items-center gap-2 hover:text-text-secondary transition-colors"
        >
          {showWorldTier2 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(
              [
                ...
                {
                  key: "socialSystem",
                  label: t.socialSystem,
                  ph: t.socialSystemPH,
                },
                { key: "economy", label: t.economy, ph: t.economyPH },
                {
                  key: "magicTechSystem",
                  label: t.magicTechSystem,
                  ph: t.magicTechSystemPH,
                },
                {
                  key: "factionRelations",
                  label: t.factionRelations,
                  ph: t.factionRelationsPH,
                },
                {
                  key: "survivalEnvironment",
                  label: t.survivalEnvironment,
                  ph: t.survivalEnvironmentPH,
                },
              ] as const
            ).map((f) => (
              <div key={f.key} className="space-y-2">
                ...
                <textarea
                  className="w-full bg-bg-tertiary border border-amber-500/20 rounded-xl p-3 text-sm h-20 resize-none text-text-primary placeholder:text-text-tertiary focus:border-amber-500 outline-none leading-relaxed"
                  placeholder={f.ph}
                  value={(config[f.key] as string) ?? ""}
                  onChange={(e) =>
                    setConfig({ ...config, [f.key]: e.target.value })
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {(
              [
                ...
                {
                  key: "truthVsBeliefs",
                  label: t.truthVsBeliefs,
                  ph: t.truthVsBeliefsPH,
                },
              ] as const
            ).map((f) => (
              <div key={f.key} className="space-y-2">
                ...
                <textarea
                  className="w-full bg-bg-tertiary border border-emerald-500/20 rounded-xl p-3 text-sm h-16 resize-none text-text-primary placeholder:text-text-tertiary focus:border-emerald-500 outline-none leading-relaxed"
                  placeholder={f.ph}
                  value={(config[f.key] as string) ?? ""}
                  onChange={(e) =>
                    setConfig({ ...config, [f.key]: e.target.value })
          ...
          <div className="h-20 flex items-end gap-px">
            {tensionData.map((td, i) => {
              ...
              return (
                <div
                  key={i}
                  className="flex-1 relative h-full group cursor-default"
                >
                  <div
                    className={`absolute bottom-0 w-full rounded-t-sm transition-all duration-300 ${isCurrentEp ? "bg-gradient-to-t from-blue-500 to-cyan-400" : "bg-gradient-to-t from-blue-600/40 to-indigo-400/20"}`}
                    style={{ height: `${height}%` }}
                  />
                  {isCurrentEp && (
                    <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 bg-blue-400 rounded-full" />
                  )}
                  <div className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 hidden group-hover:block bg-bg-tertiary text-text-secondary text-[7px] px-1 py-0.5 rounded whitespace-nowrap">
                    EP.{i + 1}: {height}%
                  </div>
                </div>
              );
      ...
      <div className="space-y-6 pt-6 border-t border-border">
        <h3 className="text-[10px] font-black text-text-tertiary uppercase tracking-widest flex items-center gap-2">
          ...
          {tl("planningExtra.narrativeGuardrails")}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          <div className="space-y-4">
            ...
            <input
              ...
              max="10000"
              step="500"
              aria-label={t.minDensity}
              className="w-full accent-blue-600 h-1.5 bg-bg-tertiary rounded-full appearance-none"
              value={config.guardrails.min}
              onChange={(e) =>
                setConfig({
                  ...config,
                  guardrails: {
          ...
          <div className="space-y-4">
            ...
            <input
              ...
              max="15000"
              step="500"
              aria-label={t.maxCapacity}
              className="w-full accent-blue-600 h-1.5 bg-bg-tertiary rounded-full appearance-none"
              value={config.guardrails.max}
              onChange={(e) =>
                setConfig({
                  ...config,
                  guardrails: {
      ...
      <div className="space-y-6 pt-6 border-t border-border">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          <div className="space-y-4">
            <div className="flex justify-between text-[10px] font-bold text-text-tertiary uppercase">
              ...
              <span className="font-mono">{config.prismPreserve ?? 100}</span>
            </div>
            <input
              type="range"
              min="0"
              ...
              onChange={(e) =>
                setConfig({
                  ...config,
                  prismPreserve: parseInt(e.target.value),
                })
              }
            />
          </div>
          <div className="space-y-4">
            <div className="flex justify-between text-[10px] font-bold text-text-tertiary uppercase">
              ...
              <span className="font-mono">{config.prismScale ?? 120}</span>
            </div>
            <input
              type="range"
              min="0"
              ...
              onChange={(e) =>
                setConfig({ ...config, prismScale: parseInt(e.target.value) })
              }
            />
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          {[
            ...
            const isActive =
              (config.prismPreserve ?? 100) === p.preserve &&
              (config.prismScale ?? 120) === p.scale;
            return (
              <button
                key={p.scale}
                onClick={() =>
                  setConfig({
                    ...config,
                    prismPreserve: p.preserve,
                    prismScale: p.scale,
                  })
                }
                className={`px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all font-mono ${isActive ? "bg-blue-600/20 border border-blue-500/40 text-blue-400" : "bg-bg-secondary border border-border text-text-tertiary hover:text-text-secondary"}`}
              >
      ...
      <div className="space-y-6 pt-6 border-t border-border">
        ...
        <div className="flex flex-wrap gap-2">
          {[
            {
              key: "OFF" as const,
              label: tl("planningExtra.prismModeOff"),
              desc: tl("planningExtra.prismModeOffDesc"),
            },
            {
              key: "FREE" as const,
              label: tl("planningExtra.prismModeFree"),
              desc: tl("planningExtra.prismModeFreeDesc"),
            },
            {
              key: "ALL" as const,
              label: tl("planningExtra.prismModeAll"),
              desc: tl("planningExtra.prismModeAllDesc"),
            },
            {
              key: "T15" as const,
              label: tl("planningExtra.prismModeT15"),
              desc: tl("planningExtra.prismModeT15Desc"),
            },
            {
              key: "M18" as const,
              label: tl("planningExtra.prismModeM18"),
              desc: tl("planningExtra.prismModeM18Desc"),
            },
            {
              key: "CUSTOM" as const,
              label: tl("planningExtra.prismModeCustom"),
              desc: tl("planningExtra.prismModeCustomDesc"),
            },
          ].map((pm) => (
            <button
              key={pm.key}
        ...
        {(() => {
          ...
          ).find((p) => p.key === (config.prismMode ?? "OFF"));
          return (
              ...
              return (
                <div key={axis} className="space-y-2">
                  <div className="flex justify-between text-[10px] font-bold text-text-tertiary uppercase">
                    ...
                    <span className="font-mono">{val}/5</span>
                  </div>
                  <input
                    type="range"
                    min="0"
