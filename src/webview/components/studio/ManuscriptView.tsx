...
export default function ManuscriptView({
  ...
  const totalChars = useMemo(
    () => manuscripts.reduce((sum, m) => sum + m.charCount, 0),
    [manuscripts],
  );
  ...
  const getAnalysis = useCallback(
    (ep: number): ChapterAnalysis | null => {
      return (
        (config.chapterAnalyses || []).find((a) => a.episode === ep) ?? null
      );
    },
  ...
  const progressPercent =
    totalTarget > 0
      ? Math.min(100, Math.round((totalChars / totalTarget) * 100))
      : 0;
    ...
    <div className="max-w-6xl mx-auto py-4 px-3 sm:py-8 sm:px-4 md:py-12 md:px-6 space-y-4 sm:space-y-6">
      ...
      <div className="bg-bg-secondary border border-border rounded-xl p-4 space-y-3">
        <div className="flex items-center justify-between text-[10px] font-bold font-mono uppercase tracking-wider">
          ...
          <span className="text-accent-purple">
            ...
            {t("manuscript.charUnit")} / {totalTarget.toLocaleString()}
            {t("manuscript.charUnit")} ({progressPercent}%)
          </span>
        </div>
        ...
        <div className="flex justify-between text-[9px] text-text-tertiary font-mono">
          <span>
            {sorted.length} / {config.totalEpisodes} {t("manuscript.eps")}
          </span>
          <span>
            {t("manuscript.target")}: {targetPerEp.toLocaleString()}
            {t("manuscript.charsPerEp")}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 sm:grid-cols-10 gap-1.5">
        {Array.from({ length: config.totalEpisodes }, (_, i) => {
          ...
          const pct = ms
            ? Math.min(100, Math.round((ms.charCount / targetPerEp) * 100))
            : 0;
          ...
          return (
            <button
              ...
              title={
                ms
                  ? `EP.${ep}: ${ms.charCount.toLocaleString()}${t("manuscript.charUnit")}`
                  : `EP.${ep}`
              }
            >
              {ep}
        ...
        {sorted.length === 0 ? (
          ...
          sorted.map((m) => (
            ...
            >
              <button
                onClick={() =>
                  setExpandedEp(expandedEp === m.episode ? null : m.episode)
                }
              ...
              >
                <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                  ...
                  <span className="text-[9px] text-text-tertiary font-mono shrink-0 hidden sm:inline">
                    {m.charCount.toLocaleString()}
                    {t("manuscript.charUnit")}
                  </span>
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            className="px-3 py-1.5 bg-bg-secondary border border-border rounded-lg text-[10px] font-bold text-text-tertiary"
                          >
                            {t("manuscript.cancel")}
                          </button>
                          <button
                            onClick={saveEdit}
                            className="px-3 py-1.5 bg-accent-purple text-white rounded-lg text-[10px] font-bold"
                          >
                            {t("manuscript.save")}
                          </button>
                        </div>
                    ...
                    <div className="p-4">
                      <div className="flex flex-wrap justify-end gap-2 mb-3">
                        <button
                          onClick={() =>
                            setAnalysisEp(
                              analysisEp === m.episode ? null : m.episode,
                            )
                          }
                        ...
                        {onEditInStudio && (
                          <button
                            onClick={() => onEditInStudio(m.content)}
                            className="p-1.5 bg-bg-tertiary/50 rounded text-text-tertiary hover:text-accent-green transition-colors"
                        ...
                        <button
                          onClick={() =>
                            setDiffEp(diffEp === m.episode ? null : m.episode)
                          }
                          className={`p-1.5 rounded transition-colors ${
                            diffEp === m.episode
                              ? "bg-blue-600/20 text-blue-400"
                              : "bg-bg-tertiary/50 text-text-tertiary hover:text-blue-400"
                          }`}
                        ...
                        <button
                          onClick={() => startEdit(m)}
                          aria-label="편집"
                          className="p-1.5 bg-bg-tertiary/50 rounded text-text-tertiary hover:text-accent-purple transition-colors"
                        >
                          <Edit3 className="w-3 h-3" />
                        </button>
                        <button
                          onClick={() => deleteManuscript(m.episode)}
                          aria-label="삭제"
                          className="p-1.5 bg-bg-tertiary/50 rounded text-text-tertiary hover:text-accent-red transition-colors"
                        >
                      ...
                      {diffEp === m.episode &&
                        (() => {
                          ...
                          const adds = diff.filter(
                            (d) => d.type === "add",
                          ).length;
                          const removes = diff.filter(
                            (d) => d.type === "remove",
                          ).length;
                          return (
                            <div className="mb-4 border border-blue-500/20 rounded-xl overflow-hidden">
                              ...
                              <div className="p-3 bg-bg-primary max-h-60 overflow-y-auto text-[11px] font-mono leading-relaxed custom-scrollbar">
                                ...
                                {diff.length > 500 && (
                                  <div className="text-center py-1 text-[9px] text-text-tertiary">
                                    +{diff.length - 500} lines truncated
                                  </div>
                                )}
                              </div>
                            </div>
                      ...
                      <div
                        className="prose prose-sm max-w-none text-text-secondary font-serif leading-[2] max-h-[60vh] sm:max-h-[50vh] overflow-y-auto overscroll-contain whitespace-pre-wrap text-sm sm:text-sm"
                        style={{ WebkitOverflowScrolling: "touch" }}
                      >
                        {m.content}
                      </div>
                      <div className="mt-3 text-[10px] text-text-tertiary font-mono">
                        {t("manuscript.lastUpdate")}:{" "}
