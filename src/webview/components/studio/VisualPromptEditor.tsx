    ...
    <div className="space-y-5">
      ...
      <div className="flex gap-3">
        ...
        <select
          value={card.shotType}
          onChange={(e) =>
            update({ shotType: e.target.value as VisualShotType })
          }
          ...
          {VISUAL_PRESETS.map((p) => (
            <button
              ...
              onClick={() =>
                update({
                  levels: { ...p.levels },
                  shotType: p.defaultShotType || card.shotType,
                })
              }
          ...
          {(
            ...
            const labels: Record<string, string> = isKO
              ? {
                  ...
                  backgroundPrompt: "배경",
                  scenePrompt: "장면",
                  compositionPrompt: "구도",
                  lightingPrompt: "조명",
                  stylePrompt: "스타일",
                  negativePrompt: "금지 요소",
                }
              : {
                  subjectPrompt: "Subject",
                  backgroundPrompt: "Background",
                  scenePrompt: "Scene",
                  compositionPrompt: "Composition",
            ...
            return (
              <div key={field}>
                <label className="text-[10px] text-text-tertiary font-semibold">
                  {labels[field]}
                </label>
                <textarea
                  value={card[field] || ""}
      ...
      <details className="group">
        <summary className="text-[10px] font-black text-text-tertiary uppercase tracking-widest cursor-pointer hover:text-text-secondary mt-2">
          {isKO
            ? "▸ Image2Image 터널링 및 시드 (고급)"
            : "▸ Image2Image Tunneling & Seed (Advanced)"}
        </summary>
        <div className="mt-3 space-y-3">
          <div>
            <label className="text-[10px] mb-1 block text-text-tertiary font-semibold">
              {isKO ? "시드값 (Seed)" : "Seed"}
            </label>
            <input
              type="number"
            ...
            <p className="text-[9px] text-zinc-500 mt-1">
              {isKO
                ? "시드 값을 고정하면 인물의 얼굴이나 아트 스타일의 시각적 일관성(Consistency)을 유지하기 쉽습니다."
                : "Fixing the seed helps maintain face/style consistency."}
            </p>
          </div>
          <div>
            <label className="text-[10px] mb-1 block text-text-tertiary font-semibold">
              {isKO ? "레퍼런스 이미지 URL" : "Reference Image URL"}
            </label>
            <input
              type="url"
            ...
            <p className="text-[9px] text-zinc-500 mt-1">
              {isKO
                ? "컨트롤넷이나 Image2Image 파이프라인에서 참조할 원본 캐릭터의 이미지 주소(URL)입니다."
                : "Source image URL for ControlNet or Image2Image pipeline."}
            </p>
          </div>
        </div>
      </details>
      ...
      <div className="bg-black/60 border border-border rounded-xl p-4">
        ...
        <p className="text-[11px] text-text-secondary leading-relaxed whitespace-pre-wrap">
          {finalPrompt ||
            (isKO
              ? "레벨을 조절하면 초안이 생성됩니다"
              : "Adjust levels to generate draft")}
        </p>
        {negPrompt && (
          <div className="mt-3 pt-3 border-t border-border">
            <div className="flex justify-between items-center mb-1">
              <span className="text-[9px] font-bold text-red-500/60 uppercase">
                Negative
              </span>
              <button
          ...
          {genImages.length > 0 && (
            <div className="grid grid-cols-2 gap-2">
              {genImages.map((img, i) => (
                <div
                  key={i}
                  className="relative group rounded-xl overflow-hidden border border-border/30 bg-black/30"
                >
                  <Image
                    src={img.url}
                    alt={`Generated ${i + 1}`}
                  ...
                  <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                    <a
                      href={img.url}
                      download={`noi-${card.title || "image"}-${i + 1}.png`}
                      target="_blank"
                  ...
                  {img.revised_prompt && (
                    <div className="absolute bottom-0 left-0 right-0 bg-black/80 px-2 py-1">
                      <p className="text-[8px] text-text-tertiary line-clamp-2">
                        {img.revised_prompt}
                      </p>
                    </div>
                  )}
      ...
      {imageGenEnabled && !imageApiKey && (
        <div className="text-[9px] text-text-tertiary bg-bg-secondary/30 border border-border/30 rounded-lg px-3 py-2">
          {isKO
            ? "설정에서 이미지 생성 API 키를 등록하면 프리뷰를 생성할 수 있습니다."
            : "Add an image generation API key in Settings to enable previews."}
        </div>
      )}
          ...
          {(card.consistencyTags?.length ?? 0) > 0 && (
            <div className="flex flex-wrap gap-1">
              {card.consistencyTags!.map((tag, i) => (
                <span
                  key={i}
                  className="px-2 py-0.5 rounded-md text-[9px] font-bold bg-purple-600/10 border border-purple-500/20 text-purple-300"
                >
                  {tag}
                  <button
                    onClick={() =>
                      update({
                        consistencyTags: card.consistencyTags!.filter(
                          (_, j) => j !== i,
                        ),
                      })
                    }
