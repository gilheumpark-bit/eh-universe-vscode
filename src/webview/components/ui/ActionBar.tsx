...
export function ActionBar({
  ...
  const handleShare = useCallback(async () => {
    try {
      const { createShareLink, copyShareLink } =
  ...
  return (
    <div
      className={`flex ${direction === "col" ? "flex-col" : ""} items-center gap-0.5 ${className}`}
    >
      {actions.includes("copy") && (
        <button
          onClick={handleCopy}
          className={`${btnClass} ${copied ? activeClass : defaultClass}`}
          title="Copy"
        >
      ...
      {actions.includes("share") && (
        <button
          onClick={handleShare}
          className={`${btnClass} ${shared ? activeClass : defaultClass}`}
          title="Share link"
        >
