    ...
    try {
      const response = await ai.models.generateContent({
        ...
        config: {
          ...
          responseSchema,
          abortSignal: AbortSignal.timeout(STRUCTURED_GENERATION_TIMEOUT_MS),
        },
      });
      try {
      ...
      const isRetryable =
        /500|502|503|504|INTERNAL|resource.*exhausted|deadline|overloaded/i.test(
          msg,
        );
...
export async function handleMagicSystems(
  apiKey: string,
  model: string,
  config: Pick<StoryConfig, "genre" | "synopsis">,
  language: AppLanguage,
...
${existingBlock}`,
    {
      ...
      items: {
        ...
        properties: {
          ...
          ranks: { type: Type.ARRAY, items: { type: Type.STRING } },
        },
...
export async function handleWorldDesign(
  apiKey: string,
  model: string,
  genre: string,
  language: AppLanguage,
  ...
  if (hints?.povCharacter)
    hintParts.push(`Main character: "${hints.povCharacter}"`);
  if (hints?.setting) hintParts.push(`Setting: "${hints.setting}"`);
  if (hints?.primaryEmotion)
    hintParts.push(`Core emotion: "${hints.primaryEmotion}"`);
...
export async function handleWorldSim(
  apiKey: string,
  model: string,
  synopsis: string,
  genre: string,
  ...
  if (worldContext?.corePremise)
    contextParts.push(`World Premise: ${worldContext.corePremise}`);
  if (worldContext?.powerStructure)
    contextParts.push(`Power Structure: ${worldContext.powerStructure}`);
  if (worldContext?.currentConflict)
    contextParts.push(`Central Conflict: ${worldContext.currentConflict}`);
  if (worldContext?.factionRelations)
    contextParts.push(
      `Known Faction Relations: ${worldContext.factionRelations}`,
    );
  const contextBlock =
...
export async function handleSceneDirection(
  apiKey: string,
  model: string,
  synopsis: string,
  characters: string[],
  ...
  if (tierContext?.corePremise)
    contextParts.push(`World Premise: ${tierContext.corePremise}`);
  if (tierContext?.powerStructure)
    contextParts.push(`Power Structure: ${tierContext.powerStructure}`);
  if (tierContext?.currentConflict)
    contextParts.push(`World Conflict: ${tierContext.currentConflict}`);
  if (tierContext?.charProfiles?.length) {
    const charBlock = tierContext.charProfiles
      .map(
        (character) =>
          `  - ${character.name}: wants "${character.desire || "?"}", conflicts with "${character.conflict || "?"}", arc toward "${character.changeArc || "?"}", forbidden line "${character.values || "?"}"`,
      )
      .join("\n");
