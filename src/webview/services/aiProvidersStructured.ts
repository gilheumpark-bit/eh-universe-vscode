...
export async function generateJsonOpenAICompat(
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You are a structured data generator. Always respond with valid JSON only. No markdown, no explanation.",
            },
      ...
      const isRetryable = /500|502|503|504|INTERNAL|resource.*exhausted/i.test(
        msg,
      );
...
export async function dispatchStructuredGeneration(
  ...
  try {
    ...
    if (provider === "gemini" && schema) {
      return {
        ok: true,
        result: await generateJsonGemini(
          apiKey,
          model,
          prompt,
          schema,
    ...
    if (provider === "claude") {
      return {
        ok: true,
        result: await generateJsonClaude(
          apiKey,
          model,
          prompt,
          schema,
