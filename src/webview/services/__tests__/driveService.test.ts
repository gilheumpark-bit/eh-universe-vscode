...
describe("getOrCreateAppFolder", () => {
  it("returns existing folder ID when found", async () => {
    mockFetch.mockResolvedValueOnce({
      ok: true,
      json: async () => ({
        files: [{ id: MOCK_FOLDER_ID, name: "NOA Studio" }],
      }),
    });
  ...
  it("throws on API error", async () => {
    ...
    await expect(getOrCreateAppFolder(MOCK_TOKEN)).rejects.toThrow(
      "Drive search failed: 401",
    );
  });
});
...
describe("loadProjectFile", () => {
  ...
  it("throws on download error", async () => {
    ...
    await expect(loadProjectFile(MOCK_TOKEN, "bad-id")).rejects.toThrow(
