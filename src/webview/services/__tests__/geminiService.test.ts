...
// engine/pipeline
const mockBuildSystemInstruction = jest
  .fn()
  .mockReturnValue("system-instruction");
...
jest.mock("@/engine/pipeline", () => ({
  buildSystemInstruction: (...args: unknown[]) =>
    mockBuildSystemInstruction(...args),
...
describe("generateCharacters", () => {
  ...
  it("normalizes unknown roles to extra", async () => {
    mockFetch.mockResolvedValueOnce(
      okJson([{ name: "Mystery", role: "unknown_role" }]),
    );
  ...
  it("defaults role to extra when role is missing", async () => {
    mockFetch.mockResolvedValueOnce(okJson([{ name: "NoRole" }]));
...
describe("generateItems", () => {
  ...
  it("normalizes artifact category to accessory via alias", async () => {
    mockFetch.mockResolvedValueOnce(
      okJson([{ name: "Ring", category: "artifact", rarity: "epic" }]),
    );
  ...
  it("normalizes key_item category to quest via alias", async () => {
    mockFetch.mockResolvedValueOnce(
      okJson([{ name: "Scroll", category: "key_item", rarity: "uncommon" }]),
    );
...
describe("generateWorldSim", () => {
  ...
  it("passes worldContext to API", async () => {
    ...
    mockFetch.mockResolvedValueOnce(
      okJson({ civilizations: [], relations: [] }),
    );
...
describe("generateStoryStream", () => {
  ...
  it("streams content and returns processed result", async () => {
    ...
    const chunks: string[] = [];
    const result = await generateStoryStream(baseConfig, "draft", (t) =>
      chunks.push(t),
    );
    ...
    expect(mockPostProcessResponse).toHaveBeenCalledWith(
      "Hello World",
      baseConfig,
      "KO",
      PlatformType.MOBILE,
