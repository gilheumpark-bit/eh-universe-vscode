...
async function deriveKey(passphrase: string): Promise<CryptoKey> {
  ...
  return crypto.subtle.deriveKey(
    {
      name: "PBKDF2",
      salt: enc.encode("noa-studio-drive-v1"),
      iterations: 100_000,
      hash: "SHA-256",
    },
    keyMaterial,
...
export async function loadProjectFile(
  ...
  const jsonText =
    _encryptionPassphrase && rawText.startsWith(ENCRYPTION_PREFIX)
...
export async function deleteProjectFile(
  ...
  const res = await driveRequest(`${DRIVE_API}/files/${fileId}`, token, {
    method: "DELETE",
  });
...
export async function syncAllProjects(
  ...
  type SyncTask =
    | { type: "upload"; index: number; fileId?: string }
    | { type: "download"; index: number; fileId: string }
  ...
  for (let i = 0; i < merged.length; i++) {
    ...
    if (local.lastUpdate > remoteTime + 5000) {
      ...
      Math.abs(local.lastUpdate - remoteTime) <= 5000 &&
      local.lastUpdate !== remoteTime
    ) {
      tasks.push({
        type: "conflict",
        info: {
  ...
  const remoteOnly = [...remoteMap.entries()].filter(
    ([pid]) => !processedRemoteIds.has(pid),
  );
  if (remoteOnly.length > 0) {
    const dlResults = await Promise.allSettled(
      remoteOnly.map(async ([, file]) => {
