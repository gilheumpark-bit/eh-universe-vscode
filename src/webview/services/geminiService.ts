...
async function fetchStructuredGemini<T>(
  ...
  const cacheable =
    body.task === "worldDesign" ||
    body.task === "worldSim" ||
    body.task === "sceneDirection";
  ...
  if (response.ok) {
    if (cacheable && cacheKey)
      structuredCache.set(cacheKey, { data, ts: Date.now() });
    return data as T;
  }
...
function buildChatMessages(
  ...
  const { messages: trimmed } = truncateMessages(
    systemInstruction,
    allMsgs,
    model,
  );
...
export const generateStoryStream = async (
  ...
  const temperature =
    options.temperature ??
    parseFloat(localStorage.getItem("noa_temperature") || "0.9");
...
export const generateCharacters = async (
  ...
  const existingNames = (config.characters || [])
    .map((c) => c.name)
    .filter(Boolean);
...
export const generateItems = async (
  ...
  const normalizeCategory = (c: string) =>
    VALID_CATEGORIES.includes(c) ? c : CATEGORY_ALIAS[c] || "misc";
  const normalizeRarity = (r: string) =>
    VALID_RARITIES.includes(r) ? r : "common";
  ...
  return results
    ...
    .map((item) => ({
      ...
      category: normalizeCategory(
        (item as Item).category || "misc",
      ) as Item["category"],
      rarity: normalizeRarity(
        (item as Item).rarity || "common",
      ) as Item["rarity"],
...
export const generateSkills = async (
  ...
  const existingNames = (config.skills || [])
    .map((s) => s.name)
    .filter(Boolean);
  const results = await fetchStructuredGemini<unknown[]>({
    task: "skills",
...
export const generateMagicSystems = async (
  ...
  const existingNames = (config.magicSystems || [])
    .map((m) => m.name)
    .filter(Boolean);
  const results = await fetchStructuredGemini<unknown[]>({
    task: "magicSystems",
