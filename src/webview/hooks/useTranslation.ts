...
async function scoreTranslation(
  ...
  const schema =
    config.mode === "fidelity"
      ? FIDELITY_SCORE_SCHEMA
      : EXPERIENCE_SCORE_SCHEMA;
      ...
      if (isMultiKeyActive() && tierLimits.translation.crossValidation) {
        try {
          ...
          if (secondaryRaw.trim()) {
            const secondaryScore = parseScoreResponse(
              secondaryRaw,
              config.mode,
            );
...
function mergeScores(
  ...
  const axisKeys = Object.keys(a).filter(
    (k) => k !== "overall" && typeof aRec[k] === "number",
  );
  ...
  // 전체 점수도 재계산
  const axisValues = axisKeys
    .map((k) => merged[k])
    .filter((v): v is number => typeof v === "number");
  merged.overall =
    axisValues.length > 0
      ? Math.round(axisValues.reduce((s, v) => s + v, 0) / axisValues.length)
      : a.overall;
...
export function useTranslation({
  ...
  const updateProgress = useCallback(
    (patch: Partial<TranslationProgress>) => {
  ...
  const translateChunk = useCallback(
    async (
      ...
      lastScore = await scoreTranslation(
        chunkText,
        translatedText,
        config,
        signal,
  ...
  // 에피소드 번역
  const translateEpisode = useCallback(
    async (
      manuscript: EpisodeManuscript,
      ...
      try {
        ...
        if (tracker.inconsistencies.length > 0) {
          logger.warn(
            "Translation",
            "Cross-chunk consistency issues:",
        ...
        if (onProfileUpdate && config.translatorProfile) {
          ...
          const termConsistency =
            chunks.length > 0
              ? chunks.filter((c) => c.passed).length / chunks.length
              : 1;
          const updated = updateTranslatorProfile(
            config.translatorProfile,
            result.avgScore,
            termConsistency,
            result.avgScore,
            errors,
  ...
  const translateBatch = useCallback(
    async (
      ...
      try {
        ...
        updateBatch({
          totalEpisodes: manuscripts.length,
          completedEpisodes: 0,
          currentEpisode: 0,
        });
        ...
        for (let i = 0; i < manuscripts.length; i++) {
          ...
          updateBatch({
            currentEpisode: i + 1,
            chunkProgress: progressRef.current,
          });

          // 자동 컨텍스트 브릿지: 이전 화 번역 결과에서 생성
          const episodeConfig = { ...baseConfig };
          if (results.length > 0) {
            episodeConfig.contextBridge = buildAutoBridge(
              results[results.length - 1],
              baseConfig.glossary,
            );
          }

          const result = await translateEpisode(
            manuscripts[i],
            episodeConfig,
            externalSignal,
            true,
