...
import {
  ...
  saveVersionedBackup,
  listVersionedBackups,
  restoreVersionedBackup,
...
export function useProjectManager(language: AppLanguage) {
    ...
    (updater: ChatSession[] | ((prev: ChatSession[]) => ChatSession[])) => {
      setProjects((prev) =>
        prev.map((p) => {
          ...
          const next =
            typeof updater === "function" ? updater(p.sessions) : updater;
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (!ok) {
        ...
        logger.warn(
          "NOA",
          `Storage full (${mb}MB). Consider exporting and clearing old sessions.`,
        );
        // Surface storage-full warning to user via custom event
        window.dispatchEvent(
          new CustomEvent("noa:storage-full", {
            detail: { usageMB: mb },
          }),
        );
      }
      if (isFeatureEnabled("OFFLINE_CACHE")) {
        backupToIndexedDB(projects).catch((err) =>
          logger.warn("IndexedDB", "Backup failed:", err),
        );
      }
  ...
  useEffect(() => {
    if (
      !hydrated ||
      projects.length === 0 ||
      !isFeatureEnabled("OFFLINE_CACHE")
    )
      return;
    ...
    const interval = setInterval(() => {
      ...
      saveVersionedBackup(current)
        ...
        .catch((err) =>
          logger.warn("IndexedDB", "Versioned backup failed:", err),
        );
    }, BACKUP_INTERVAL);
  ...
  const renameProject = useCallback((projectId: string, newName: string) => {
    setProjects((prev) =>
      prev.map((p) =>
        p.id === projectId
          ? { ...p, name: newName, lastUpdate: Date.now() }
          : p,
      ),
    );
  }, []);
  ...
  const moveSessionToProject = useCallback(
    (sessionId: string, targetProjectId: string) => {
      setProjects((prev) => {
        const src = prev.find((p) =>
  ...
  const setConfig = useCallback(
    (newConfig: StoryConfig | ((prev: StoryConfig) => StoryConfig)) => {
      if (typeof newConfig === "function") {
        updateCurrentSession({
          config: newConfig(currentSession?.config ?? INITIAL_CONFIG),
        });
      } else {
  ...
  return {
    ...
    versionedBackups: isFeatureEnabled("OFFLINE_CACHE")
      ? versionedBackups
      : undefined,
