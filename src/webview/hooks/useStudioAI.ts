...
import { Message, ChatSession, AppLanguage } from "@/lib/studio-types";
import {
  type HFCPState as HFCPStateType,
  processHFCPTurn,
} from "@/engine/hfcp";
...
import {
  ...
  buildRetryHint,
} from "@/engine/quality-gate";
import {
  generateSuggestions,
  getDefaultSuggestionConfig,
} from "@/engine/proactive-suggestions";
import {
  updateProfile,
  loadProfile,
  saveProfile,
  buildProfileHint,
} from "@/engine/writer-profile";
import { getNarrativeDepth } from "@/lib/noa/lora-swap";
import {
  executePipeline,
  getDefaultPipelineConfig,
  type PipelineExecution,
} from "@/engine/auto-pipeline";
...
export function useStudioAI({
  ...
  const [directorReport, setDirectorReport] = useState<DirectorReport | null>(
    null,
  );
  ...
  const handleSend = useCallback(
    async (
      ...
      if (!text.trim() || isGenerating || !currentSessionId || !currentSession)
        return;
      ...
      if (!canGenerate()) {
        setUxError?.({
          error: new Error("Free tier limit reached"),
          retry: () => {},
        });
        return;
      }
      ...
      const OUTPUT_MODE_LABELS: Record<string, string> = {
        ...
        "description-boost":
          "[출력 모드: 묘사 강화 — 배경/감각/내면 묘사 중심]",
        "ending-hook":
          "[출력 모드: 엔딩 훅 강화 — 마지막 3문장에 강한 클리프행어]",
        bridge:
          "[출력 모드: 연결부 — 이전 에피소드와 자연스럽게 이어지는 브릿지]",
      };
      const outputModePrefix =
        advancedOutputMode && OUTPUT_MODE_LABELS[advancedOutputMode]
      ...
      if (advancedSettings) {
        ...
        if (adv.constraints) {
          ...
          parts.push(
            `- 대화 비율(Dialogue Ratio): 약 ${adv.constraints.dialogueRatio}%`,
          );
          parts.push(`- 템포(Tempo): ${adv.constraints.tempo}`);
          parts.push(
            `- 문장 길이(Sentence Length): ${adv.constraints.sentenceLen}`,
          );
          parts.push(
            `- 감정 노출도(Emotion Exposure): ${adv.constraints.emotionExposure}`,
          );
        }
        if (adv.includes)
          parts.push(`- 필수 포함 요소(Must Include): ${adv.includes}`);
        if (adv.excludes)
          parts.push(`- 절대 금지 요소(Must Exclude): ${adv.excludes}`);
      ...
      const userMsg: Message = {
        ...
        timestamp: Date.now(),
        meta: {
          hfcpMode: hfcpResult.mode,
          hfcpVerdict: hfcpResult.verdict,
          hfcpScore: hfcpResult.score,
        } as Message["meta"],
      };
      ...
      try {
        ...
        const configForAI = {
          ...
          simulatorRef: {
            ...
            genreSelections:
              capturedConfig.worldSimData?.genreSelections ||
              capturedConfig.simulatorRef?.genreSelections,
          },
        };
        ...
        const maxAttempts =
          gateConfig.enabled && gateConfig.autoMode === "full_auto"
            ? gateConfig.maxRetries
            : 1;
        ...
        while (attempt <= maxAttempts) {
          ...
          result = await generateStoryStream(
              ...
              setSessions((prev) =>
                prev.map((s) => {
                  if (s.id === capturedSessionId) {
                    const visualPrefix =
                      attempt > 1
                        ? `[Quality Gate: 자동 재작성 시도 ${attempt}/${maxAttempts}]\n\n`
                        : "";
                    const msgs = s.messages.map((m) =>
                      m.id === aiMsgId
                        ? { ...m, content: visualPrefix + displayContent }
                        : m,
                    );
            ...
            },
            {
              ...
              temperature: (() => {
                const d = getNarrativeDepth();
                const base = parseFloat(
                  localStorage.getItem("noa_temperature") || "0.9",
                );
                return Math.max(0.1, Math.min(1.5, base + (d - 1.0) * 0.4));
              })(),
            },
          );
          ...
          try {
            dReport = analyzeManuscript(
              finalContent,
              capturedConfig.publishPlatform,
            );
          } catch {
            /* manuscript analysis advisory — non-blocking */
          }
          qTag = calculateQualityTag(
            dReport,
            capturedConfig.narrativeIntensity || "standard",
          );

          gateResult = evaluateQuality(
            finalContent,
            capturedConfig,
          ...
          currentRetryHint = buildRetryHint(
            gateResult,
            attempt,
            language === "KO",
          );
        ...
        try {
          ...
          const pastReports = (currentSession?.messages || [])
            ...
            .map((m) => {
              ...
              return {
                tension: r.metrics?.tension ?? 50,
                pacing: r.metrics?.pacing ?? 60,
                immersion: r.metrics?.immersion ?? 60,
                eos: r.eosScore ?? 50,
                grade: r.grade ?? "B",
              };
            });
          ...
          chars.forEach((ch) => {
            charLastAppearance[ch.name] = capturedConfig.episode ?? 1;
          });
          ...
          /* suggestions are advisory — never block */
        }
        ...
        try {
          ...
          /* profile learning is meta — never block */
        }
          ...
          prev.map((s) => {
            if (s.id === capturedSessionId) {
                ...
                m.id === aiMsgId
                  ? {
                      ...
                      meta: {
                        ...
                        // eslint-disable-next-line @typescript-eslint/no-explicit-any
                        qualityFindings: qTag.visibleFindings.map((f: any) => ({
                          kind: f.kind,
                          severity: f.severity,
                          message: f.message,
                          lineNo: f.lineNo,
              ...
              if (cleanText.length > 100) {
                ...
                const existing = (s.config.manuscripts || []).find(
                  (m) => m.episode === ep,
                );
                const manuscript = {
                  episode: ep,
                  title: capturedConfig.title
                    ? `${capturedConfig.title} EP.${ep}`
                    : `EP.${ep}`,
                  content: cleanText,
                ...
                const manuscripts = existing
                  ? (s.config.manuscripts || []).map((m) =>
                      m.episode === ep ? manuscript : m,
                    )
                  : [...(s.config.manuscripts || []), manuscript];
                return {
                  ...s,
                  messages: msgs,
                  config: { ...s.config, manuscripts },
                };
              }
              return { ...s, messages: msgs };
            }
        ...
        if (error instanceof DOMException && error.name === "AbortError") {
          ...
          if (
            classified.code === StudioErrorCode.KEY_MISSING ||
            classified.code === StudioErrorCode.KEY_INVALID
          ) {
  ...
  const handleRegenerate = useCallback(
    async (assistantMsgId: string) => {
      ...
      const savedVersions = currentMsg.content
        ? [...prevVersions, currentMsg.content]
        : prevVersions;
            ...
            const msgs = s.messages.map((m) =>
              m.id === assistantMsgId
                ? {
                    ...
                    currentVersionIndex: savedVersions.length,
                  }
                : m,
            );
            return { ...s, messages: msgs };
      ...
      try {
        const configForChat = {
          ...
          simulatorRef: {
            ...
            genreSelections:
              capturedConfig2.worldSimData?.genreSelections ||
              capturedConfig2.simulatorRef?.genreSelections,
          },
        };
        ...
        const finalContent =
          stripEngineArtifacts(fullContent) || result.content;
        setLastReport(result.report);
        setSessions((prev) =>
          prev.map((s) => {
            if (s.id === capturedSessionId2) {
        ...
        if (error instanceof DOMException && error.name === "AbortError") {
          ...
          setUxError({
            error: classified,
            retry: classified.retryable
              ? () => handleRegenerate(assistantMsgId)
              : undefined,
          });
        }
      } finally {
