...
function createHarness() {
  const ref: { current: ReturnType<typeof useCodeStudioComposer> | null } = {
    current: null,
  };
...
describe("useCodeStudioComposer", () => {
  ...
  it("transitionMode enforces allowed transitions", () => {
    ...
    let result: boolean;
    act(() => {
      result = get().transitionMode("generating");
    });
    ...
    act(() => {
      result = get().transitionMode("review");
    });
    ...
    act(() => {
      result = get().transitionMode("verifying");
    });
    expect(result!).toBe(true);
  ...
  it("composing is true only in generating mode", () => {
    ...
    act(() => {
      get().transitionMode("generating");
    });
    expect(get().composing).toBe(true);

    act(() => {
      get().transitionMode("verifying");
    });
    expect(get().composing).toBe(false);
    cleanup();
  });
  ...
  it("accept marks a change as accepted", async () => {
    ...
    expect(get().changes.find((c) => c.fileId === "f1")?.status).toBe(
      "accepted",
    );
    expect(get().changes.find((c) => c.fileId === "f2")?.status).toBe(
      "pending",
    );
    cleanup();
  });
  ...
  it("acceptAll / rejectAll bulk-updates pending changes", async () => {
    ...
    expect(get().changes.find((c) => c.fileId === "f3")?.status).toBe(
      "rejected",
    );
    cleanup();
  });
