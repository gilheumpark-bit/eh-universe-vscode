...
const mockSyncAll = syncAllProjects as jest.MockedFunction<
  typeof syncAllProjects
>;
...
function createHarness(
  ...
  const params = {
    ...
    accessToken:
      "accessToken" in overrides ? overrides.accessToken : "token-123",
    refreshAccessToken:
      overrides.refreshAccessToken ?? jest.fn().mockResolvedValue("new-token"),
...
describe("useStudioSync", () => {
  ...
  it("sync reminder controls", () => {
    ...
    act(() => {
      get().setShowSyncReminder(true);
    });
    expect(get().showSyncReminder).toBe(true);

    act(() => {
      get().setShowSyncReminder(false);
    });
    expect(get().showSyncReminder).toBe(false);
    cleanup();
  });
