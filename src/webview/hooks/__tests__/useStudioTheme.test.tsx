...
function createThemeHarness() {
  const ref: { current: ReturnType<typeof useStudioTheme> | null } = {
    current: null,
  };
...
function createLanguageHarness() {
  const ref: { current: ReturnType<typeof useStudioLanguage> | null } = {
    current: null,
  };
  const container = document.createElement("div");
  document.body.appendChild(container);
...
describe("useStudioTheme", () => {
  ...
  it("manages search state", () => {
    ...
    act(() => {
      get().setShowSearch(true);
    });
    expect(get().showSearch).toBe(true);

    act(() => {
      get().setSearchQuery("test");
    });
