...
export function useCodeStudioComposer(): UseCodeStudioComposerReturn {
  ...
  const updateStatus = useCallback(
    (fileId: string, status: "accepted" | "rejected") => {
      setChanges((prev) =>
        prev.map((c) => (c.fileId === fileId ? { ...c, status } : c)),
      );
    },
    [],
  );
  ...
  const accept = useCallback(
    (fileId: string) => updateStatus(fileId, "accepted"),
    [updateStatus],
  );
  const reject = useCallback(
    (fileId: string) => updateStatus(fileId, "rejected"),
    [updateStatus],
  );
