...
export function useStudioQuickStart({
  ...
  const [pendingQuickStart, setPendingQuickStart] =
    useState<PendingQuickStart | null>(null);
  ...
  const handleQuickStart = async (genre: Genre, userPrompt: string) => {
    ...
    try {
      ...
      const world = await generateWorldDesign(genre, language, {
        synopsis: userPrompt,
      });
  ...
  useEffect(() => {
    ...
    if (!currentSession || currentSession.id !== pendingQuickStart.sessionId)
      return;
