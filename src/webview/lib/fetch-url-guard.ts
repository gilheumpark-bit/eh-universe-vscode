...
export function assertUrlAllowedForFetch(
  rawUrl: string,
  ...
  if (/^\d{1,3}(\.\d{1,3}){3}$/.test(host)) {
    if (isPrivateOrReservedIPv4(host)) {
      return {
        ok: false,
        reason: "사설/로컬 주소로의 요청은 허용되지 않습니다.",
      };
    }
  }
...
export function rateLimitFetchUrl(
  clientKey: string,
  ...
  if (b.count > MAX_REQ) {
    return {
      ok: false,
      retryAfterSec: Math.ceil((WINDOW_MS - (now - b.windowStart)) / 1000) || 1,
    };
  }
  return { ok: true };
}
