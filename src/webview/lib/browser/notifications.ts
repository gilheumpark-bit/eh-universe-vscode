...
export function notifyNovelComplete(episode: number, grade: string): void {
  notify("소설 생성 완료", `EP.${episode} — 등급: ${grade}`, {
    tag: "novel-gen",
  });
}

export function notifyCodeVerifyComplete(
  agentCount: number,
  confidence: number,
): void {
  notify("코드 검증 완료", `${agentCount}개 에이전트, 신뢰도: ${confidence}%`, {
    tag: "code-verify",
  });
}

export function notifyTranslationComplete(
  episodes: number,
  lang: string,
): void {
  notify("번역 완료", `${episodes}개 에피소드 → ${lang}`, {
    tag: "translation",
  });
}
