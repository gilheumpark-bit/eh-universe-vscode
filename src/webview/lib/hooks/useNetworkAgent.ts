  ...
  const searchAgent = useCallback(
    async (
      ...
      if (!idToken) {
        logger.warn(
          "useNetworkAgent",
          "searchAgent skipped: no Firebase ID token",
        );
      ...
      try {
        ...
        for (let i = 0; i < rawKey.length; i++)
          safeKey += rawKey.charCodeAt(i).toString(16);
        ...
        if (db) {
          ...
          if (cachedDoc && cachedDoc.exists()) {
            ...
            const isFresh =
              Date.now() - (cachedData.timestamp || 0) < 24 * 60 * 60 * 1000;
            if (isFresh) {
              return {
        ...
        if (summaryText.includes("<SILENCE>")) {
          summaryText =
            "[SYSTEM] ⚠️ 의미 없는 요청 혹은 세계관과 무관한 트롤링으로 판단되어 AI가 응답을 거부합니다. (HSE - 침묵할 권리 행사)";
        }
        ...
        if (db && cachedRef && finalResult.summary) {
          setDoc(cachedRef, {
            ...
            logger.warn("useNetworkAgent", "Cache write failed", err),
          );
        }
  ...
  const ingestAgent = useCallback(
    async (
      ...
      if (!tok) {
        logger.warn(
          "useNetworkAgent",
          "ingestAgent skipped: no Firebase ID token",
        );
        setError("Unauthorized");
        return false;
      }
