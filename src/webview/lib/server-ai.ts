...
const LOCAL_PROVIDERS: ReadonlySet<ServerProviderId> = new Set([
  "ollama",
  "lmstudio",
]);
...
export function isServerProviderId(value: unknown): value is ServerProviderId {
  return (
    typeof value === "string" &&
    SERVER_PROVIDERS.includes(value as ServerProviderId)
  );
}
...
export function hasServerProviderCredentials(
  provider: ServerProviderId,
): boolean {
  if (provider === "gemini") {
...
export function getHostedProviderAvailability(): Record<
  ServerProviderId,
  boolean
> {
