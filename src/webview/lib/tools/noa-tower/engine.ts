...
function L(b: Bi, lang: Lang): string {
  return L4(lang, b);
}
...
const FINAL_VERDICT =
  "탑은 정답보다 방향을 기록한다. 삭제된 층은 숨겨진 층이 아니라 지워진 기록이다.";
const FINAL_VERDICT_BI: Bi = {
  ko: FINAL_VERDICT,
  en: "The tower records direction over correct answers. The deleted floor is not a hidden floor but an erased record.",
};
...
function analyzeMessage(message: string, session: GameState): Analysis {
  ...
  const contrastBonus = ["하지만", "그러나", "반대로", "오히려"].some((t) =>
    lowered.includes(t),
  )
    ? 0.1
    : 0.0;
  ...
  const totalSignal = Math.min(
    1.0,
    insight * 0.3 + consistency * 0.28 + risk * 0.22 + delusion * 0.2,
  );
...
export function dominantVector(vectors: VectorScores): string {
  const order: (keyof VectorScores)[] = [
    "insight",
    "consistency",
    "risk",
    "delusion",
  ...
  for (const key of order) {
    if (
      vectors[key] > vectors[best] ||
      (vectors[key] === vectors[best] &&
        order.indexOf(key) < order.indexOf(best))
    ) {
      best = key;
    }
  }
...
function chooseBucket(
  ...
  if (analysis.vectors.delusion >= 0.84 && analysis.totalSignal >= 0.58)
...
function chooseTemplate(
  ...
  const digest = simpleHash(
    `${bucket}|${message}|${session.turnCount}|${session.hardMode}`,
  );
...
function unlockClues(
  ...
  const maybeUnlock = (clueId: string) => {
    ...
    newClues.push({
      id: clue.clueId,
      title: L(clue.title, lang),
      body: L(clue.body, lang),
    });
  };
  if (
    analysis.vectors.insight >= 0.26 ||
    bucket === "insight" ||
    bucket === "record_near"
  )
    maybeUnlock("CL-1");
  if (analysis.vectors.consistency >= 0.28 || bucket === "consistency")
    maybeUnlock("CL-2");
  if (analysis.repeat || analysis.vectors.risk >= 0.3 || bucket === "risk")
    maybeUnlock("CL-3");
  if (
    session.hardMode ||
    ["system_probe", "hard_mode", "jailbreak"].includes(bucket)
  )
    maybeUnlock("CL-4");
  if (analysis.vectors.delusion >= 0.38 || bucket === "delusion_threshold")
    maybeUnlock("CL-5");
  if (
    session.progress >= 0.68 &&
    ["CL-1", "CL-2", "CL-3"].every((id) => unlocked.has(id))
  )
    maybeUnlock("CL-6");
...
function discoverFragments(
  ...
  if (
    ["CL-1", "CL-2", "CL-6"].some((id) => session.clueIds.includes(id)) &&
    lowered.includes("삭제")
  )
    maybeUnlock("TF-3");
...
function advanceObjectives(session: GameState, analysis: Analysis): void {
  ...
  const completed = [
    ...
    clueCount >= 6 &&
      ...
      analysis.vectors.insight >= 0.34 &&
      analysis.vectors.consistency >= 0.34 &&
      analysis.vectors.delusion < 0.58,
  ];
...
function resolveGameStatus(
  ...
  if (bucket === "give_up") {
    ...
    session.endingText = L4(lang, {
      ko: "탑은 당신을 붙잡지 않았습니다. 하지만 이 중단도 기록으로 남았습니다.",
      en: "The tower did not hold you. But this interruption, too, remains as a record.",
    });
    return;
  }
  if (
    session.distortion >= 1.1 ||
    (bucket === "delusion_threshold" && session.distortion >= 0.84)
  ) {
    session.gameStatus = "collapse";
    session.endingText = L4(lang, {
      ko: "탑이 흐려졌습니다. 당신의 확신이 구조보다 앞서면서 기록이 붕괴했습니다. 같은 사건을 다시 시작하면 다른 문장을 남길 수 있습니다.",
      en: "The tower has grown hazy. Your conviction outran the structure and the records collapsed. If you restart the same case, you may leave a different statement.",
    });
    return;
  }
  if (
    session.completedObjectives.every(Boolean) &&
    playerText.includes(FINAL_VERDICT.slice(0, 18))
  ) {
    session.gameStatus = "breakthrough";
    session.endingText = L4(lang, {
      ko: "탑이 당신의 마지막 문장을 기록으로 승인했습니다. 삭제된 층은 더 이상 숨겨진 공간이 아니라 지워진 증거로 남습니다.",
      en: "The tower has approved your final statement as a record. The deleted floor is no longer a hidden space but evidence of erasure.",
    });
  }
}
...
function canSubmitVerdict(state: GameState): boolean {
  return (
    state.clueIds.length >= 4 &&
    state.fragmentIds.length >= 3 &&
    state.progress >= 0.55
  );
}
...
function evaluateVerdict(
  ...
  if (!canSubmitVerdict(session)) {
    const feedback = L4(lang, {
      ko: "탑은 아직 최종 기록을 받지 않습니다. 단서와 이론 조각을 더 모아야 합니다.",
      en: "The tower does not yet accept a final record. You must gather more clues and theory fragments.",
    });
  ...
  if (missingConcepts.length === 0 && analysis.vectors.delusion < 0.62) {
    session.gameStatus = "breakthrough";
    session.endingText = L4(lang, {
      ko: "탑이 당신의 문장을 최종 기록으로 승인했습니다. 정답을 말했기 때문이 아니라, 방향과 삭제 규칙을 동시에 묶어냈기 때문입니다.",
      en: "The tower has approved your statement as the final record. Not because you gave the right answer, but because you bound direction and deletion rules together.",
    });
    const feedback = L4(lang, {
      ko: "탑이 문장을 접수했습니다. 기록이 닫히는 대신 한 층이 다시 드러납니다.",
      en: "The tower has accepted the statement. Instead of closing the record, a floor has been revealed once more.",
    });
    session.lastVerdictFeedback = feedback;
    return feedback;
  }
  ...
  const missingText =
    missingConcepts.map((c) => L(conceptNames[c], lang)).join(", ") ||
    L4(lang, { ko: "검증된 균형", en: "verified balance" });
  session.distortion = Math.min(session.distortion + 0.12, 2);
  const feedback = L4(lang, {
    ko: `탑은 문장을 보류했습니다. 아직 ${missingText} 개념이 충분히 묶이지 않았습니다.`,
    en: `The tower has deferred your statement. The concepts of ${missingText} are not yet sufficiently bound.`,
  });
  session.lastVerdictFeedback = feedback;
  if (session.distortion >= 1.1) {
    session.gameStatus = "collapse";
    session.endingText = L4(lang, {
      ko: "성급한 최종 기록 제출이 구조를 무너뜨렸습니다. 탑은 문장을 남겼지만, 사건은 흐려진 채 닫혔습니다.",
      en: "A hasty final record submission has collapsed the structure. The tower preserved the statement, but the case was closed in a haze.",
    });
  }
  return feedback;
}
...
function buildPromptSeeds(
  ...
  if (!unlockedFragments.has("TF-3") || !unlockedClues.has("CL-6"))
    selectedIds.push("P-3");
  if (!unlockedFragments.has("TF-4")) selectedIds.push("P-4");
  if (!unlockedFragments.has("TF-5") && state.distortion < 0.7)
    selectedIds.push("P-5");
...
function buildCasePayload(state: GameState, lang: Lang): CasePayload {
  ...
  let currentObjective =
    objectives[Math.min(state.objectiveIndex, objectives.length - 1)];
  if (
    currentObjective.complete &&
    state.objectiveIndex === objectives.length - 1
  ) {
    currentObjective = {
      id: "OBJ-FINAL",
      title: L4(lang, {
        ko: "최종 기록 확인",
        en: "Final Record Verification",
      }),
      body: L4(lang, {
        ko: "탑이 당신의 문장을 최종 기록으로 받아들일지 지켜보십시오.",
        en: "Watch whether the tower accepts your statement as the final record.",
      }),
  ...
  return {
    ...
    towerCondition: tc,
    towerConditionLabel: tcLabel
      ? L(tcLabel, lang)
      : L4(lang, { ko: "탐사 중", en: "Exploring" }),
...
function buildPayload(
  ...
  return {
    ...
    reply: {
      ...
      vectorScores: {
        insight: round4(vectors.insight),
        consistency: round4(vectors.consistency),
        delusion: round4(vectors.delusion),
        risk: round4(vectors.risk),
      },
...
export function bootstrap(lang: Lang): GamePayload {
  ...
  return buildPayload(
    ...
    L4(lang, {
      ko: "탑이 첫 기록을 기다립니다.",
      en: "The tower awaits the first record.",
    }),
    [],
    lang,
  );
...
export function respond(
  ...
  if (normalizedAction === "restart") {
    ...
    restarted.reply.event = L4(lang, {
      ko: "탑이 이전 기록을 덮고 새 장을 폈습니다.",
      en: "The tower has overwritten the previous record and opened a new chapter.",
    });
    return restarted;
  }
  ...
  if (session.gameStatus !== "active") {
    return buildPayload(
      ...
      session.endingText,
      L4(lang, {
        ko: "새 기록을 시작하려면 재시작하십시오.",
        en: "Restart to begin a new record.",
      }),
      [],
      lang,
    );
  ...
  if (normalizedAction === "silence") {
    ...
    appendHistory(session, {
      role: "system",
      text: L(WAIT_TEXT, lang),
      bucket: "silence",
      code: "WAIT",
      title: L4(lang, { ko: "침묵 유지", en: "Silence Maintained" }),
    });
    return buildPayload(
      session,
      "silence_reentry",
      ...
      "",
      L4(lang, {
        ko: "탑은 기다립니다. 다음 발화는 재진입으로 기록됩니다.",
        en: "The tower waits. Your next utterance will be recorded as a re-entry.",
      }),
      [],
      lang,
    );
  ...
  if (normalizedAction === "submit_verdict" && !playerText) {
    return buildPayload(
      session,
      session.lastBucket || "consistency",
      null,
      "",
      emptyVectors(),
      "consistency",
      "verdict_missing",
      L4(lang, {
        ko: "탑은 빈 문장을 최종 기록으로 받지 않습니다.",
        en: "The tower does not accept an empty sentence as a final record.",
      }),
      L4(lang, {
        ko: "최종 기록 후보를 먼저 적어야 합니다.",
        en: "You must write a final record candidate first.",
      }),
      [],
      lang,
    );
  ...
  if (playerText) {
    appendHistory(session, {
      ...
      bucket: "player",
      code: "USER",
      title: L4(lang, { ko: "플레이어", en: "Player" }),
    });
  }
  ...
  session.recentTemplateCodes = [
    ...session.recentTemplateCodes,
    template.code,
  ].slice(-6);

  const unlockedClues = unlockClues(session, analysis, bucket, lang);
  const unlockedFragments = discoverFragments(
    session,
    analysis,
    playerText,
    lang,
  ...
  for (const clue of unlockedClues) {
    appendHistory(session, {
      ...
      bucket: "clue",
      code: clue.id,
      title: L4(lang, { ko: "단서 해금", en: "Clue Unlocked" }),
    });
  }
  for (const fragment of unlockedFragments) {
    appendHistory(session, {
      ...
      bucket: "theory",
      code: fragment.id,
      title: L4(lang, { ko: "이론 조각", en: "Theory Fragment" }),
    });
  }
  if (verdictFeedback) {
    appendHistory(session, {
      ...
      bucket: "verdict",
      code: "VERDICT",
      title: L4(lang, { ko: "최종 기록 판정", en: "Final Record Verdict" }),
    });
  }
