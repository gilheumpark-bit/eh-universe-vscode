...
export async function gitBranches(): Promise<GitBranch[]> {
  const raw = await run([
    "branch",
    "-a",
    "--format=%(refname:short) %(HEAD) %(objectname:short)",
  ]);
  return raw
    .split("\n")
    .filter(Boolean)
    ...
    for (let i = 0; i < hunkPositions.length; i++) {
      ...
      const end =
        i + 1 < hunkPositions.length
          ? hunkPositions[i + 1].start - hunkPositions[i + 1].header[0].length
          : chunk.length;
...
export async function gitLog(count = 50): Promise<GitLogEntry[]> {
  ...
  return raw
    ...
    .filter(Boolean)
    .map((line) => {
      const [hash, shortHash, author, email, date, message, refsStr] =
        line.split(sep);
      return {
        hash,
        shortHash,
