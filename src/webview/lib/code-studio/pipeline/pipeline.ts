...
function analyzeSimulation(code: string, _language: string): PipelineStage {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/while\s*\(\s*true\s*\)/.test(lines[i])) {
      ...
      if (!/break/.test(scope)) {
        findings.push(
          `L${i + 1}: Potential infinite loop: while(true) without break`,
        );
        score -= 30;
      }
    }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/\bawait\s+/.test(lines[i])) {
      ...
      if (!/\btry\s*\{/.test(precedingBlock)) {
        findings.push(
          `L${i + 1}: Async await without try/catch error handling`,
        );
        score -= 10;
...
function analyzeGeneration(code: string, _language: string): PipelineStage {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/(?:function|const\s+\w+\s*=.*=>)/.test(lines[i]) && !inFunc) {
      ...
      funcStart = i;
      funcName =
        lines[i].match(/(?:function\s+(\w+)|const\s+(\w+))/)?.[1] || "";
    }
    if (inFunc && lines[i].trim() === "}" && i - funcStart > 50) {
      findings.push(
        `L${funcStart + 1}: Function "${funcName || "anonymous"}" is ${i - funcStart} lines (consider splitting)`,
      );
      score -= 10;
      inFunc = false;
    }
  ...
  if (consoleLines.length > 3) {
    findings.push(
      `${consoleLines.length} console.log calls at lines ${consoleLines.slice(0, 5).join(",")}${consoleLines.length > 5 ? "..." : ""}`,
    );
    score -= 5;
  }
    ...
    for (const name of names) {
      ...
      if (clean) {
        const codeWithoutImport =
          ...
          code.slice(importMatch.index + importMatch[0].length);
...
function analyzeSizeDensity(code: string, _language: string): PipelineStage {
  ...
  if (blankRatio > 0.4) {
    findings.push(
      `Blank line ratio ${(blankRatio * 100).toFixed(0)}% — code may be too sparse`,
    );
    score -= 10;
  } else if (blankRatio < 0.05 && totalLines > 30) {
    findings.push(
      `Blank line ratio ${(blankRatio * 100).toFixed(0)}% — code is very dense, consider adding spacing`,
    );
    score -= 5;
  }
  ...
  // 평균 줄 길이
  const avgLineLength =
    nonBlankLines.reduce((s, l) => s + l.length, 0) /
    Math.max(1, nonBlankLines.length);
  if (avgLineLength > 100) {
    findings.push(
      `Average line length ${avgLineLength.toFixed(0)} chars — consider wrapping`,
    );
    score -= 10;
  }
  ...
  if (funcCount > 0) {
    ...
    if (linesPerFunc > 40) {
      findings.push(
        `~${linesPerFunc} lines/function — consider splitting large functions`,
      );
      score -= 10;
    }
  }
  ...
  if (longLineIndices.length > 5) {
    findings.push(
      `${longLineIndices.length} lines exceed 120 chars (first at L${longLineIndices[0]})`,
    );
    score -= 5;
  }
...
function analyzeAssetTrace(code: string, _language: string): PipelineStage {
  ...
  if (externalDeps.length > 10) {
    findings.push(
      `${externalDeps.length} external deps — heavy third-party dependency`,
    );
    score -= 5;
  }
  ...
  if (relativeImports > 5) {
    findings.push(
      `${relativeImports} same-dir imports — watch for circular dependencies`,
    );
    score -= 5;
  }
  ...
  if (reExports > 5) {
    findings.push(
      `${reExports} re-exports — potential barrel file, check for tree-shaking impact`,
    );
    score -= 5;
  }
  ...
  if (dynamicImports > 0) {
    findings.push(
      `${dynamicImports} dynamic import(s) — verify code-splitting intent`,
    );
  }
  ...
  if (findings.length === 0) {
    findings.push(
      `${externalDeps.length} external, ${internalDeps.length} internal deps`,
    );
  }
...
function analyzeStability(code: string, _language: string): PipelineStage {
  ...
  if (maxDepth >= 2) {
    findings.push(
      `L${nestedLine + 1}: Nested loops detected (O(n^2) potential)`,
    );
    score -= 15;
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/\.\.\.(?:arr|list|items|data|array)/i.test(lines[i])) {
      findings.push(
        `L${i + 1}: Spread operator on potentially large array — consider alternatives`,
      );
      score -= 5;
      break;
    }
  ...
  if (
    ...
    for (let i = 0; i < lines.length; i++) {
      if (/addEventListener/.test(lines[i])) {
        findings.push(
          `L${i + 1}: addEventListener without removeEventListener — potential memory leak`,
        );
        score -= 15;
        break;
      }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/readFileSync|writeFileSync/.test(lines[i])) {
      findings.push(
        `L${i + 1}: Synchronous file I/O — consider async alternatives`,
      );
      score -= 10;
      break;
    }
...
function analyzeReleaseIP(code: string, _language: string): PipelineStage {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (
      ...
      findings.push(
        `L${i + 1}: Hardcoded secret detected — use environment variables`,
      );
      score -= 40;
      break;
    }
...
function analyzeGovernance(code: string, _language: string): PipelineStage {
  ...
  if (exports > 20) {
    findings.push(
      `${exports} exports — module may have too many responsibilities`,
    );
    score -= 10;
  }
...
function detectUnusedImportsAST(
  ...
  const importedNames = new Map<
    string,
    { line: number; node: import("typescript").Node }
  >();
  ...
  ts.forEachChild(sourceFile, (node) => {
    if (ts.isImportDeclaration(node) && node.importClause) {
      ...
      if (clause.name) {
        ...
        importedNames.set(clause.name.text, {
          line: pos.line + 1,
          node: clause.name,
        });
      }
...
async function runASTAnalysis(
  ...
  if (!ts) {
    logger.warn(
      "pipeline",
      "TypeScript compiler not available for AST analysis",
    );
    return [];
  }
...
function mergeASTFindings(
  ...
  for (const af of astFindings) {
    ...
    const isDuplicate = merged.some((existing) => {
      ...
      if (
        existingLower.includes(`l${af.line}:`) &&
        existingLower.includes(af.message.toLowerCase().slice(0, 20))
      ) {
        return true;
      }
    ...
    if (!isDuplicate) {
      ...
      scoreAdjust -=
        af.severity === "error" ? 15 : af.severity === "warning" ? 5 : 2;
    }
  }
 ...
 */
export function runStaticPipeline(
  code: string,
  language: string,
): PipelineResult {
  ...
  const overallScore = Math.round(
    stages.reduce((s, t) => s + t.score, 0) / stages.length,
  );
  const overallStatus =
    overallScore >= 80 ? "pass" : overallScore >= 60 ? "warn" : "fail";
...
export async function runStaticPipelineWithAST(
  ...
  const resolvedName =
    fileName ||
    `file.${language.includes("tsx") ? "tsx" : language.includes("jsx") ? "jsx" : language.includes("javascript") ? "js" : "ts"}`;
  ...
  const unusedImportFindings = astFindings.filter((f) =>
    f.message.startsWith("Unused import"),
  );
  const unusedVarFindings = astFindings.filter((f) =>
    f.message.startsWith("Unused variable"),
  );
  const missingReturnFindings = astFindings.filter((f) =>
    f.message.includes("missing return type"),
  );
  const emptyFuncFindings = astFindings.filter((f) =>
    f.message.startsWith("Empty function"),
  );

  const stages = [...baseResult.stages];

  // Merge unused imports/vars into Validation (index 2)
  stages[2] = mergeASTFindings(
    stages[2],
    [...unusedImportFindings, ...unusedVarFindings],
    "validation",
  );
  ...
  const overallScore = Math.round(
    dedupedStages.reduce((s, t) => s + t.score, 0) / dedupedStages.length,
  );
  const overallStatus =
    overallScore >= 80 ? "pass" : overallScore >= 60 ? "warn" : "fail";
