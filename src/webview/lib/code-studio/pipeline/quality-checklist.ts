...
export function runTier1(code: string, fileName: string): CheckItem[] {
  ...
  results.push(
    check(
      ...
      {
        ko: "에러를 삼키면 디버깅 불가",
        en: "Swallowed errors prevent debugging",
      },
      emptyCatch === 0 ? "pass" : emptyCatch <= 2 ? "warn" : "fail",
      emptyCatch > 0 ? `${emptyCatch}건 발견` : undefined,
      emptyCatch,
      0,
      true,
    ),
  ...
  const dangerousCalls = (
    code.match(/\beval\s*\(|\bFunction\s*\(|new\s+Function\b/g) ?? []
  ).length;
  results.push(
    check(
      "S02",
      ...
      dangerousCalls > 0 ? `${dangerousCalls}건` : undefined,
      dangerousCalls,
      0,
    ),
  );
  ...
  results.push(
    check(
      ...
      anyCount > 0 ? `${anyCount}건` : undefined,
      anyCount,
      0,
      true,
    ),
  ...
  const guardRatio =
    propAccess > 0 ? Math.round((nullGuards / propAccess) * 100) : 100;
  results.push(
    check(
      "S04",
      "safety",
      ...
      {
        ko: "속성 접근 대비 ?. ?? 사용률",
        en: "Optional chaining usage vs property access",
      },
      guardRatio >= 15 ? "pass" : guardRatio >= 5 ? "warn" : "fail",
      `${guardRatio}% (${nullGuards}/${propAccess})`,
      guardRatio,
      15,
    ),
  );
  ...
  results.push(
    check(
      ...
      {
        ko: "프로덕션 성능 저하 + 정보 노출",
        en: "Performance impact + info leakage",
      },
      consoleLogs === 0 ? "pass" : consoleLogs <= 3 ? "warn" : "fail",
      consoleLogs > 0 ? `${consoleLogs}건` : undefined,
      consoleLogs,
      0,
      true,
    ),
  ...
  results.push(
    check(
      ...
      {
        ko: "가독성·테스트·유지보수 저하",
        en: "Readability and maintenance concern",
      },
      largeFns === 0 ? "pass" : largeFns <= 2 ? "warn" : "fail",
      largeFns > 0 ? `${largeFns}개 함수` : undefined,
      largeFns,
      0,
    ),
  );
  ...
  results.push(
    check(
      ...
      {
        ko: "비동기 에러 미처리 → 런타임 크래시",
        en: "Async errors crash at runtime",
      },
      unguardedAwait === 0 ? "pass" : unguardedAwait <= 2 ? "warn" : "fail",
      unguardedAwait > 0 ? `${unguardedAwait}건` : undefined,
      unguardedAwait,
      0,
      true,
    ),
  ...
  const abortUsage = (
    code.match(/AbortController|AbortSignal|signal\s*:/g) ?? []
  ).length;
  ...
  results.push(
    check(
      ...
      {
        ko: "네트워크 지연 시 무한 대기",
        en: "Infinite hang on network delay",
      },
      untimedFetch === 0 ? "pass" : "fail",
      fetchCalls > 0 ? `${untimedFetch}/${fetchCalls} 미보호` : "해당 없음",
      untimedFetch,
      0,
    ),
  );
  ...
  const cleanupPatterns = (
    code.match(
      /\bfinally\b|cleanup|dispose|removeEventListener|clearTimeout|clearInterval|\.unsubscribe/g,
    ) ?? []
  ).length;
  ...
  results.push(
    check(
      ...
      {
        ko: "finally/cleanup/removeEventListener 존재 여부",
        en: "Cleanup patterns presence",
      },
  ...
  results.push(
    check(
      ...
      unusedImports > 0 ? `${unusedImports}건` : undefined,
      unusedImports,
      0,
      true,
    ),
  ...
  // M03: 매직 넘버
  const magicNumbers = (
    code.match(/(?<![.\w])(?:[2-9]\d{2,}|[1-9]\d{3,})(?!\w)/g) ?? []
  ).length;
  results.push(
    check(
      "M03",
      ...
      magicNumbers > 0 ? `${magicNumbers}건` : undefined,
      magicNumbers,
      2,
    ),
  );
  ...
  const secretPatterns = (
    code.match(
      /(?:password|secret|token|apikey|api_key)\s*[:=]\s*['"][^'"]{8,}['"]/gi,
    ) ?? []
  ).length;
  results.push(
    check(
      ...
      {
        ko: "API키/비밀번호 코드 내 노출",
        en: "API keys/passwords exposed in code",
      },
      secretPatterns === 0 ? "pass" : "fail",
      secretPatterns > 0 ? `${secretPatterns}건 — 즉시 제거 필요` : undefined,
      secretPatterns,
      0,
    ),
  );
  ...
  // X02: innerHTML 사용
  const innerHtml = (code.match(/innerHTML|dangerouslySetInnerHTML/g) ?? [])
    .length;
  results.push(
    check(
      "X02",
      ...
      innerHtml > 0 ? `${innerHtml}건 — 입력 검증 필요` : undefined,
      innerHtml,
      0,
    ),
  );
...
export function buildPrecisionPrompt(
  ...
  const domainInstructions: Record<CheckDomain, string> = {
    safety:
      "Focus on: null dereference paths, type coercion bugs, error propagation chains. Trace each any usage and empty catch to its consumer.",
    performance:
      "Focus on: algorithm complexity analysis (exact Big-O), render cycle counts, memory allocation patterns, bundle size impact of imports.",
    reliability:
      "Focus on: failure cascade paths (what breaks if fetch fails?), retry/timeout coverage map, state corruption during concurrent operations.",
    maintainability:
      "Focus on: coupling analysis (how many files change if this function changes?), naming consistency audit, dead code reachability.",
    security:
      "Focus on: input validation paths (trace user input to output), auth bypass scenarios, data exposure through error messages.",
  };

  const instructions = targets
    .map((d) => `\n### ${d.toUpperCase()}\n${domainInstructions[d]}`)
    .join("");
  ...
  const score =
    scored.length > 0
      ? Math.round(
          scored.reduce(
            (sum, i) =>
              sum + (i.status === "pass" ? 100 : i.status === "warn" ? 60 : 0),
            0,
          ) / scored.length,
        )
      : 100;

  const hasSecurityFail = all.some(
    (i) => i.domain === "security" && i.status === "fail",
  );
  const hasSafetyFail = all.some(
    (i) => i.domain === "safety" && i.status === "fail",
  );
