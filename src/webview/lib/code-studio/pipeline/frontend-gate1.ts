...
const REQUIRED_STATES = [
  {
    pattern: /isLoading|loading|isPending|isFetching|skeleton/i,
    name: "Loading",
  },
  {
    pattern: /isEmpty|\.length\s*===?\s*0|no\s*data|no\s*results|검색\s*결과/i,
    name: "Empty",
  },
...
export function scan5States(
  ...
  const hasFetching =
    /fetch\(|useQuery|useSWR|axios\.|\.get\(|\.post\(|api\/|getServerSide/i.test(
      code,
    );
...
export function scanDeadDOM(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/<button\b/.test(line)) {
      ...
      if (
        !/onClick|on[A-Z]\w*=|type\s*=\s*["']submit/.test(chunk) &&
        !/<\/button>/.test(line)
      ) {
        findings.push({
          type: "dead-dom",
    ...
    if (
      /<input\b/.test(line) &&
      !/type\s*=\s*["'](?:submit|hidden|button)/.test(line)
    ) {
      const chunk = lines.slice(i, Math.min(i + 3, lines.length)).join(" ");
      if (
        !/onChange|onInput|value\s*=|defaultValue|ref\s*=|register/.test(chunk)
      ) {
        findings.push({
          type: "dead-dom",
          element: "<input>",
...
/** Gate 1 전체 실행 */
export function runFrontendGate1(
  code: string,
  fileName: string = "unknown",
): {
