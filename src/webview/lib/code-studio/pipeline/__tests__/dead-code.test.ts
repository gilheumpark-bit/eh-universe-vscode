...
describe("scanDeadCode", () => {
  it("detects unreachable code after return", () => {
    const file = makeFile(
      "1",
      "a.ts",
      ["function foo() {", "  return 1;", "  const x = 2;", "}"].join("\n"),
    );
  ...
  it("detects unused variables", () => {
    const file = makeFile(
      "1",
      "b.ts",
      ["const unused = 42;", "const used = 10;", "console.log(used);"].join(
        "\n",
      ),
    );
    const findings = scanDeadCode([file]);
    const unusedVars = findings.filter(
      (f) => f.kind === "unused-variable" && f.symbol === "unused",
    );
  ...
  it("ignores underscore-prefixed variables", () => {
    ...
    const findings = scanDeadCode([file]);
    const unusedVars = findings.filter(
      (f) => f.kind === "unused-variable" && f.symbol === "_ignored",
    );
  ...
  it("detects unused imports", () => {
    const file = makeFile(
      "1",
      "d.ts",
      ["import { unused } from './lib';", "const x = 1;"].join("\n"),
    );
    const findings = scanDeadCode([file]);
  ...
  it("returns empty for clean code", () => {
    ...
    const nonExportFindings = findings.filter(
      (f) => f.kind !== "unused-export",
    );
