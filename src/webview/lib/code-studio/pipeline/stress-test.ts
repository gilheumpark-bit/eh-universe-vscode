...
const DEFAULT_SCENARIOS: StressScenario[] = [
  ...
  },
  {
    id: "spike-burst",
    name: "Spike Test",
    description: "0 to 1000 users instantly",
    type: "spike",
    virtualUsers: 1000,
    durationSec: 30,
    rampUpSec: 0,
  },
  {
    id: "soak-24h",
    name: "Soak Test",
    description: "200 users for extended period",
    type: "soak",
    virtualUsers: 200,
    durationSec: 3600,
    rampUpSec: 60,
  },
  {
    id: "breakpoint",
    name: "Breakpoint",
    description: "Increase until failure",
    type: "breakpoint",
    virtualUsers: 2000,
    durationSec: 180,
    rampUpSec: 180,
...
function computeStaticMetrics(code: string): StaticMetrics {
  ...
  const functionCount = (
    code.match(
      /(?:function\s+\w+|(?:const|let|var)\s+\w+\s*=\s*(?:async\s+)?(?:\([^)]*\)|[a-zA-Z_]\w*)\s*=>|(?:async\s+)?(?:get|set|static\s+)?\w+\s*\([^)]*\)\s*\{)/g,
    ) ?? []
  ).length;
  ...
  for (const line of lines) {
    ...
    if (
      /^\s*(?:for\s*\(|while\s*\(|do\s*\{|\.forEach\(|\.map\(|\.reduce\()/.test(
        trimmed,
      )
    ) {
      currentLoopDepth++;
  ...
  const fetchCallCount = (
    code.match(/\bfetch\s*\(|axios\.|\.get\s*\(|\.post\s*\(/g) ?? []
  ).length;
  const setTimeoutCount = (code.match(/\bsetTimeout\b|\bsetInterval\b/g) ?? [])
    .length;
  const eventListenerCount = (code.match(/addEventListener|\.on\s*\(/g) ?? [])
    .length;
  const consoleLogCount = (code.match(/console\.\w+\s*\(/g) ?? []).length;

  // 재귀 함수 (간이 탐지: 함수 이름이 본문에서 자기 자신 호출)
  const fnNames = (code.match(/function\s+(\w+)/g) ?? []).map((m) =>
    m.replace("function ", ""),
  );
  ...
  const cyclomaticEstimate = (
    code.match(/\bif\b|\belse\b|\bswitch\b|\bcase\b|\b\?\s*[^:]/g) ?? []
  ).length;

  return {
    totalLines,
    functionCount,
    nestedLoopDepth: maxLoopDepth,
...
function buildMetricsBlock(m: StaticMetrics): string {
  ...
  if (m.eventListenerCount > 3)
    warnings.push(
      `⚠ addEventListener ${m.eventListenerCount}건 — 메모리 릭 위험`,
    );
  if (m.recursiveFunctionCount > 0)
    warnings.push(
      `⚠ 재귀 함수 ${m.recursiveFunctionCount}건 — 스택 오버플로우 위험`,
    );
...
export async function analyzeStress(
  ...
  try {
    ...
    const grade: StressResult["grade"] =
      errorRate < 0.01 && avgMs < 100
        ? "A"
        : errorRate < 0.05 && avgMs < 300
          ? "B"
          : errorRate < 0.1 && avgMs < 1000
            ? "C"
            : errorRate < 0.2
              ? "D"
              : "F";
    ...
    return {
      ...
      metrics: {
        ...
        totalRequests:
          p.totalRequests ?? scenario.virtualUsers * scenario.durationSec,
