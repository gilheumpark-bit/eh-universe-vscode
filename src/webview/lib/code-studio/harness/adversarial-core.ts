...
export function analyzeSpyPatterns(code: string): {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // 함수 시작 감지
    if (
      /(?:async\s+)?(?:function\s+\w+|(?:const|let)\s+\w+\s*=\s*(?:async\s+)?(?:\([^)]*\)|[a-z_$]\w*)\s*=>)/.test(
        line,
      )
    ) {
      inFunction = true;
    ...
    if (inFunction) {
      ...
      if (braceDepth <= 0 && functionBody.length > 1) {
        ...
        const hasExternalCall =
          /fetch\(|axios|\.query|\.insert|\.update|\.delete|\.save|\.send|dispatch|emit|navigate|push\(/.test(
            body,
          );
        const hasHardcodedReturn =
          /return\s+(?:\{[^}]*status[^}]*\}|true|false|\[\]|\{\}|null|undefined|["'][^"']*["'])\s*;/.test(
            body,
          );
        ...
        if (!hasExternalCall && hasHardcodedReturn && functionBody.length > 2) {
          suspiciousReturns.push({
            ...
            pattern:
              "Function returns hardcoded value without any external call (DB/API/State)",
          });
          findings.push(
            `Line ${functionStart + 1}: Spy Alert — 외부 호출 없이 하드코딩 값만 반환하는 의심 함수`,
          );
        }
...
export function extractFunctionParams(
  code: string,
): Array<{ name: string; params: string[]; line: number }> {
...
export function analyzeFuzzVulnerabilities(code: string): string[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inAsync) {
      ...
      if (braceDepth <= 0 && i > asyncStart) {
        if (!hasTryCatch) {
          findings.push(
            `Line ${asyncStart + 1}: Fuzz Alert — async 함수에 try-catch가 없습니다. 네트워크 에러 시 크래시됩니다.`,
          );
        }
        inAsync = false;
      }
  ...
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (
      /\.\s*map\s*\(|\.\s*filter\s*\(|\.\s*reduce\s*\(|\.\s*forEach\s*\(/.test(
        line,
      )
    ) {
      ...
      if (
        !/\.length|!==?\s*null|!==?\s*undefined|\?\.|Array\.isArray/.test(
          prevLines,
        ) &&
        !/\?\.\s*map|\?\.\s*filter/.test(line)
      ) {
        findings.push(
          `Line ${i + 1}: Fuzz Alert — 배열 메서드 호출 전 null/undefined 체크가 없습니다.`,
        );
      }
    }
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    if (/parseInt\(|Number\(|parseFloat\(/.test(lines[i])) {
      if (!/isNaN|typeof.*number|!==?\s*undefined/.test(lines[i])) {
        findings.push(
          `Line ${i + 1}: Fuzz Alert — 숫자 변환 전 입력값 검증이 없습니다. NaN/Infinity 가능.`,
        );
      }
    }
  }
...
export function generateMutations(
  code: string,
): Array<{ line: number; original: string; mutated: string; type: string }> {
...
export function buildHarnessFeedback(
  ...
  if (hasSpy) {
    ...
    instruction =
      "하드코딩된 return 값을 실제 로직으로 교체하라. 기존 인터페이스는 유지하라.";
  } else if (hasFuzz) {
    ...
    instruction =
      "방어 로직(if/try-catch)을 추가하여 전체 코드를 다시 제출하라. 기존 정상 동작 로직은 절대 건드리지 마라.";
