...
export {
  ...
  type HarnessConfig,
  type ParsedError,
} from "./build-test-loop";

// AST Hollow Scanner (Gate 1 Backend)
export {
  scanForHollowCode,
  scanProjectForHollowCode,
  type HollowCodeFinding,
} from "../pipeline/ast-hollow-scanner";
...
export {
  ...
  scan5States,
  scanDeadDOM,
  type FrontendGateFinding,
} from "../pipeline/frontend-gate1";

// Frontend Gate 2: Design Token Linter
export {
  runFrontendGate2,
  scanDesignTokens,
  type DesignTokenFinding,
} from "../pipeline/frontend-gate2";
...
export {
  ...
  buildSkeletonPrompt,
  buildDesignPrompt,
  type HeadlessFirstConfig,
  type HeadlessFirstResult,
} from "./headless-first";
...
export async function runMasterHarness(
  ...
  const gate1: GateResult = {
    ...
    score:
      hollowErrors.length === 0
        ? 100
        : Math.max(0, 100 - hollowErrors.length * 20),
  ...
  if (!gate1.passed) {
    ...
    return {
      ...
      gateFailed: "GATE-AST",
      results,
      feedback: buildHarnessFeedback(
        results,
        hollowErrors.map((f) => f.message),
        [],
      ),
  ...
  const gate2Findings = [
    ...fg1Errors.map((f) => f.message),
    ...fg2Errors.map((f) => f.message),
  ];
  const gate2: GateResult = {
    ...
    score:
      gate2Findings.length === 0
        ? 100
        : Math.max(0, 100 - gate2Findings.length * 10),
  ...
  if (!gate2.passed) {
    ...
    return {
      ...
      gatesRun: 2,
      gatesPassed: 1,
      gateFailed: "GATE-BUILD",
      results,
      feedback: buildHarnessFeedback(results, [], gate2Findings),
      totalDurationMs: Date.now() - start,
    };
  }
  ...
  try {
    ...
    const gate3: GateResult = {
      ...
      score:
        dynamicResult.results.length > 0
          ? Math.round(
              dynamicResult.results.reduce((s, r) => s + r.score, 0) /
                dynamicResult.results.length,
            )
          : 100,
    ...
    if (!gate3.passed) {
      ...
      return {
        ...
        feedback: buildHarnessFeedback(
          results,
          dynamicFailed.flatMap((r) => r.findings),
          [],
        ),
        totalDurationMs: Date.now() - start,
      };
    ...
    return {
      ...
      gateFailed: "GATE-SPY",
      results,
      feedback: buildHarnessFeedback(results, [`${err}`], []),
      totalDurationMs: Date.now() - start,
    };
  }
