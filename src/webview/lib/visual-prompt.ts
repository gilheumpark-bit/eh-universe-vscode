...
export function buildLevelPromptFragment(levels: VisualLevelPack): string {
  ...
  if (levels.emotionIntensity > 0)
    parts.push(EMOTION_INTENSITY[levels.emotionIntensity]);
  if (levels.compositionDrama > 0)
    parts.push(COMPOSITION_DRAMA[levels.compositionDrama]);
  if (levels.styleStrength > 0)
    parts.push(STYLE_STRENGTH[levels.styleStrength]);
  if (levels.symbolismWeight > 0)
    parts.push(SYMBOLISM_WEIGHT[levels.symbolismWeight]);
