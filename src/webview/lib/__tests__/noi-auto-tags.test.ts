import {
  extractConsistencyTags,
  extractAllConsistencyTags,
...
describe("noi-auto-tags", () => {
  describe("extractConsistencyTags", () => {
    ...
    it("extracts hair keywords (Korean)", () => {
      const tags = extractConsistencyTags(
        makeChar({ appearance: "흑발에 장발" }),
      );
    ...
    it("extracts body keywords", () => {
      const tags = extractConsistencyTags(
        makeChar({ appearance: "근육질의 장신, 흉터가 있는" }),
      );
    ...
    it("extracts attire keywords", () => {
      const tags = extractConsistencyTags(
        makeChar({ appearance: "갑옷을 입고 망토를 두른" }),
      );
    ...
    it("extracts English keywords", () => {
      const tags = extractConsistencyTags(
        makeChar({ appearance: "blonde ponytail, blue eyes, hoodie" }),
      );
  ...
  describe("buildConsistencyFragment", () => {
    ...
    it("returns formatted fragment", () => {
      const result = buildConsistencyFragment(
        makeChar({ name: "Alice", appearance: "은발" }),
      );
    ...
    it("joins multiple tags with comma", () => {
      const result = buildConsistencyFragment(
        makeChar({ name: "Bob", appearance: "흑발 적안 갑옷" }),
      );
