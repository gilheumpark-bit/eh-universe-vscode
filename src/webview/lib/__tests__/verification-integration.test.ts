...
const { runStaticPipeline } =
  require("@/lib/code-studio/pipeline/pipeline") as {
...
// eslint-disable-next-line @typescript-eslint/no-require-imports
const { generateFixes } =
  require("@/lib/code-studio/pipeline/pipeline-utils") as {
    generateFixes: jest.Mock;
  };
// eslint-disable-next-line @typescript-eslint/no-require-imports
const { scanProject } =
  require("@/lib/code-studio/features/patent-scanner") as {
...
describe("Verification Integration", () => {
  ...
  describe("Full verification flow", () => {
    ...
    test("failing code with fixable issues improves across rounds", async () => {
      ...
      runStaticPipeline
        .mockReturnValueOnce(
          makePipelineResult(60, "warn", [
            {
              name: "lint",
              status: "warn",
              score: 60,
              message: "warnings",
              findings: ["L1: unused import"],
            },
          ]),
        )
      ...
      findBugsStatic.mockReturnValueOnce([]).mockReturnValueOnce([]);
      ...
      const lastScore =
        result.iterations[result.iterations.length - 1].combinedScore;
    ...
    test("hard gate blocks despite high score", async () => {
      ...
      expect(result.hardGateFailures.some((f) => f.includes("critical"))).toBe(
        true,
      );
    });
          ...
          makePipelineResult(40, "fail", [
            {
              ...
              score: 40,
              message: "",
              findings: ["L1: unused import", "L3: console.log"],
            },
          ]),
        )
        .mockReturnValueOnce(
          makePipelineResult(65, "warn", [
            {
              name: "lint",
              status: "warn",
              score: 65,
              message: "",
              findings: ["L2: console.log"],
            },
          ]),
        )
        .mockReturnValueOnce(makePipelineResult(90, "pass"));
