...
function makePipelineResult(score: number): PipelineResult {
  return {
    stages: [
      {
        name: "lint",
        status: score >= 77 ? "pass" : "warn",
        score,
        message: "",
        findings: ["L1: issue found"],
      },
    ],
    overallScore: score,
...
const FILE = "test.ts";
const FILES = [
  { id: "test-1", name: "test.ts", type: "file" as const, content: CODE },
];
...
describe("Safe Fix Filtering", () => {
  ...
  test("unused import removal is auto-applied", async () => {
    ...
    const result = await runVerificationLoop(CODE, LANG, FILE, FILES, {
      enableStress: false,
      enableIP: false,
      maxIterations: 1,
    });
    ...
    expect(
      result.iterations[0].fixesApplied + result.iterations[0].fixesSkipped,
