...
} from "../code-studio/pipeline/verification-loop";
import type {
  PipelineResult,
  PipelineStage,
} from "../code-studio/pipeline/pipeline";
...
jest.mock("../code-studio/pipeline/pipeline", () => ({
  runStaticPipeline: (...args: unknown[]) =>
    mockRunStaticPipeline(...(args as [string, string])),
}));
jest.mock("../code-studio/pipeline/bugfinder", () => ({
  findBugsStatic: (...args: unknown[]) =>
    mockFindBugsStatic(...(args as [string, string])),
}));
jest.mock("../code-studio/pipeline/pipeline-utils", () => ({
  generateFixes: (...args: unknown[]) =>
    mockGenerateFixes(...(args as [unknown, unknown])),
}));
jest.mock("../code-studio/pipeline/stress-test", () => ({
  runStressReport: (...args: unknown[]) =>
    mockRunStressReport(...(args as [string, string])),
}));
jest.mock("../code-studio/features/patent-scanner", () => ({
  scanProject: (...args: unknown[]) =>
    mockScanProject(...(args as [FileNode[]])),
}));
...
describe("runVerificationLoop", () => {
  ...
  test("no-fixes stop — findings exist but no fixable ones", async () => {
    ...
    mockRunStaticPipeline.mockReturnValue(
      makePipelineResult(50, "fail", ["L1: some issue"]),
    );
    mockFindBugsStatic.mockReturnValue([]);
    mockGenerateFixes.mockReturnValue([]);
  ...
  test("onProgress fires for each round", async () => {
    ...
    await runVerificationLoop(
      ...
      baseConfig,
      (iteration) => {
        progressCalls.push(iteration.round);
      },
    );
  ...
  test("handles bugfinder throw gracefully", async () => {
    mockRunStaticPipeline.mockReturnValue(makePipelineResult(90, "pass"));
    mockFindBugsStatic.mockImplementation(() => {
      throw new Error("bugfinder crash");
    });
    mockGenerateFixes.mockReturnValue([]);
