...
function parseHtmlContent(
  ...
  for (const sel of candidates) {
    ...
    if (textEl && textEl.textContent && textEl.textContent.trim().length > 100)
      break;
  }
  ...
  if (/^ko/i.test(langAttr) || /[가-힣]/.test(text.slice(0, 200)))
    language = "ko";
  else if (
    /^ja/i.test(langAttr) ||
    /[\u3040-\u309f\u30a0-\u30ff]/.test(text.slice(0, 200))
  )
    language = "ja";
  else if (/^zh/i.test(langAttr) || /[\u4e00-\u9fff]/.test(text.slice(0, 200)))
    language = "zh";
  else if (
    /^en/i.test(langAttr) ||
    /^[a-zA-Z\s,.!?]+$/.test(text.slice(0, 200))
  )
    language = "en";
