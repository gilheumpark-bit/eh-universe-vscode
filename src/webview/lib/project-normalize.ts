  ...
  return value
    .map((raw: unknown) => {
      ...
      return {
        ...
        characterProfiles:
          typeof r?.characterProfiles === "string" ? r.characterProfiles : "",
...
export function mergeProjectSnapshots(
  primary: ProjectSnapshot[],
  secondary: ProjectSnapshot[],
): ProjectSnapshot[] {
...
export function projectFingerprint(
  snapshot: Omit<ProjectSnapshot, "updated_at">,
): string {
...
export function mergeStoryBible(existing: string, incoming: string): string {
  ...
  if (existing.includes(nextBlock))
    return limitText(existing, STORY_BIBLE_LIMIT);
...
export function buildReferenceBundle(
  ...
  const selectedProjects = projectList
    .filter(
      (project) =>
        project.id !== currentProjectId && referenceIds.includes(project.id),
    )
