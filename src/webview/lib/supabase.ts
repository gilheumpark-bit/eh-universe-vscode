...
export async function saveProjectToCloud(
  ...
  if (!supabase || !supabaseUrl || !supabaseAnonKey) {
    logger.warn(
      "EH Translator",
      "Supabase DB credentials missing. Skipping cloud save.",
    );
    return { error: "DB_DISABLED" };
  }

  const { data, error } = await supabase.from("eh_projects").upsert(
    {
      id: projectId,
      user_id: userId,
      project_data: state,
...
export async function listUserProjects(userId: string) {
  ...
  return data.map(
    (row: {
      id: string;
      updated_at: string;
      project_data?: { projectName?: string; chapters?: unknown[] } | null;
    }) => ({
      id: row.id,
 ...
 */
export async function getProjectsForReference(
  userId: string,
  referenceIds: string[],
) {
  if (
  ...
  if (error || !data) return [];
  return data.map(
    (row: { project_data: Record<string, unknown> | null }) => row.project_data,
  );
}
