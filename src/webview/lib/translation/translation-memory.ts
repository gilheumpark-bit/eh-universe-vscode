...
export function addToTM(
  ...
  const existing = entries.find(
    (e) => e.source === source && e.targetLang === targetLang,
  );
  if (existing) {
  ...
  return matches
    .sort(
      (a, b) =>
        ...
        (b.entry.confirmed ? 1 : 0) - (a.entry.confirmed ? 1 : 0),
    )
    .slice(0, 5);
}
  ...
  try {
    ...
    for (const entry of imported) {
      ...
      const dup = existing.find(
        (e) => e.source === entry.source && e.targetLang === entry.targetLang,
      );
      if (!dup) {
        existing.push({
          ...entry,
