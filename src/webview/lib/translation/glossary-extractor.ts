    ...
    return parsed
      .filter(
        (item: Record<string, unknown>) =>
          item.term && typeof item.term === "string",
      )
      .map((item: Record<string, unknown>) => ({
        term: String(item.term),
        frequency: 1,
        type: (["name", "place", "term", "title", "organization"].includes(
          String(item.type),
        )
          ? item.type
          : "term") as GlossaryCandidate["type"],
...
export function extractTermsRuleBased(text: string): GlossaryCandidate[] {
  ...
  const koNamePattern =
    /([가-힣]{2,4})(?:은|는|이|가|을|를|의|에게|에서|와|과|로|도|만|까지)/g;
  let match;
  ...
  return Array.from(candidates.values())
    .map((c) => ({
      ...
      confidence: Math.min(
        1,
        c.confidence + (c.frequency > 3 ? 0.3 : c.frequency > 1 ? 0.15 : 0),
      ),
    }))
