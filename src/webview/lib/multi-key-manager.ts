...
export function getSlotForRole(
  ...
  // 정확한 역할 매칭 우선
  const exact = config.slots.find(
    (s) => s.enabled && s.apiKey && s.assignedRole === role,
  );
  if (exact) return exact;
  // general fallback
  const general = config.slots.find(
    (s) => s.enabled && s.apiKey && s.assignedRole === "general",
  );
  return general ?? null;
}

/** 크로스밸리데이션용: 같은 역할 또는 general인 슬롯 2개 이상 */
export function getSlotsForCrossValidation(
  config: MultiKeyConfig,
  role: AgentRole,
): KeySlot[] {
  const candidates = config.slots.filter(
    (s) =>
      s.enabled &&
      s.apiKey &&
      (s.assignedRole === role || s.assignedRole === "general"),
  );
...
/** 전체 사용량 요약 */
export function getTotalUsage(config: MultiKeyConfig): {
  calls: number;
  tokens: number;
  cost: number;
} {
    ...
    slots: config.slots.map((s) => {
      ...
      return {
        ...
        usage: {
          ...
          lastUsed: Date.now(),
          estimatedCostUSD:
            s.usage.estimatedCostUSD + (outputTokens / 1000) * costPer1K,
        },
      };
    }),
  };
...
/** 사용량 초기화 */
export function resetSlotUsage(
  config: MultiKeyConfig,
  slotId: string,
): MultiKeyConfig {
  return {
